Version Scheme
--------------
TurtleDex uses the following versioning scheme, vX.X.X
...
- extend `ttdxc renter` with optional currency conversion based on
...
- Add `/renter/clean` API route with `ttdxc renter clean` to remove lost files
 from renter. Also added `ttdxc renter lost` to view lost files that would be
 removed.
...
- Added a job to `.gitlab-ci.yml` to trigger TurtleDex Antfarm version tests on TurtleDex
  master updates and on TurtleDex nightly executions.
- Add `ttdxc skynet isblocked` command as a helper to check if a skylink is
blocked since `ttdxc skynet blocklist` returns a list of hashed merkleroots, so
...
- Add `--portal` flag to `ttdxc skynet pin`
...
- move health summary from `ttdxc renter -v` to `ttdxc renter health`
- add timeout param to read registry api call
- add root flag to download endpoints
- add root flag to list downloads endpoint
- Add support for skykey delete in ttdxc.
- Add support for uploading entire directories as skyfiles (e.g. `ttdxc skynet
  ...
  [POST]` endpoint.
- Added feature to use pipes with 'ttdxc skynet upload'. e.g. 
  'dd if=/dev/zero bs=1M count=1000 | ttdxc skynet upload 1GB.dat'
...
- Fix skykey default type in ttdxc

**Other**
- Split out `ttdxc renter workers` download and upload info
...
- Add `configModules` to the API so that the ttdxd modules can be return in
  `/daemon/settings [GET]`
...
- Add `disabledefaultpath` - a new optional path parameter when creating
  ...
  skyfile.
- Add 'ttdxc' commands for the FeeManager
- Add `TypePrivateID` Skykeys with skyfile encryption support
- Added available and priority memory output to `ttdxc renter -v`
...
- Fix panic in the wal of the ttdxdir and siafile if a delete update was
  submitted as the last update in a set of updates.

**Other**
- Add `EphemeralAccountExpiry` and `MaxEphemeralAccountBalance` to the Host's
  ExternalSettings
- Add testing infrastructure to validate the output of ttdxc commands.
- Add root ttdxc Cobra command test with subtests.
...
- Add `ttdxc renter workers ea` command to ttdxc
- Add `ttdxc renter workers pt` command to ttdxc
- Add `ttdxc renter workers rj` command to ttdxc
- Add `ttdxc renter workers hsj` command to ttdxc
...
### v1.4.11
**Bugs Fixed**
- Fixed bug where a TurtleDex dir could be created with the same path as an already
existing TurtleDex dir and no error was returned.
...
### v1.4.9
**Key Updates**
- Add `FeeManager` to ttdxd to allow for applications to charge a fee
- Add start time for the API server for ttdxd uptime
- Add new `/consensus/subscribe/:id` endpoint to allow subscribing to consensus
  change events
- Add /skykeys endpoint and `ttdxc skykey ls` command
...
- Update skykey ttdxc functions to accept httpClient and remove global httpClient
  reference from ttdxc testing
- Skykeycmd test broken down to subtests.
- Create ttdxc testing helpers.
...
- Add `startheight` and `endheight` flags for `ttdxc wallet transactions`
  pagination
- Add progress bars to Skynet uploads. Those can be disabled by passing the
  `--silent` flag.
- Add the Execute Program RPC to the host
- Added Skykey API endpoints and ttdxc commands.
- Add /skynet/portals API endpoints.
- Add MinBaseRPCPrice and MinSectorAccessPrice to `ttdxc host -v`
...
- Add SIAD_DATA_DIR environment variable which tells `ttdxd` where to store the
  ttdxd-specific data. This complements the SIA_DATA_DIR variable which tells
  `ttdxd` where to store general TurtleDex data, such as the API password,
  configuration, etc.
- Update the `ttdxc renter` summaries to use the `root` flags for the API calls
...
- Add `ttdxc renter workers` command to ttdxc
...
- Refactor the environment variables into the `build` package to address bug
  where `ttdxc` and `ttdxd` could be using different API Passwords.
- Fix bug in converting siafile to skyfile and enable testing.
- Fixed bug in bubble code that would overwrite the `ttdxdir` metadata with old
  metadata
- Fixed the output of `ttdxc skynet ls` not counting subdirectories.
...
- Fixed bug in `ttdxc renter -v` where the health summary wasn't considering
...
- Created `Merge Request.md` to document the merge request standards and
  process.
- Remove backslash check in TurtleDexPath validation, add `\` to list of accepted
  characters
- `ttdxc skynet upload` with the `--dry-run` flag will now print more clear
...
- Expand `TurtleDexPath` unit testing to address more edge cases.
...
- Add `--fee-included` parameter to `ttdxc wallet send ttdcs` that allows
   sending an exact wallet balance with the fees included.
- Extend `ttdxc hostdb view` to include all the fields returned from the API.
- `ttdxc renter delete` now accepts a list of files.
- add pause and resume uploads to ttdxc
- Extended `ttdxc renter` to include number of passive and disabled contracts
- Add contract data to `ttdxc renter`
...
**Bugs Fixed**
- Fixed file health output of `ttdxc renter -v` not adding to 100% by adding
...
- Fix divide by zero panic in `renterFileHealthSummary` for `ttdxc renter -v`
- Fix negative currency panic in `ttdxc renter contracts view`

**Other**
- Add timeout parameter to Skylink pin route
- Also apply timeout when fetching the individual chunks
- Add TurtleDexMux stream handler to the host
...
- Add `--root` parameter to `ttdxc renter delete` that allows passing absolute
...
- Fix default expected upload/download values displaying 0 when setting an
  initial allowance.
- `ttdxc skynet upload` now supports uploading directories. All files are
...
- The `ttdxc renter setallowance` UX is considerably improved.
- Add XChaCha20 CipherKey.
- Add Skykey Manager.
- Add `ttdxc skynet unpin` subcommand.
- Extend `ttdxc renter -v` to show breakdown of file health.
- Add Skynet-Disable-Force header to allow disabling the force update feature
  on Skynet uploads
- Add bandwidth usage to `ttdxc gateway`
...
- Fix bug where `ttdxc renter -v` wasn't working due to the wrong flag being
  used.
- Fixed bug in siafile snapshot code where the `hostKey()` method was not used
  to safely acquire the host pubkey.
- Fixed `ttdxc skynet ls` not working when files were passed as input. It is now
...
- fix ttdxc update for new release verification
...
- Add `data-pieces` and `parity-pieces` flags to `ttdxc renter upload`
- Integrate TurtleDexMux
...
- HostDB Data race fixed and documentation updated to explain the data race
  concern
- `Name` and `Dir` methods of the TurtleDexpath used the `filepath` package when they
...
- TurtleDexfiles were not pruning hosts correctly
- Unable to upload a new file if 'force' is set and no file exists to delete
- TurtleDexc would not always delete a file or folder correctly
...
- ttdxc Alerts command
- Critical alerts displayed on every ttdxc call
- Single File Get in ttdxc
...
- Community images added for Built On TurtleDex
- JSON tag code analyzer 
- ResponseWriter code analyzer
- boltdb added to github.com/turtledex
...
- Fix wallet bugs
- Add ttdxc command to cancel allowance
...
- Add ttdxc command to support storage leaderboard
...
- Restored (but deprecated) some ttdxc commands
...
- TurtleDexfunds can be loaded and sent
