...
// contract.
func (h *Host) managedAddRenewCollateral(hostCollateral types.Currency, so storageObligation, txnSet []types.Transaction) (builder modules.TransactionBuilder, newParents []types.Transaction, newInputs []types.TurtleDexcoinInput, newOutputs []types.TurtleDexcoinOutput, err error) {
	...
	err = builder.FundTurtleDexcoins(hostCollateral)
	if err != nil {
		builder.Drop()
	...
	for _, inputIndex := range newInputIndices {
		newInputs = append(newInputs, updatedTxn.TurtleDexcoinInputs[inputIndex])
	}
	for _, outputIndex := range newOutputIndices {
		newOutputs = append(newOutputs, updatedTxn.TurtleDexcoinOutputs[outputIndex])
	}
...
func (h *Host) managedRPCRenewContractRHP2(conn net.Conn) error {
	...
	err = encoding.ReadObject(conn, &renterPK, modules.NegotiateMaxTurtleDexPubkeySize)
	if err != nil {
...
func (h *Host) managedVerifyRenewedContract(so storageObligation, txnSet []types.Transaction, renterPK types.TurtleDexPublicKey) (types.Currency, error) {
