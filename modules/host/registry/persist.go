...
type (
	...
	// compressedPublicKey is a version of the types.TurtleDexPublicKey which is
...
func newCompressedPublicKey(spk types.TurtleDexPublicKey) (cpk compressedPublicKey, _ error) {
...
// newTurtleDexPublicKey creates a sia public key from a compressed public key.
func newTurtleDexPublicKey(cpk compressedPublicKey) (spk types.TurtleDexPublicKey, _ error) {
	if cpk.Algorithm != signatureEd25519 {
		return spk, errors.New("newTurtleDexPublicKey: unknown signature algorithm")
	}
...
func (entry persistedEntry) Value(index int64) (*value, error) {
	...
	spk, err := newTurtleDexPublicKey(entry.Key)
	if err != nil {
		return nil, errors.AddContext(err, "Value: failed to convert compressed key to TurtleDexPublicKey")
	}
	return &value{
		key:         spk,
		tweak:       entry.Tweak,
