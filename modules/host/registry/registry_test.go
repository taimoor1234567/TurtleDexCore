...
func TestRegistryRace(t *testing.T) {
	...
	keys := make([]types.TurtleDexPublicKey, 0, numEntries)
	...
	worker := func(key types.TurtleDexPublicKey, sk crypto.SecretKey, rv modules.SignedRegistryValue, nextExpiry, nextRevision *uint64) {
		for {
...
func BenchmarkRegistryUpdate(b *testing.B) {
	...
	keys := make([]types.TurtleDexPublicKey, 0, nEntries)
