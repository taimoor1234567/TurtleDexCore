	...
	return StorageObligationSnapshot{
		...
		staticRevisionTxn: types.Transaction{
			FileContractRevisions: []types.FileContractRevision{
				{
					NewValidProofOutputs:  make([]types.TurtleDexcoinOutput, 2),
					NewMissedProofOutputs: make([]types.TurtleDexcoinOutput, 3),
				},
			},
		},
	}
...
func (so storageObligation) payouts() (valid []types.TurtleDexcoinOutput, missed []types.TurtleDexcoinOutput) {
	valid = make([]types.TurtleDexcoinOutput, 2)
	missed = make([]types.TurtleDexcoinOutput, 3)
	if len(so.RevisionTransactionSet) > 0 {
...
func (h *Host) threadedHandleActionItem(soid types.FileContractID) {
	...
	if !so.RevisionConfirmed && len(so.RevisionTransactionSet) > 0 && blockHeight >= so.expiration()-revisionSubmissionBuffer {
		...
		err = builder.FundTurtleDexcoins(requiredFee)
		if err != nil {
