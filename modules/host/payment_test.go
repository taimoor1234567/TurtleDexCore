...
func TestVerifyEAFundRevision(t *testing.T) {
	...
	curr := types.FileContractRevision{
		NewValidProofOutputs: []types.TurtleDexcoinOutput{
			{Value: types.NewCurrency64(10)},
			{Value: types.NewCurrency64(1)},
		},
		NewMissedProofOutputs: []types.TurtleDexcoinOutput{
			{Value: types.NewCurrency64(10)},
			{Value: types.NewCurrency64(1)},
	...
	badOutputs := []types.TurtleDexcoinOutput{payment.NewMissedProofOutputs[0]}
	...
	badCurr = deepCopy(curr)
	badCurr.NewMissedProofOutputs = append([]types.TurtleDexcoinOutput{}, curr.NewMissedProofOutputs[:2]...)
	err = verifyEAFundRevision(badCurr, payment, height, amount)
...
func testPayByContract(t *testing.T, pair *renterHostPair) {
	...
	amount := types.TurtleDexcoinPrecision.Div64(2)
...
func (ht *hostTester) addNoOpRevision(so storageObligation, renterPK types.TurtleDexPublicKey) (storageObligation, error) {
...
func (ht *hostTester) addNewRevision(so storageObligation, renterPK types.TurtleDexPublicKey, newFileSize uint64, newFileMerkleRoot crypto.Hash) (storageObligation, error) {
	builder, err := ht.wallet.StartTransaction()
	if err != nil {
		return storageObligation{}, err
	}
...
func TestRevisionFromRequest(t *testing.T) {
	...
	pbcr := modules.PayByContractRequest{
		NewValidProofValues: []types.Currency{
			types.TurtleDexcoinPrecision.Mul64(10),
			types.TurtleDexcoinPrecision.Mul64(100),
		},
		NewMissedProofValues: []types.Currency{
			types.TurtleDexcoinPrecision.Mul64(1000),
			types.TurtleDexcoinPrecision.Mul64(10000),
			types.TurtleDexcoinPrecision.Mul64(100000),
		},
	}

	// valid case
	rev := revisionFromRequest(recent, pbcr)
	if !rev.NewValidProofOutputs[0].Value.Equals(types.TurtleDexcoinPrecision.Mul64(10)) {
		t.Fatal("valid output 0 doesn't match")
	}
	if !rev.NewValidProofOutputs[1].Value.Equals(types.TurtleDexcoinPrecision.Mul64(100)) {
		t.Fatal("valid output 1 doesn't match")
	}
	if !rev.NewMissedProofOutputs[0].Value.Equals(types.TurtleDexcoinPrecision.Mul64(1000)) {
		t.Fatal("missed output 0 doesn't match")
	}
	if !rev.NewMissedProofOutputs[1].Value.Equals(types.TurtleDexcoinPrecision.Mul64(10000)) {
		t.Fatal("missed output 1 doesn't match")
	}
	if !rev.NewMissedProofOutputs[2].Value.Equals(types.TurtleDexcoinPrecision.Mul64(100000)) {
