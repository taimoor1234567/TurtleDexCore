...
func (h *TestHost) RegistryGet(pubKey types.TurtleDexPublicKey, tweak crypto.Hash) (modules.SignedRegistryValue, bool) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *TestHost) RegistryUpdate(rv modules.SignedRegistryValue, pubKey types.TurtleDexPublicKey, expiry types.BlockHeight) (modules.SignedRegistryValue, error) {
	h.mu.Lock()
	defer h.mu.Unlock()
