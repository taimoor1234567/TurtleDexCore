...
func (h *Host) threadedNotifySubscribers(pubKey types.TurtleDexPublicKey, rv modules.SignedRegistryValue) {
	err := h.tg.Add()
	if err != nil {
		return
	}
...
// a subsriber.
func subscriptionResponseStream(info *subscriptionInfo, sm *siamux.TurtleDexMux) (siamux.Stream, error) {
...
// writer.
func sendNotification(stream io.Writer, spk types.TurtleDexPublicKey, rv modules.SignedRegistryValue) error {
