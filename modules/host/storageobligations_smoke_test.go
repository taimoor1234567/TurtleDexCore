...
func (ht *hostTester) newTesterStorageObligation() (storageObligation, error) {
	...
	// up paying.
	payout := types.TurtleDexcoinPrecision.Mul64(10e3)
...
func TestPruneStaleStorageObligations(t *testing.T) {
	...
	var contractCost types.Currency = types.NewCurrency64(1).Mul(types.TurtleDexcoinPrecision)
	var lockedCollateral types.Currency = types.NewCurrency64(1e3).Mul(types.TurtleDexcoinPrecision)
	for i := 0; i < 3; i++ {
		so, err := ht.newTesterStorageObligation()
		if err != nil {
			t.Fatal(err)
...
func TestSingleSectorStorageObligationStack(t *testing.T) {
	...
	sectorCost := types.TurtleDexcoinPrecision.Mul64(550)
...
func TestMultiSectorStorageObligationStack(t *testing.T) {
	...
	sectorCost2 := types.TurtleDexcoinPrecision.Mul64(650)
