...
func (fm *fuseManager) Mount(mountPoint string, sp modules.TurtleDexPath, opts modules.MountOptions) (err error) {
	...
	rootDirNode, err := fm.renter.staticFileSystem.OpenTurtleDexDir(sp)
	if err != nil {
...
func (fm *fuseManager) MountInfo() []modules.MountInfo {
	...
	for mountPoint, filesystem := range fm.mountPoints {
		siaPath := filesystem.renter.staticFileSystem.DirTurtleDexPath(filesystem.root.staticDirNode)
