...
func TestSaveLoad(t *testing.T) {
	...
	c.oldContracts = map[types.FileContractID]modules.RenterContract{
		{0}: {ID: types.FileContractID{0}, HostPublicKey: types.TurtleDexPublicKey{Key: []byte("foo")}},
		{1}: {ID: types.FileContractID{1}, HostPublicKey: types.TurtleDexPublicKey{Key: []byte("bar")}},
		{2}: {ID: types.FileContractID{2}, HostPublicKey: types.TurtleDexPublicKey{Key: []byte("baz")}},
	}
	...
	if _, foundOutput := contract.parentOutputs[types.TurtleDexcoinOutputID{4}]; !foundOutput {
		t.Fatal("watchdog not restored properly", contract.parentOutputs)
	}
