...
func newModules(testdir string) (modules.ConsensusSet, modules.Wallet, modules.TransactionPool, *siamux.TurtleDexMux, modules.HostDB, closeFn, error) {
...
// host.
func newStream(mux *siamux.TurtleDexMux, h modules.Host) (siamux.Stream, error) {
	hes := h.ExternalSettings()
	muxAddress := fmt.Sprintf("%s:%s", hes.NetAddress.Host(), hes.TurtleDexMuxPort)
	muxPK := modules.TurtleDexPKToMuxPK(h.PublicKey())
	return mux.NewStream(modules.HostTurtleDexMuxSubscriberName, muxAddress, muxPK)
}
...
func TestIntegrationSetAllowance(t *testing.T) {
	...
	a.Funds = types.TurtleDexcoinPrecision
	a.Hosts = 0
	err = c.SetAllowance(a)
	...
	a.Funds = types.TurtleDexcoinPrecision.Mul64(100)
	err = c.SetAllowance(a)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHostMaxDuration(t *testing.T) {
	...
	// Create allowance
	a := modules.Allowance{
		Funds:              types.TurtleDexcoinPrecision.Mul64(100),
		Hosts:              1,
		Period:             30,
		RenewWindow:        20,
...
func TestPayment(t *testing.T) {
	...
	// newStream is a helper to get a ready-to-use stream that is connected to a
	// host.
	newStream := func(mux *siamux.TurtleDexMux, h modules.Host) (siamux.Stream, error) {
		hes := h.ExternalSettings()
		muxAddress := fmt.Sprintf("%s:%s", hes.NetAddress.Host(), hes.TurtleDexMuxPort)
		muxPK := modules.TurtleDexPKToMuxPK(h.PublicKey())
		return mux.NewStream(modules.HostTurtleDexMuxSubscriberName, muxAddress, muxPK)
