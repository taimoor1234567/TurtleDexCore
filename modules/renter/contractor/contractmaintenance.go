...
type (
	...
	fileContractRenewal struct {
		...
		hostPubKey types.TurtleDexPublicKey
	}
)
...
func (c *Contractor) managedEstimateRenewFundingRequirements(contract modules.RenterContract, blockHeight types.BlockHeight, allowance modules.Allowance) (types.Currency, error) {
	...
	// contract).
	beforeTurtleDexfundFeesEstimate := maintenanceCost.Add(newUploadsCost).Add(newDownloadsCost).Add(contractPrice)
	afterTurtleDexfundFeesEstimate := types.Tax(blockHeight, beforeTurtleDexfundFeesEstimate).Add(beforeTurtleDexfundFeesEstimate)
	...
	estimatedCost := afterTurtleDexfundFeesEstimate.Add(txnFees)
...
func (c *Contractor) managedPrunedRedundantAddressRange() {
	...
	pks := make([]types.TurtleDexPublicKey, 0, len(allContracts))
...
// I/O.
func (c *Contractor) managedRenew(id types.FileContractID, hpk types.TurtleDexPublicKey, contractFunding types.Currency, newEndHeight types.BlockHeight, hostSettings modules.HostExternalSettings) (_ modules.RenterContract, err error) {
	...
	if c.staticDeps.Disrupt("DefaultRenewSettings") {
		...
		host.HostExternalSettings.TurtleDexMuxPort = hostSettings.TurtleDexMuxPort
	}
	...
	err = txnBuilder.FundTurtleDexcoins(params.Funding)
	if err != nil {
...
func (c *Contractor) threadedContractMaintenance() {
	...
	allContracts := c.staticContracts.ViewAll()
	c.mu.RLock()
	var blacklist []types.TurtleDexPublicKey
	var addressBlacklist []types.TurtleDexPublicKey
	for _, contract := range allContracts {
