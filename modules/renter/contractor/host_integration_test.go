...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/host"
	...
	modWallet "github.com/turtledex/TurtleDexCore/modules/wallet"
...
func newCustomTestingHost(testdir string, cs modules.ConsensusSet, tp modules.TransactionPool, mux *siamux.TurtleDexMux, deps modules.Dependencies) (modules.Host, closeFn, error) {
...
func newTestingHost(testdir string, cs modules.ConsensusSet, tp modules.TransactionPool, mux *siamux.TurtleDexMux) (modules.Host, closeFn, error) {
...
func newTestingContractor(testdir string, g modules.Gateway, cs modules.ConsensusSet, tp modules.TransactionPool, rl *ratelimit.RateLimit, deps modules.Dependencies) (*Contractor, closeFn, error) {
	...
	siaMuxDir := filepath.Join(testdir, modules.TurtleDexMuxDir)
	mux, err := modules.NewTurtleDexMux(siaMuxDir, testdir, "localhost:0", "localhost:0")
	if err != nil {
		return nil, nil, err
	}
...
func newCustomTestingTrio(name string, mux *siamux.TurtleDexMux, hdeps, cdeps modules.Dependencies) (modules.Host, *Contractor, modules.TestMiner, closeFn, error) {
	testdir := build.TempDir("contractor", name)
...
func TestIntegrationFormContract(t *testing.T) {
	...
	_, _, err = c.managedNewContract(hostEntry, types.TurtleDexcoinPrecision.Mul64(50), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestFormContractSmallAllowance(t *testing.T) {
	...
	c.mu.Lock()
	c.allowance = modules.DefaultAllowance
	c.allowance.Funds = types.TurtleDexcoinPrecision.Mul64(1)
...
func TestIntegrationReviseContract(t *testing.T) {
	...
	// form a contract with the host
	_, contract, err := c.managedNewContract(hostEntry, types.TurtleDexcoinPrecision.Mul64(50), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationRenew(t *testing.T) {
	...
	contract, err = c.managedRenew(contract.ID, contract.HostPublicKey, types.TurtleDexcoinPrecision.Mul64(50), c.blockHeight+200, hostSettings)
	if err != nil {
		t.Fatal(err)
	}
	...
	contract, err = c.managedRenew(contract.ID, contract.HostPublicKey, types.TurtleDexcoinPrecision.Mul64(50), c.blockHeight+100, hostSettings)
	if err != nil {
		t.Fatal(err)
	}
...
// own this contract. See https://github.com/turtledex/TurtleDexCore/issues/2327.
func TestContractPresenceLeak(t *testing.T) {
	...
	// form a contract with the host
	_, contract, err := c.managedNewContract(hostEntry, types.TurtleDexcoinPrecision.Mul64(10), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
