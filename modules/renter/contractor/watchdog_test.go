...
//                       +
//
func createTestTransactionTree(numRoots int, chainLength int) (txnSet []types.Transaction, roots []types.Transaction, subRootTx types.Transaction, fcTxn types.Transaction, rootParentOutputs map[types.TurtleDexcoinOutputID]bool) {
	roots = make([]types.Transaction, 0, numRoots)
	rootParents := make(map[types.TurtleDexcoinOutputID]bool)

	// All the root txs create outputs spent in subRootTx.
	subRootTx = types.Transaction{
		TurtleDexcoinInputs: make([]types.TurtleDexcoinInput, 0, numRoots),
	...
	for i := 0; i < numRoots; i++ {
		nextRootTx := types.Transaction{
			TurtleDexcoinInputs: []types.TurtleDexcoinInput{
				{
					ParentID: types.TurtleDexcoinOutputID(crypto.HashObject(fastrand.Bytes(32))),
				},
			},
			TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{
				{UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32)))},
		...
		rootParents[nextRootTx.TurtleDexcoinInputs[0].ParentID] = true

		subRootTx.TurtleDexcoinInputs = append(subRootTx.TurtleDexcoinInputs, types.TurtleDexcoinInput{
			ParentID: nextRootTx.TurtleDexcoinOutputID(0),
		})
	...
	for i := 0; i < chainLength; i++ {
		...
		for i := 0; i < len(txnSet); i++ {
			if len(txnSet[i].TurtleDexcoinInputs) > 1 {
	...
	fcTxn = types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{ParentID: txnSet[0].TurtleDexcoinOutputID(0)}},
...
func addChildren(txnSet []types.Transaction, numDependencies int, hasOutputs bool) []types.Transaction {
	...
	if !hasOutputs {
		for i := 0; i < numDependencies; i++ {
			txnSet[0].TurtleDexcoinOutputs = append(txnSet[0].TurtleDexcoinOutputs, types.TurtleDexcoinOutput{
				UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32))),
			})
		}
	}
	...
	for i := 0; i < numDependencies; i++ {
		newChild := types.Transaction{
			TurtleDexcoinInputs:  make([]types.TurtleDexcoinInput, 1),
			TurtleDexcoinOutputs: make([]types.TurtleDexcoinOutput, 0),
		}

		newChild.TurtleDexcoinInputs[0] = types.TurtleDexcoinInput{
			ParentID: txnSet[0].TurtleDexcoinOutputID(uint64(i)),
		}

		newChild.TurtleDexcoinOutputs = append(newChild.TurtleDexcoinOutputs, types.TurtleDexcoinOutput{
			UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32))),
		})
...
func TestWatchdogRevisionCheck(t *testing.T) {
	...
	a := modules.Allowance{
		Funds:              types.TurtleDexcoinPrecision.Mul64(100), // 100 SC
		Hosts:              1,
		Period:             50,
		RenewWindow:        10,
...
func TestWatchdogGetParents(t *testing.T) {
	...
	for i := 0; i < 10-1; i++ {
		parentID := txnSet[i].TurtleDexcoinInputs[0].ParentID
		expectedID := txnSet[i+1].TurtleDexcoinOutputID(0)
	...
	if parentOids[0] != rootTx.TurtleDexcoinInputs[0].ParentID {
	...
	for i := 0; i < len(rootTx2.TurtleDexcoinInputs); i++ {
		foundParent := false
		for j := 0; j < len(parentOids2); j++ {
			if parentOids2[i] == rootTx2.TurtleDexcoinInputs[j].ParentID {
				foundParent = true
			}
		}
		if !foundParent {
	...
	bigRootIDs := make(map[types.TurtleDexcoinOutputID]bool)

	// All the root txs create outputs spent in subRootTx.
	subRootTx := types.Transaction{
		TurtleDexcoinInputs: make([]types.TurtleDexcoinInput, numBigRoots),
	}
	...
	for i := 0; i < numBigRoots; i++ {
		...
		subRootTx.TurtleDexcoinInputs[i] = types.TurtleDexcoinInput{
			ParentID: nextRootTx.TurtleDexcoinOutputID(0),
		}
		bigRoots = append(bigRoots, nextRootTx)
		bigRootIDs[nextRootTx.TurtleDexcoinInputs[0].ParentID] = true
	}
