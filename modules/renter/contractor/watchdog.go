...
type watchdog struct {
	...
	// outputDependencies maps TurtleDexcoin outputs to the file contracts that are
	...
	// confirmed TurtleDexcoin outputs used to create the file contract transaction set.
	...
	outputDependencies map[types.TurtleDexcoinOutputID]map[types.FileContractID]struct{}
...
type fileContractStatus struct {
	...
	// on-chain are removed from this set. If a TurtleDexcoin output that this file
	...
	// parentOutputs stores TurtleDexcoinOutputID of outputs which this file contract is
	...
	parentOutputs map[types.TurtleDexcoinOutputID]struct{}
...
func newWatchdog(contractor *Contractor) *watchdog {
	...
	return &watchdog{
		...
		outputDependencies: make(map[types.TurtleDexcoinOutputID]map[types.FileContractID]struct{}),
...
func (w *watchdog) callMonitorContract(args monitorContractArgs) error {
	...
	if !args.recovered {
		...
		saneInputs = saneInputs && len(args.sweepTxn.TurtleDexcoinInputs) != 0
	...
	fileContractStatus := &fileContractStatus{
		...
		parentOutputs:        make(map[types.TurtleDexcoinOutputID]struct{}),
...
// addOutputDependency marks the contract with fcID as dependent on this TurtleDexcoin
// output.
func (w *watchdog) addOutputDependency(outputID types.TurtleDexcoinOutputID, fcID types.FileContractID) {
...
// removeOutputDependency removes the given TurtleDexcoinOutput from the dependencies
// of this file contract.
func (w *watchdog) removeOutputDependency(outputID types.TurtleDexcoinOutputID, fcID types.FileContractID) {
	dependentFCs, ok := w.outputDependencies[outputID]
	if !ok {
...
// getParentOutputIDs returns the IDs of the parent TurtleDexcoinOutputs used in the
// transaction set. That is, it returns the TurtleDexcoinOutputs that this transaction
// set is dependent on.
func getParentOutputIDs(txnSet []types.Transaction) []types.TurtleDexcoinOutputID {
	// Create a map of created and spent outputs. The parent outputs are those
	// that are spent but not created in this set.
	createdOutputs := make(map[types.TurtleDexcoinOutputID]bool)
	spentOutputs := make(map[types.TurtleDexcoinOutputID]bool)
	for _, txn := range txnSet {
		for _, scInput := range txn.TurtleDexcoinInputs {
			spentOutputs[scInput.ParentID] = true
		}
		for i := range txn.TurtleDexcoinOutputs {
			createdOutputs[txn.TurtleDexcoinOutputID(uint64(i))] = true
		}
	}

	// Remove all intermediary outputs that were created in the set from the set
	// of spentOutputs.
	parentOutputs := make([]types.TurtleDexcoinOutputID, 0)
...
func (w *watchdog) scanRevertedBlock(block types.Block) {
	...
	outputsCreatedInBlock := make(map[types.TurtleDexcoinOutputID]*types.Transaction)
	for i := 0; i < len(block.Transactions); i++ {
		txn := &block.Transactions[i]
		for i := range txn.TurtleDexcoinOutputs {
			outputsCreatedInBlock[txn.TurtleDexcoinOutputID(uint64(i))] = txn
		}
...
func (w *watchdog) updateDependenciesFromRevertedBlock(createdOutputs map[types.TurtleDexcoinOutputID]*types.Transaction) {
	// Create a queue of outputs to check.
	outputQueue := make([]types.TurtleDexcoinOutputID, 0)
	outputsInQueue := make(map[types.TurtleDexcoinOutputID]struct{})
	...
	var outputID types.TurtleDexcoinOutputID
...
func (w *watchdog) checkUnconfirmedContract(fcID types.FileContractID, contractData *fileContractStatus) {
	...
	for _, txn := range contractData.formationTxnSet {
		setSize += txn.MarshalTurtleDexSize()
	}
...
func (w *watchdog) sweepContractInputs(fcID types.FileContractID, contractData *fileContractStatus) {
	...
	setSize := contractData.sweepTxn.MarshalTurtleDexSize()
	for _, sweepParent := range contractData.sweepParents {
		setSize += sweepParent.MarshalTurtleDexSize()
	}
	...
	// sweep.
	numOuts := len(txn.TurtleDexcoinOutputs)
	if numOuts == 0 {
		w.contractor.log.Println("expected at least 1 output in sweepTxn", len(txn.TurtleDexcoinOutputs))
		return
	}
	replacementOutput := types.TurtleDexcoinOutput{
		Value:      txn.TurtleDexcoinOutputs[numOuts-1].Value.Sub(txnFee),
		UnlockHash: txn.TurtleDexcoinOutputs[numOuts-1].UnlockHash,
	}
	err = sweepBuilder.ReplaceTurtleDexcoinOutput(uint64(numOuts-1), replacementOutput)
	if err != nil {
