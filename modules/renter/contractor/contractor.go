...
func (emptyWorkerPool) Worker(_ types.TurtleDexPublicKey) (modules.Worker, error) {
...
func (c *Contractor) ContractPublicKey(pk types.TurtleDexPublicKey) (crypto.PublicKey, bool) {
	c.mu.RLock()
...
func (c *Contractor) PeriodSpending() (modules.ContractorSpending, error) {
	...
	for _, contract := range allContracts {
		...
		spending.ContractFees = spending.ContractFees.Add(contract.TurtleDexfundFee)
	...
	for _, contract := range c.oldContracts {
		...
		if contract.StartHeight >= c.currentPeriod {
			...
			spending.PreviousSpending = spending.PreviousSpending.Add(contract.ContractFee).Add(contract.TxnFee).
				Add(contract.TurtleDexfundFee).Add(contract.DownloadSpending).Add(contract.UploadSpending).Add(contract.StorageSpending)
		} else {
			// Calculate Previous spending
			spending.PreviousSpending = spending.PreviousSpending.Add(contract.ContractFee).Add(contract.TxnFee).
				Add(contract.TurtleDexfundFee).Add(contract.DownloadSpending).Add(contract.UploadSpending).Add(contract.StorageSpending)
...
func (c *Contractor) ProvidePayment(stream io.ReadWriter, host types.TurtleDexPublicKey, rpc types.Specifier, amount types.Currency, refundAccount modules.AccountID, blockHeight types.BlockHeight) error {
