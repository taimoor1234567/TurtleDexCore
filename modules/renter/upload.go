...
func (r *Renter) Upload(up modules.FileUploadParams) error {
	...
	if up.Force {
		err := r.DeleteFile(up.TurtleDexPath)
	...
	dirTurtleDexPath, err := up.TurtleDexPath.Dir()
	if err != nil {
		return err
	}
	...
	cipherKey := crypto.GenerateTurtleDexKey(up.CipherType)

	// Create the TurtleDexfile and add to renter
	err = r.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, cipherKey, uint64(sourceInfo.Size()), sourceInfo.Mode(), up.DisablePartialChunk)
	if err != nil {
		return errors.AddContext(err, "could not create a new sia file")
	}
	entry, err := r.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	if err != nil {
	...
	// Bubble the health of the TurtleDexFile directory to ensure the health is
