...
func TestRefreshPaths(t *testing.T) {
	...
	paths := []modules.TurtleDexPath{
		modules.RootTurtleDexPath(),
		{Path: ""},
		{Path: "root"},
		{Path: "root/SubDir1"},
	...
	uniquePaths := []modules.TurtleDexPath{
		{Path: "root/SubDir1/SubDir1/SubDir1"},
		{Path: "root/SubDir1/SubDir2"},
		{Path: "root/SubDir2/SubDir1"},
		{Path: "root/SubDir2/SubDir2/SubDir2"},
	...
	dirsToRefresh.mu.Unlock()
	parentPaths := []modules.TurtleDexPath{
		{Path: ""},
		{Path: "root"},
		{Path: "root/SubDir1"},
		{Path: "root/SubDir1/SubDir1"},
	...
	for _, sp := range uniquePaths {
		...
		up.TurtleDexPath, err = sp.Join("testFile")
		if err != nil {
			t.Fatal(err)
		}
	...
	di, err := rt.renter.DirList(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
