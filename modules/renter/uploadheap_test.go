...
func testManagedBuildUnfinishedChunks(t *testing.T) {
	...
	siaPath, err := modules.NewTurtleDexPath("stuckFile")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 10e3, persist.DefaultDiskPermissionsTest, false)
	if err != nil {
		t.Fatal(err)
	}
...
func testManagedBuildChunkHeap(t *testing.T) {
	...
	f1, err := rt.renter.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	rt.renter.managedBuildChunkHeap(modules.RootTurtleDexPath(), hosts, targetUnstuckChunks, offline, goodForRenew)
...
func addChunksOfDifferentHealth(r *Renter, numChunks int, priority, fileRecentlySuccessful, stuck, remote bool) error {
	var UID siafile.TurtleDexfileUID
	if priority {
		UID = "priority"
...
func testManagedAddChunksToHeap(t *testing.T) {
	...
	dirTurtleDexPaths := rt.renter.newUniqueRefreshPaths()
	names := []string{"rootFile", "subdir/File", "subdir2/file"}
	for _, name := range names {
		siaPath, err := modules.NewTurtleDexPath(name)
		if err != nil {
			t.Fatal(err)
		}
		up.TurtleDexPath = siaPath
		...
		err = rt.renter.CreateDir(dirTurtleDexPath, modules.DefaultDirPerm)
		if err != nil && !errors.Contains(err, filesystem.ErrExists) {
			t.Fatal(err)
		}
		err = dirTurtleDexPaths.callAdd(dirTurtleDexPath)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	err = dirTurtleDexPaths.callRefreshAllBlocking()
	if err != nil {
		t.Fatal(err)
	}
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		rd, err := rt.renter.DirList(modules.RootTurtleDexPath())
		if err != nil {
			return err
		}
		if rd[0].Health == 0 {
...
func testAddRemoteChunksToHeap(t *testing.T) {
	...
	for _, name := range names {
		// Create the TurtleDexPath for the file
		siaPath, err := modules.NewTurtleDexPath(name)
		if err != nil {
			t.Fatal(err)
		}
		...
		dirTurtleDexPaths.callAdd(dirTurtleDexPath)
	}
...
func testAddDirectoryBackToHeap(t *testing.T) {
	...
	// Create file
	rsc, _ := modules.NewRSCode(1, 1)
	siaPath, err := modules.NewTurtleDexPath("test")
	if err != nil {
		t.Fatal(err)
	}
	source, err := rt.createZeroByteFileOnDisk()
	...
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), modules.SectorSize, persist.DefaultDiskPermissionsTest, false)
	if err != nil {
		t.Fatal(err)
	}
	f, err := rt.renter.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	...
	if !d.staticTurtleDexPath.Equals(modules.RootTurtleDexPath()) {
		t.Fatal("Expected Directory siapath to be the root siaPath but was", d.staticTurtleDexPath.String())
	}
...
func testUploadHeapMaps(t *testing.T) {
	...
	for i := uint64(0); i < numHeapChunks; i++ {
		...
		chunk := &unfinishedUploadChunk{
			id: uploadChunkID{
				fileUID: siafile.TurtleDexfileUID(fmt.Sprintf("chunk - %v", i)),
				index:   i,
			},
			fileEntry:                 sf.Copy(),
			stuck:                     stuck,
...
func testChunkSwitchStuckStatus(t *testing.T) {
	...
	chunk := &unfinishedUploadChunk{
		id: uploadChunkID{
			fileUID: siafile.TurtleDexfileUID("chunk"),
			index:   0,
		},
		staticMemoryManager: rt.renter.repairMemoryManager,
	}
...
func testManagedTryUpdate(t *testing.T) {
	...
	for i, test := range tests {
		...
		existingChunk := &unfinishedUploadChunk{
			id: uploadChunkID{
				fileUID: siafile.TurtleDexfileUID(test.name),
				index:   uint64(i),
			},
