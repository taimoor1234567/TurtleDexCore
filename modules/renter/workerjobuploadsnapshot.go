...
func (j *jobUploadSnapshot) callExecute() {
	...
	err = w.renter.managedUploadSnapshotHost(meta, j.staticTurtleDexFileData, sess, w)
	if err != nil {
...
func (r *Renter) managedUploadSnapshotHost(meta modules.UploadedBackup, dotTurtleDex []byte, host contractor.Session, w *worker) error {
	// Get the wallet seed.
	...
	var sectors [][]byte
	for buf := bytes.NewBuffer(dotTurtleDex); buf.Len() > 0; {
...
func (w *worker) UploadSnapshot(ctx context.Context, meta modules.UploadedBackup, dotTurtleDex []byte) error {
