...
func (r *Renter) managedAddRandomStuckChunks(hosts map[string]struct{}) ([]modules.TurtleDexPath, error) {
	var dirTurtleDexPaths []modules.TurtleDexPath
	...
	for i := 0; i < maxRandomStuckChunksAddToHeap && spaceInHeap; i++ {
		...
		dirTurtleDexPath, err := r.managedStuckDirectory()
		if err != nil {
			return dirTurtleDexPaths, errors.AddContext(err, "unable to get random stuck directory")
		}

		// Get Random stuck file from directory
		siaPath, err := r.managedStuckFile(dirTurtleDexPath)
		if err != nil {
			return dirTurtleDexPaths, errors.AddContext(err, "unable to get random stuck file in dir "+dirTurtleDexPath.String())
		}
		...
		if err != nil {
			return dirTurtleDexPaths, errors.AddContext(err, "unable to push random stuck chunk from '"+siaPath.String()+"' of '"+dirTurtleDexPath.String()+"'")
		}
		...
		// the iteration
		dirTurtleDexPaths = append(dirTurtleDexPaths, dirTurtleDexPath)
		r.repairLog.Printf("Added %v stuck chunks from %s", currentNumRandomStuckChunks-prevNumRandomStuckChunks, dirTurtleDexPath.String())
		prevNumStuckChunks = currentNumStuckChunks
		prevNumRandomStuckChunks = currentNumRandomStuckChunks
		spaceInHeap = prevNumRandomStuckChunks < maxRandomStuckChunksInHeap && prevNumStuckChunks < maxStuckChunksInHeap
	}
	return dirTurtleDexPaths, nil
}
...
// stuck stack.
func (r *Renter) managedAddStuckChunksFromStuckStack(hosts map[string]struct{}) ([]modules.TurtleDexPath, error) {
	var dirTurtleDexPaths []modules.TurtleDexPath
	...
	for r.stuckStack.managedLen() > 0 && numStuckChunks < maxStuckChunksInHeap {
		// Pop the first file TurtleDexPath
		...
		if err != nil && !errors.Contains(err, errNoStuckChunks) {
			return dirTurtleDexPaths, errors.AddContext(err, "unable to add stuck chunks to heap")
		}
		...
		if err != nil {
			return dirTurtleDexPaths, errors.AddContext(err, "unable to get directory siapath")
		}
		dirTurtleDexPaths = append(dirTurtleDexPaths, dirTurtleDexPath)
		numStuckChunks, _ = r.uploadHeap.managedNumStuckChunks()
	}
...
func (r *Renter) managedAddStuckChunksToHeap(siaPath modules.TurtleDexPath, hosts map[string]struct{}, offline, goodForRenew map[string]bool) (err error) {
	// Open File
	sf, err := r.staticFileSystem.OpenTurtleDexFile(siaPath)
	if err != nil {
...
func (r *Renter) managedOldestHealthCheckTime() (modules.TurtleDexPath, time.Time, error) {
	// Check the ttdxdir metadata for the root files directory
	siaPath := modules.RootTurtleDexPath()
	metadata, err := r.managedDirectoryMetadata(siaPath)
	if err != nil {
		return modules.TurtleDexPath{}, time.Time{}, err
	}
	...
	for (metadata.AggregateNumSubDirs > healthLoopNumBatchSubDirs || metadata.AggregateNumFiles > healthLoopNumBatchFiles) && metadata.NumSubDirs > 0 {
		...
		select {
		case <-r.tg.StopChan():
			return modules.TurtleDexPath{}, time.Time{}, errors.New("Renter shutdown before oldestHealthCheckTime could be found")
		default:
		}

		// Check for sub directories
		subDirTurtleDexPaths, err := r.managedSubDirectories(siaPath)
		if err != nil {
			return modules.TurtleDexPath{}, time.Time{}, err
		}
		...
		updated := false
		for _, subDirPath := range subDirTurtleDexPaths {
			// Check to make sure renter hasn't been shutdown
			select {
			case <-r.tg.StopChan():
				return modules.TurtleDexPath{}, time.Time{}, errors.New("Renter shutdown before oldestHealthCheckTime could be found")
...
func (r *Renter) managedStuckDirectory() (modules.TurtleDexPath, error) {
	...
	for {
		...
		// Check to make sure renter hasn't been shutdown
		case <-r.tg.StopChan():
			return modules.TurtleDexPath{}, nil
		default:
		}

		directories, err := r.managedDirList(siaPath)
		if err != nil {
			return modules.TurtleDexPath{}, err
		}
		...
		for i := len(directories) - 1; i >= 0; i-- {
			...
			if i == 0 {
				siaPath = directories[0].TurtleDexPath
				return siaPath, nil
			}
			...
			siaPath = directories[i].TurtleDexPath
...
func (r *Renter) managedStuckFile(dirTurtleDexPath modules.TurtleDexPath) (siapath modules.TurtleDexPath, err error) {
	...
	siaDir, err := r.staticFileSystem.OpenTurtleDexDir(dirTurtleDexPath)
	if err != nil {
		return modules.TurtleDexPath{}, errors.AddContext(err, "unable to open siaDir "+dirTurtleDexPath.String())
	}
	defer func() {
	...
	if aggregateNumStuckChunks == 0 || numStuckChunks == 0 || numFiles == 0 {
		...
		err = fmt.Errorf("managedStuckFile should not have been called on %v, AggregateNumStuckChunks: %v, NumStuckChunks: %v, NumFiles: %v", dirTurtleDexPath.String(), aggregateNumStuckChunks, numStuckChunks, numFiles)
		return modules.TurtleDexPath{}, err
	}
	...
	// unless we need to
	fileinfos, err := r.staticFileSystem.ReadDir(dirTurtleDexPath)
	if err != nil {
		return modules.TurtleDexPath{}, errors.AddContext(err, "unable to open ttdxdir: "+dirTurtleDexPath.String())
	}
	// Iterate over the fileinfos
	for _, fi := range fileinfos {
		// Check for TurtleDexFile
		if fi.IsDir() || filepath.Ext(fi.Name()) != modules.TurtleDexFileExtension {
			continue
		}

		// Get TurtleDexPath
		sp, err := dirTurtleDexPath.Join(strings.TrimSuffix(fi.Name(), modules.TurtleDexFileExtension))
		if err != nil {
			return modules.TurtleDexPath{}, errors.AddContext(err, "unable to join the siapath with the file: "+fi.Name())
		}

		// Open TurtleDexFile, grab the number of stuck chunks and close the file
		f, err := r.staticFileSystem.OpenTurtleDexFile(sp)
		if err != nil {
			return modules.TurtleDexPath{}, errors.AddContext(err, "could not open siafileset for "+sp.String())
		}
		numStuckChunks := int(f.NumStuckChunks())
		if err := f.Close(); err != nil {
			return modules.TurtleDexPath{}, errors.AddContext(err, "failed to close filenode "+sp.String())
		}
	...
	if siapath.IsEmpty() {
		...
		go r.callThreadedBubbleMetadata(dirTurtleDexPath)
		return modules.TurtleDexPath{}, errors.New("no files selected from directory " + dirTurtleDexPath.String())
	}
	return siapath, nil
}

// managedSubDirectories reads a directory and returns a slice of all the sub
// directory TurtleDexPaths
func (r *Renter) managedSubDirectories(siaPath modules.TurtleDexPath) ([]modules.TurtleDexPath, error) {
	// Read directory
	fileinfos, err := r.staticFileSystem.ReadDir(siaPath)
	if err != nil {
		return nil, err
	}
	// Find all sub directory TurtleDexPaths
	folders := make([]modules.TurtleDexPath, 0, len(fileinfos))
	for _, fi := range fileinfos {
		if fi.IsDir() {
...
func (r *Renter) threadedStuckFileLoop() {
	...
	for {
		...
		// directory.
		stuckStackDirTurtleDexPaths, err := r.managedAddStuckChunksFromStuckStack(hosts)
		if err != nil {
		...
		dirTurtleDexPaths = append(dirTurtleDexPaths, stuckStackDirTurtleDexPaths...)

		// Try add random stuck chunks to upload heap
		randomDirTurtleDexPaths, err := r.managedAddRandomStuckChunks(hosts)
		if err != nil {
			r.repairLog.Println("WARN: error adding random stuck chunks to upload heap:", err)
		}
		dirTurtleDexPaths = append(dirTurtleDexPaths, randomDirTurtleDexPaths...)
		...
		for _, dirTurtleDexPath := range dirTurtleDexPaths {
			err = bubblePaths.callAdd(dirTurtleDexPath)
			if err != nil {
				r.repairLog.Printf("Error adding refresh path of %s: %v", dirTurtleDexPath.String(), err)
			}
		}
		err = bubblePaths.callRefreshAllBlocking()
		if err != nil {
			r.repairLog.Print("Error bubbling dirTurtleDexPaths", err)
			select {
...
func (r *Renter) managedPrepareForBubble(rootDir modules.TurtleDexPath, force bool) (*uniqueRefreshPaths, error) {
	// Initiate helpers
	...
	dlf := func(di modules.DirectoryInfo) {
		...
		addErr := urp.callAdd(di.TurtleDexPath)
		if addErr != nil {
			r.log.Printf("WARN: unable to add siapath `%v` to uniqueRefreshPaths; err: %v", di.TurtleDexPath, addErr)
			err = errors.Compose(err, addErr)
			return
		}
		// Update files in the directory.
		updateErr := r.managedUpdateFileMetadatasParams(di.TurtleDexPath, offlineMap, goodForRenewMap, contracts, used)
		if updateErr != nil {
			r.log.Println("Error calling managedUpdateFileMetadatas on `", di.TurtleDexPath, "`:", updateErr)
	...
	entry, openErr := r.staticFileSystem.OpenTurtleDexDir(rootDir)
	if openErr != nil {
...
// happen sparingly.
func (r *Renter) managedUpdateFileMetadatas(dirTurtleDexPath modules.TurtleDexPath) error {
	// Get cached offline and goodforrenew maps.
	offlineMap, goodForRenewMap, contracts, used := r.managedRenterContractsAndUtilities()
	return r.managedUpdateFileMetadatasParams(dirTurtleDexPath, offlineMap, goodForRenewMap, contracts, used)
}
...
func (r *Renter) managedUpdateFileMetadatasParams(dirTurtleDexPath modules.TurtleDexPath, offlineMap map[string]bool, goodForRenewMap map[string]bool, contracts map[string]modules.RenterContract, used []types.TurtleDexPublicKey) error {
	fis, err := r.staticFileSystem.ReadDir(dirTurtleDexPath)
	if err != nil {
	...
	for _, fi := range fis {
		...
		if ext == modules.TurtleDexFileExtension {
			fName := strings.TrimSuffix(fi.Name(), modules.TurtleDexFileExtension)
			fileTurtleDexPath, err := dirTurtleDexPath.Join(fName)
			if err != nil {
			...
			// Update the file.
			err = func() error {
				sf, err := r.staticFileSystem.OpenTurtleDexFile(fileTurtleDexPath)
				if err != nil {
					return err
				}
...
func (r *Renter) managedUpdateFileMetadata(sf *filesystem.FileNode, offlineMap, goodForRenew map[string]bool, contracts map[string]modules.RenterContract, used []types.TurtleDexPublicKey) (err error) {
