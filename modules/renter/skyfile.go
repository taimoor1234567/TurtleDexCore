...
// in a single sector which is put on the TurtleDex network using 1-of-N redundancy.
...
import (
	...
	"github.com/turtledex/TurtleDexCore/fixtures"
...
func fileUploadParams(siaPath modules.TurtleDexPath, dataPieces, parityPieces int, force bool, ct crypto.CipherType) (modules.FileUploadParams, error) {
...
func baseSectorUploadParamsFromSUP(sup modules.SkyfileUploadParameters) (modules.FileUploadParams, error) {
	...
	return fileUploadParams(sup.TurtleDexPath, 1, int(sup.BaseChunkRedundancy)-1, sup.Force, crypto.TypePlain)
}
...
// CreateSkylinkFromTurtleDexfile creates a skyfile from a siafile. This requires
// uploading a new skyfile which contains fanout information pointing to the
// siafile data. The TurtleDexPath provided in 'sup' indicates where the new base
...
func (r *Renter) CreateSkylinkFromTurtleDexfile(sup modules.SkyfileUploadParameters, siaPath modules.TurtleDexPath) (_ modules.Skylink, err error) {
	// Encryption is not supported for TurtleDexFile conversion.
	...
	fileNode, err := r.staticFileSystem.OpenTurtleDexFile(siaPath)
	if err != nil {
...
func (r *Renter) managedCreateSkylinkFromFileNode(sup modules.SkyfileUploadParameters, skyfileMetadata modules.SkyfileMetadata, fileNode *filesystem.FileNode, fanoutReader io.Reader) (modules.Skylink, error) {
	...
	if r.isFileNodeBlocked(fileNode) {
		...
		return modules.Skylink{}, errors.Compose(ErrSkylinkBlocked, r.DeleteFile(sup.TurtleDexPath))
	}
...
func (r *Renter) managedCreateFileNodeFromReader(up modules.FileUploadParams, reader io.Reader) (*filesystem.FileNode, error) {
	...
	hpk := types.TurtleDexPublicKey{} // blank host key
	ec := fileNode.ErasureCode()
	...
	for chunkIndex := uint64(0); ; chunkIndex++ {
		// Grow the TurtleDexFile to the right size.
		err := fileNode.TurtleDexFile.GrowNumChunks(chunkIndex + 1)
		if err != nil {
			return nil, err
		}
		...
		err = func() (err error) {
			...
			for pieceIndex, dataPieceEnc := range dataEncoded {
				if err := fileNode.TurtleDexFile.AddPiece(hpk, chunkIndex, uint64(pieceIndex), crypto.MerkleRoot(dataPieceEnc)); err != nil {
					return err
				}
			}
...
func (r *Renter) managedUploadBaseSector(sup modules.SkyfileUploadParameters, baseSector []byte, skylink modules.Skylink) (err error) {
	...
	// Add the skylink to the TurtleDexfile.
	err = fileNode.AddSkylink(skylink)
...
// leading chunk of a skyfile to the TurtleDex network and returns the skylink that
...
// data to a large siafile and upload it to the TurtleDex network using
// 'callUploadStreamFromReader'. The final skylink is created by calling
// 'CreateSkylinkFromTurtleDexfile' on the resulting siafile.
func (r *Renter) managedUploadSkyfileLargeFile(sup modules.SkyfileUploadParameters, fileReader modules.SkyfileUploadReader) (modules.Skylink, error) {
	...
	siaPath, err := modules.NewTurtleDexPath(sup.TurtleDexPath.String() + modules.ExtendedSuffix)
	if err != nil {
		return modules.Skylink{}, errors.AddContext(err, "unable to create TurtleDexPath for large skyfile extended data")
	}
...
func (r *Renter) PinSkylink(skylink modules.Skylink, lup modules.SkyfileUploadParameters, timeout time.Duration, pricePerMS types.Currency) error {
	...
	// Create the siapath for the skyfile extra data. This is going to be the
	// same as the skyfile upload siapath, except with a suffix.
	fup.TurtleDexPath, err = modules.NewTurtleDexPath(lup.TurtleDexPath.String() + modules.ExtendedSuffix)
	if err != nil {
		return errors.AddContext(err, "unable to create TurtleDexPath for large skyfile extended data")
	}
...
func (r *Renter) RestoreSkyfile(reader io.Reader) (modules.Skylink, error) {
	...
	extendedPath, err := modules.NewTurtleDexPath(sup.TurtleDexPath.String() + modules.ExtendedSuffix)
	if err != nil {
	...
	if sl.CipherType == crypto.TypeThreefish {
		// For converted files we need to generate a TurtleDexKey
		fup.CipherKey, err = crypto.NewTurtleDexKey(sl.CipherType, sl.KeyData[:])
		if err != nil {
	...
	if err != nil {
		...
		return modules.Skylink{}, errors.Compose(err, r.DeleteFile(sup.TurtleDexPath))
	}
...
func (r *Renter) UploadSkyfile(sup modules.SkyfileUploadParameters, reader modules.SkyfileUploadReader) (skylink modules.Skylink, err error) {
	...
	defer func() {
		if err != nil || sup.DryRun {
			if err := r.DeleteFile(sup.TurtleDexPath); err != nil && !errors.Contains(err, filesystem.ErrNotExist) {
				r.log.Printf("error deleting siafile after upload error: %v", err)
			}

			extendedPath := sup.TurtleDexPath.String() + modules.ExtendedSuffix
			extendedTurtleDexPath, _ := modules.NewTurtleDexPath(extendedPath)
			if err := r.DeleteFile(extendedTurtleDexPath); err != nil && !errors.Contains(err, filesystem.ErrNotExist) {
...
func (r *Renter) isFileNodeBlocked(fileNode *filesystem.FileNode) bool {
	...
	for _, skylinkstr := range skylinkstrs {
		...
		if err != nil {
			...
			r.log.Printf("WARN: previous skylink for siafile %v could not be loaded from string; potentially corrupt skylink: %v", fileNode.TurtleDexFilePath(), skylinkstr)
			continue
		}
		// Check if skylink is blocked
		if r.staticSkynetBlocklist.IsBlocked(skylink) {
