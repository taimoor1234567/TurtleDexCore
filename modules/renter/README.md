...
that a user has uploaded to TurtleDex. This includes the location and health of these
...
no logspam and no messages that would only make sense to ttdxd developers.
...
downloading data from TurtleDex hosts.
...
leverages a testgroup with a renter, a miner, and several hosts to mimic the TurtleDex
...
**Outbound Complexities**
 - callUploadStreamFromReader is used to upload new data to the TurtleDex network when
...
download or partially download a sector from the TurtleDex network knowing only the
...
There are 3 main functions that work together to make up TurtleDex's file repair
...
that timestamp and the TurtleDexPath of the directory.
...
the TurtleDexPath of the TurtleDexFile it came from is added to the Renter's `stuckStack`
...
number of TurtleDexFiles that have had stuck chunks successfully repaired in a LIFO
stack. If the LIFO stack already has `maxSuccessfulStuckRepairFiles` in it, when
a new TurtleDexFile is pushed onto the stack the oldest TurtleDexFile is dropped from the
stack so the new TurtleDexFile can be added. Additionally, if TurtleDexFile is being added
that is already being tracked, then the original reference is removed and the
TurtleDexFile is added to the top of the Stack. If there have been successful stuck
...
since all the chunks in a TurtleDexFile have the same redundancy settings and were
