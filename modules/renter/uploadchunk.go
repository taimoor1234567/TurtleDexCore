...
type uploadChunkID struct {
	fileUID siafile.TurtleDexfileUID // Unique to each file.
...
type unfinishedUploadChunk struct {
	...
	staticAvailableChan       chan struct{} // used to signal that the chunk is available on the TurtleDex network. Error needs to be checked.
	staticUploadCompletedChan chan struct{} // used to signal that the chunk has finished uploading to the TurtleDex network. Error needs to be checked.
...
// staticAvailable returns whether or not the chunk is available yet on the TurtleDex
// network.
...
func (r *Renter) managedDownloadLogicalChunkData(chunk *unfinishedUploadChunk) error {
	...
	// Prepare snapshot.
	snap, err := chunk.fileEntry.SnapshotRange(r.staticFileSystem.FileTurtleDexPath(chunk.fileEntry), uint64(chunk.offset), downloadLength)
	if err != nil {
		return err
	}
	...
	d.OnComplete(func(_ error) error {
		...
		return chunk.fileEntry.TurtleDexFile.UpdateAccessTime()
	})
...
func (r *Renter) threadedFetchAndRepairChunk(chunk *unfinishedUploadChunk) {
	...
	if err != nil {
		...
		// data.
		err := fmt.Errorf("Unable to fetch the logical data for chunk %v of %s - marking as stuck: %v", chunk.staticIndex, chunk.staticTurtleDexPath, err)
		chunk.err = err
		chunk.mu.Unlock()
		...
		// If TurtleDex is not currently online, the chunk doesn't need to be marked
		// as stuck.
		if !r.g.Online() {
			return
		}
		...
		if err != nil {
			r.repairLog.Printf("Error marking chunk %v of file %s as stuck: %v", chunk.staticIndex, chunk.staticTurtleDexPath, err)
		}
		return
	}
...
func (r *Renter) managedFetchLogicalChunkData(uc *unfinishedUploadChunk) error {
	...
	err := func() error {
		...
		if os.IsNotExist(err) {
			...
			r.log.Println("WARN: local file not found on disk, setting localpath to '' to avoid corruption for", uc.fileEntry.TurtleDexFilePath())
...
func (r *Renter) managedCleanUpUploadChunk(uc *unfinishedUploadChunk) {
	...
	if chunkComplete && !released {
		if uc.piecesCompleted >= uc.staticPiecesNeeded {
			r.repairLog.Printf("Completed repair for chunk %v of %s, %v pieces were completed out of %v", uc.staticIndex, uc.staticTurtleDexPath, uc.piecesCompleted, uc.staticPiecesNeeded)
		} else {
			r.repairLog.Printf("Repair of chunk %v of %s was unsuccessful, %v pieces were completed out of %v", uc.staticIndex, uc.staticTurtleDexPath, uc.piecesCompleted, uc.staticPiecesNeeded)
		}
	...
	if chunkComplete && !released {
		...
		if !r.deps.Disrupt("disableCloseUploadEntry") {
			...
			if err != nil {
				r.log.Println("WARN: unable to close file entry for chunk", uc.fileEntry.TurtleDexFilePath())
			}
		}
...
func (r *Renter) managedSetStuckAndClose(uc *unfinishedUploadChunk, stuck bool) error {
	...
	if err != nil {
		return fmt.Errorf("WARN: unable to update chunk stuck status for file and close it %v: %v", uc.fileEntry.TurtleDexFilePath(), err)
	}
	return nil
}
...
func (r *Renter) managedUpdateUploadChunkStuckStatus(uc *unfinishedUploadChunk) {
	...
	if !r.deps.Disrupt("IgnoreFailedRepairs") || successfulRepair {
		if err := uc.fileEntry.SetStuck(index, !successfulRepair); err != nil {
			r.log.Printf("WARN: could not set chunk %v stuck status for file %v: %v", uc.id, uc.fileEntry.TurtleDexFilePath(), err)
		}
	}
	...
	if stuck && successfulRepair && stuckRepair {
		...
		if uc.fileEntry.NumStuckChunks() > 0 {
			r.stuckStack.managedPush(r.staticFileSystem.FileTurtleDexPath(uc.fileEntry))
		}
