...
func (w *worker) managedDropUploadChunks() {
	...
	for chunkToDrop := chunksToDrop.Pop(); chunkToDrop != nil; chunkToDrop = chunksToDrop.Pop() {
		...
		w.renter.repairLog.Debugf("dropping chunk %v of %s because the worker is dropping all chunks", chunkToDrop.staticIndex, chunkToDrop.staticTurtleDexPath)
	}
}
...
func (w *worker) managedPerformUploadChunkJob() {
	...
	if err != nil {
		failureErr := fmt.Errorf("Worker failed to add new piece to TurtleDexFile: %v", err)
		w.managedUploadFailed(uc, pieceIndex, failureErr)
		return
	}
...
func (w *worker) managedUploadFailed(uc *unfinishedUploadChunk, pieceIndex uint64, failureErr error) {
	w.renter.repairLog.Printf("Worker upload failed. Worker: %v, Chunk: %v of %s, Error: %v", w.staticHostPubKey, uc.staticIndex, uc.staticTurtleDexPath, failureErr)
