...
// MarshalTurtleDex implements the encoding.TurtleDexMarshaller interface, writing the
// file data to w.
func (f *file) MarshalTurtleDex(w io.Writer) error {
...
func (f *file) UnmarshalTurtleDex(r io.Reader) error {
...
// loadTurtleDexFiles walks through the directory searching for siafiles and loading
// them into memory.
func (r *Renter) compatV137ConvertTurtleDexFiles(tracking map[string]v137TrackedFile, oldContracts []modules.RenterContract) error {
	...
	err := filepath.Walk(r.persistDir, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() || filepath.Ext(path) != modules.TurtleDexFileExtension {
			return nil
		}

		// Check if file was already converted.
		_, err = siafile.LoadTurtleDexFile(path, r.wal)
		if err == nil {
			return nil
		}
		...
		_, err = r.compatV137loadTurtleDexFilesFromReader(file, tracking, oldContracts)
		if err != nil {
...
// v137FileToTurtleDexFile converts a legacy file to a TurtleDexFile. Fields that can't be
// populated using the legacy file remain blank.
func (r *Renter) v137FileToTurtleDexFile(f *file, repairPath string, oldContracts []modules.RenterContract) (*filesystem.FileNode, error) {
	// Create a mapping of contract ids to host keys.
	contracts := r.hostContractor.Contracts()
	idToPk := make(map[types.FileContractID]types.TurtleDexPublicKey)
	...
	fileData := siafile.FileData{
		...
		Deleted:     f.deleted,
		UID:         siafile.TurtleDexfileUID(f.staticUID),
	}
	...
	return r.staticFileSystem.NewTurtleDexFileFromLegacyData(fileData)
}

// compatV137LoadTurtleDexFilesFromReader reads .sia data from reader and registers
// the contained files in the renter. It returns the nicknames of the loaded
// files.
func (r *Renter) compatV137loadTurtleDexFilesFromReader(reader io.Reader, tracking map[string]v137TrackedFile, oldContracts []modules.RenterContract) ([]string, error) {
	// read header
	var header [15]byte
	var version string
	var numFiles uint64
	...
	for i := range files {
		...
		for {
			siaPath, err := modules.NewTurtleDexPath(files[i].name)
			if err != nil {
				return nil, err
			}
	...
	for i, f := range files {
		...
		// v137FileToTurtleDexFile adds siafile to the TurtleDexFileSet so it does not need to
		// be returned here
		entry, err := r.v137FileToTurtleDexFile(f, repairPath, oldContracts)
		if err != nil {
...
func (r *Renter) convertPersistVersionFrom140To142(path string) error {
	...
	newHomePath := modules.HomeFolder.TurtleDexDirSysPath(fsRoot)
	newTurtleDexFilesPath := modules.UserFolder.TurtleDexDirSysPath(fsRoot)
	newSnapshotsPath := modules.BackupFolder.TurtleDexDirSysPath(fsRoot)
	if err := os.MkdirAll(newHomePath, 0700); err != nil {
		return errors.AddContext(err, "failed to create new home dir")
	}
	if err := os.Rename(filepath.Join(r.persistDir, "siafiles"), newTurtleDexFilesPath); err != nil && !os.IsNotExist(err) {
...
// version, converting legacy TurtleDexFiles in the process.
func (r *Renter) convertPersistVersionFrom133To140(path string, oldContracts []modules.RenterContract) error {
	...
	// Load potential legacy TurtleDexFiles.
	if err := r.compatV137ConvertTurtleDexFiles(p.Tracking, oldContracts); err != nil {
