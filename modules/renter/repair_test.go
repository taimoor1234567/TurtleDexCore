...
// equalBubbledMetadata is a helper that checks for equality in the ttdxdir
// metadata that gets bubbled
//
// Since we can't check timestamps for equality cause they are often set to
// `time.Now()` by methods, we allow a timestamp to be off by a certain delta.
func equalBubbledMetadata(md1, md2 ttdxdir.Metadata, delta time.Duration) error {
...
// aggregate ttdxdir metadata fields that gets bubbled
//
// Since we can't check timestamps for equality cause they are often set to
// `time.Now()` by methods, we allow a timestamp to be off by a certain delta.
func equalBubbledAggregateMetadata(md1, md2 ttdxdir.Metadata, delta time.Duration) error {
...
// non-aggregate ttdxdir metadata fields that gets bubbled
//
// Since we can't check timestamps for equality cause they are often set to
// `time.Now()` by methods, we allow a timestamp to be off by a certain delta.
func equalBubbledDirectoryMetadata(md1, md2 ttdxdir.Metadata, delta time.Duration) error {
	// Check Health
...
// openAndUpdateDir is a helper method for updating a ttdxdir metadata
func (rt *renterTester) openAndUpdateDir(siapath modules.TurtleDexPath, metadata ttdxdir.Metadata) error {
	ttdxdir, err := rt.renter.staticFileSystem.OpenTurtleDexDir(siapath)
	if err != nil {
		return err
	}
	err = ttdxdir.UpdateMetadata(metadata)
	return errors.Compose(err, ttdxdir.Close())
}
...
func TestBubbleHealth(t *testing.T) {
	...
	defaultMetadata := ttdxdir.Metadata{
	...
	err = build.Retry(20, time.Second, func() error {
		...
		metadata, err := rt.renter.managedDirectoryMetadata(modules.RootTurtleDexPath())
		if err != nil {
			return err
		}
	...
	subDir2, err := modules.NewTurtleDexPath("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkTime := time.Now()
	metadataUpdate := ttdxdir.Metadata{
		AggregateHealth:              1,
		Health:                       1,
		StuckHealth:                  0,
	...
	if err := rt.openAndUpdateDir(modules.RootTurtleDexPath(), metadataUpdate); err != nil {
		t.Fatal(err)
	}
	siaPath = subDir1
	...
	// Define test helper
	bubbleAndVerifyMetadata := func(testCase string, dirToBubble, expectedMDDir modules.TurtleDexPath, anf, ansd uint64) {
		...
		var expectedMetadata, metadata ttdxdir.Metadata
	...
	fileTurtleDexPath, err := subDir1_2.Join("test")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 100, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	if err != nil {
		t.Fatal(err)
	}
	f, err := rt.renter.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestOldestHealthCheckTime(t *testing.T) {
	...
	for _, dir := range directories {
		// Create Directory
		dirTurtleDexPath := newTurtleDexPath(dir)
		if err := rt.renter.CreateDir(dirTurtleDexPath, modules.DefaultDirPerm); err != nil {
			t.Fatal(err)
		}
		err = rt.openAndUpdateDir(dirTurtleDexPath, nowMD)
		if err != nil {
			t.Fatal(err)
		}
		// Put two files in the directory
		for i := 0; i < 2; i++ {
			fileTurtleDexPath, err := dirTurtleDexPath.Join(persist.RandomSuffix())
			if err != nil {
				t.Fatal(err)
			}
			sf, err := rt.renter.createRenterTestFile(fileTurtleDexPath)
			if err != nil {
				t.Fatal(err)
			}
	...
	err1 := rt.openAndUpdateDir(modules.RootTurtleDexPath(), nowMD)
	...
	oldestHealthCheckUpdate := ttdxdir.Metadata{
		Health:                       1,
		StuckHealth:                  0,
		AggregateLastHealthCheckTime: oldestCheckTime,
		LastHealthCheckTime:          oldestCheckTime,
	}
	subDir1_2 := newTurtleDexPath("root/SubDir1/SubDir2")
	...
	subDir1 := newTurtleDexPath("root/SubDir1")
	...
	for i := uint64(0); i < healthLoopNumBatchFiles; i++ {
		fileTurtleDexPath, err := subDir1.Join(persist.RandomSuffix())
		if err != nil {
			t.Fatal(err)
		}
		sf, err := rt.renter.createRenterTestFile(fileTurtleDexPath)
	...
	entry, err := rt.renter.staticFileSystem.OpenTurtleDexDir(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
...
func TestNumFiles(t *testing.T) {
	...
	up := modules.FileUploadParams{
		Source:      "",
		TurtleDexPath:     modules.RandomTurtleDexPath(),
		ErasureCode: rsc,
	}
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 100, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	if err != nil {
	...
	// Add file to subdir
	up.TurtleDexPath, err = subDir1_2.Join(hex.EncodeToString(fastrand.Bytes(8)))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 100, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	...
	up.TurtleDexPath, err = modules.SkynetFolder.Join(hex.EncodeToString(fastrand.Bytes(8)))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 100, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		dirInfo, err := rt.renter.staticFileSystem.DirInfo(modules.RootTurtleDexPath())
		if err != nil {
			return err
		}
		// TurtleDexfiles
...
func TestDirectorySize(t *testing.T) {
	...
	fileSize := uint64(100)
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), fileSize, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDirectoryModTime(t *testing.T) {
	...
	// static before we create the TurtleDexFiles to be able to ensure the ModTimes of
	// the TurtleDexFiles are the most recent
	time.Sleep(time.Second)

	// Add files
	sp1 := modules.RandomTurtleDexPath()
	rsc, _ := modules.NewRSCode(1, 1)
	up := modules.FileUploadParams{
		Source:      "",
		TurtleDexPath:     sp1,
	...
	up.TurtleDexPath = sp2
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), fileSize, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
	if err != nil {
		t.Fatal(err)
	}
	f1, err := rt.renter.staticFileSystem.OpenTurtleDexFile(sp1)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	f2, err := rt.renter.staticFileSystem.OpenTurtleDexFile(sp2)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestRandomStuckDirectory(t *testing.T) {
	...
	err = rt.renter.SetFileStuck(up.TurtleDexPath, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	var count1_2, countRoot, countTurtleDexFiles int
	for i := 0; i < 100; i++ {
		...
		if dir.Equals(modules.RootTurtleDexPath()) {
			countRoot++
			continue
		}
...
func TestRandomStuckFile(t *testing.T) {
	...
	siaPath1 := rt.renter.staticFileSystem.FileTurtleDexPath(file1)
	...
	siaPath2 := rt.renter.staticFileSystem.FileTurtleDexPath(file2)
	...
	siaPath3 := rt.renter.staticFileSystem.FileTurtleDexPath(file3)
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		i++
		if i%10 == 0 {
			err = rt.renter.managedBubbleMetadata(modules.RootTurtleDexPath())
			if err != nil {
				return err
			}
		}
	...
	checkFindRandomFile(t, rt.renter, modules.RootTurtleDexPath(), siaPath1, siaPath2, siaPath3)

	// Create a directory
	dir, err := modules.NewTurtleDexPath("Dir")
	if err != nil {
		t.Fatal(err)
	}
	...
	newTurtleDexPath1, err := dir.Join(siaPath1.String())
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.RenameFile(siaPath1, newTurtleDexPath1)
	if err != nil {
		t.Fatal(err)
	}
	newTurtleDexPath2, err := dir.Join(siaPath2.String())
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.RenameFile(siaPath2, newTurtleDexPath2)
	if err != nil {
		t.Fatal(err)
	}
	newTurtleDexPath3, err := dir.Join(siaPath3.String())
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.RenameFile(siaPath3, newTurtleDexPath3)
	if err != nil {
		t.Fatal(err)
	}
	// Since we disabled the health loop for this test, call it manually to
	...
	checkFindRandomFile(t, rt.renter, dir, newTurtleDexPath1, newTurtleDexPath2, newTurtleDexPath3)
}

// checkFindRandomFile is a helper function that checks the output from
// managedStuckFile in a loop
func checkFindRandomFile(t *testing.T, r *Renter, dir, siaPath1, siaPath2, siaPath3 modules.TurtleDexPath) {
...
func TestCalculateFileMetadata(t *testing.T) {
	...
	rsc, _ := modules.NewRSCode(1, 1)
	siaPath, err := modules.NewTurtleDexPath("rootFile")
	if err != nil {
		t.Fatal(err)
	}
	up := modules.FileUploadParams{
	...
	sf, err := rt.renter.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	err = rt.renter.managedUpdateFileMetadatas(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}

	// Check calculated metadata
	bubbledMetadatas, err := rt.renter.managedCalculateFileMetadatas([]modules.TurtleDexPath{up.TurtleDexPath})
	if err != nil {
		t.Fatal(err)
	}
...
// TestCreateMissingTurtleDexDir confirms that the repair code creates a ttdxdir file
// if one is not found
func TestCreateMissingTurtleDexDir(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Confirm the ttdxdir file is on disk
	siaDirPath := modules.RootTurtleDexPath().TurtleDexDirMetadataSysPath(rt.renter.staticFileSystem.Root())
	_, err = os.Stat(siaDirPath)
	if err != nil {
		t.Fatal(err)
	}

	// Remove .ttdxdir file on disk
	err = os.Remove(siaDirPath)
	if err != nil {
		t.Fatal(err)
	}

	// Confirm ttdxdir is gone
	_, err = os.Stat(siaDirPath)
	if !os.IsNotExist(err) {
		t.Fatal("Err should have been IsNotExist", err)
	}

	// Create ttdxdir file with managedDirectoryMetadata
	_, err = rt.renter.managedDirectoryMetadata(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddStuckChunksToHeap(t *testing.T) {
	...
	err = rt.renter.staticFileSystem.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.RandomCipherType()), 100, persist.DefaultDiskPermissionsTest, false)
	if err != nil {
		t.Fatal(err)
	}
	f, err := rt.renter.staticFileSystem.OpenTurtleDexFile(up.TurtleDexPath)
	...
	err = rt.renter.managedAddStuckChunksToHeap(up.TurtleDexPath, hosts, offline, goodForRenew)
...
func TestRandomStuckFileRegression(t *testing.T) {
	...
	siaPath := rt.renter.staticFileSystem.FileTurtleDexPath(file)
	...
	stuckTurtleDexPath, err := rt.renter.managedStuckFile(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	if !stuckTurtleDexPath.Equals(siaPath) {
		t.Fatalf("Stuck siapath should have been the one file in the directory, expected %v got %v", siaPath, stuckTurtleDexPath)
	}
}
...
func TestPrepareForBubble(t *testing.T) {
	...
	emptyDir, err := modules.NewTurtleDexPath("emptyDir")
	if err != nil {
		t.Fatal(err)
	}
	...
	urp.mu.Lock()
	_, ok = urp.parentDirs[modules.RootTurtleDexPath()]
	urp.mu.Unlock()
	if !ok {
		t.Error("unexpected")
	}
	...
	resetTimes := func() {
		...
		oldMetadata := ttdxdir.Metadata{
			AggregateLastHealthCheckTime: old,
			LastHealthCheckTime:          old,
		}
		oldTurtleDexPaths := []modules.TurtleDexPath{modules.HomeFolder, modules.BackupFolder, modules.VarFolder}
		for _, sp := range oldTurtleDexPaths {
		...
		currentMetadata := ttdxdir.Metadata{
			AggregateLastHealthCheckTime: time.Now(),
			LastHealthCheckTime:          time.Now(),
		}
		err = rt.openAndUpdateDir(modules.RootTurtleDexPath(), currentMetadata)
		if err != nil {
			t.Fatal(err)
		}
		// Update emptyDir, UserFolder, and SkynetFolder to have future lastHealthCheckTime
		future := time.Now().AddDate(1, 0, 0)
		futureMetadata := ttdxdir.Metadata{
			AggregateLastHealthCheckTime: future,
			LastHealthCheckTime:          future,
		}
		futureTurtleDexPaths := []modules.TurtleDexPath{emptyDir, modules.UserFolder, modules.SkynetFolder}
		for _, sp := range futureTurtleDexPaths {
	...
	urp, err = rt.renter.managedPrepareForBubble(modules.RootTurtleDexPath(), false)
	if err != nil {
		t.Fatal(err)
	}
	...
	// call prepare for bubble on root
	urp, err = rt.renter.managedPrepareForBubble(modules.RootTurtleDexPath(), true)
	if err != nil {
		t.Fatal(err)
	}
