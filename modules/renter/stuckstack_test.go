...
func TestStuckStack(t *testing.T) {
	stack := stuckStack{
		stack:    make([]modules.TurtleDexPath, 0, maxSuccessfulStuckRepairFiles),
		siaPaths: make(map[modules.TurtleDexPath]struct{}),
	}
	...
	// Create some TurtleDexPaths to add to the stack
	sp1, _ := modules.NewTurtleDexPath("siaPath1")
	sp2, _ := modules.NewTurtleDexPath("siaPath2")
	...
	if !siaPath.Equals(sp1) {
		...
		t.Log("sp1:", sp1)
		t.Fatal("TurtleDexPaths not equal")
	}
	if stack.managedLen() != 0 {
		t.Fatal("Expected length of 0 got", stack.managedLen())
	}
	...
	for i := 0; i < 2*maxSuccessfulStuckRepairFiles; i++ {
		sp, _ := modules.NewTurtleDexPath(strconv.Itoa(i))
