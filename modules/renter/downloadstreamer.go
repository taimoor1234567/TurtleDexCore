...
// the sia network.
func (r *Renter) Streamer(siaPath modules.TurtleDexPath, disableLocalFetch bool) (_ string, _ modules.Streamer, err error) {
	...
	node, err := r.staticFileSystem.OpenTurtleDexFile(siaPath)
	if err != nil {
		return "", nil, err
	}
	defer func() {
...
func (r *Renter) StreamerByNode(node *filesystem.FileNode, disableLocalFetch bool) (modules.Streamer, error) {
	...
	// Grab the current TurtleDexPath of the FileNode and then create a snapshot.
	sp := r.staticFileSystem.FileTurtleDexPath(node)
