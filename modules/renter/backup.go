...
func (r *Renter) managedCreateBackup(dst string, secret []byte) (err error) {
	...
	if err := r.managedTarTurtleDexFiles(tw); err != nil {
		twErr := tw.Close()
		gzwErr := gzw.Close()
...
func (r *Renter) LoadBackup(src string, secret []byte) (err error) {
	...
	root, err := r.staticFileSystem.OpenTurtleDexDir(modules.UserFolder)
	if err != nil {
		return err
	}
	defer func() {
...
// managedTarTurtleDexFiles creates a tarball from the renter's siafiles and writes
// it to dst.
func (r *Renter) managedTarTurtleDexFiles(tw *tar.Writer) error {
	...
	return r.staticFileSystem.Walk(modules.UserFolder, func(path string, info os.FileInfo, statErr error) (err error) {
		...
		if !info.IsDir() && filepath.Ext(path) != modules.TurtleDexFileExtension &&
			filepath.Ext(path) != modules.TurtleDexDirExtension {
			return nil
		}
		...
		// Handle siafiles and ttdxdirs differently.
		var file io.Reader
		if filepath.Ext(path) == modules.TurtleDexFileExtension {
			// Get the siafile.
			siaPath, err := modules.UserFolder.Join(strings.TrimSuffix(relPath, modules.TurtleDexFileExtension))
			if err != nil {
				return err
			}
			...
			header.Size = fi.Size()
		} else if filepath.Ext(path) == modules.TurtleDexDirExtension {
			// Get the ttdxdir.
			var siaPath modules.TurtleDexPath
			siaPathStr := strings.TrimSuffix(relPath, modules.TurtleDexDirExtension)
			...
			// Get a reader to read from the ttdxdir.
...
func (r *Renter) managedUntarDir(tr *tar.Reader) error {
	...
	for {
		...
		if name := filepath.Base(info.Name()); name == modules.TurtleDexDirExtension {
			// Load the file as a .ttdxdir
			var md ttdxdir.Metadata
			err = json.Unmarshal(b, &md)
			if err != nil {
				return errors.AddContext(err, "could not unmarshal")
			}
			// Try creating a new TurtleDexDir.
			var siaPath modules.TurtleDexPath
			...
			err := r.staticFileSystem.NewTurtleDexDir(siaPath, modules.DefaultDirPerm)
			if errors.Contains(err, filesystem.ErrExists) {
				// .ttdxdir exists already
				continue
			} else if err != nil {
				// unexpected error
			...
			// Update the metadata.
			dirEntry, err := r.staticFileSystem.OpenTurtleDexDir(siaPath)
			if err != nil {
			...
			// Close Directory
			dirEntry.Close()
		} else if filepath.Ext(info.Name()) == modules.TurtleDexFileExtension {
			// Add the file to the TurtleDexFileSet.
			reader := bytes.NewReader(b)
			siaPath, err := modules.UserFolder.Join(strings.TrimSuffix(header.Name, modules.TurtleDexFileExtension))
			if err != nil {
				return errors.AddContext(err, "could not join folders")
			}
			err = r.staticFileSystem.AddTurtleDexFileFromReader(reader, siaPath)
			if err != nil {
