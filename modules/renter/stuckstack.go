...
type (
	...
	stuckStack struct {
		stack    []modules.TurtleDexPath
		siaPaths map[modules.TurtleDexPath]struct{}
...
func (ss *stuckStack) managedPop() (sp modules.TurtleDexPath) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
...
func (ss *stuckStack) managedPush(siaPath modules.TurtleDexPath) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
