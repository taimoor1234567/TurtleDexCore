...
type (
	...
	download struct {
		...
		staticTurtleDexPath         modules.TurtleDexPath    // The path of the siafile at the time the download started.
...
func (r *Renter) managedDownload(p modules.RenterDownloadParameters) (_ *download, err error) {
	...
	entry, err := r.staticFileSystem.OpenTurtleDexFile(p.TurtleDexPath)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	// Prepare snapshot.
	snap, err := entry.SnapshotRange(p.TurtleDexPath, p.Offset, p.Length)
	if err != nil {
		return nil, err
	}
...
func (r *Renter) managedNewDownload(params downloadParams) (*download, error) {
	...
	d := &download{
		...
		staticTurtleDexPath:         params.file.TurtleDexPath(),
...
func (d *download) Start() error {
	...
	for chunkIndex := minChunk; chunkIndex <= maxChunk; chunkIndex++ {
		...
		for pieceIndex, pieceSet := range pieces {
			for _, piece := range pieceSet {
				...
				if exists {
					d.r.log.Println("ERROR: Worker has multiple pieces uploaded for the same chunk.", params.file.TurtleDexPath(), chunkIndex, pieceIndex, piece.HostPubKey.String())
				}
	...
	for i := minChunk; i <= maxChunk; i++ {
		udc := &unfinishedDownloadChunk{
			...
			staticChunkIndex: i,
			staticCacheID:    fmt.Sprintf("%v:%v", d.staticTurtleDexPath, i),
...
func (r *Renter) DownloadByUID(uid modules.DownloadID) (modules.DownloadInfo, bool) {
	...
	return modules.DownloadInfo{
		...
		TurtleDexPath:         d.staticTurtleDexPath,
