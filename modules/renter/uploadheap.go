...
func (r *Renter) managedBuildUnfinishedChunk(entry *filesystem.FileNode, chunkIndex uint64, hosts map[string]struct{}, hostPublicKeys map[string]types.TurtleDexPublicKey, priority bool, offline, goodForRenew map[string]bool, mm *memoryManager) (*unfinishedUploadChunk, error) {
	// Copy entry
	...
	uuc := &unfinishedUploadChunk{
		...
		staticTurtleDexPath: entryCopy.TurtleDexFilePath(),
...
// file.
//
// NOTE: each unfinishedUploadChunk needs its own TurtleDexFileSetEntry. This is due
// to the TurtleDexFiles being removed from memory. Since the renter does not keep the
// TurtleDexFiles in memory the unfinishedUploadChunks need to close the TurtleDexFile when
// they are done and so cannot share a TurtleDexFileSetEntry as the first chunk to
...
func (r *Renter) managedBuildUnfinishedChunks(entry *filesystem.FileNode, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool, mm *memoryManager) []*unfinishedUploadChunk {
	...
	if workerPoolLen < minPieces {
		...
		if allowance.Hosts < uint64(minPieces) && target == targetUnstuckChunks {
			...
			r.log.Printf("WARN: allownace had insufficient hosts for chunk to reach minimum redundancy, have %v need %v for file %v", allowance.Hosts, minPieces, entry.TurtleDexFilePath())
	...
	pks := make(map[string]types.TurtleDexPublicKey)
...
func (r *Renter) managedAddChunksToHeap(hosts map[string]struct{}) (*uniqueRefreshPaths, error) {
	...
	for r.uploadHeap.managedLen() < maxUploadHeapChunks && r.directoryHeap.managedLen() > 0 {
		...
		r.managedBuildChunkHeap(dir.staticTurtleDexPath, hosts, targetUnstuckChunks, offline, goodForRenew)
		...
		err = siaPaths.callAdd(dir.staticTurtleDexPath)
		if err != nil {
			r.repairLog.Println("WARN: error adding siapath to tracked paths to bubble:", err)
		}
		r.repairLog.Printf("Added %v chunks from %s to the repair heap", chunksAdded, dir.staticTurtleDexPath)
	}
...
func (r *Renter) managedBuildAndPushRandomChunk(siaPath modules.TurtleDexPath, hosts map[string]struct{}, target repairTarget, mm *memoryManager) error {
	// Open file
	file, err := r.staticFileSystem.OpenTurtleDexFile(siaPath)
	if err != nil {
		return err
	}
...
func (r *Renter) callBuildAndPushChunks(files []*filesystem.FileNode, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool) {
	...
	dirTurtleDexPath, err := r.staticFileSystem.FileTurtleDexPath(files[0]).Dir()
	if err != nil {
		r.log.Println("WARN: unable to get directory TurtleDexPath and add directory back to directory heap:", err)
		return
	}
	...
	d := &directory{
		...
		explored:        true,
		health:          wh.health,
		staticTurtleDexPath:   dirTurtleDexPath,
	}
...
func (r *Renter) managedBuildChunkHeap(dirTurtleDexPath modules.TurtleDexPath, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool) {
	// Get Directory files
	...
	for _, fi := range fileinfos {
		...
		if fi.IsDir() || ext != modules.TurtleDexFileExtension {
			continue
		}

		// Open TurtleDexFile
		siaPath, err := dirTurtleDexPath.Join(strings.TrimSuffix(fi.Name(), ext))
		if err != nil {
		...
		if target == targetStuckChunks && file.NumStuckChunks() == 0 {
			...
			err = file.Close()
			if err != nil {
				r.log.Println("WARN: Could not close file:", file.TurtleDexFilePath(), err)
			}
			continue
		}
	...
	if len(files) == 0 {
		r.log.Debugln("No files pulled from `", dirTurtleDexPath, "` to build the repair heap")
		return
	}
	...
	if len(files) > maxUploadHeapChunks && target == targetUnstuckChunks {
		...
		for i := maxUploadHeapChunks; i < len(files); i++ {
			err = files[i].Close()
			if err != nil {
				r.log.Println("WARN: Could not close file:", files[i].TurtleDexFilePath(), err)
			}
		}
...
func (r *Renter) managedRefreshHostsAndWorkers() map[string]struct{} {
	...
	// key is the String() representation of the host's TurtleDexPublicKey.
	//
...
func (r *Renter) managedRepairLoop() error {
	...
	for r.uploadHeap.managedLen() >= minUploadHeapSize || smallRepair || time.Now().After(repairBreakTime) {
		...
		chunkPath := nextChunk.staticTurtleDexPath
...
func (r *Renter) threadedUploadAndRepair() {
	...
	for {
		...
		// Add chunks to heap.
		dirTurtleDexPaths, err := r.managedAddChunksToHeap(hosts)
		if err != nil {
		...
		dirTurtleDexPaths.callRefreshAll()
	}
}
