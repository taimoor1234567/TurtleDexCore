...
func (wp *workerPool) Worker(hostPubKey types.TurtleDexPublicKey) (modules.Worker, error) {
	return wp.callWorker(hostPubKey)
}

// callWorker will return the worker associated with the provided public key.
// If no worker is found, an error will be returned.
func (wp *workerPool) callWorker(hostPubKey types.TurtleDexPublicKey) (*worker, error) {
	wp.mu.Lock()
	defer wp.mu.Unlock()
