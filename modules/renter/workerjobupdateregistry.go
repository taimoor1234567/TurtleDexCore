...
type (
	...
	jobUpdateRegistry struct {
		staticTurtleDexPublicKey        types.TurtleDexPublicKey
...
func (w *worker) newJobUpdateRegistry(ctx context.Context, responseChan chan *jobUpdateRegistryResponse, spk types.TurtleDexPublicKey, srv modules.SignedRegistryValue) *jobUpdateRegistry {
	return &jobUpdateRegistry{
		staticTurtleDexPublicKey:        spk,
...
func (j *jobUpdateRegistry) callExecute() {
	...
	if errors.Contains(err, registry.ErrLowerRevNum) || errors.Contains(err, registry.ErrSameRevNum) {
		...
		// with the error.
		if err := rv.Verify(j.staticTurtleDexPublicKey.ToPublicKey()); err != nil {
		...
		cachedRevision, cached := w.staticRegistryCache.Get(j.staticTurtleDexPublicKey, j.staticSignedRegistryValue.Tweak)
		if cached && cachedRevision > rv.Revision {
			...
			w.staticRegistryCache.Set(j.staticTurtleDexPublicKey, rv, true) // adjust the cache
			return
		}
		sendResponse(&rv, err)
		return
	...
	w.staticRegistryCache.Set(j.staticTurtleDexPublicKey, j.staticSignedRegistryValue, false)
...
func (j *jobUpdateRegistry) managedUpdateRegistry() (modules.SignedRegistryValue, error) {
	...
	if build.VersionCmp(w.staticCache().staticHostVersion, "1.5.5") < 0 {
		pb.V154AddUpdateRegistryInstruction(j.staticTurtleDexPublicKey, j.staticSignedRegistryValue)
	} else {
		pb.AddUpdateRegistryInstruction(j.staticTurtleDexPublicKey, j.staticSignedRegistryValue)
	}
...
func (w *worker) UpdateRegistry(ctx context.Context, spk types.TurtleDexPublicKey, rv modules.SignedRegistryValue) error {
