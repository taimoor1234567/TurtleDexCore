...
// directory is a helper struct that represents a ttdxdir in the
// repairDirectoryHeap
...
type directoryHeap struct {
	...
	// heap
	heapDirectories map[modules.TurtleDexPath]*directory
...
func (dh *directoryHeap) managedPop() (d *directory) {
	...
	if dh.heap.Len() > 0 {
		d = heap.Pop(&dh.heap).(*directory)
		delete(dh.heapDirectories, d.staticTurtleDexPath)
	}
	return d
}
...
func (dh *directoryHeap) managedPush(d *directory) {
	...
	_, exists := dh.heapDirectories[d.staticTurtleDexPath]
	if exists {
		if !dh.update(d) {
	...
	heap.Push(&dh.heap, d)
	dh.heapDirectories[d.staticTurtleDexPath] = d
}
...
func (dh *directoryHeap) managedReset() {
	dh.mu.Lock()
	defer dh.mu.Unlock()
	dh.heapDirectories = make(map[modules.TurtleDexPath]*directory)
...
func (dh *directoryHeap) update(d *directory) bool {
	heapDir, exists := dh.heapDirectories[d.staticTurtleDexPath]
	if !exists {
		return false
	}
	...
	heapDir.mu.Unlock()
	dh.heapDirectories[d.staticTurtleDexPath] = heapDir
	heap.Fix(&dh.heap, heapDir.index)
	return true
}

// managedPushDirectory adds a directory to the directory heap
func (dh *directoryHeap) managedPushDirectory(siaPath modules.TurtleDexPath, metadata ttdxdir.Metadata, explored bool) {
	d := &directory{
...
func (r *Renter) managedNextExploredDirectory() (*directory, error) {
	...
	for {
		...
		if err != nil {
			contextStr := fmt.Sprintf("unable to push subdirectories for `%v`", d.staticTurtleDexPath)
...
func (r *Renter) managedPushSubDirectories(d *directory) error {
	subDirs, err := r.managedSubDirectories(d.staticTurtleDexPath)
	if err != nil {
		contextStr := fmt.Sprintf("unable to get subdirectories for `%v`", d.staticTurtleDexPath)
...
// managedPushUnexploredDirectory reads the health from the ttdxdir metadata and
// pushes an unexplored directory element onto the heap
func (r *Renter) managedPushUnexploredDirectory(siaPath modules.TurtleDexPath) (err error) {
	// Grab the ttdxdir metadata. If it doesn't exist, create it.
