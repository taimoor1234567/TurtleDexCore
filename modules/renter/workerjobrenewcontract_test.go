...
func TestRenewContract(t *testing.T) {
	...
	funding := types.TurtleDexcoinPrecision
	...
	err = txnBuilder.FundTurtleDexcoins(funding)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedValidRenterOutput := types.TurtleDexcoinOutput{
	...
	expectedValidHostOutput := types.TurtleDexcoinOutput{
		Value:      hostPayout,
		UnlockHash: params.Host.UnlockHash,
	}
	expectedMissedHostOutput := types.TurtleDexcoinOutput{
		Value:      hostCollateral.Sub(baseCollateral).Add(params.Host.ContractPrice),
		UnlockHash: params.Host.UnlockHash,
	}
	expectedVoidOutput := types.TurtleDexcoinOutput{
