...
// bubbledTurtleDexDirMetadata is a wrapper for ttdxdir.Metadata that also contains the
// siapath for convenience.
type bubbledTurtleDexDirMetadata struct {
	sp modules.TurtleDexPath
	ttdxdir.Metadata
}

// bubbledTurtleDexFileMetadata is a wrapper for siafile.BubbledMetadata that also
// contains the siapath for convenience.
type bubbledTurtleDexFileMetadata struct {
	sp modules.TurtleDexPath
...
// updates the ttdxdir metadata on disk then calls callThreadedBubbleMetadata on
...
func (r *Renter) BubbleMetadata(siaPath modules.TurtleDexPath, force, recursive bool) error {
...
func (r *Renter) managedPrepareBubble(siaPath modules.TurtleDexPath) bool {
...
// be bubbled up
func (r *Renter) managedCalculateDirectoryMetadata(siaPath modules.TurtleDexPath) (ttdxdir.Metadata, error) {
	// Set default metadata values to start
	now := time.Now()
	metadata := ttdxdir.Metadata{
		...
		AggregateRemoteHealth:        ttdxdir.DefaultDirHealth,
		...
		NumSubDirs:          uint64(0),
		RemoteHealth:        ttdxdir.DefaultDirHealth,
		RepairSize:          uint64(0),
		Size:                uint64(0),
	...
	var fileTurtleDexPaths, dirTurtleDexPaths []modules.TurtleDexPath
	for _, fi := range fileinfos {
		...
		if ext == modules.TurtleDexFileExtension {
			...
			fileTurtleDexPath, err := siaPath.Join(fName)
			if err != nil {
				r.log.Println("unable to join siapath with dirpath while calculating directory metadata:", err)
				continue
			}
			fileTurtleDexPaths = append(fileTurtleDexPaths, fileTurtleDexPath)
		} else if fi.IsDir() {
			// Directory is found, read the directory metadata file
			dirTurtleDexPath, err := siaPath.Join(fi.Name())
			if err != nil {
				r.log.Println("unable to join siapath with dirpath while calculating directory metadata:", err)
				continue
			}
	...
	bubbledMetadatas, err := r.managedCalculateFileMetadatas(fileTurtleDexPaths)
	if err != nil {
	...
	dirMetadatas, err := r.managedDirectoryMetadatas(dirTurtleDexPaths)
	if err != nil {
		r.log.Printf("failed to calculate file metadata: %v", err)
	}
	...
	for len(bubbledMetadatas)+len(dirMetadatas) > 0 {
		...
		if len(bubbledMetadatas) > 0 {
			...
			fileTurtleDexPath := bubbledMetadata.sp
			fileMetadata := bubbledMetadata.bm
			// If 75% or more of the redundancy is missing, register an alert
			// for the file.
			uid := string(fileMetadata.UID)
			if maxHealth := math.Max(fileMetadata.Health, fileMetadata.StuckHealth); maxHealth >= AlertTurtleDexfileLowRedundancyThreshold {
				r.staticAlerter.RegisterAlert(modules.AlertIDTurtleDexfileLowRedundancy(uid), AlertMSGTurtleDexfileLowRedundancy,
					AlertCauseTurtleDexfileLowRedundancy(fileTurtleDexPath, maxHealth, fileMetadata.Redundancy),
					modules.SeverityWarning)
			} else {
				r.staticAlerter.UnregisterAlert(modules.AlertIDTurtleDexfileLowRedundancy(uid))
			}
			...
			isExtended := strings.Contains(fileTurtleDexPath.String(), modules.ExtendedSuffix)
...
func (r *Renter) managedCalculateFileMetadata(siaPath modules.TurtleDexPath, hostOfflineMap, hostGoodForRenewMap map[string]bool) (bubbledTurtleDexFileMetadata, error) {
	// Open TurtleDexFile in a read only state so that it doesn't need to be
	// closed
	sf, err := r.staticFileSystem.OpenTurtleDexFile(siaPath)
	if err != nil {
		return bubbledTurtleDexFileMetadata{}, err
	}
	defer func() {
	...
	if r.isFileNodeBlocked(sf) {
		...
		return bubbledTurtleDexFileMetadata{}, errors.Compose(r.staticFileSystem.DeleteFile(siaPath), ErrSkylinkBlocked)
	}
	...
	if !onDisk && redundancy < 1 {
		r.log.Debugf("File not found on disk and possibly unrecoverable: LocalPath %v; TurtleDexPath %v", sf.LocalPath(), siaPath.String())
	}
	...
	// Return the metadata
	return bubbledTurtleDexFileMetadata{
		sp: siaPath,
		bm: siafile.BubbledMetadata{
			Health:              health,
...
func (r *Renter) managedCalculateFileMetadatas(siaPaths []modules.TurtleDexPath) (_ []bubbledTurtleDexFileMetadata, err error) {
	/// Get cached offline and goodforrenew maps.
	hostOfflineMap, hostGoodForRenewMap, _, _ := r.managedRenterContractsAndUtilities()

	// Define components
	mds := make([]bubbledTurtleDexFileMetadata, 0, len(siaPaths))
	siaPathChan := make(chan modules.TurtleDexPath, numBubbleWorkerThreads)
	var errs error
	var errMu, mdMu sync.Mutex

	// Create function for loading TurtleDexFiles and calculating the metadata
	metadataWorker := func() {
...
func (r *Renter) managedCompleteBubbleUpdate(siaPath modules.TurtleDexPath) {
	r.bubbleUpdatesMu.Lock()
	defer r.bubbleUpdatesMu.Unlock()
...
// managedDirectoryMetadatas returns all the metadatas of the TurtleDexDirs for the
// provided siaPaths
func (r *Renter) managedDirectoryMetadatas(siaPaths []modules.TurtleDexPath) ([]bubbledTurtleDexDirMetadata, error) {
	// Define components
	mds := make([]bubbledTurtleDexDirMetadata, 0, len(siaPaths))
	siaPathChan := make(chan modules.TurtleDexPath, numBubbleWorkerThreads)
	var errs error
	var errMu, mdMu sync.Mutex
	...
	metadataWorker := func() {
		for siaPath := range siaPathChan {
			...
			mdMu.Lock()
			mds = append(mds, bubbledTurtleDexDirMetadata{
				siaPath,
				md,
			})
			mdMu.Unlock()
...
// metadata
func (r *Renter) managedDirectoryMetadata(siaPath modules.TurtleDexPath) (_ ttdxdir.Metadata, err error) {
	...
	if !fi.IsDir() {
		return ttdxdir.Metadata{}, fmt.Errorf("%v is not a directory", siaPath)
	}

	//  Open TurtleDexDir
	siaDir, err := r.staticFileSystem.OpenTurtleDexDirCustom(siaPath, true)
	if err != nil {
		return ttdxdir.Metadata{}, err
	}
	defer func() {
...
func (r *Renter) managedUpdateLastHealthCheckTime(siaPath modules.TurtleDexPath) error {
	// Read directory
	fileinfos, err := r.staticFileSystem.ReadDir(siaPath)
	if err != nil {
		r.log.Printf("WARN: Error in reading files in directory %v : %v\n", siaPath.String(), err)
	...
	for _, fi := range fileinfos {
		...
		// Check for TurtleDexFiles and Directories
		if fi.IsDir() {
			// Directory is found, read the directory metadata file
			dirTurtleDexPath, err := siaPath.Join(fi.Name())
			if err != nil {
			...
			dirMetadata, err := r.managedDirectoryMetadata(dirTurtleDexPath)
			if err != nil {
				return err
			}
	...
	entry, err := r.staticFileSystem.OpenTurtleDexDir(siaPath)
	if err != nil {
		return err
	}
...
func (r *Renter) callThreadedBubbleMetadata(siaPath modules.TurtleDexPath) {
	if err := r.tg.Add(); err != nil {
		return
	}
	defer r.tg.Done()
...
// bubble preparation.
func (r *Renter) managedPerformBubbleMetadata(siaPath modules.TurtleDexPath) (err error) {
	...
	siaDir, err := r.staticFileSystem.OpenTurtleDexDir(siaPath)
	if err != nil {
...
// and updates the ttdxdir metadata on disk then calls callThreadedBubbleMetadata
// on the parent directory so that it is only blocking for the current directory
func (r *Renter) managedBubbleMetadata(siaPath modules.TurtleDexPath) error {
