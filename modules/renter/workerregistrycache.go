...
func (rc *registryRevisionCache) Get(pubKey types.TurtleDexPublicKey, tweak crypto.Hash) (uint64, bool) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
...
func (rc *registryRevisionCache) Set(pubKey types.TurtleDexPublicKey, rv modules.SignedRegistryValue, force bool) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
