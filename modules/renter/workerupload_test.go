...
func TestCheckUploadGouging(t *testing.T) {
	...
	minAllowance := modules.Allowance{
		...
		// One ttdc is multiplied by the number of elements that are checked
		// for gouging, and then divided by the gounging denominator.
		Funds:  types.TurtleDexcoinPrecision.Mul64(4).Div64(uploadGougingFractionDenom).Sub(oneCurrency),
		Period: 1, // 1 block.
	...
	minHostSettings := modules.HostExternalSettings{
		BaseRPCPrice:         types.TurtleDexcoinPrecision,
		SectorAccessPrice:    types.TurtleDexcoinPrecision,
		UploadBandwidthPrice: types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize),
		StoragePrice:         types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize),
	}
	...
	maxAllowance.MaxRPCPrice = types.TurtleDexcoinPrecision.Add(oneCurrency)
	maxAllowance.MaxContractPrice = oneCurrency
	maxAllowance.MaxDownloadBandwidthPrice = oneCurrency
	maxAllowance.MaxSectorAccessPrice = types.TurtleDexcoinPrecision.Add(oneCurrency)
	maxAllowance.MaxStoragePrice = types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize).Add(oneCurrency)
	maxAllowance.MaxUploadBandwidthPrice = types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize).Add(oneCurrency)
	...
	failAllowance.MaxRPCPrice = types.TurtleDexcoinPrecision.Sub(oneCurrency)
	...
	failAllowance = maxAllowance
	failAllowance.MaxSectorAccessPrice = types.TurtleDexcoinPrecision.Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
	...
	failAllowance = maxAllowance
	failAllowance.MaxStoragePrice = types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize).Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
	...
	failAllowance = maxAllowance
	failAllowance.MaxUploadBandwidthPrice = types.TurtleDexcoinPrecision.Div64(modules.StreamUploadSize).Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
