...
// updateTurtleDexDirHealth is a helper method to update the health and the aggregate
// health of a ttdxdir
func (r *Renter) updateTurtleDexDirHealth(siaPath modules.TurtleDexPath, health, aggregateHealth float64) (err error) {
...
func addDirectoriesToHeap(r *Renter, numDirs int, explored, remote bool) {
	...
	for i := 0; i < numDirs; i++ {
		...
		d := &directory{
			...
			staticTurtleDexPath:         modules.RandomTurtleDexPath(),
		}
...
func TestDirectoryHeap(t *testing.T) {
	...
	err = rt.renter.managedPushUnexploredDirectory(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	if !d.staticTurtleDexPath.Equals(modules.RootTurtleDexPath()) {
		t.Fatal("Directory should be root directory but is", d.staticTurtleDexPath)
	}

	// Make sure pushing an unexplored dir that doesn't exist works.
	randomSP, err := modules.RootTurtleDexPath().Join(modules.RandomTurtleDexPath().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Try again but this time just remove the .ttdxdir file.
	err = os.Remove(randomSP.TurtleDexDirMetadataSysPath(rt.renter.staticFileSystem.Root()))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.managedPushUnexploredDirectory(randomSP)
...
func TestPushSubDirectories(t *testing.T) {
	...
	siaPath1, err := modules.NewTurtleDexPath("SubDir1")
	if err != nil {
		t.Fatal(err)
	}
	siaPath2, err := modules.NewTurtleDexPath("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	...
	// Update metadata
	err = rt.renter.updateTurtleDexDirHealth(siaPath1, float64(2), float64(2))
	if err != nil {
		t.Fatal(err)
	}

	err = rt.renter.updateTurtleDexDirHealth(siaPath2, float64(3), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	...
	d := &directory{
		staticTurtleDexPath: modules.RootTurtleDexPath(),
	}
	...
	d = rt.renter.directoryHeap.managedPop()
	if !d.staticTurtleDexPath.Equals(siaPath2) {
		t.Fatalf("Expected directory %v but found %v", siaPath2.String(), d.staticTurtleDexPath.String())
	}
	...
	d = rt.renter.directoryHeap.managedPop()
	if !d.staticTurtleDexPath.Equals(siaPath1) {
		t.Fatalf("Expected directory %v but found %v", siaPath1.String(), d.staticTurtleDexPath.String())
	}
...
func TestNextExploredDirectory(t *testing.T) {
	...
	// Update metadata
	err = rt.renter.updateTurtleDexDirHealth(modules.RootTurtleDexPath(), float64(0), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath1, float64(1), float64(2))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath1_1, float64(1), float64(1))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath1_2, float64(2), float64(2))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath2, float64(1), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath2_1, float64(1), float64(1))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateTurtleDexDirHealth(siaPath2_2, float64(3), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !d.staticTurtleDexPath.Equals(siaPath2_2) {
		t.Fatalf("Expected directory %v but found %v", siaPath2_2.String(), d.staticTurtleDexPath.String())
	}
	if d.aggregateHealth != float64(3) {
		t.Fatal("Expected AggregateHealth to be 3 but was", d.aggregateHealth)
	}
	...
	if !d.staticTurtleDexPath.Equals(siaPath1_2) {
		t.Fatalf("Expected directory %v but found %v", siaPath1_2.String(), d.staticTurtleDexPath.String())
	}
	if d.aggregateHealth != float64(2) {
		t.Fatal("Expected AggregateHealth to be 2 but was", d.aggregateHealth)
	}
