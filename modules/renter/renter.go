...
import (
	...
	"github.com/turtledex/threadgroup"
	...
	"github.com/turtledex/TurtleDexCore/modules/renter/hostdb"
	"github.com/turtledex/TurtleDexCore/modules/renter/skynetblocklist"
	"github.com/turtledex/TurtleDexCore/modules/renter/skynetportals"
	"github.com/turtledex/TurtleDexCore/persist"
	"github.com/turtledex/TurtleDexCore/skykey"
	siasync "github.com/turtledex/TurtleDexCore/sync"
...
type hostContractor interface {
	...
	ContractByPublicKey(types.TurtleDexPublicKey) (modules.RenterContract, bool)

	// ContractPublicKey returns the public key capable of verifying the renter's
	// signature on a contract.
	ContractPublicKey(pk types.TurtleDexPublicKey) (crypto.PublicKey, bool)
	...
	ContractUtility(types.TurtleDexPublicKey) (modules.ContractUtility, bool)
	...
	Editor(types.TurtleDexPublicKey, <-chan struct{}) (contractor.Editor, error)

	// IsOffline reports whether the specified host is considered offline.
	IsOffline(types.TurtleDexPublicKey) bool

	// Downloader creates a Downloader from the specified contract ID,
	// allowing the retrieval of sectors.
	Downloader(types.TurtleDexPublicKey, <-chan struct{}) (contractor.Downloader, error)

	// Session creates a Session from the specified contract ID.
	Session(types.TurtleDexPublicKey, <-chan struct{}) (contractor.Session, error)
...
type renterFuseManager interface {
	...
	Mount(mountPoint string, sp modules.TurtleDexPath, opts modules.MountOptions) (err error)
...
type cachedUtilities struct {
	...
	used         []types.TurtleDexPublicKey
}

// A Renter is responsible for tracking all of the files that a user has
// uploaded to TurtleDex, as well as the locations and health of these files.
type Renter struct {
	// Skynet Management
	...
	//
	// repairMemoryManager is used for repair work scheduled by ttdxd
	//
...
// PriceEstimation estimates the cost in ttdcs of performing various storage
...
func (r *Renter) PriceEstimation(allowance modules.Allowance) (modules.RenterPriceEstimation, modules.Allowance, error) {
	...
	var pks []types.TurtleDexPublicKey
	...
	if len(hosts) < int(allowance.Hosts) {
		// Re-initialize the list with TurtleDexPublicKeys to hold the public keys from the current
...
func (r *Renter) managedRenterContractsAndUtilities() (offline map[string]bool, goodForRenew map[string]bool, contracts map[string]modules.RenterContract, used []types.TurtleDexPublicKey) {
	id := r.mu.Lock()
...
func (r *Renter) managedUpdateRenterContractsAndUtilities() {
	var used []types.TurtleDexPublicKey
	goodForRenew := make(map[string]bool)
	offline := make(map[string]bool)
	...
	// Update the used hosts of the TurtleDexfile. Only consider the ones that
	// are goodForRenew.
	for _, contract := range allContracts {
		pk := contract.HostPublicKey
...
func (r *Renter) SetFileTrackingPath(siaPath modules.TurtleDexPath, newPath string) (err error) {
	if err := r.tg.Add(); err != nil {
		return err
	}
	defer r.tg.Done()
...
func (r *Renter) Filter() (modules.FilterMode, map[string]types.TurtleDexPublicKey, error) {
	var fm modules.FilterMode
	hosts := make(map[string]types.TurtleDexPublicKey)
	if err := r.tg.Add(); err != nil {
		return fm, hosts, err
	}
	defer r.tg.Done()
...
func (r *Renter) SetFilterMode(lm modules.FilterMode, hosts []types.TurtleDexPublicKey) error {
	if err := r.tg.Add(); err != nil {
		return err
	}
	defer r.tg.Done()
...
func (r *Renter) Host(spk types.TurtleDexPublicKey) (modules.HostDBEntry, bool, error) {
...
// ContractUtility returns the utility field for a given contract, along
// with a bool indicating if it exists.
func (r *Renter) ContractUtility(pk types.TurtleDexPublicKey) (modules.ContractUtility, bool) {
...
// Mount mounts the files under the specified siapath under the 'mountPoint' folder on
// the local filesystem.
func (r *Renter) Mount(mountPoint string, sp modules.TurtleDexPath, opts modules.MountOptions) error {
...
func renterBlockingStartup(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, hdb modules.HostDB, w modules.Wallet, hc hostContractor, mux *siamux.TurtleDexMux, persistDir string, rl *ratelimit.RateLimit, deps modules.Dependencies) (*Renter, error) {
	if g == nil {
	...
	r := &Renter{
		...
		directoryHeap: directoryHeap{
			heapDirectories: make(map[modules.TurtleDexPath]*directory),
		},
	...
	if !r.deps.Disrupt("DisableRepairAndHealthLoops") {
		...
		err = r.managedPushUnexploredDirectory(modules.RootTurtleDexPath())
		if err != nil {
			return nil, err
		}
...
func NewCustomRenter(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, hdb modules.HostDB, w modules.Wallet, hc hostContractor, mux *siamux.TurtleDexMux, persistDir string, rl *ratelimit.RateLimit, deps modules.Dependencies) (*Renter, <-chan error) {
...
func New(g modules.Gateway, cs modules.ConsensusSet, wallet modules.Wallet, tpool modules.TransactionPool, mux *siamux.TurtleDexMux, rl *ratelimit.RateLimit, persistDir string) (*Renter, <-chan error) {
	errChan := make(chan error, 1)
