...
type (
	// DirNode is a node which references a TurtleDexDir.
	DirNode struct {
		node
		...
		// lazyTurtleDexDir is the TurtleDexDir of the DirNode. 'lazy' means that it will
		// only be loaded on demand and destroyed as soon as the length of
		// 'threads' reaches 0.
		lazyTurtleDexDir **ttdxdir.TurtleDexDir
	}
)
...
// Delete is a wrapper for TurtleDexDir.Delete.
...
// Deleted is a wrapper for TurtleDexDir.Deleted.
...
// DirReader is a wrapper for TurtleDexDir.DirReader.
func (n *DirNode) DirReader() (*ttdxdir.DirReader, error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	sd, err := n.siaDir()
	if err != nil {
...
// Metadata is a wrapper for TurtleDexDir.Metadata.
func (n *DirNode) Metadata() (ttdxdir.Metadata, error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	sd, err := n.siaDir()
	if os.IsNotExist(err) {
		return ttdxdir.Metadata{}, ErrNotExist
	}
	if err != nil {
		return ttdxdir.Metadata{}, err
	}
	return sd.Metadata(), nil
}

// Path is a wrapper for TurtleDexDir.Path.
...
// UpdateBubbledMetadata is a wrapper for TurtleDexDir.UpdateBubbledMetadata.
func (n *DirNode) UpdateBubbledMetadata(md ttdxdir.Metadata) error {
	n.mu.Lock()
	defer n.mu.Unlock()
	sd, err := n.siaDir()
	if err != nil {
...
// UpdateLastHealthCheckTime is a wrapper for TurtleDexDir.UpdateLastHealthCheckTime.
...
// UpdateMetadata is a wrapper for TurtleDexDir.UpdateMetadata.
func (n *DirNode) UpdateMetadata(md ttdxdir.Metadata) error {
	n.mu.Lock()
	defer n.mu.Unlock()
	sd, err := n.siaDir()
	if err != nil {
...
func (n *DirNode) managedList(fsRoot string, recursive, cached bool, offlineMap map[string]bool, goodForRenewMap map[string]bool, contractsMap map[string]modules.RenterContract, flf modules.FileListFunc, dlf modules.DirListFunc) error {
	...
	dirWorker := func() {
		for sd := range dirLoadChan {
			...
			if sd.managedAbsPath() == fsRoot {
				di, err = sd.managedInfo(modules.RootTurtleDexPath())
			} else {
				di, err = sd.managedInfo(nodeTurtleDexPath(fsRoot, &sd.node))
			}
			sd.Close()
	...
	fileWorker := func() {
		for load := range fileLoadChan {
			...
			if cached {
				fi, err = sf.staticCachedInfo(nodeTurtleDexPath(fsRoot, &sf.node))
			} else {
				fi, err = sf.managedFileInfo(nodeTurtleDexPath(fsRoot, &sf.node), offlineMap, goodForRenewMap, contractsMap)
			}
			if errors.Contains(err, ErrNotExist) {
				continue
			}
...
// managedRecursiveList returns the files and dirs within the TurtleDexDir.
func (n *DirNode) managedRecursiveList(recursive, cached bool, fileLoadChan chan func() (*FileNode, error), dirLoadChan chan *DirNode) error {
	...
	for _, info := range fis {
		...
		if !info.IsDir() && filepath.Ext(info.Name()) != modules.TurtleDexFileExtension {
			continue
		}
		if info.IsDir() {
		...
		fileNames = append(fileNames, strings.TrimSuffix(info.Name(), modules.TurtleDexFileExtension))
	}
	// Handle dirs first.
...
func (n *DirNode) closeDirNode() {
	n.node.closeNode()
	// If no more threads use the directory we delete the TurtleDexDir to invalidate
	// the cache.
	if len(n.threads) == 0 {
		*n.lazyTurtleDexDir = nil
	}
}

// managedNewTurtleDexFileFromReader will read a siafile and its chunks from the given
// reader and add it to the directory. This will always load the file from the
// given reader.
func (n *DirNode) managedNewTurtleDexFileFromExisting(sf *siafile.TurtleDexFile, chunks siafile.Chunks) error {
	// Get the initial path of the siafile.
	path := sf.TurtleDexFilePath()
	...
	sf.UpdateUniqueID()
	sf.SetTurtleDexFilePath(currentPath)
	// Save the file to disk.
	if err := sf.SaveWithChunks(chunks); err != nil {
		return err
	}
	// Add the node to the dir.
	fileName := strings.TrimSuffix(filepath.Base(currentPath), modules.TurtleDexFileExtension)
	fn := &FileNode{
...
// managedNewTurtleDexFileFromLegacyData adds an existing TurtleDexFile to the filesystem
// using the provided siafile.FileData object.
func (n *DirNode) managedNewTurtleDexFileFromLegacyData(fileName string, fd siafile.FileData) (*FileNode, error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	// Check if the path is taken.
	path := filepath.Join(n.absPath(), fileName+modules.TurtleDexFileExtension)
	if _, err := os.Stat(filepath.Join(path)); !os.IsNotExist(err) {
		return nil, ErrExists
	}
	// Check if the file or folder exists already.
	key := strings.TrimSuffix(fileName, modules.TurtleDexFileExtension)
...
func (n *DirNode) managedUniquePrefix(path string, uid siafile.TurtleDexfileUID) (string, bool) {
	suffix := 0
	currentPath := path
	for {
		fileName := strings.TrimSuffix(filepath.Base(currentPath), modules.TurtleDexFileExtension)
		...
		if exists && oldFile.UID() == uid {
			...
			suffix++
			currentPath = strings.TrimSuffix(path, modules.TurtleDexFileExtension)
			currentPath = fmt.Sprintf("%v_%v%v", currentPath, suffix, modules.TurtleDexFileExtension)
...
// siaDir is a wrapper for the lazyTurtleDexDir field.
func (n *DirNode) siaDir() (*ttdxdir.TurtleDexDir, error) {
	if *n.lazyTurtleDexDir != nil {
		return *n.lazyTurtleDexDir, nil
	}
	sd, err := ttdxdir.LoadTurtleDexDir(n.absPath(), modules.ProdDependencies, n.staticWal)
	if os.IsNotExist(err) {
...
func (n *DirNode) managedDeleteFile(fileName string) error {
	...
	sysPath := filepath.Join(n.absPath(), fileName+modules.TurtleDexFileExtension)
...
// managedInfo builds and returns the DirectoryInfo of a TurtleDexDir.
func (n *DirNode) managedInfo(siaPath modules.TurtleDexPath) (modules.DirectoryInfo, error) {
	// Grab the ttdxdir metadata
...
func (n *DirNode) childExists(name string) bool {
	...
	_, errDir := os.Stat(filepath.Join(n.absPath(), name+modules.TurtleDexFileExtension))
...
// managedNewTurtleDexFile creates a new TurtleDexFile in the directory.
func (n *DirNode) managedNewTurtleDexFile(fileName string, source string, ec modules.ErasureCoder, mk crypto.CipherKey, fileSize uint64, fileMode os.FileMode, disablePartialUpload bool) error {
	n.mu.Lock()
	defer n.mu.Unlock()
	...
	_, err := siafile.New(filepath.Join(n.absPath(), fileName+modules.TurtleDexFileExtension), source, n.staticWal, ec, mk, fileSize, fileMode, nil, disablePartialUpload)
	return errors.AddContext(err, "NewTurtleDexFile: failed to create file")
}

// managedNewTurtleDexDir creates the TurtleDexDir with the given dirName as its child. We
// try to create the TurtleDexDir if it exists in memory but not on disk, as it may
// have just been deleted. We also do not return an error if the TurtleDexDir exists
// in memory and on disk already, which may be due to a race.
func (n *DirNode) managedNewTurtleDexDir(dirName string, rootPath string, mode os.FileMode) error {
	n.mu.Lock()
	defer n.mu.Unlock()
	...
	// Check that no dir or file exists on disk.
	_, err := os.Stat(filepath.Join(n.absPath(), dirName+modules.TurtleDexFileExtension))
	if !os.IsNotExist(err) {
		return ErrExists
	}
	_, err = ttdxdir.New(filepath.Join(n.absPath(), dirName), rootPath, mode, n.staticWal)
	if os.IsExist(err) {
		return nil
	}
	return err
...
// readonlyOpenFile opens a TurtleDexFile but doesn't add it to the parent. That's
...
func (n *DirNode) readonlyOpenFile(fileName string) (*FileNode, error) {
	...
	// Load file from disk.
	filePath := filepath.Join(n.absPath(), fileName+modules.TurtleDexFileExtension)
	sf, err := siafile.LoadTurtleDexFile(filePath, n.staticWal)
	if errors.Contains(err, siafile.ErrUnknownPath) || os.IsNotExist(err) {
		return nil, ErrNotExist
	}
	if err != nil {
		return nil, errors.AddContext(err, fmt.Sprintf("failed to load TurtleDexFile '%v' from disk", filePath))
	}
	fn = &FileNode{
...
func (n *DirNode) openDir(dirName string) (*DirNode, error) {
	...
	dirMDPath := filepath.Join(dirPath, modules.TurtleDexDirExtension)
...
// managedOpenDir opens a TurtleDexDir.
...
func (n *DirNode) removeFile(child *FileNode) {
	...
	if !exists || child.TurtleDexFile != currentChild.TurtleDexFile {
		return // Nothing to do
	}
...
func (n *DirNode) managedRename(newName string, oldParent, newParent *DirNode) error {
	...
	*n.path = newBase
	...
	for _, file := range filesToRename {
		*file.path = *file.path + modules.TurtleDexFileExtension
		file.UnmanagedSetTurtleDexFilePath(*file.path)
	}
	// Rename all dirs in memory.
	for _, dir := range dirsToRename {
		if *dir.lazyTurtleDexDir == nil {
			continue // dir isn't loaded
		}
		err = (*dir.lazyTurtleDexDir).SetPath(*dir.path)
		if err != nil {
