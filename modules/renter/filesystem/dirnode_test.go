...
// properly
func checkMetadataInit(md ttdxdir.Metadata) error {
	// Check Aggregate Fields
	if md.AggregateHealth != ttdxdir.DefaultDirHealth {
		return fmt.Errorf("TurtleDexDir AggregateHealth not set properly: got %v expected %v", md.AggregateHealth, ttdxdir.DefaultDirHealth)
	}
	...
	if md.AggregateMinRedundancy != ttdxdir.DefaultDirRedundancy {
		return fmt.Errorf("TurtleDexDir AggregateMinRedundancy not set properly: got %v expected %v", md.AggregateMinRedundancy, ttdxdir.DefaultDirRedundancy)
	}
	if md.AggregateModTime.IsZero() {
		return errors.New("AggregateModTime not initialized")
	}
	if md.AggregateNumFiles != 0 {
		return fmt.Errorf("TurtleDexDir AggregateNumFiles not set properly: got %v expected 0", md.AggregateNumFiles)
	}
	if md.AggregateNumStuckChunks != 0 {
		return fmt.Errorf("TurtleDexDir AggregateNumStuckChunks not initialized properly, expected 0, got %v", md.AggregateNumStuckChunks)
	}
	if md.AggregateNumSubDirs != 0 {
		return fmt.Errorf("TurtleDexDir AggregateNumSubDirs not initialized properly, expected 0, got %v", md.AggregateNumSubDirs)
	}
	if md.AggregateStuckHealth != ttdxdir.DefaultDirHealth {
		return fmt.Errorf("TurtleDexDir AggregateStuckHealth not set properly: got %v expected %v", md.AggregateStuckHealth, ttdxdir.DefaultDirHealth)
	}
	if md.AggregateSize != 0 {
		return fmt.Errorf("TurtleDexDir AggregateSize not set properly: got %v expected 0", md.AggregateSize)
	}

	// Check TurtleDexDir Fields
	if md.Health != ttdxdir.DefaultDirHealth {
		return fmt.Errorf("TurtleDexDir Health not set properly: got %v expected %v", md.Health, ttdxdir.DefaultDirHealth)
	}
	if !md.LastHealthCheckTime.IsZero() {
		return fmt.Errorf("LastHealthCheckTime should be zero but was %v", md.LastHealthCheckTime)
	}
	if md.MinRedundancy != ttdxdir.DefaultDirRedundancy {
		return fmt.Errorf("TurtleDexDir MinRedundancy not set properly: got %v expected %v", md.MinRedundancy, ttdxdir.DefaultDirRedundancy)
	}
	if md.ModTime.IsZero() {
		return errors.New("ModTime not initialized")
	}
	if md.NumFiles != 0 {
		return fmt.Errorf("TurtleDexDir NumFiles not initialized properly, expected 0, got %v", md.NumFiles)
	}
	if md.NumStuckChunks != 0 {
		return fmt.Errorf("TurtleDexDir NumStuckChunks not initialized properly, expected 0, got %v", md.NumStuckChunks)
	}
	if md.NumSubDirs != 0 {
		return fmt.Errorf("TurtleDexDir NumSubDirs not initialized properly, expected 0, got %v", md.NumSubDirs)
	}
	if md.StuckHealth != ttdxdir.DefaultDirHealth {
		return fmt.Errorf("TurtleDexDir stuck health not set properly: got %v expected %v", md.StuckHealth, ttdxdir.DefaultDirHealth)
	}
	if md.Size != 0 {
		return fmt.Errorf("TurtleDexDir Size not set properly: got %v expected 0", md.Size)
	}
	return nil
}
...
func equalMetadatas(md, md2 ttdxdir.Metadata) error {
	// Check Aggregate Fields
...
// TestUpdateTurtleDexDirSetMetadata probes the UpdateMetadata method of the TurtleDexDirSet
func TestUpdateTurtleDexDirSetMetadata(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	sp := newTurtleDexPath("path/to/dir")
