...
func (fs *FileSystem) CachedListCollect(siaPath modules.TurtleDexPath, recursive bool) (fis []modules.FileInfo, dis []modules.DirectoryInfo, err error) {
...
// newTestFileSystemWithFile creates a new FileSystem and TurtleDexFile and makes sure
// that they are linked
func newTestFileSystemWithFile(name string) (*FileNode, *FileSystem, error) {
	dir := testDir(name)
	fs := newTestFileSystem(dir)
	sp := modules.RandomTurtleDexPath()
	fs.addTestTurtleDexFile(sp)
	sf, err := fs.OpenTurtleDexFile(sp)
	return sf, fs, err
}

// newTestFileSystemWithDir creates a new FileSystem and TurtleDexDir and makes sure
// that they are linked
func newTestFileSystemWithDir(name string) (*DirNode, *FileSystem, error) {
	dir := testDir(name)
	fs := newTestFileSystem(dir)
	sp := modules.RandomTurtleDexPath()
	if err := fs.NewTurtleDexDir(sp, modules.DefaultDirPerm); err != nil {
		panic(err) // Reflect behavior of newTestFileSystemWithFile.
	}
	sd, err := fs.OpenTurtleDexDir(sp)
	return sd, fs, err
}
...
// newTurtleDexPath creates a new siapath from the specified string.
func newTurtleDexPath(path string) modules.TurtleDexPath {
	sp, err := modules.NewTurtleDexPath(path)
	if err != nil {
		panic(err)
	}
	return sp
...
// addTestTurtleDexFile is a convenience method to add a TurtleDexFile for testing to a
// FileSystem.
func (fs *FileSystem) addTestTurtleDexFile(siaPath modules.TurtleDexPath) {
	if err := fs.addTestTurtleDexFileWithErr(siaPath); err != nil {
		panic(err)
	}
}

// addTestTurtleDexFileWithErr is a convenience method to add a TurtleDexFile for testing to
// a FileSystem.
func (fs *FileSystem) addTestTurtleDexFileWithErr(siaPath modules.TurtleDexPath) error {
	ec, err := modules.NewRSSubCode(10, 20, crypto.SegmentSize)
	if err != nil {
		return err
	}
	err = fs.NewTurtleDexFile(siaPath, "", ec, crypto.GenerateTurtleDexKey(crypto.TypeDefaultRenter), uint64(fastrand.Intn(100)), persist.DefaultDiskPermissionsTest, false)
	if err != nil {
		return err
	}
	return nil
...
// TestNewTurtleDexDir tests if creating a new directory using NewTurtleDexDir creates the
...
// TestNewTurtleDexFile tests if creating a new file using NewTurtleDexFiles creates the
// correct folder structure and file.
func TestNewTurtleDexFile(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	sp := newTurtleDexPath("sub/foo/file")
	fs.addTestTurtleDexFile(sp)
	if err := fs.NewTurtleDexDir(sp, modules.DefaultDirPerm); !errors.Contains(err, ErrExists) {
	...
	if _, err := os.Stat(filepath.Join(root, sp.String()+modules.TurtleDexFileExtension)); err != nil {
		t.Fatal(err)
	}
	// Create a file in the root dir.
	sp = newTurtleDexPath("file")
	fs.addTestTurtleDexFile(sp)
	if err := fs.NewTurtleDexDir(sp, modules.DefaultDirPerm); !errors.Contains(err, ErrExists) {
		t.Fatal("err should be ErrExists but was", err)
	}
...
// TestOpenTurtleDexDir confirms that a previoiusly created TurtleDexDir can be opened and
// that the filesystem tree is extended accordingly in the process.
func TestOpenTurtleDexDir(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	fs := newTestFileSystem(root)
	// Create dir /foo
	sp := newTurtleDexPath("foo")
	if err := fs.NewTurtleDexDir(sp, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	// Open the newly created dir.
	foo, err := fs.OpenTurtleDexDir(sp)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	// Create dir /sub/foo. This time don't use NewTurtleDexDir but OpenTurtleDexDir with
	// the create flag set to `true`.
	sp = newTurtleDexPath("sub/foo")
	sd, err := fs.OpenTurtleDexDirCustom(sp, true)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	rootSD, err := fs.OpenTurtleDexDir(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	sd2, err := fs.OpenTurtleDexDir(sp)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	sdSub, err := fs.OpenTurtleDexDir(newTurtleDexPath("sub"))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
// TestOpenTurtleDexFile confirms that a previously created TurtleDexFile can be opened and
// that the filesystem tree is extended accordingly in the process.
func TestOpenTurtleDexFile(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	if *sf.path != filepath.Join(root, (*sf.name)+modules.TurtleDexFileExtension) {
	...
	sp = newTurtleDexPath("/sub1/sub2/file")
	fs.addTestTurtleDexFile(sp)
	// Open the newly created file.
	sf2, err := fs.OpenTurtleDexFile(sp)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
// TestCloseTurtleDexDir tests that closing an opened directory shrinks the tree
// accordingly.
func TestCloseTurtleDexDir(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	sd1, err := fs.OpenTurtleDexDir(sp)
	if err != nil {
		t.Fatal(err)
	}
	sd2, err := fs.OpenTurtleDexDirCustom(sp, true)
	if err != nil {
		t.Fatal(err)
	}
...
// TestCloseTurtleDexFile tests that closing an opened file shrinks the tree
// accordingly.
func TestCloseTurtleDexFile(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	sf1, err := fs.OpenTurtleDexFile(sp)
	if err != nil {
		t.Fatal(err)
	}
	sf2, err := fs.OpenTurtleDexFile(sp)
...
func TestDeleteFile(t *testing.T) {
	...
	if _, err := fs.OpenTurtleDexFile(sp); !errors.Contains(err, ErrNotExist) {
		t.Fatal("err should be ErrNotExist but was:", err)
	}
	if err := fs.addTestTurtleDexFileWithErr(sp); err != nil {
...
func TestDeleteDirectory(t *testing.T) {
	...
	fs := newTestFileSystem(root)
	// Add some files.
	fs.addTestTurtleDexFile(newTurtleDexPath("dir/foo/bar/file1"))
	fs.addTestTurtleDexFile(newTurtleDexPath("dir/foo/bar/file2"))
	fs.addTestTurtleDexFile(newTurtleDexPath("dir/foo/bar/file3"))
	// Delete "foo"
	if err := fs.DeleteDir(newTurtleDexPath("/dir/foo")); err != nil {
		t.Fatal(err)
	}
...
func TestRenameFile(t *testing.T) {
	...
	fs := newTestFileSystem(root)
	// Add a file to the root dir.
	foo := newTurtleDexPath("foo")
	foobar := newTurtleDexPath("foobar")
	barfoo := newTurtleDexPath("bar/foo")
	fs.addTestTurtleDexFile(foo)
	// Rename the file.
	if err := fs.RenameFile(foo, foobar); err != nil {
		t.Fatal(err)
	}
	// Check if the file was renamed.
	if _, err := fs.OpenTurtleDexFile(foo); !errors.Contains(err, ErrNotExist) {
		t.Fatal("expected ErrNotExist but got:", err)
	}
	sf, err := fs.OpenTurtleDexFile(foobar)
	if err != nil {
		t.Fatal("expected ErrNotExist but got:", err)
	}
	sf.Close()
	...
	sf, err = fs.OpenTurtleDexFile(barfoo)
	if err != nil {
		t.Fatal("expected ErrNotExist but got:", err)
	}
	sf.Close()
...
func TestThreadedAccess(t *testing.T) {
	...
	for _, fp := range filePaths {
		fs.addTestTurtleDexFile(newTurtleDexPath(fp))
	}
	...
	for i := 0; i < numThreads; i++ {
		...
		go func() {
			...
			for {
				...
				sp := newTurtleDexPath(filePaths[fastrand.Intn(len(filePaths))])
				sf, err := fs.OpenTurtleDexFile(sp)
				if err != nil {
					t.Fatal(err)
				}
...
// TestTurtleDexDirRename tests the Rename method of the ttdxdirset.
func TestTurtleDexDirRename(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	...
	f := func(entry *DirNode) {
		...
		for {
			...
			err := entry.UpdateMetadata(ttdxdir.Metadata{})
			if err != nil {
				t.Error(err)
				return
			}
	...
	for _, dir := range dirStructure {
		sp, err := modules.NewTurtleDexPath(dir)
		if err != nil {
			t.Fatal(err)
		}
		err = fs.NewTurtleDexDir(sp, modules.DefaultDirPerm)
		if err != nil {
			t.Fatal(err)
		}
		entry, err := fs.OpenTurtleDexDir(sp)
		if err != nil {
			t.Fatal(err)
		}
	...
	// Rename dir1 to dir2.
	oldPath, err1 := modules.NewTurtleDexPath(dirStructure[0])
	newPath, err2 := modules.NewTurtleDexPath("dir2")
	...
	for _, dir := range dirStructure {
		...
		entry, err := fs.OpenTurtleDexDir(newDir)
		if err != nil {
			t.Fatal(err)
		}
		defer func() {
...
// TestAddTurtleDexFileFromReader tests the AddTurtleDexFileFromReader method's behavior.
func TestAddTurtleDexFileFromReader(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	if err := sfs.AddTurtleDexFileFromReader(bytes.NewReader(d), sfs.FileTurtleDexPath(sf)); err != nil {
		t.Fatal(err)
	}
	numTurtleDexFiles := 0
	err = sfs.Walk(modules.RootTurtleDexPath(), func(path string, info os.FileInfo, err error) error {
		if filepath.Ext(path) == modules.TurtleDexFileExtension {
			numTurtleDexFiles++
		}
		return nil
	})
	...
	if numTurtleDexFiles != 1 {
		t.Fatalf("Found %v siafiles but expected %v", numTurtleDexFiles, 1)
	}
	// Load the same siafile again, but change the UID.
	b, err := ioutil.ReadFile(sf.TurtleDexFilePath())
	if err != nil {
		t.Fatal(err)
	}
	reader := bytes.NewReader(b)
	newSF, newChunks, err := siafile.LoadTurtleDexFileFromReaderWithChunks(reader, sf.TurtleDexFilePath(), sfs.staticWal)
	if err != nil {
		t.Fatal(err)
	}
	// Save the file to a temporary location with the new uid.
	newSF.UpdateUniqueID()
	newSF.SetTurtleDexFilePath(sf.TurtleDexFilePath() + "_tmp")
	...
	// UID.
	b, err = ioutil.ReadFile(newSF.TurtleDexFilePath())
	if err != nil {
		t.Fatal(err)
	}
	// Remove file at temporary location after reading it.
	if err := os.Remove(newSF.TurtleDexFilePath()); err != nil {
		t.Fatal(err)
	}
	reader = bytes.NewReader(b)
	var newSFTurtleDexPath modules.TurtleDexPath
	if err := newSFTurtleDexPath.FromSysPath(sf.TurtleDexFilePath(), sfs.Root()); err != nil {
		t.Fatal(err)
	}
	if err := sfs.AddTurtleDexFileFromReader(reader, newSFTurtleDexPath); err != nil {
		t.Fatal(err)
	}
	// Reload newSF with the new expected path.
	newSFPath := filepath.Join(filepath.Dir(sf.TurtleDexFilePath()), newSFTurtleDexPath.String()+"_1"+modules.TurtleDexFileExtension)
	newSF, err = siafile.LoadTurtleDexFile(newSFPath, sfs.staticWal)
	if err != nil {
		t.Fatal(err)
	}
	...
	if numTurtleDexFiles != 2 {
		t.Fatalf("Found %v siafiles but expected %v", numTurtleDexFiles, 2)
	}
	// The UID should have changed.
	if newSF.UID() == preImportUID {
		t.Fatal("newSF UID should have changed after importing the file")
	}
	if !strings.HasSuffix(newSF.TurtleDexFilePath(), "_1"+modules.TurtleDexFileExtension) {
		t.Fatal("TurtleDexFile should have a suffix but didn't")
	}
	// Should be able to open the new file from disk.
	if _, err := os.Stat(newSF.TurtleDexFilePath()); err != nil {
		t.Fatal(err)
	}
}

// TestTurtleDexFileSetDeleteOpen checks that deleting an entry from the set followed
// by creating a TurtleDexfile with the same name without closing the deleted entry
// works as expected.
func TestTurtleDexFileSetDeleteOpen(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	for i := 0; i < 10; i++ {
		...
		err := sfs.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, sk, fileSize, fileMode, up.DisablePartialChunk)
		if err != nil {
			t.Fatal(err)
		}
		entry, err := sfs.OpenTurtleDexFile(up.TurtleDexPath)
		if err != nil {
			t.Fatal(err)
		}
		// Delete TurtleDexFile
		if err := sfs.DeleteFile(sfs.FileTurtleDexPath(entry)); err != nil {
			t.Fatal(err)
		}
		// The map should be empty.
		if len(sfs.files) != 0 {
			t.Fatal("TurtleDexFileMap should have 1 file")
		}
		// Append the entry to make sure we can close it later.
		entries = append(entries, entry)
	}
	// The TurtleDexFile shouldn't exist anymore.
	_, err := sfs.OpenTurtleDexFile(siaPath)
	if !errors.Contains(err, ErrNotExist) {
		t.Fatal("TurtleDexFile shouldn't exist anymore")
	}
	// Close the entries.
...
// TestTurtleDexFileSetOpenClose tests that the threadCount of the siafile is
// incremented and decremented properly when Open() and Close() are called
func TestTurtleDexFileSetOpenClose(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	siaPath := sfs.FileTurtleDexPath(entry)
	exists, _ := sfs.FileExists(siaPath)
	if !exists {
		t.Fatal("No TurtleDexFileSetEntry found")
	}
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(sfs.files) != 1 {
		t.Fatalf("Expected TurtleDexFileSet map to be of length 1, instead is length %v", len(sfs.files))
	}
	...
	// Confirm file and partialsTurtleDexFile were removed from memory
	if len(sfs.files) != 0 {
		t.Fatalf("Expected TurtleDexFileSet map to contain 0 files, instead is length %v", len(sfs.files))
	}

	// Open siafile again and confirm threadCount was incremented
	entry, err = sfs.OpenTurtleDexFile(siaPath)
	if err != nil {
		t.Fatal(err)
	}
	if len(entry.threads) != 1 {
...
func TestFilesInMemory(t *testing.T) {
	...
	//
	// Open file
	entry1, err := sfs.OpenTurtleDexFile(siaPath)
	if err != nil {
		t.Fatal(err)
	}
	// Confirm there is 1 file in memory
	...
	entry2, err := sfs.OpenTurtleDexFile(siaPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenameFileInMemory(t *testing.T) {
	...
	exists, _ := sfs.FileExists(siaPath)
	if !exists {
		t.Fatal("No TurtleDexFile found")
	}
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = os.Stat(entry.TurtleDexFilePath())
	if err != nil {
		t.Fatal(err)
	}
	// Rename second instance
	newTurtleDexPath := modules.RandomTurtleDexPath()
	err = sfs.RenameFile(siaPath, newTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteFileInMemory(t *testing.T) {
	...
	// Create FileSystem with TurtleDexFile
	entry, sfs, err := newTestFileSystemWithFile(t.Name())
	if err != nil {
		t.Fatal(err)
	}
...
// TestDeleteCorruptTurtleDexFile confirms that the siafileset will delete a siafile
// even if it cannot be opened
func TestDeleteCorruptTurtleDexFile(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	siaPath, err := modules.NewTurtleDexPath("badFile")
	if err != nil {
		t.Fatal(err)
	}
	siaFilePath := siaPath.TurtleDexFileSysPath(sfs.Root())
...
// TestTurtleDexDirDelete tests the DeleteDir method of the siafileset.
func TestTurtleDexDirDelete(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	// Prepare a ttdxdirset
	root := filepath.Join(testDir(t.Name()), "fs-root")
	os.RemoveAll(root)
	fs := newTestFileSystem(root)
	...
	for _, dir := range dirStructure {
		...
		up := modules.FileUploadParams{Source: "", TurtleDexPath: fileSP, ErasureCode: ec}
		err = fs.NewTurtleDexFile(up.TurtleDexPath, up.Source, up.ErasureCode, crypto.GenerateTurtleDexKey(crypto.TypeDefaultRenter), 100, persist.DefaultDiskPermissionsTest, up.DisablePartialChunk)
		if err != nil {
			t.Fatal(err)
		}
		sf, err := fs.OpenTurtleDexFile(up.TurtleDexPath)
		if err != nil {
			t.Fatal(err)
		}
		// 50% chance to spawn goroutine. It's not realistic to assume that all dirs
	...
	time.Sleep(time.Second)
	// Delete dir1.
	sp, err := modules.NewTurtleDexPath("dir1")
	if err != nil {
		t.Fatal(err)
	}
	...
	wg.Wait()
	time.Sleep(time.Second)
	// The root siafile dir should be empty except for 1 .ttdxdir file.
	files, err := fs.ReadDir(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	if len(files) != 1 {
	...
	for _, file := range files {
		if filepath.Ext(file.Name()) != modules.TurtleDexDirExtension &&
			filepath.Ext(file.Name()) != modules.PartialsTurtleDexFileExtension {
			t.Fatal("Encountered unexpected file:", file.Name())
		}
	}
}

// TestTurtleDexDirRenameWithFiles tests the RenameDir method of the filesystem with
// files.
func TestTurtleDexDirRenameWithFiles(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	// Prepare a filesystem.
	...
	for _, dir := range dirStructure {
		...
		err = fs.NewTurtleDexFile(fileSP, source, rc, sk, fileSize, fileMode, true)
		if err != nil {
			t.Fatal(err)
		}
		sf, err := fs.OpenTurtleDexFile(fileSP)
		if err != nil {
			t.Fatal(err)
		}
		// 50% chance to spawn goroutine. It's not realistic to assume that all dirs
	...
	for _, dir := range dirStructure {
		...
		numFiles := 0
		for _, fi := range fis {
			if !fi.IsDir() && filepath.Ext(fi.Name()) == modules.TurtleDexFileExtension {
				numFiles++
			}
		}
		if numFiles != 1 {
...
func TestRenameDirInMemory(t *testing.T) {
	...
	// Create FileSystem with corresponding ttdxdir.
	entry, fs, err := newTestFileSystemWithDir(t.Name())
	if err != nil {
		t.Fatal(err)
	}
	siaPath := fs.DirTurtleDexPath(entry)
	exists, _ := fs.DirExists(siaPath)
	if !exists {
		t.Fatal("No TurtleDexDir found")
	}
	...
	entry2, err := fs.OpenTurtleDexDir(siaPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	newTurtleDexPath := modules.RandomTurtleDexPath()
	err = fs.RenameDir(siaPath, newTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteDirInMemory(t *testing.T) {
	...
	dirPath := fs.DirTurtleDexPath(entry)
	exists, _ := fs.DirExists(dirPath)
	if !exists {
		t.Fatal("No TurtleDexDirSetEntry found")
	}
	...
	fs.addTestTurtleDexFile(filepath1)
	fileEntry1, err := fs.OpenTurtleDexFile(filepath1)
	if err != nil {
		t.Fatal(err)
	}
	...
	fs.addTestTurtleDexFile(filepath2)
	fileEntry2, err := fs.OpenTurtleDexFile(filepath2)
	if err != nil {
		t.Fatal(err)
	}
	...
	entry2, err := fs.OpenTurtleDexDir(dirPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Create a new dir at the same path.
	err = fs.NewTurtleDexDir(dirPath, modules.DefaultDirPerm)
	if err != nil {
		t.Fatal(err)
	}
	// Get the entry.
	entry3, err := fs.OpenTurtleDexDir(dirPath)
	if err != nil {
		t.Fatal(err)
	}
...
// TestLazyTurtleDexDir tests that siaDir correctly reads and sets the lazyTurtleDexDir
// field.
func TestLazyTurtleDexDir(t *testing.T) {
	if testing.Short() && !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
	...
	if *foo2.lazyTurtleDexDir != sd {
		t.Fatal("foo2.lazyTurtleDexDir isn't set correctly", foo2.lazyTurtleDexDir)
	}
}
...
func TestOpenCloseRoot(t *testing.T) {
	...
	rootNode, err := fs.OpenTurtleDexDir(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	err = rootNode.Close()
...
// TestFailedOpenFileFolder makes sure that a failed call to OpenTurtleDexFile or
...
func TestFailedOpenFileFolder(t *testing.T) {
	...
	// Open "foo" as a file.
	_, err = fs.OpenTurtleDexFile(foo)
	if !errors.Contains(err, ErrNotExist) {
		t.Fatal("err should be ErrNotExist but was", err)
	}
	if len(fs.files) != 0 || len(fs.directories) != 0 {
...
func testFileDirConflict(t *testing.T, open bool) {
	...
	// Create a file.
	filepath := newTurtleDexPath("dir1/file1")
	fs.addTestTurtleDexFile(filepath)

	if open {
		// Open the file. This shouldn't affect later checks.
		node, err := fs.OpenTurtleDexFile(filepath)
		if err != nil {
			t.Fatal(err)
		}
		defer func() {
	...
	err = fs.addTestTurtleDexFileWithErr(filepath)
	...
	filepath2 := newTurtleDexPath("dir1/file2")
	fs.addTestTurtleDexFile(filepath2)
	...
	err = fs.NewTurtleDexDir(filepath, modules.DefaultDirPerm)
	if !errors.Contains(err, ErrExists) {
		t.Fatalf("Expected err %v, got %v", ErrExists, err)
	}

	// Create a dir.
	dirpath := newTurtleDexPath("dir2/dir3")
	err = fs.NewTurtleDexDir(dirpath, modules.DefaultDirPerm)
	if err != nil {
		t.Fatal(err)
	}
	...
	if open {
		...
		node, err := fs.OpenTurtleDexDir(dirpath)
		if err != nil {
			t.Fatal(err)
		}
		defer func() {
	...
	err = fs.addTestTurtleDexFileWithErr(dirpath)
	if !errors.Contains(err, ErrExists) {
		t.Fatalf("Expected err %v, got %v", ErrExists, err)
	}
	...
	dirpath2 := newTurtleDexPath("dir2/dir4")
	err = fs.NewTurtleDexDir(dirpath2, modules.DefaultDirPerm)
	if err != nil {
		t.Fatal(err)
	}
...
func TestList(t *testing.T) {
	...
	for _, d := range dirStructure {
		// Create directory
		siaPath := newTurtleDexPath(d)
		err := fs.NewTurtleDexDir(siaPath, persist.DefaultDiskPermissionsTest)
		if err != nil {
			t.Fatal(err)
		}

		// Add a file
		fileTurtleDexPath, err := siaPath.Join("file")
		if err != nil {
			t.Fatal(err)
		}
		fs.addTestTurtleDexFile(fileTurtleDexPath)
	}

	// Get the cached information
	fis, dis, err := fs.CachedListCollect(newTurtleDexPath(dirStructure[0]), true)
	if err != nil {
		t.Fatal(err)
	}
