...
// marshalMetadata marshals the metadata of the TurtleDexFile using json encoding.
func marshalMetadata(md Metadata) ([]byte, error) {
	return json.Marshal(md)
}

// marshalPubKeyTable marshals the public key table of the TurtleDexFile using TurtleDex
// encoding.
func marshalPubKeyTable(pubKeyTable []HostPublicKey) ([]byte, error) {
	...
	for _, pk := range pubKeyTable {
		if err := pk.MarshalTurtleDex(&buf); err != nil {
			return nil, err
		}
	}
...
// unmarshalMetadata unmarshals the json encoded metadata of the TurtleDexFile.
...
func unmarshalPubKeyTable(raw []byte) (keys []HostPublicKey, err error) {
	...
	for {
		...
		if err = key.UnmarshalTurtleDex(r); errors.Contains(err, io.EOF) {
			break
		} else if err != nil {
			return nil, err
		}
