...
type (
	// Snapshot is a snapshot of a TurtleDexFile. A snapshot is a deep-copy and
	...
	Snapshot struct {
		...
		staticTurtleDexPath         modules.TurtleDexPath
		staticLocalPath       string
		staticPartialChunks   []PartialChunkInfo
		staticUID             TurtleDexfileUID
	}
)

// SnapshotReader is a helper type that allows reading a raw TurtleDexFile from disk
...
// TurtleDexfile from disk. Note that the underlying siafile holds a readlock until
...
func (sf *TurtleDexFile) SnapshotReader() (*SnapshotReader, error) {
	// Lock the file.
	sf.mu.RLock()
	if sf.deleted {
		sf.mu.RUnlock()
		return nil, errors.AddContext(ErrDeleted, "can't copy deleted TurtleDexFile")
	}
	// Open file.
...
// TurtleDexPath returns the TurtleDexPath of the file.
func (s *Snapshot) TurtleDexPath() modules.TurtleDexPath {
	return s.staticTurtleDexPath
}
...
func (s *Snapshot) UID() TurtleDexfileUID {
	return s.staticUID
}

// readlockChunks reads all chunks from the siafile within the range [min;max].
func (sf *TurtleDexFile) readlockChunks(min, max int) ([]chunk, error) {
	// Copy chunks.
...
// readlockSnapshot creates a snapshot of the TurtleDexFile.
func (sf *TurtleDexFile) readlockSnapshot(sp modules.TurtleDexPath, chunks []chunk) (*Snapshot, error) {
	...
	for _, chunk := range chunks {
		...
		if cci, ok := sf.isIncludedPartialChunk(uint64(chunk.Index)); ok {
			pieces, err := sf.partialsTurtleDexFile.Pieces(cci.Index)
			if err != nil {
				return nil, err
			}
	...
	return &Snapshot{
		...
		staticTurtleDexPath:         sp,
		staticLocalPath:       localPath,
		staticUID:             uid,
	}, nil
}

// Snapshot creates a snapshot of the TurtleDexFile.
func (sf *TurtleDexFile) Snapshot(sp modules.TurtleDexPath) (*Snapshot, error) {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// SnapshotRange creates a snapshot of the TurtleDexfile over a specific range.
func (sf *TurtleDexFile) SnapshotRange(sp modules.TurtleDexPath, offset, length uint64) (*Snapshot, error) {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// snapshot from it.
func SnapshotFromReader(sp modules.TurtleDexPath, r io.Reader) (*Snapshot, error) {
	d, err := ioutil.ReadAll(r)
	if err != nil {
		return nil, err
	}
	sf, chunks, err := LoadTurtleDexFileFromReaderWithChunks(bytes.NewReader(d), "", nil)
	if err != nil {
		return nil, err
	}
