...
var (
	...
	// ErrUnknownThread is an error when a TurtleDexFile is trying to be closed by a
...
type (
	// TurtleDexFile is the disk format for files uploaded to the TurtleDex network.  It
	...
	TurtleDexFile struct {
		// staticMetadata is the mostly static staticMetadata of a TurtleDexFile. The reserved
		...
		mu      sync.RWMutex
		wal     *writeaheadlog.WAL // the wal that is used for TurtleDexFiles

		// siaFilePath is the path to the .sia file on disk.
		siaFilePath string

		// partialTurtleDexFile is the TurtleDexFile that holds or could hold the partial chunk of
		// this siafile. Since we don't know if a file is going to have a partial
		// chunk we simply keep the megafiles always open and assign them to TurtleDexFiles
		// with matching redundancy.
		partialsTurtleDexFile *TurtleDexFile
	}
	...
	// the table offset.
	Piece struct {
		HostPubKey types.TurtleDexPublicKey // public key of the host
		MerkleRoot crypto.Hash        // merkle root of the piece
	}

	// HostPublicKey is an entry in the HostPubKey table.
	HostPublicKey struct {
		PublicKey types.TurtleDexPublicKey // public key of host
...
func (hpk HostPublicKey) MarshalTurtleDex(w io.Writer) error {
...
// TurtleDexFilePath returns the siaFilePath field of the TurtleDexFile.
func (sf *TurtleDexFile) TurtleDexFilePath() string {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	return sf.siaFilePath
}

// Lock acquires the TurtleDexFile's mutex for calling Unmanaged exported methods.
func (sf *TurtleDexFile) Lock() {
	sf.mu.Lock()
}

// Unlock releases the TurtleDexFile's mutex.
func (sf *TurtleDexFile) Unlock() {
	sf.mu.Unlock()
}

// UnmanagedSetDeleted sets the deleted field of the TurtleDexFile without
// holding the lock.
func (sf *TurtleDexFile) UnmanagedSetDeleted(deleted bool) {
	sf.deleted = deleted
}

// UnmanagedSetTurtleDexFilePath sets the siaFilePath field of the TurtleDexFile without
// holding the lock.
func (sf *TurtleDexFile) UnmanagedSetTurtleDexFilePath(newTurtleDexFilePath string) {
	sf.siaFilePath = newTurtleDexFilePath
}

// UnmarshalTurtleDex implements the encoding.TurtleDexUnmarshaler interface.
func (hpk *HostPublicKey) UnmarshalTurtleDex(r io.Reader) error {
...
// New create a new TurtleDexFile.
func New(siaFilePath, source string, wal *writeaheadlog.WAL, erasureCode modules.ErasureCoder, masterKey crypto.CipherKey, fileSize uint64, fileMode os.FileMode, partialsTurtleDexFile *TurtleDexFile, disablePartialUpload bool) (*TurtleDexFile, error) {
	// TODO remove this
	...
	file := &TurtleDexFile{
		...
		partialsTurtleDexFile: partialsTurtleDexFile,
		siaFilePath:     siaFilePath,
		wal:             wal,
	}
	// Init chunks.
	numChunks := fileSize / file.staticChunkSize()
	if fileSize%file.staticChunkSize() != 0 && partialsTurtleDexFile != nil && !disablePartialUpload {
		...
		numChunks++
	} else if fileSize%file.staticChunkSize() != 0 && partialsTurtleDexFile == nil {
		return nil, errors.New("can't create a file with a partial chunk without assigning a partialsTurtleDexFile")
	}
...
// GrowNumChunks increases the number of chunks in the TurtleDexFile to numChunks. If
// the file already contains >= numChunks chunks then GrowNumChunks is a no-op.
func (sf *TurtleDexFile) GrowNumChunks(numChunks uint64) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
// RemoveLastChunk removes the last chunk of the TurtleDexFile and truncates the file
// accordingly.
func (sf *TurtleDexFile) RemoveLastChunk() error {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	return sf.removeLastChunk()
}

// SetFileSize changes the fileSize of the TurtleDexFile.
func (sf *TurtleDexFile) SetFileSize(fileSize uint64) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	if sf.deleted {
	...
	if !sf.staticMetadata.DisablePartialChunk && uint64(sf.staticMetadata.FileSize)%sf.staticChunkSize() != 0 {
		...
		if sf.partialsTurtleDexFile == nil {
			return errors.New("can't turn file without partial chunk into a file with one if partialsTurtleDexFile == nil")
		}
	}
...
func (sf *TurtleDexFile) AddPiece(pk types.TurtleDexPublicKey, chunkIndex, pieceIndex uint64, merkleRoot crypto.Hash) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	if cci, ok := sf.isIncludedPartialChunk(chunkIndex); ok {
		return sf.partialsTurtleDexFile.AddPiece(pk, cci.Index, pieceIndex, merkleRoot)
	}
...
func (sf *TurtleDexFile) chunkHealth(chunk chunk, offlineMap map[string]bool, goodForRenewMap map[string]bool) (h float64, uh float64, _ uint64, err error) {
	...
	if cci, ok := sf.isIncludedPartialChunk(uint64(chunk.Index)); ok && !incomplete {
		return sf.partialsTurtleDexFile.ChunkHealth(int(cci.Index), offlineMap, goodForRenewMap)
	}
...
func (sf *TurtleDexFile) ChunkHealth(index int, offlineMap map[string]bool, goodForRenewMap map[string]bool) (float64, float64, uint64, error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) Delete() (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) Deleted() bool {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	return sf.deleted
}
...
func (sf *TurtleDexFile) ErasureCode() modules.ErasureCoder {
...
// the end of the file.
func (sf *TurtleDexFile) SaveWithChunks(chunks Chunks) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) SaveHeader() (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) SaveMetadata() (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	if sf.deleted {
...
func (sf *TurtleDexFile) Expiration(contracts map[string]modules.RenterContract) types.BlockHeight {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	for _, pc := range sf.staticMetadata.PartialChunks {
		...
		ps, err := sf.partialsTurtleDexFile.Pieces(pc.Index)
		if err == nil {
...
// calculated for the TurtleDexFile and returned.
//
...
func (sf *TurtleDexFile) Health(offline map[string]bool, goodForRenew map[string]bool) (h, sh, uh, ush float64, nsc, rbr, sb uint64) {
	numPieces := sf.staticMetadata.staticErasureCode.NumPieces()
	minPieces := sf.staticMetadata.staticErasureCode.MinPieces()
...
func (sf *TurtleDexFile) HostPublicKeys() (spks []types.TurtleDexPublicKey) {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	// Only return the keys, not the whole entry.
	keys := make([]types.TurtleDexPublicKey, 0, len(sf.pubKeyTable))
...
func (sf *TurtleDexFile) IsIncludedPartialChunk(chunkIndex uint64) bool {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) IsIncompletePartialChunk(chunkIndex uint64) bool {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// fully uploaded yet.
func (sf *TurtleDexFile) NumChunks() uint64 {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) Pieces(chunkIndex uint64) ([][]Piece, error) {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	...
	if cc, ok := sf.isIncludedPartialChunk(chunkIndex); ok {
		return sf.partialsTurtleDexFile.Pieces(cc.Index) // get pieces from linked siafile
	}
	if sf.isIncompletePartialChunk(chunkIndex) {
...
func (sf *TurtleDexFile) Redundancy(offlineMap map[string]bool, goodForRenewMap map[string]bool) (r, ur float64, err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) SetAllStuck(stuck bool) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
// completed.
func (sf *TurtleDexFile) SetChunkStatusCompleted(pci uint64) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) SetStuck(index uint64, stuck bool) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) StuckChunkByIndex(index uint64) (bool, error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) UID() TurtleDexfileUID {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// of the TurtleDexFile. The keys of all used hosts should be passed to the method
// and the TurtleDexFile will update the flag for hosts it knows of to 'true' and set
// hosts which were not passed in to 'false'.
func (sf *TurtleDexFile) UpdateUsedHosts(used []types.TurtleDexPublicKey) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	if sf.partialsTurtleDexFile != nil {
		return sf.partialsTurtleDexFile.UpdateUsedHosts(used)
	}
	return nil
}
...
// redundant pieces.
func (sf *TurtleDexFile) defragChunk(chunk *chunk) {
...
func (sf *TurtleDexFile) hostKey(offset uint32) HostPublicKey {
...
// isIncludedPartialChunk returns 'true' if the provided index points to a
// partial chunk which has been added to the partials sia file already.
func (sf *TurtleDexFile) isIncludedPartialChunk(chunkIndex uint64) (PartialChunkInfo, bool) {
...
// host.
func (sf *TurtleDexFile) pruneHosts() (_ []writeaheadlog.Update, err error) {
...
// renew.
func (sf *TurtleDexFile) GoodPieces(chunkIndex int, offlineMap map[string]bool, goodForRenewMap map[string]bool) (uint64, uint64) {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// of unique pieces that are good for renew, meaning the contract is set to
// renew.
func (sf *TurtleDexFile) goodPieces(chunk chunk, offlineMap map[string]bool, goodForRenewMap map[string]bool) (uint64, uint64) {
	numPiecesGoodForRenew := uint64(0)
	numPiecesGoodForUpload := uint64(0)

	// Handle partial chunk.
	if cci, ok := sf.isIncludedPartialChunk(uint64(chunk.Index)); ok {
		return sf.partialsTurtleDexFile.GoodPieces(int(cci.Index), offlineMap, goodForRenewMap)
	}
	if sf.isIncompletePartialChunk(uint64(chunk.Index)) {
		return 0, 0
	}
...
func (sf *TurtleDexFile) UploadProgressAndBytes() (float64, uint64, error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	return sf.uploadProgressAndBytes()
}

// Chunk returns the chunk of a TurtleDexFile at a given index.
func (sf *TurtleDexFile) Chunk(chunkIndex uint64) (chunk, error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
// growNumChunks increases the number of chunks in the TurtleDexFile to numChunks. If
// the file already contains >= numChunks chunks then GrowNumChunks is a no-op.
func (sf *TurtleDexFile) growNumChunks(numChunks uint64) (updates []writeaheadlog.Update, err error) {
	if sf.deleted {
		return nil, errors.AddContext(ErrDeleted, "can't grow number of chunks of deleted file")
	}
	// Don't allow a TurtleDexFile with a partial chunk to grow.
	if sf.staticMetadata.HasPartialChunk {
...
func (sf *TurtleDexFile) removeLastChunk() (err error) {
	if sf.deleted {
...
func (sf *TurtleDexFile) setStuck(index uint64, stuck bool) (err error) {
	...
	if cci, ok := sf.isIncludedPartialChunk(index); ok {
		return sf.partialsTurtleDexFile.SetStuck(cci.Index, stuck)
	}
...
func (sf *TurtleDexFile) uploadedBytes() (uint64, uint64, error) {
