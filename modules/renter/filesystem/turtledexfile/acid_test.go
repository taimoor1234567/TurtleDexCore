...
// TestTurtleDexFileFaultyDisk simulates interacting with a TurtleDexFile on a faulty disk.
func TestTurtleDexFileFaultyDisk(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	hostkeys := make([]types.TurtleDexPublicKey, 0, 50)
	for i := 0; i < 50; i++ {
		spk := types.TurtleDexPublicKey{}
		fastrand.Read(spk.Key)
		hostkeys = append(hostkeys, types.TurtleDexPublicKey{})
	}

	// The outer loop is responsible for simulating a restart of ttdxd by
...
OUTER:
	for {
		...
		for {
			...
			if fastrand.Intn(100) < 80 {
				...
				snap, err := sf.Snapshot(modules.RandomTurtleDexPath())
				if err != nil {
	...
	LOAD:
		for tries := 0; ; tries++ {
			...
			/*
				 PARTIAL TODO:
						 TODO: Uncomment once we enable partial chunks again
							_, err = loadTurtleDexFile(sf.partialsTurtleDexFile.siaFilePath, wal, fdd)
							if err != nil {
								if errors.Contains(err, dependencies.ErrDiskFault) {
									numRecoveries++
									continue // try again
			...
			*/
			sf, err = loadTurtleDexFile(sf.siaFilePath, wal, fdd)
			sf.deps = fdd
			if err != nil {
				if errors.Contains(err, dependencies.ErrDiskFault) {
					numRecoveries++
			...
			*/
			...
			sf.SetPartialsTurtleDexFile(nil)
			break
		}
	}
