...
// TestSnapshot tests if a snapshot is created correctly from a TurtleDexFile.
func TestSnapshot(t *testing.T) {
	...
	// Make sure the snapshot has the same fields as the TurtleDexFile.
	...
	if sp != snap.staticTurtleDexPath {
...
// snapshots of TurtleDexfiles with different sizes.
func benchmarkSnapshot(b *testing.B, fileSize uint64) {
	...
	for i := uint64(0); i < sf.NumChunks(); i++ {
		for j := uint64(0); j < uint64(rc.NumPieces()); j++ {
			if err := sf.AddPiece(types.TurtleDexPublicKey{}, i, j, crypto.Hash{}); err != nil {
				b.Fatal(err)
			}
		}
	}
	...
	for i := 0; i < b.N; i++ {
		_, err := sf.Snapshot(modules.RootTurtleDexPath())
		if err != nil {
			b.Fatal(err)
		}
	}
