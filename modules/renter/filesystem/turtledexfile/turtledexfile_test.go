...
// setCombinedChunkOfTestFile adds one or two Combined chunks to a TurtleDexFile for
// tests to be able to use a TurtleDexFile that already has its partial chunk
// contained within a combined chunk. If the TurtleDexFile doesn't have a partial
// chunk, this is a no-op. The combined chunk will be stored in the provided
// 'dir'.
func setCombinedChunkOfTestFile(sf *TurtleDexFile) error {
	if true {
...
// TurtleDexFile for testing and changes its status to completed.
func setCustomCombinedChunkOfTestFile(sf *TurtleDexFile, numCombinedChunks int) error {
	if true {
		// PARTIAL TODO: remove when partial chunks are enabled again
		return nil
	}
...
func TestFileRedundancy(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		pk := types.TurtleDexPublicKey{Key: []byte{byte(i)}}
	...
	for _, nData := range nDatas {
		...
		for i := uint64(0); i < f.NumChunks()-1; i++ {
			err := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(0)}}, i, 0, crypto.Hash{})
			if err != nil {
				t.Fatal(err)
			}
		}
		...
		for i := uint64(0); i < f.NumChunks()-1; i++ {
			err := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(1)}}, i, 1, crypto.Hash{})
			if err != nil {
				t.Fatal(err)
			}
		}
		...
		err = f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(2)}}, f.NumChunks()-1, 0, crypto.Hash{})
		if err != nil {
			t.Fatal(err)
		}
		...
		for iChunk := uint64(0); iChunk < f.NumChunks(); iChunk++ {
			for iPiece := uint64(1); iPiece < uint64(f.ErasureCode().MinPieces()); iPiece++ {
				err := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(3)}}, iChunk, iPiece, crypto.Hash{})
				if err != nil {
					t.Fatal(err)
				}
			}
			err := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(4)}}, iChunk, uint64(f.ErasureCode().MinPieces()), crypto.Hash{})
			if err != nil {
				t.Fatal(err)
			}
		}
		...
		for iChunk := uint64(0); iChunk < f.NumChunks(); iChunk++ {
			for iPiece := uint64(0); iPiece < uint64(f.ErasureCode().MinPieces()); iPiece++ {
				err := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(5)}}, iChunk, iPiece, crypto.Hash{})
				if err != nil {
					t.Fatal(err)
				}
			}
...
func TestFileHealth(t *testing.T) {
	...
	*/
	for i := 0; i < 2; i++ {
		spk := types.TurtleDexPublicKey{Algorithm: types.SignatureEd25519, Key: []byte{byte(i)}}
	...
	spk := types.TurtleDexPublicKey{Algorithm: types.SignatureEd25519, Key: []byte{0}}
	offlineMap[spk.String()] = false
	goodForRenewMap[spk.String()] = true
	...
	spk = types.TurtleDexPublicKey{Algorithm: types.SignatureEd25519, Key: []byte{1}}
	offlineMap[spk.String()] = false
	goodForRenewMap[spk.String()] = true
	if err := f.AddPiece(spk, 0, 1, crypto.Hash{}); err != nil {
		t.Fatal(err)
...
// TestGrowNumChunks is a unit test for the TurtleDexFile's GrowNumChunks method.
func TestGrowNumChunks(t *testing.T) {
	...
	checkFile := func(sf *TurtleDexFile, numChunks, size uint64) {
...
func TestChunkHealth(t *testing.T) {
	...
	spk = types.TurtleDexPublicKey{Algorithm: types.SignatureEd25519, Key: []byte{2}}
	offlineMap[spk.String()] = false
	goodForRenewMap[spk.String()] = true
...
func TestStuckChunks(t *testing.T) {
	...
	sf, err := LoadTurtleDexFile(sf.TurtleDexFilePath(), sf.wal)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUploadedBytes(t *testing.T) {
	...
	// chunk
	for i := 0; i < 4; i++ {
		err := f.AddPiece(types.TurtleDexPublicKey{}, uint64(0), 0, crypto.Hash{})
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestFileUploadProgressPinning(t *testing.T) {
	...
	for chunkIndex := uint64(0); chunkIndex < f.NumChunks(); chunkIndex++ {
		for pieceIndex := uint64(0); pieceIndex < uint64(f.ErasureCode().NumPieces()); pieceIndex++ {
			err1 := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(0)}}, chunkIndex, pieceIndex, crypto.Hash{})
			err2 := f.AddPiece(types.TurtleDexPublicKey{Key: []byte{byte(1)}}, chunkIndex, pieceIndex, crypto.Hash{})
...
func TestFileExpiration(t *testing.T) {
	...
	pk1 := types.TurtleDexPublicKey{Key: []byte{0}}
	pk2 := types.TurtleDexPublicKey{Key: []byte{1}}
	pk3 := types.TurtleDexPublicKey{Key: []byte{2}}
...
// BenchmarkLoadTurtleDexFile benchmarks loading an existing siafile's metadata into
// memory.
func BenchmarkLoadTurtleDexFile(b *testing.B) {
	// Get new file params
	...
	hostKeys := make([]types.TurtleDexPublicKey, rc.NumPieces())
	...
	for loads := 0; loads < b.N; loads++ {
		sf, err = LoadTurtleDexFile(siaFilePath, wal)
		if err != nil {
			b.Fatal(err)
		}
	}
