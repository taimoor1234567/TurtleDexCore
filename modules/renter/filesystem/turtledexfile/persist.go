...
var (
	// errUnknownTurtleDexFileUpdate is returned when applyUpdates finds an update
	// that is unknown
	errUnknownTurtleDexFileUpdate = errors.New("unknown siafile update")
)
...
// LoadTurtleDexFile is a wrapper for loadTurtleDexFile that uses the production
// dependencies.
func LoadTurtleDexFile(path string, wal *writeaheadlog.WAL) (*TurtleDexFile, error) {
	return loadTurtleDexFile(path, wal, modules.ProdDependencies)
}

// LoadTurtleDexFileFromReader allows loading a TurtleDexFile from a different location that
// directly from disk as long as the source satisfies the TurtleDexFileSource
// interface.
func LoadTurtleDexFileFromReader(r io.ReadSeeker, path string, wal *writeaheadlog.WAL) (*TurtleDexFile, error) {
	return loadTurtleDexFileFromReader(r, path, wal, modules.ProdDependencies)
}

// LoadTurtleDexFileFromReaderWithChunks does not only read the header of the TurtleDexfile
...
// disk later.
func LoadTurtleDexFileFromReaderWithChunks(r io.ReadSeeker, path string, wal *writeaheadlog.WAL) (*TurtleDexFile, Chunks, error) {
	sf, err := LoadTurtleDexFileFromReader(r, path, wal)
	if err != nil {
...
// LoadTurtleDexFileMetadata is a wrapper for loadTurtleDexFileMetadata that uses the
// production dependencies.
func LoadTurtleDexFileMetadata(path string) (Metadata, error) {
	return loadTurtleDexFileMetadata(path, modules.ProdDependencies)
}

// SetPartialChunks informs the TurtleDexFile about a partial chunk that has been
// saved by the partial chunk set. As such it should be exclusively called by
// the partial chunk set. It updates the metadata of the TurtleDexFile and also adds a
// new chunk to the partial TurtleDexFile if necessary. At the end it applies the
// updates of the partial chunk set, the TurtleDexFile and the partial TurtleDexFile
// atomically.
func (sf *TurtleDexFile) SetPartialChunks(combinedChunks []modules.PartialChunk, updates []writeaheadlog.Update) (err error) {
	...
	// Lock both the TurtleDexFile and partials TurtleDexFile. We need to atomically update
	// both of them.
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	sf.partialsTurtleDexFile.mu.Lock()
	defer sf.partialsTurtleDexFile.mu.Unlock()
	...
	for _, c := range combinedChunks {
		...
		if c.InPartialsFile {
			pc.Index = uint64(sf.partialsTurtleDexFile.numChunks - 1)
		} else {
			pc.Index = uint64(sf.partialsTurtleDexFile.numChunks)
			u, err := sf.partialsTurtleDexFile.addCombinedChunk()
			if err != nil {
				return err
			}
...
// SetPartialsTurtleDexFile sets the partialsTurtleDexFile field of the TurtleDexFile. This is
// usually done for non-partials TurtleDexFiles after loading them from disk.
func (sf *TurtleDexFile) SetPartialsTurtleDexFile(partialsTurtleDexFile *TurtleDexFile) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	sf.partialsTurtleDexFile = partialsTurtleDexFile
}

// SetTurtleDexFilePath sets the path of the siafile on disk.
func (sf *TurtleDexFile) SetTurtleDexFilePath(path string) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	sf.siaFilePath = path
}
...
// TurtleDexFile. This method can apply updates from different TurtleDexFiles and should
// only be run before the TurtleDexFiles are loaded from disk right after the startup
// of ttdxd. Otherwise we might run into concurrency issues.
func applyUpdates(deps modules.Dependencies, updates ...writeaheadlog.Update) error {
	for _, u := range updates {
		err := func() error {
			...
			default:
				return errUnknownTurtleDexFileUpdate
			}
		}()
		if err != nil {
...
// loadTurtleDexFile loads a TurtleDexFile from disk.
func loadTurtleDexFile(path string, wal *writeaheadlog.WAL, deps modules.Dependencies) (*TurtleDexFile, error) {
	// Open the file.
	f, err := deps.Open(path)
	if err != nil {
		return nil, err
	}
	sf, err := loadTurtleDexFileFromReader(f, path, wal, deps)
...
// directly from disk as long as the source satisfies the TurtleDexFileSource
// interface.
func loadTurtleDexFileFromReader(r io.ReadSeeker, path string, wal *writeaheadlog.WAL, deps modules.Dependencies) (*TurtleDexFile, error) {
	// Create the TurtleDexFile
	sf := &TurtleDexFile{
		deps:        deps,
		siaFilePath: path,
		wal:         wal,
...
// loadTurtleDexFileMetadata loads only the metadata of a TurtleDexFile from disk.
func loadTurtleDexFileMetadata(path string, deps modules.Dependencies) (md Metadata, err error) {
	// Open the file.
	f, err := deps.Open(path)
	if err != nil {
		return Metadata{}, err
...
func readInsertUpdate(update writeaheadlog.Update) (path string, index int64, data []byte, err error) {
	if !IsTurtleDexFileUpdate(update) {
		err = errors.New("readUpdate can't read non-TurtleDexFile update")
		build.Critical(err)
		return
	}
...
func (sf *TurtleDexFile) allocateHeaderPage() (_ writeaheadlog.Update, err error) {
...
// applyUpdates applies updates to the TurtleDexFile. Only updates that belong to the
// TurtleDexFile on which applyUpdates is called can be applied. Everything else will
// be considered a developer error and cause the update to not be applied to
// avoid corruption.  applyUpdates also syncs the TurtleDexFile for convenience since
// it already has an open file handle.
func (sf *TurtleDexFile) applyUpdates(updates ...writeaheadlog.Update) (err error) {
...
func (sf *TurtleDexFile) chunk(chunkIndex int) (_ chunk, err error) {
	...
	if cci, ok := sf.isIncludedPartialChunk(uint64(chunkIndex)); ok {
		c, err := sf.partialsTurtleDexFile.Chunk(cci.Index)
...
func (sf *TurtleDexFile) iterateChunks(iterFunc func(chunk *chunk) (bool, error)) ([]writeaheadlog.Update, error) {
	if sf.deleted {
	...
	err := sf.iterateChunksReadonly(func(chunk chunk) error {
		...
		if modified && ok {
			...
			updates = append(updates, sf.partialsTurtleDexFile.saveChunkUpdate(chunk))
		} else if modified {
...
func (sf *TurtleDexFile) iterateChunksReadonly(iterFunc func(chunk chunk) error) (err error) {
	if sf.deleted {
...
func (sf *TurtleDexFile) chunkOffset(chunkIndex int) int64 {
	if chunkIndex < 0 {
...
func (sf *TurtleDexFile) createAndApplyTransaction(updates ...writeaheadlog.Update) (err error) {
	// Sanity check that file hasn't been deleted.
	if sf.deleted {
...
// createAndApplyTransaction method of the TurtleDexFile. This will result in 2 fsyncs
...
// createDeleteUpdate is a helper method that creates a writeaheadlog for
// deleting a file.
func (sf *TurtleDexFile) createDeleteUpdate() writeaheadlog.Update {
...
// specific part of the TurtleDexFile. e.g. the metadata
...
// directly but wrapped into another helper that creates an update for a
// specific part of the TurtleDexFile. e.g. the metadata
func (sf *TurtleDexFile) createInsertUpdate(index int64, data []byte) writeaheadlog.Update {
	return createInsertUpdate(sf.siaFilePath, index, data)
}

// readAndApplyInsertUpdate reads the insert update for a TurtleDexFile and then
// applies it
func (sf *TurtleDexFile) readAndApplyInsertUpdate(f modules.File, update writeaheadlog.Update) error {
	// Decode update.
	path, index, data, err := readInsertUpdate(update)
	if err != nil {
		return err
	...
	// Sanity check path. Update should belong to TurtleDexFile.
	if sf.siaFilePath != path {
		build.Critical(fmt.Sprintf("can't apply update for file %s to TurtleDexFile %s", path, sf.siaFilePath))
		return nil
	}
...
func (sf *TurtleDexFile) readAndApplyTruncateUpdate(f modules.File, u writeaheadlog.Update) error {
...
// saveFile saves the TurtleDexFile's header and the provided chunks atomically.
func (sf *TurtleDexFile) saveFile(chunks []chunk) (err error) {
	// Sanity check that file hasn't been deleted.
	if sf.deleted {
...
func (sf *TurtleDexFile) saveChunkUpdate(chunk chunk) writeaheadlog.Update {
...
// pubKeyTable of the TurtleDexFile to disk using the writeaheadlog. If the metadata
// and overlap due to growing too large and would therefore corrupt if they
// were written to disk, a new page is allocated.
// NOTE: For consistency chunk updates always need to be created after the
// header or metadata updates.
func (sf *TurtleDexFile) saveHeaderUpdates() (_ []writeaheadlog.Update, err error) {
...
// saveMetadataUpdates saves the metadata of the TurtleDexFile but not the
...
// saveHeader is potentially faster for TurtleDexFiles with a header that can not be
// marshaled within a single page.
// NOTE: For consistency chunk updates always need to be created after the
// header or metadata updates.
func (sf *TurtleDexFile) saveMetadataUpdates() ([]writeaheadlog.Update, error) {
	// Marshal the pubKeyTable.
	pubKeyTable, err := marshalPubKeyTable(sf.pubKeyTable)
	if err != nil {
		return nil, err
