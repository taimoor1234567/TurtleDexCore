...
// equalFiles is a helper that compares two TurtleDexFiles for equality.
func equalFiles(sf, sf2 *TurtleDexFile) error {
...
// addRandomHostKeys adds n random host keys to the TurtleDexFile's pubKeyTable. It
// doesn't write them to disk.
func (sf *TurtleDexFile) addRandomHostKeys(n int) {
	for i := 0; i < n; i++ {
		...
		sf.pubKeyTable = append(sf.pubKeyTable, HostPublicKey{
			PublicKey: types.TurtleDexPublicKey{
				Algorithm: algorithm,
				Key:       key,
			},
			Used: true,
...
// customTestFileAndWAL creates an empty TurtleDexFile for testing and also returns
// the WAL used in the creation and the path of the WAL.
func customTestFileAndWAL(siaFilePath, source string, rc modules.ErasureCoder, sk crypto.CipherKey, fileSize uint64, numChunks int, fileMode os.FileMode) (*TurtleDexFile, *writeaheadlog.WAL, string) {
	...
	if err != nil {
		panic(fmt.Sprint("failed to load partialsTurtleDexFile", err))
	}
	// Check that the partials file is sane.
	if partialsTurtleDexFile.numChunks > 0 {
		panic(fmt.Sprint("partialsTurtleDexFile shouldn't have any chunks but had ", partialsTurtleDexFile.numChunks))
	}
	/* PARTIAL TODO:
	partialsEntry := &TurtleDexFileSetEntry{
		partialsTurtleDexFile,
...
// contain.
func newBlankTestFileAndWAL(minChunks int) (*TurtleDexFile, *writeaheadlog.WAL, string) {
...
// newBlankTestFile is a helper method to create a TurtleDexFile for testing without
// any hosts or uploaded pieces.
func newBlankTestFile() *TurtleDexFile {
	sf, _, _ := newBlankTestFileAndWAL(1)
	return sf
}

// newTestFile creates a TurtleDexFile for testing where each chunk has a random
// number of pieces.
func newTestFile() *TurtleDexFile {
	sf := newBlankTestFile()
	...
	for chunkIndex := 0; chunkIndex < sf.numChunks; chunkIndex++ {
		for pieceIndex := 0; pieceIndex < sf.ErasureCode().NumPieces(); pieceIndex++ {
			...
			for i := 0; i < numPieces; i++ {
				pk := types.TurtleDexPublicKey{Key: fastrand.Bytes(crypto.EntropySize)}
				mr := crypto.Hash{}
				fastrand.Read(mr[:])
...
func newTestFileParams(minChunks int, partialChunk bool) (string, modules.TurtleDexPath, string, modules.ErasureCoder, crypto.CipherKey, uint64, int, os.FileMode) {
...
// creates a directory for the file.
func newTestFileParamsWithRC(minChunks int, partialChunk bool, rc modules.ErasureCoder) (string, modules.TurtleDexPath, string, modules.ErasureCoder, crypto.CipherKey, uint64, int, os.FileMode) {
	...
	// Create the path to the file.
	siaFilePath := siaPath.TurtleDexFileSysPath(filepath.Join(os.TempDir(), "siafiles", hex.EncodeToString(fastrand.Bytes(16))))
	dir, _ := filepath.Split(siaFilePath)
...
func TestNewFile(t *testing.T) {
	...
	// Save the TurtleDexFile to make sure cached fields are persisted too.
	...
	sf2, err := LoadTurtleDexFile(sf.siaFilePath, sf.wal)
	if err != nil {
		t.Fatal("failed to load TurtleDexFile from disk", err)
	}
	sf2.SetPartialsTurtleDexFile(sf.partialsTurtleDexFile)
	// Compare the files.
...
func TestApplyUpdates(t *testing.T) {
	...
	t.Run("TestTurtleDexFileApplyUpdates", func(t *testing.T) {
		siaFile := newTestFile()
...
func TestZeroByteFileCompat(t *testing.T) {
	...
	sf, err := loadTurtleDexFile(siaFilePath, wal, modules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveLargeHeader(t *testing.T) {
	...
	// Add some host keys. This should force the TurtleDexFile to allocate a new page
	// for the pubKeyTable.
...
func testApply(t *testing.T, siaFile *TurtleDexFile, apply func(...writeaheadlog.Update) error) {
...
func TestUpdateUsedHosts(t *testing.T) {
	...
	var err error
	sf, err = LoadTurtleDexFile(sf.siaFilePath, sf.wal)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUniqueIDMissing(t *testing.T) {
	...
	// Load the file again.
	sf, err = LoadTurtleDexFile(sf.siaFilePath, wal)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetCombinedChunkSingle(t *testing.T) {
	...
	// Create two TurtleDexFiles with partial chunks and link them by giving the second
	...
	if sf.partialsTurtleDexFile.NumChunks() != 1 {
		t.Fatal("expected partialsTurtleDexFile to have one chunk but had", sf.partialsTurtleDexFile.NumChunks())
	}
	...
	if sf.partialsTurtleDexFile.NumChunks() != 2 {
		t.Fatal("expected partialsTurtleDexFile to have two chunks but had", sf.partialsTurtleDexFile.NumChunks())
	}
	if sf2.partialsTurtleDexFile.NumChunks() != 2 {
		t.Fatal("expected partialsTurtleDexFile to have two chunks but had", sf.partialsTurtleDexFile.NumChunks())
	}
}
