...
type (
	...
	// chunk relevant to TurtleDexFiles. A TurtleDexFile with a partial chunk may contain 1 or
	...
	PartialChunkInfo struct {
		...
		Index  uint64                  `json:"index"`  // Index of the combined chunk within partialsTurtleDexFile
	...
	// TurtleDexfileUID is a unique identifier for siafile which is used to track
	// siafiles even after renaming them.
	TurtleDexfileUID string

	// Metadata is the metadata of a TurtleDexFile and is JSON encoded.
	...
	Metadata struct {
		UniqueID TurtleDexfileUID `json:"uniqueid"` // unique identifier for file
		...
		// persisted whenever another method persists the metadata or when the TurtleDexFile
		// is closed.
		//
		...
		//
		// LastHealthCheckTime is the timestamp of the last time the TurtleDexFile's
		// health was checked by Health()
		//
		// NumStuckChunks is the number of all the TurtleDexFile's chunks that have
	...
	BubbledMetadata struct {
		...
		StuckBytes          uint64
		StuckHealth         float64
		UID                 TurtleDexfileUID
	}
)

// AccessTime returns the AccessTime timestamp of the file.
func (sf *TurtleDexFile) AccessTime() time.Time {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	return sf.staticMetadata.AccessTime
}

// AddSkylink will add a skylink to the TurtleDexFile.
func (sf *TurtleDexFile) AddSkylink(s modules.Skylink) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
func (sf *TurtleDexFile) ChangeTime() time.Time {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) PartialChunks() []PartialChunkInfo {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) CreateTime() time.Time {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) LastHealthCheckTime() time.Time {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) LocalPath() string {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) MasterKey() crypto.CipherKey {
	return sf.staticMasterKey()
}

// Metadata returns the TurtleDexFile's metadata, resolving any fields related to
// partial chunks.
func (sf *TurtleDexFile) Metadata() Metadata {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// Mode returns the FileMode of the TurtleDexFile.
func (sf *TurtleDexFile) Mode() os.FileMode {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) ModTime() time.Time {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
// metadata
func (sf *TurtleDexFile) NumStuckChunks() uint64 {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
...
func (sf *TurtleDexFile) PieceSize() uint64 {
...
func (sf *TurtleDexFile) Rename(newTurtleDexFilePath string) error {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	return sf.rename(newTurtleDexFilePath)
}
...
// the file is atomic across all operating systems, we create a wal transaction
// that moves over all the chunks one-by-one and deletes the src file.
func (sf *TurtleDexFile) rename(newTurtleDexFilePath string) (err error) {
	if sf.deleted {
	...
	if _, err := os.Stat(newTurtleDexFilePath); err == nil {
		return ErrPathOverload
	}
	// Create path to renamed location.
	dir, _ := filepath.Split(newTurtleDexFilePath)
...
func (sf *TurtleDexFile) SetMode(mode os.FileMode) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	// backup the changed metadata before changing it. Revert the change on
	// error.
...
// TurtleDexFile metadata
func (sf *TurtleDexFile) SetLastHealthCheckTime() {
	sf.mu.Lock()
	defer sf.mu.Unlock()
...
// the file from disk.
func (sf *TurtleDexFile) SetLocalPath(path string) (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	// backup the changed metadata before changing it. Revert the change on
	// error.
...
func (sf *TurtleDexFile) Size() uint64 {
	sf.mu.RLock()
	defer sf.mu.RUnlock()
	return uint64(sf.staticMetadata.FileSize)
}

// UpdateUniqueID creates a new random uid for the TurtleDexFile.
func (sf *TurtleDexFile) UpdateUniqueID() {
	sf.staticMetadata.UniqueID = uniqueID()
}

// UpdateAccessTime updates the AccessTime timestamp to the current time.
func (sf *TurtleDexFile) UpdateAccessTime() (err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	// backup the changed metadata before changing it. Revert the change on
	// error.
...
func (sf *TurtleDexFile) numStuckChunks() uint64 {
	...
	for _, cc := range sf.staticMetadata.PartialChunks {
		stuck, err := sf.partialsTurtleDexFile.StuckChunkByIndex(cc.Index)
		if err != nil {
...
func (sf *TurtleDexFile) staticChunkSize() uint64 {
...
func (sf *TurtleDexFile) staticMasterKey() crypto.CipherKey {
	sk, err := crypto.NewTurtleDexKey(sf.staticMetadata.StaticMasterKeyType, sf.staticMetadata.StaticMasterKey)
	if err != nil {
		// This should never happen since the constructor of the TurtleDexFile takes
...
// uniqueID creates a random unique TurtleDexfileUID.
func uniqueID() TurtleDexfileUID {
	return TurtleDexfileUID(persist.UID())
}
