...
type (
	// FileNode is a node which references a TurtleDexFile.
	FileNode struct {
		node
		closed bool

		*siafile.TurtleDexFile
	}
)

// AddPiece wraps siafile.AddPiece to guarantee that it's not called when the
// fileNode was already closed.
func (n *FileNode) AddPiece(pk types.TurtleDexPublicKey, chunkIndex, pieceIndex uint64, merkleRoot crypto.Hash) (err error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	if n.closed {
	...
	return n.TurtleDexFile.AddPiece(pk, chunkIndex, pieceIndex, merkleRoot)
}
...
func (n *FileNode) managedMode() os.FileMode {
	n.node.mu.Lock()
	defer n.node.mu.Unlock()
	return n.TurtleDexFile.Mode()
}

// managedFileInfo returns the FileInfo of the file node.
func (n *FileNode) managedFileInfo(siaPath modules.TurtleDexPath, offline map[string]bool, goodForRenew map[string]bool, contracts map[string]modules.RenterContract) (modules.FileInfo, error) {
	// Build the FileInfo
	var onDisk bool
...
func (n *FileNode) managedRename(newName string, oldParent, newParent *DirNode) error {
	...
	newPath := filepath.Join(newParent.absPath(), newName) + modules.TurtleDexFileExtension
	// Rename the file.
	err := n.TurtleDexFile.Rename(newPath)
...
func (n *FileNode) staticCachedInfo(siaPath modules.TurtleDexPath) (modules.FileInfo, error) {
	md := n.Metadata()
