...
// newTurtleDexDirTestDir creates a test directory for a ttdxdir test
func newTurtleDexDirTestDir(testDir string) (string, error) {
	rootPath := filepath.Join(os.TempDir(), "ttdxdirs", testDir)
...
// newTestDir creates a new TurtleDexDir for testing, the test Name should be passed
// in as the rootDir
func newTestDir(rootDir string) (*TurtleDexDir, error) {
	rootPath, err := newTurtleDexDirTestDir(rootDir)
	if err != nil {
		return nil, err
	}
	wal, _ := newTestWAL()
	return New(modules.RandomTurtleDexPath().TurtleDexDirSysPath(rootPath), rootPath, modules.DefaultDirPerm, wal)
}
...
// TestIsTurtleDexDirUpdate tests the IsTurtleDexDirUpdate method.
func TestIsTurtleDexDirUpdate(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	if !IsTurtleDexDirUpdate(metadataUpdate) {
		t.Error("metadataUpdate should be a TurtleDexDirUpdate but wasn't")
	}
	if !IsTurtleDexDirUpdate(deleteUpdate) {
		t.Error("deleteUpdate should be a TurtleDexDirUpdate but wasn't")
	}
	if IsTurtleDexDirUpdate(emptyUpdate) {
		t.Error("emptyUpdate shouldn't be a TurtleDexDirUpdate but was one")
	}
}
...
func TestCreateReadMetadataUpdate(t *testing.T) {
	...
	path := filepath.Join(sd.path, modules.TurtleDexDirExtension)
	...
	// Check path
	path2 := filepath.Join(sd.path, modules.TurtleDexDirExtension)
	if path != path2 {
...
func TestApplyUpdates(t *testing.T) {
	...
	t.Run("TestApplyUpdates", func(t *testing.T) {
		ttdxdir, err := newTestDir(t.Name())
		if err != nil {
			t.Fatal(err)
		}
		testApply(t, ttdxdir, ApplyUpdates)
	})
	t.Run("TestTurtleDexDirApplyUpdates", func(t *testing.T) {
		ttdxdir, err := newTestDir(t.Name())
		if err != nil {
			t.Fatal(err)
		}
		testApply(t, ttdxdir, ttdxdir.applyUpdates)
	})
	t.Run("TestCreateAndApplyTransaction", func(t *testing.T) {
		...
		testApply(t, ttdxdir, ttdxdir.createAndApplyTransaction)
	})
}

// testApply tests if a given method applies a set of updates correctly.
func testApply(t *testing.T, ttdxdir *TurtleDexDir, apply func(...writeaheadlog.Update) error) {
	// Create an update to the metadata
	metadata := ttdxdir.metadata
	metadata.Health = 1.0
	path := filepath.Join(ttdxdir.path, modules.TurtleDexDirExtension)
	...
	// Open file.
	sd, err := LoadTurtleDexDir(ttdxdir.path, modules.ProdDependencies, ttdxdir.wal)
	if err != nil {
		t.Fatal("Failed to load ttdxdir", err)
	}
	// Check Time separately due to how the time is persisted
	if !metadata.AggregateLastHealthCheckTime.Equal(sd.metadata.AggregateLastHealthCheckTime) {
		t.Fatalf("AggregateLastHealthCheckTimes not equal, got %v expected %v", metadata.AggregateLastHealthCheckTime, sd.metadata.AggregateLastHealthCheckTime)
...
func TestManagedCreateAndApplyTransactions(t *testing.T) {
	...
	// Apply update.
	if err := CreateAndApplyTransaction(ttdxdir.wal, update); err != nil {
		t.Fatal("Failed to apply update", err)
	}
	// Open file.
	sd, err := LoadTurtleDexDir(ttdxdir.path, modules.ProdDependencies, ttdxdir.wal)
...
func TestCreateAndApplyTransactionPanic(t *testing.T) {
	...
	func() {
		defer assertRecover()
		_ = ttdxdir.createAndApplyTransaction(update)
	}()
	func() {
		defer assertRecover()
		_ = CreateAndApplyTransaction(ttdxdir.wal, update)
	}()
}
