...
// DirReader is a helper type that allows reading a raw .ttdxdir from disk while
...
// DirReader creates a io.ReadCloser that can be used to read the raw TurtleDexDir
// from disk.
func (sd *TurtleDexDir) DirReader() (*DirReader, error) {
	sd.mu.Lock()
	if sd.deleted {
		sd.mu.Unlock()
