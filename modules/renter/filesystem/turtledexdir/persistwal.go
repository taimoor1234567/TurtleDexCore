...
// called on startup before any ttdxdir is loaded from disk
func readAndApplyDeleteUpdate(update writeaheadlog.Update) error {
	if !IsTurtleDexDirUpdate(update) {
		err := errors.New("readAndApplyDeleteUpdate can't read non-TurtleDexDir update")
		build.Critical(err)
		return err
	}
	// Delete from disk
...
func readAndApplyMetadataUpdate(deps modules.Dependencies, update writeaheadlog.Update) (err error) {
	if !IsTurtleDexDirUpdate(update) {
		err := errors.New("readAndApplyMetadataUpdate can't read non-TurtleDexDir update")
		build.Critical(err)
		return err
	}
	// Decode update.
...
// TurtleDexDir.
func (sd *TurtleDexDir) applyUpdates(updates ...writeaheadlog.Update) error {
...
func (sd *TurtleDexDir) createAndApplyTransaction(updates ...writeaheadlog.Update) (err error) {
...
func (sd *TurtleDexDir) createDeleteUpdate() writeaheadlog.Update {
	return writeaheadlog.Update{
...
func (sd *TurtleDexDir) readAndApplyMetadataUpdate(file modules.File, update writeaheadlog.Update) error {
	// Decode update.
	data, path, err := readMetadataUpdate(update)
	if err != nil {
		return err
	...
	// Sanity check path belongs to ttdxdir
	if path != sd.mdPath() {
		build.Critical(fmt.Sprintf("can't apply update for file %s to TurtleDexDir %s", path, sd.path))
		return nil
	}
...
// saveMetadataUpdate saves the metadata of the TurtleDexDir
func (sd *TurtleDexDir) saveMetadataUpdate() (writeaheadlog.Update, error) {
