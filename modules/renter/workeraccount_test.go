...
// TurtleDexPublicKey, this method returns the TurtleDexPublicKey alongisde the secret key
func newRandomHostKey() (types.TurtleDexPublicKey, crypto.SecretKey) {
	sk, pk := crypto.GenerateKeyPair()
	return types.TurtleDexPublicKey{
...
func testAccountCheckFundAccountGouging(t *testing.T) {
	...
	allowance := modules.Allowance{
		Funds: types.TurtleDexcoinPrecision.Mul64(1e3),
	}
	...
	targetBalance := types.TurtleDexcoinPrecision
	...
	pt = newDefaultPriceTable()
	pt.FundAccountCost = types.TurtleDexcoinPrecision.MulFloat(0.075)
	err = checkFundAccountGouging(pt, allowance, targetBalance)
...
func testAccountCreation(t *testing.T, rt *renterTester) {
	...
	hostKey := types.TurtleDexPublicKey{
		Algorithm: types.SignatureEd25519,
		Key:       pk[:],
	}
	...
	// validate the account id is built using a valid TurtleDexPublicKey and the
...
func testAccountTracking(t *testing.T, rt *renterTester) {
	...
	deposit := types.TurtleDexcoinPrecision
	...
	withdrawal := types.TurtleDexcoinPrecision.Div64(100)
