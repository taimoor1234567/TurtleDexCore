...
func calcSnapshotUploadProgress(fileUploadProgress float64, dotTurtleDexUploadProgress float64) float64 {
	return 0.8*fileUploadProgress + 0.2*dotTurtleDexUploadProgress
}
...
func (r *Renter) UploadedBackups() ([]modules.UploadedBackup, []types.TurtleDexPublicKey, error) {
	...
	hosts := make([]types.TurtleDexPublicKey, 0, len(r.persist.SyncedContracts))
...
// of operations.
func (r *Renter) BackupsOnHost(hostKey types.TurtleDexPublicKey) ([]modules.UploadedBackup, error) {
	if err := r.tg.Add(); err != nil {
		return nil, err
	}
	defer r.tg.Done()
...
func (r *Renter) DownloadBackup(dst string, name string) (err error) {
	...
	_, dotTurtleDex, err := r.managedDownloadSnapshot(uid)
	if err != nil {
		return err
	}
	// Store it in the backup file set.
	backupTurtleDexPath, err := modules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
	if err := r.staticFileSystem.WriteFile(backupTurtleDexPath, dotTurtleDex, 0666); err != nil {
		return err
	}
	// Load the .sia file.
...
func (r *Renter) managedDownloadSnapshotTable(host *worker) ([]snapshotEntry, error) {
	...
	// decrypt the table
	c, _ := crypto.NewTurtleDexKey(crypto.TypeThreefish, secret[:])
...
func (r *Renter) managedUploadSnapshot(meta modules.UploadedBackup, dotTurtleDex []byte) error {
	...
	for _, w := range workers {
		job := &jobUploadSnapshot{
			staticTurtleDexFileData: dotTurtleDex,
...
func (r *Renter) managedDownloadSnapshot(uid [16]byte) (ub modules.UploadedBackup, dotTurtleDex []byte, err error) {
	if err := r.tg.Add(); err != nil {
	...
	for i := range contracts {
		err := func() (err error) {
			...
			for _, root := range entry.DataSectors {
				...
				dotTurtleDex = append(dotTurtleDex, data...)
				if uint64(len(dotTurtleDex)) >= entry.Size {
					dotTurtleDex = dotTurtleDex[:entry.Size]
					break
				}
			}
	...
	for {
		...
		flf := func(info modules.FileInfo) {
			...
			for _, meta = range r.persist.UploadedBackups {
				sp, _ := info.TurtleDexPath.Rebase(modules.BackupFolder, modules.RootTurtleDexPath())
			...
			r.log.Println("Uploading snapshot", info.TurtleDexPath)
			err := func() error {
				// Grab the entry for the uploaded backup's siafile.
				entry, err := r.staticFileSystem.OpenTurtleDexFile(info.TurtleDexPath)
				if err != nil {
				...
				dotTurtleDex, err := ioutil.ReadAll(sr)
				...
				meta.Size = uint64(len(dotTurtleDex))
				if err := r.managedUploadSnapshot(meta, dotTurtleDex); err != nil {
					return errors.Compose(err, entry.Close())
				}
				...
				if err := r.staticFileSystem.DeleteFile(info.TurtleDexPath); err != nil {
					return err
				}
				return nil
			}()
		...
		err = func() (err error) {
			...
			for _, uid := range missing {
				ub, dotTurtleDex, err := r.managedDownloadSnapshot(uid)
				if err != nil {
					// TODO: if snapshot can't be found on any host, delete it
					return err
				}
				if err := w.UploadSnapshot(r.tg.StopCtx(), ub, dotTurtleDex); err != nil {
					return err
				}
