...
const (
	// persistVersion defines the TurtleDex version that the persistence was
	// last updated
	persistVersion = "1.4.2"
)

const (
	// AlertMSGTurtleDexfileLowRedundancy indicates that a file is below 75% redundancy.
	AlertMSGTurtleDexfileLowRedundancy = "The TurtleDexFile mentioned in the 'Cause' is below 75% redundancy"
	// AlertTurtleDexfileLowRedundancyThreshold is the health threshold at which we start
	// registering the LowRedundancy alert for a TurtleDexfile.
	AlertTurtleDexfileLowRedundancyThreshold = 0.75
)

// AlertCauseTurtleDexfileLowRedundancy creates a customized "cause" for a siafile
// with a certain path and health.
func AlertCauseTurtleDexfileLowRedundancy(siaPath modules.TurtleDexPath, health, redundancy float64) string {
	return fmt.Sprintf("TurtleDexfile '%v' has a health of %v and redundancy of %v", siaPath.String(), health, redundancy)
}
...
var (
	...
	// in the cache size on TurtleDex's end growing to match the size of the
