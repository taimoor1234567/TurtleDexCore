...
var (
	...
	DefaultTestAllowance = modules.Allowance{
		Funds:       types.TurtleDexcoinPrecision.Mul64(500),
		Hosts:       uint64(50),
	...
	DefaultHostDBEntry = modules.HostDBEntry{
		HostExternalSettings: modules.HostExternalSettings{
			...
			Collateral:    types.NewCurrency64(250).Mul(types.TurtleDexcoinPrecision).Div(modules.BlockBytesPerMonthTerabyte),
			MaxCollateral: types.NewCurrency64(750).Mul(types.TurtleDexcoinPrecision),

			BaseRPCPrice:           types.TurtleDexcoinPrecision.Mul64(100).Div64(1e9),
			ContractPrice:          types.NewCurrency64(5).Mul(types.TurtleDexcoinPrecision),
			DownloadBandwidthPrice: types.TurtleDexcoinPrecision.Mul64(100).Div64(1e12),
			SectorAccessPrice:      types.TurtleDexcoinPrecision.Mul64(2).Div64(1e6),
			StoragePrice:           types.NewCurrency64(100).Mul(types.TurtleDexcoinPrecision).Div(modules.BlockBytesPerMonthTerabyte),
...
func calculateWeightFromUInt64Price(price, collateral uint64) (weight types.Currency, err error) {
	...
	entry.StoragePrice = types.NewCurrency64(price).Mul(types.TurtleDexcoinPrecision).Div(modules.BlockBytesPerMonthTerabyte)
	entry.Collateral = types.NewCurrency64(collateral).Mul(types.TurtleDexcoinPrecision).Div(modules.BlockBytesPerMonthTerabyte)
