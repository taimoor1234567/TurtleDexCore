...
func newHDBTesterDeps(name string, deps modules.Dependencies) (*hdbTester, error) {
	...
	mux, err := modules.NewTurtleDexMux(filepath.Join(testDir, modules.TurtleDexMuxDir), testDir, "localhost:0", "localhost:0")
	if err != nil {
		return nil, err
	}
...
func (hdbt *hdbTester) initWallet() error {
	hdbt.walletKey = crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
...
func TestRandomHosts(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		var exclude []types.TurtleDexPublicKey
...
func TestRemoveNonexistingHostFromHostTree(t *testing.T) {
	...
	err = hdbt.hdb.staticHostTree.Remove(types.TurtleDexPublicKey{})
	if err == nil {
...
func TestCheckForIPViolations(t *testing.T) {
	...
	badHosts, err := hdbt.hdb.CheckForIPViolations([]types.TurtleDexPublicKey{entry1.PublicKey, entry2.PublicKey})
	if err != nil {
		t.Fatal(err)
	}
	if len(badHosts) != 0 {
	...
	badHosts, err = hdbt.hdb.CheckForIPViolations([]types.TurtleDexPublicKey{entry1.PublicKey, entry2.PublicKey, entry3.PublicKey})
	if err != nil {
		t.Fatal(err)
	}
	if len(badHosts) != 1 {
	...
	badHosts, err = hdbt.hdb.CheckForIPViolations([]types.TurtleDexPublicKey{entry2.PublicKey, entry1.PublicKey, entry3.PublicKey})
	if err != nil {
		t.Fatal(err)
	}
	if len(badHosts) != 1 {
	...
	// 'youngest' entry.
	badHosts, err = hdbt.hdb.CheckForIPViolations([]types.TurtleDexPublicKey{entry3.PublicKey, entry1.PublicKey, entry2.PublicKey})
	if err != nil {
		t.Fatal(err)
	}
	if len(badHosts) != 1 {
