...
func TestHostTree(t *testing.T) {
	...
	firstInsertions := 64
	var keys []types.TurtleDexPublicKey
	...
	var removed []types.TurtleDexPublicKey
...
func TestRandomHosts(t *testing.T) {
	...
	// returned.
	uniqueHosts := tree.SelectRandom(3, []types.TurtleDexPublicKey{
	...
	randHosts = tree.SelectRandom(3, []types.TurtleDexPublicKey{{}, {}, {}}, nil)
	if len(randHosts) != 3 {
		t.Error("didn't get 3 hosts")
	}
...
func TestHostTreeFilter(t *testing.T) {
	...
	// host.
	if numHosts := len(tree.SelectRandom(2, nil, []types.TurtleDexPublicKey{entry2.PublicKey})); numHosts != 1 {
		t.Error("Expected only one host but was", numHosts)
	}

	// Call SelectRandom again but ignore host 3. This should give us no host.
	if numHosts := len(tree.SelectRandom(2, nil, []types.TurtleDexPublicKey{entry3.PublicKey})); numHosts != 0 {
