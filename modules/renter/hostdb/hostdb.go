...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/renter/hostdb/hosttree"
...
var (
	...
	errNilTurtleDexMux             = errors.New("cannot create hostdb with nil siamux")
...
type HostDB struct {
	...
	// hosts. The mapkey is a serialized TurtleDexPublicKey.
	...
	filteredHosts      map[string]types.TurtleDexPublicKey
...
func (hdb *HostDB) remove(pk types.TurtleDexPublicKey) error {
...
func hostdbBlockingStartup(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, siamux *siamux.TurtleDexMux, persistDir string, deps modules.Dependencies) (*HostDB, error) {
	...
	hdb := &HostDB{
		...
		filteredHosts:  make(map[string]types.TurtleDexPublicKey),
		knownContracts: make(map[string]contractInfo),
	...
	// in the hostdb.
	hdb.mu.Lock()
	knownContractHosts := make([]types.TurtleDexPublicKey, 0, len(hdb.knownContracts))
...
func New(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, siamux *siamux.TurtleDexMux, persistDir string) (*HostDB, <-chan error) {
...
// the consensusSet.
func NewCustomHostDB(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, siamux *siamux.TurtleDexMux, persistDir string, deps modules.Dependencies) (*HostDB, <-chan error) {
...
func (hdb *HostDB) CheckForIPViolations(hosts []types.TurtleDexPublicKey) ([]types.TurtleDexPublicKey, error) {
	if err := hdb.tg.Add(); err != nil {
		return nil, err
	}
	defer hdb.tg.Done()
	...
	var badHosts []types.TurtleDexPublicKey
...
func (hdb *HostDB) Host(spk types.TurtleDexPublicKey) (modules.HostDBEntry, bool, error) {
	if err := hdb.tg.Add(); err != nil {
...
func (hdb *HostDB) Filter() (modules.FilterMode, map[string]types.TurtleDexPublicKey, error) {
	if err := hdb.tg.Add(); err != nil {
...
func (hdb *HostDB) SetFilterMode(fm modules.FilterMode, hosts []types.TurtleDexPublicKey) error {
	if err := hdb.tg.Add(); err != nil {
	...
	if fm == modules.HostDBDisableFilter {
		...
		hdb.staticFilteredTree = hdb.staticHostTree
		hdb.filteredHosts = make(map[string]types.TurtleDexPublicKey)
		hdb.filterMode = fm
		return nil
	}
