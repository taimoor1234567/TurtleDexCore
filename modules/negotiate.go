...
const (
	// MinimumSupportedRenterHostProtocolVersion is the minimum version of TurtleDex
...
const (
	...
	// NegotiateMaxTurtleDexPubkeySize defines the maximum size that a TurtleDexPubkey is
	// allowed to be when being sent over the wire during negotiation.
	NegotiateMaxTurtleDexPubkeySize = 1e3
...
type (
	...
	HostExternalSettings struct {
		...
		// is only to cover the ttdc fees that the host will suffer when
		...
		TurtleDexMuxPort string `json:"siamuxport"`
	}
...
// TurtleDexMuxAddress returns the address of the host's siamux.
func (hes HostExternalSettings) TurtleDexMuxAddress() string {
	return fmt.Sprintf("%s:%s", hes.NetAddress.Host(), hes.TurtleDexMuxPort)
}
...
type (
	...
	LoopFormContractRequest struct {
		...
		RenterKey    types.TurtleDexPublicKey
	}
	...
	LoopContractAdditions struct {
		...
		Inputs  []types.TurtleDexcoinInput
...
// protocol.
func NewRenterSession(conn net.Conn, hostPublicKey types.TurtleDexPublicKey) (*RenterHostSession, LoopChallengeRequest, error) {
...
// transaction.
func CreateAnnouncement(addr NetAddress, pk types.TurtleDexPublicKey, sk crypto.SecretKey) (signedAnnouncement []byte, err error) {
...
func DecodeAnnouncement(fullAnnouncement []byte) (na NetAddress, spk types.TurtleDexPublicKey, err error) {
...
// it.
func RPCBeginSubscription(stream siamux.Stream, pp PaymentProvider, host types.TurtleDexPublicKey, pt *RPCPriceTable, initialBudget types.Currency, fundAcc AccountID, bh types.BlockHeight, subscriber types.Specifier) error {
	// initiate the RPC
...
func RPCFundSubscription(stream siamux.Stream, host types.TurtleDexPublicKey, pp PaymentProvider, aid AccountID, bh types.BlockHeight, fundAmt types.Currency) error {
	// Send the type of the request.
	buf := bytes.NewBuffer(nil)
