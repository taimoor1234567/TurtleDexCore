...
type PaymentProvider interface {
	...
	ProvidePayment(stream io.ReadWriter, host types.TurtleDexPublicKey, rpc types.Specifier, amount types.Currency, refundAccount AccountID, blockHeight types.BlockHeight) error
}
...
type (
	...
	// It should always be a valid representation of types.TurtleDexPublicKey or an
	// empty string.
	AccountID struct {
		spk string
	}
...
func NewAccountID() (id AccountID, sk crypto.SecretKey) {
	...
	id.FromSPK(types.TurtleDexPublicKey{
...
// FromSPK creates an AccountID from a TurtleDexPublicKey. This assumes that the
// provided key is valid and won't perform additional checks.
func (aid *AccountID) FromSPK(spk types.TurtleDexPublicKey) {
	if spk.Equals(types.TurtleDexPublicKey{}) {
		*aid = ZeroAccountID
		return
	}
...
// MarshalTurtleDex implements the TurtleDexMarshaler interface.
func (aid AccountID) MarshalTurtleDex(w io.Writer) error {
	if aid.IsZeroAccount() {
		return types.TurtleDexPublicKey{}.MarshalTurtleDex(w)
	}
	return aid.SPK().MarshalTurtleDex(w)
}

// UnmarshalTurtleDex implements the TurtleDexMarshaler interface.
func (aid *AccountID) UnmarshalTurtleDex(r io.Reader) error {
	var spk types.TurtleDexPublicKey
	err := spk.UnmarshalTurtleDex(r)
	if err != nil {
		return err
	}
	aid.FromSPK(spk)
...
func (aid AccountID) PK() (pk crypto.PublicKey) {
	...
	if len(spk.Key) != len(pk) {
		panic("key len mismatch between crypto.Publickey and types.TurtleDexPublicKey")
	}
	copy(pk[:], spk.Key)
	return
}

// SPK returns the account id as a types.TurtleDexPublicKey.
func (aid AccountID) SPK() (spk types.TurtleDexPublicKey) {
	if aid.IsZeroAccount() {
		build.Critical("should never use the zero account")
	}
	err := spk.LoadString(aid.spk)
	if err != nil {
		build.Critical("account id should never fail to be loaded as a TurtleDexPublicKey")
	}
	return
}
