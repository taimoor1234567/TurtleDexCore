...
// TestTurtleDexMuxCompat verifies the TurtleDexMux is initialized in compatibility mode
// when the host's persistence metadata version is v1.4.2
func TestTurtleDexMuxCompat(t *testing.T) {
	// ensure the host's persistence file does not exist
	siaDataDir := filepath.Join(os.TempDir(), t.Name())
	siaMuxDir := filepath.Join(siaDataDir, TurtleDexMuxDir)
	...
	mux, err := NewTurtleDexMux(siaMuxDir, siaDataDir, "localhost:0", "localhost:0")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(actualPK[:], expectedPK[:]) {
		t.Log(actualPK)
		t.Log(expectedPK)
		t.Fatal("TurtleDexMux's public key was different after reloading the mux")
	}
	if !bytes.Equal(actualSK[:], expectedSK[:]) {
		t.Log(actualSK)
		t.Log(expectedSK)
		t.Fatal("TurtleDexMux's private key was different after reloading the mux")
	}
	mux.Close()
	...
	if !bytes.Equal(actualPK[:], spk.Key) {
		t.Log(actualPK)
		t.Log(spk.Key)
		t.Fatal("TurtleDexMux's public key was not equal to the host's pubkey")
	}
...
// TestTurtleDexMuxAbsolutePath verifies we can not create the TurtleDexMux using a relative
// path for neither the siamux dir nor the sia data dir.
func TestTurtleDexMuxAbsolutePath(t *testing.T) {
	t.Parallel()

	assertRecover := func() {
		if r := recover(); r == nil {
			t.Fatalf("Expected a panic when a relative path is passed to the TurtleDexMux")
		}
	}
	...
	for _, relPath := range []string{"", ".", ".."} {
		func() {
			defer assertRecover()
			NewTurtleDexMux(absPath, relPath, "localhost:0", "localhost:0")
		}()
		func() {
			defer assertRecover()
			NewTurtleDexMux(relPath, absPath, "localhost:0", "localhost:0")
		}()
	}
}
