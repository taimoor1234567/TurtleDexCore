...
const (
	...
	// HostTurtleDexMuxSubscriberName is the name used by the host to register a
	// listener on the TurtleDexMux.
	HostTurtleDexMuxSubscriberName = "host"
...
const (
	...
	// support 6 month contracts when TurtleDex leaves beta.
...
var (
	...
	// 4096-byte messages.
	DefaultBaseRPCPrice = types.TurtleDexcoinPrecision.Mul64(100).Div64(1e9) // 100 nS
	...
	// keep the data even if the price of ttdc fluctuates, the price of raw
	// storage fluctuates, or the host realizes that there is unexpected
	// opportunity cost in being a host.
	DefaultCollateral = types.TurtleDexcoinPrecision.Mul64(100).Div(BlockBytesPerMonthTerabyte) // 100 SC / TB / Month
	...
	// relatively small file contract, but millions of ttdcs could be locked
	...
	// bit.
	DefaultMaxCollateral = types.TurtleDexcoinPrecision.Mul64(5e3)
	...
	// transaction.
	DefaultContractPrice = types.TurtleDexcoinPrecision.Div64(100).Div64(1e3).Mul64(EstimatedFileContractRevisionAndProofTransactionSetSize)

	// DefaultDownloadBandwidthPrice defines the default price of upload
	// bandwidth. The default is set to 10 ttdcs per gigabyte, because
	// download bandwidth is expected to be plentiful but also in-demand.
	DefaultDownloadBandwidthPrice = types.TurtleDexcoinPrecision.Mul64(25).Div(BytesPerTerabyte) // 25 SC / TB
	...
	DefaultMaxEphemeralAccountBalance = types.TurtleDexcoinPrecision
	...
	DefaultSectorAccessPrice = types.TurtleDexcoinPrecision.Mul64(2).Div64(1e6) // 2 uS
	...
	DefaultStoragePrice = types.TurtleDexcoinPrecision.Mul64(50).Div(BlockBytesPerMonthTerabyte) // 50 SC / TB / Month

	// DefaultUploadBandwidthPrice defines the default price of upload
	// bandwidth. The default is set to 1 ttdc per GB, because the host is
	...
	// data.
	DefaultUploadBandwidthPrice = types.TurtleDexcoinPrecision.Mul64(1).Div(BytesPerTerabyte) // 1 SC / TB
	...
	CompatV1412DefaultMaxEphemeralAccountBalance = types.TurtleDexcoinPrecision
)
...
type (
	...
	StorageObligation struct {
		...
		ValidProofOutputs  []types.TurtleDexcoinOutput `json:"validproofoutputs"`
		MissedProofOutputs []types.TurtleDexcoinOutput `json:"missedproofoutputs"`
	}
...
// fields set. NetAddress, RemainingStorage, TotalStorage, UnlockHash, RevisionNumber and TurtleDexMuxPort are not set.
