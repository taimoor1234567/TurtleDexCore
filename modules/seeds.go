...
func (is identifierSeed) identifier(txn types.Transaction) (ci contractIdentifier) {
	s := crypto.HashAll(is, txn.TurtleDexcoinInputs[0].ParentID)
	copy(ci[:], s[:])
	return
}
...
func (iss identifierSigningSeed) identifierSigningKey(txn types.Transaction) (cisk contractIdentifierSigningKey) {
	s1 := crypto.HashAll(iss, txn.TurtleDexcoinInputs[0].ParentID, 0)
	s2 := crypto.HashAll(iss, txn.TurtleDexcoinInputs[0].ParentID, 1)
	copy(cisk[:32], s1[:])
	copy(cisk[32:], s2[:])
	return
}
...
func GenerateContractKeyPair(renterSeed EphemeralRenterSeed, txn types.Transaction) (sk crypto.SecretKey, pk crypto.PublicKey) {
	return GenerateContractKeyPairWithOutputID(renterSeed, txn.TurtleDexcoinInputs[0].ParentID)
}
...
func GenerateContractKeyPairWithOutputID(renterSeed EphemeralRenterSeed, inputParentID types.TurtleDexcoinOutputID) (sk crypto.SecretKey, pk crypto.PublicKey) {
	// Get the secret key seed and wipe it afterwards.
	csks := renterSeed.contractSecretKeySeed()
	defer fastrand.Read(csks[:])

	// Combine the seed with the first TurtleDexcoinInput's parentID to create unique entropy for a
	// txn.
...
// signed identifier using a renter key and the first ttdc input of a
// transaction.
...
// from memory safely.
func PrefixedSignedIdentifier(renterSeed EphemeralRenterSeed, txn types.Transaction, hostKey types.TurtleDexPublicKey) (ContractSignedIdentifier, crypto.Ciphertext) {
	...
	sk, err := crypto.NewTurtleDexKey(crypto.TypeThreefish, signingKey[:])
	if err != nil {
	...
	copy(csi[:16], PrefixNonTurtleDex[:])
...
func (csi ContractSignedIdentifier) IsValid(renterSeed EphemeralRenterSeed, txn types.Transaction, hostKey crypto.Ciphertext) (types.TurtleDexPublicKey, bool, error) {
	// Get the seeds and wipe them after we are done using them.
	cis := renterSeed.contractIdentifierSeed()
	defer fastrand.Read(cis[:])
	ciss := renterSeed.contractIdentifierSigningSeed()
	...
	sk, err := crypto.NewTurtleDexKey(crypto.TypeThreefish, signingKey[:])
	if err != nil {
		build.Critical("Unable to generate New TurtleDex Key", err)
		return types.TurtleDexPublicKey{}, false, errors.AddContext(err, "error getting new TurtleDex PublicKey")
	}
	// Pad the identifier and sign it but then only use 32 bytes of the
	// signature.
	signature := sk.EncryptBytes(append(identifier[:], make([]byte, 32)...))[:32]
	...
	if !bytes.Equal(signature, csi[48:80]) {
		return types.TurtleDexPublicKey{}, false, ErrCSIDoesNotMatchSeed
	}
	// Decrypt the hostKey.
	hk, err := sk.DecryptBytes(hostKey)
	if err != nil {
		return types.TurtleDexPublicKey{}, false, errors.AddContext(err, "error decrypting bytes")
	}
	// Decode the hostKey.
	var spk types.TurtleDexPublicKey
	if err := encoding.Unmarshal(hk, &spk); err != nil {
		return types.TurtleDexPublicKey{}, false, errors.AddContext(err, "error unmarshalling")
	}
	return spk, true, nil
}
