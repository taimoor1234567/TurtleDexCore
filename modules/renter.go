...
type WorkerPool interface {
	Worker(types.TurtleDexPublicKey) (Worker, error)
}
...
var (
	...
	DefaultAllowance = Allowance{
		Funds:       types.TurtleDexcoinPrecision.Mul64(2500),
...
type RenterStats struct {
	...
	TotalTurtleDexfiles uint64 `json:"totalsiafiles"`
	TotalTurtleDexdirs  uint64 `json:"totalttdxdirs"`
	TotalSize     uint64 `json:"totalsize"`
...
// DirectoryInfo provides information about a ttdxdir
type DirectoryInfo struct {
	...
	TurtleDexPath             TurtleDexPath     `json:"siapath"`
...
func (d DirectoryInfo) Name() string { return d.TurtleDexPath.Name() }
...
type DownloadInfo struct {
	...
	TurtleDexPath         TurtleDexPath `json:"siapath"`         // The siapath of the file used for the download.
...
// Name implements os.FileInfo.
func (f FileInfo) Name() string { return f.TurtleDexPath.Name() }
...
type RecoverableContract struct {
	...
	// with.
	HostPublicKey types.TurtleDexPublicKey `json:"hostpublickey"`
	// InputParentID is the ParentID of the first TurtleDexcoinInput of the
	// transaction that contains this contract.
	InputParentID types.TurtleDexcoinOutputID `json:"inputparentid"`
...
type RenterContract struct {
	...
	HostPublicKey types.TurtleDexPublicKey
	...
	//
	// TurtleDexfundFee is the amount of money spent on siafund fees when creating the
	...
	TurtleDexfundFee  types.Currency
}
...
type ContractorSpending struct {
	...
	// includes the ContractFee, TxnFee and TurtleDexfundFee
...
type (
	...
	WorkerStatus struct {
		...
		HostPubKey      types.TurtleDexPublicKey   `json:"hostpubkey"`
...
type Renter interface {
	...
	ContractUtility(pk types.TurtleDexPublicKey) (ContractUtility, bool)
	...
	Mount(mountPoint string, sp TurtleDexPath, opts MountOptions) error
	...
	SetFileStuck(siaPath TurtleDexPath, stuck bool) error
	...
	UploadedBackups() ([]UploadedBackup, []types.TurtleDexPublicKey, error)

	// BackupsOnHost returns the backups stored on the specified host.
	BackupsOnHost(hostKey types.TurtleDexPublicKey) ([]UploadedBackup, error)

	// DeleteFile deletes a file entry from the renter.
	DeleteFile(siaPath TurtleDexPath) error
	...
	File(siaPath TurtleDexPath) (FileInfo, error)
	...
	FileList(siaPath TurtleDexPath, recursive, cached bool, flf FileListFunc) error

	// Filter returns the renter's hostdb's filterMode and filteredHosts
	Filter() (FilterMode, map[string]types.TurtleDexPublicKey, error)

	// SetFilterMode sets the renter's hostdb filter mode
	SetFilterMode(fm FilterMode, hosts []types.TurtleDexPublicKey) error

	// Host provides the DB entry and score breakdown for the requested host.
	Host(pk types.TurtleDexPublicKey) (HostDBEntry, bool, error)

	// InitialScanComplete returns a boolean indicating if the initial scan of the
	// hostdb is completed.
	InitialScanComplete() (bool, error)

	// PriceEstimation estimates the cost in ttdcs of performing various
	...
	RenameFile(siaPath, newTurtleDexPath TurtleDexPath) error

	// RenameDir changes the path of a dir.
	RenameDir(oldPath, newPath TurtleDexPath) error
	...
	// used.
	ReadRegistry(spk types.TurtleDexPublicKey, tweak crypto.Hash, timeout time.Duration) (SignedRegistryValue, error)
	...
	SetFileTrackingPath(siaPath TurtleDexPath, newPath string) error

	// UpdateRegistry updates the registries on all workers with the given
	// registry value.
	UpdateRegistry(spk types.TurtleDexPublicKey, srv SignedRegistryValue, timeout time.Duration) error
	...
	// from the TurtleDex network and also returns the fileName of the streamed
	// resource.
	Streamer(siapath TurtleDexPath, disableLocalFetch bool) (string, Streamer, error)

	// Upload uploads a file using the input parameters.
	Upload(FileUploadParams) error

	// UploadStreamFromReader reads from the provided reader until io.EOF is
	// reached and upload the data to the TurtleDex network.
	UploadStreamFromReader(up FileUploadParams, reader io.Reader) error

	// CreateDir creates a directory for the renter
	CreateDir(siaPath TurtleDexPath, mode os.FileMode) error

	// DeleteDir deletes a directory from the renter
	DeleteDir(siaPath TurtleDexPath) error

	// DirList lists the directories in a ttdxdir
	DirList(siaPath TurtleDexPath) ([]DirectoryInfo, error)
	...
	// CreateSkylinkFromTurtleDexfile will create a skylink from a siafile. This will
	...
	// separately as well.
	CreateSkylinkFromTurtleDexfile(SkyfileUploadParameters, TurtleDexPath) (Skylink, error)
	...
	// DownloadSkylink will fetch a file from the TurtleDex network using the given
	...
	// UploadSkyfile will upload data to the TurtleDex network from a reader and
	...
	// updates the ttdxdir metadata on disk then calls callThreadedBubbleMetadata
	...
	BubbleMetadata(siaPath TurtleDexPath, force, recursive bool) error
}

// Streamer is the interface implemented by the Renter's streamer type which
// allows for streaming files uploaded to the TurtleDex network.
...
type HostDB interface {
	...
	// SetFilterMode sets the renter's hostdb filter mode
	SetFilterMode(lm FilterMode, hosts []types.TurtleDexPublicKey) error
	...
	IncrementSuccessfulInteractions(types.TurtleDexPublicKey) error

	// IncrementFailedInteractions increments the number of failed interactions with
	// a host for a given key
	IncrementFailedInteractions(types.TurtleDexPublicKey) error
	...
	RandomHosts(int, []types.TurtleDexPublicKey, []types.TurtleDexPublicKey) ([]HostDBEntry, error)
	...
	// renter.
	RandomHostsWithAllowance(int, []types.TurtleDexPublicKey, []types.TurtleDexPublicKey, Allowance) ([]HostDBEntry, error)
