...
// TestTurtleDexdConfigPersistCompat confirms that the compat code for the writebps
// json tag update performs as expected
func TestTurtleDexdConfigPersistCompat(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Create ttdxdconfig
	testDir := build.TempDir("ttdxdconfig", t.Name())
...
// saveLoadCheck is a helper to check saving and loading the ttdxd config file
// and verifying the correct values for the WriteBPS fields
func saveLoadCheck(sc *TurtleDexdConfig, writeBPS, writeBPSDeprepacted int64) error {
	// Set both fields
...
func checkWriteBPS(sc *TurtleDexdConfig, writeBPS int64) error {
