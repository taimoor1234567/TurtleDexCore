...
func TestMapHeapSimple(t *testing.T) {
	...
	for _, i := range randSlice {
		e1 := &mapElement{
			set: &splitSet{
				averageFee:   types.TurtleDexcoinPrecision.Mul64(uint64(i)),
				size:         uint64(10 * i),
	...
	for i := 0; i < 1000; i++ {
		...
		if maxPop.set.averageFee.Cmp(types.TurtleDexcoinPrecision.Mul64(uint64(999-i))) != 0 {
			t.Error("Unexpected currency value in result from max-heap pop.")
		}
		if minPop.set.averageFee.Cmp(types.TurtleDexcoinPrecision.Mul64(uint64(i))) != 0 {
