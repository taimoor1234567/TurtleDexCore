...
func TestLoadSeed(t *testing.T) {
	...
	ttdcBal, _, _, err := w.ConfirmedBalance()
	if err != nil {
		t.Fatal(err)
	}
	if !ttdcBal.Equals64(0) {
	...
	ttdcBal2, _, _, err := w.ConfirmedBalance()
	if err != nil {
		t.Fatal(err)
	}
	if ttdcBal2.Cmp64(0) <= 0 {
...
// its ttdc outputs to the wallet.
...
func TestSweepSeedFunds(t *testing.T) {
	...
	err = wt.wallet.LoadTurtleDexgKeys(wt.walletMasterKey, []string{"../../types/siag0of1of1.siakey"})
	if err != nil {
		t.Error(err)
	}
	...
	_, err = wt.wallet.SendTurtleDexfunds(types.NewCurrency64(12), sk.UnlockConditions.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}
	// Send some ttdcs to the address, but not enough to cover the
	// transaction fee.
	_, err = wt.wallet.SendTurtleDexcoins(types.NewCurrency64(1), sk.UnlockConditions.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}
...
func TestSweepSeedSentFunds(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		_, err = wt.wallet.SendTurtleDexfunds(types.NewCurrency64(1), uc.UnlockHash())
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
	...
	_, err = wt.wallet.SendTurtleDexfunds(types.NewCurrency64(10), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	if err := wt.addBlockNoPayout(); err != nil {
...
// transfer of its ttdc and siafund outputs to the wallet.
func TestSweepSeedCoinsAndFunds(t *testing.T) {
	...
	// Send some siafunds to the address.
	for i := 0; i < 12; i++ {
		_, err = wt.wallet.SendTurtleDexfunds(types.NewCurrency64(1), sk.UnlockConditions.UnlockHash())
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
	...
	// Send some ttdcs to the address -- must be more than the transaction
	// fee.
	for i := 0; i < 100; i++ {
		_, err = wt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(10), sk.UnlockConditions.UnlockHash())
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
