...
func TestSignTransaction(t *testing.T) {
	...
	// get a ttdc output and a siafund output
	...
	for _, o := range outputs {
		if o.FundType == types.SpecifierTurtleDexcoinOutput {
			sco = o
		} else if o.FundType == types.SpecifierTurtleDexfundOutput {
			sfo = o
		}
	}
	...
	txn := types.Transaction{
		TurtleDexfundInputs: []types.TurtleDexfundInput{{
			ParentID:         types.TurtleDexfundOutputID(sfo.ID),
		...
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID:         types.TurtleDexcoinOutputID(sco.ID),
...
func TestSignTransactionNoWallet(t *testing.T) {
	...
	txn := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID:         types.TurtleDexcoinOutputID{1}, // doesn't need to actually exist
		...
		TurtleDexfundInputs: []types.TurtleDexfundInput{{
			ParentID:         types.TurtleDexfundOutputID{2}, // doesn't need to actually exist
			UnlockConditions: sk.UnlockConditions,
		}},
		TurtleDexfundOutputs: []types.TurtleDexfundOutput{{
			Value:      types.NewCurrency64(1),
...
func TestUnspentOutputs(t *testing.T) {
	...
	_, err = wt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(77), addr)
	if err != nil {
		t.Fatal(err)
	}
	wt.miner.AddBlock()
