...
func ComputeValuedTransactions(pts []modules.ProcessedTransaction, blockHeight types.BlockHeight) ([]modules.ValuedTransaction, error) {
	...
	for _, pt := range pts {
		...
		// transaction.
		var outgoingTurtleDexcoins types.Currency
		for _, input := range pt.Inputs {
		...
		var incomingTurtleDexcoins types.Currency
		for _, output := range pt.Outputs {
			...
			if output.FundType == types.SpecifierTurtleDexcoinOutput && output.WalletAddress {
				incomingTurtleDexcoins = incomingTurtleDexcoins.Add(output.Value)
			}
		}
		...
		st := modules.ValuedTransaction{
			...
			ConfirmedIncomingValue: incomingTurtleDexcoins,
			ConfirmedOutgoingValue: outgoingTurtleDexcoins,
		}
