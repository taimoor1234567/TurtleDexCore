...
func (w *Wallet) UnspentOutputs() ([]modules.UnspentOutput, error) {
	...
	dbForEachTurtleDexcoinOutput(w.dbTx, func(scoid types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput) {
	...
	dbForEachTurtleDexfundOutput(w.dbTx, func(sfoid types.TurtleDexfundOutputID, sfo types.TurtleDexfundOutput) {
		outputs = append(outputs, modules.UnspentOutput{
...
func (w *Wallet) UnlockConditions(addr types.UnlockHash) (uc types.UnlockConditions, err error) {
	...
	uc.PublicKeys = append([]types.TurtleDexPublicKey(nil), uc.PublicKeys...)
	return uc, nil
}
...
func (w *Wallet) RemoveWatchAddresses(addrs []types.UnlockHash, unused bool) error {
	...
	err := func() error {
		...
		if !unused {
			...
			// TurtleDexcoinOutputs bucket. Iterate through the bucket and remove
			// any outputs that we are no longer watching.
			var outputIDs []types.TurtleDexcoinOutputID
			dbForEachTurtleDexcoinOutput(w.dbTx, func(scoid types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput) {
			...
			for _, scoid := range outputIDs {
				if err := dbDeleteTurtleDexcoinOutput(w.dbTx, scoid); err != nil {
					return err
				}
			}
