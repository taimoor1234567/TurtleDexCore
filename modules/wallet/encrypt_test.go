...
func postEncryptionTesting(m modules.TestMiner, w *Wallet, masterKey crypto.CipherKey) {
	...
	_, err = w.SendTurtleDexcoins(types.CalculateCoinbase(0), types.UnlockHash{})
	if err != nil {
		panic(err)
	}
	_, err = m.AddBlock()
...
func TestLock(t *testing.T) {
	...
	// Lock the wallet.
	ttdcBalance, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	err = wt.wallet.Lock()
	...
	ttdcBalance2, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	if !ttdcBalance2.Equals(ttdcBalance) {
		t.Error("ttdc balance reporting changed upon closing the wallet")
	}
	...
	ttdcBalance3, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	if ttdcBalance3.Cmp(ttdcBalance2) <= 0 {
...
func TestReset(t *testing.T) {
	...
	originalKey := crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
	...
	newKey := crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
