...
type seedScanner struct {
	...
	seed             modules.Seed
	ttdcOutputs   map[types.TurtleDexcoinOutputID]scannedOutput
	siafundOutputs   map[types.TurtleDexfundOutputID]scannedOutput
...
func (s *seedScanner) ProcessConsensusChange(cc modules.ConsensusChange) {
	...
	for _, diff := range cc.TurtleDexcoinOutputDiffs {
		if diff.Direction == modules.DiffApply {
			if index, exists := s.keys[diff.TurtleDexcoinOutput.UnlockHash]; exists && diff.TurtleDexcoinOutput.Value.Cmp(s.dustThreshold) > 0 {
				s.ttdcOutputs[diff.ID] = scannedOutput{
					id:        types.OutputID(diff.ID),
					value:     diff.TurtleDexcoinOutput.Value,
					seedIndex: index,
				}
			}
			...
			if _, exists := s.keys[diff.TurtleDexcoinOutput.UnlockHash]; exists {
				delete(s.ttdcOutputs, diff.ID)
			}
		}
	}
	for _, diff := range cc.TurtleDexfundOutputDiffs {
		if diff.Direction == modules.DiffApply {
			...
			if index, exists := s.keys[diff.TurtleDexfundOutput.UnlockHash]; exists {
				s.siafundOutputs[diff.ID] = scannedOutput{
					id:        types.OutputID(diff.ID),
					value:     diff.TurtleDexfundOutput.Value,
					seedIndex: index,
				}
			}
		} else if diff.Direction == modules.DiffRevert {
			// NOTE: DiffRevert means the output was either spent or was in a
			// block that was reverted.
			if _, exists := s.keys[diff.TurtleDexfundOutput.UnlockHash]; exists {
	...
	for _, diff := range cc.TurtleDexcoinOutputDiffs {
		index, exists := s.keys[diff.TurtleDexcoinOutput.UnlockHash]
		if exists {
	...
	for _, diff := range cc.TurtleDexfundOutputDiffs {
		index, exists := s.keys[diff.TurtleDexfundOutput.UnlockHash]
		if exists {
			s.log.Debugln("Seed scanner found a key used at index", index)
			if index > s.largestIndexSeen {
				s.largestIndexSeen = index
...
func newSeedScanner(seed modules.Seed, log *persist.Logger) *seedScanner {
	return &seedScanner{
		...
		ttdcOutputs: make(map[types.TurtleDexcoinOutputID]scannedOutput),
		siafundOutputs: make(map[types.TurtleDexfundOutputID]scannedOutput),
