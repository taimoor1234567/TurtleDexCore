...
type (
	spentTurtleDexcoinOutputSet map[types.TurtleDexcoinOutputID]types.TurtleDexcoinOutput
	spentTurtleDexfundOutputSet map[types.TurtleDexfundOutputID]types.TurtleDexfundOutput
)
...
func (w *Wallet) updateLookahead(tx *bolt.Tx, cc modules.ConsensusChange) (bool, error) {
	...
	for _, diff := range cc.TurtleDexcoinOutputDiffs {
		if index, ok := w.lookahead[diff.TurtleDexcoinOutput.UnlockHash]; ok {
			if index > largestIndex {
				largestIndex = index
			}
		}
	}
	for _, diff := range cc.TurtleDexfundOutputDiffs {
		if index, ok := w.lookahead[diff.TurtleDexfundOutput.UnlockHash]; ok {
			if index > largestIndex {
				largestIndex = index
			}
		}
...
func (w *Wallet) updateConfirmedSet(tx *bolt.Tx, cc modules.ConsensusChange) error {
	for _, diff := range cc.TurtleDexcoinOutputDiffs {
		...
		if !w.isWalletAddress(diff.TurtleDexcoinOutput.UnlockHash) {
			continue
		}

		var err error
		if diff.Direction == modules.DiffApply {
			w.log.Println("Wallet has gained a spendable ttdc output:", diff.ID, "::", diff.TurtleDexcoinOutput.Value.HumanString())
			err = dbPutTurtleDexcoinOutput(tx, diff.ID, diff.TurtleDexcoinOutput)
		} else {
			w.log.Println("Wallet has lost a spendable ttdc output:", diff.ID, "::", diff.TurtleDexcoinOutput.Value.HumanString())
			err = dbDeleteTurtleDexcoinOutput(tx, diff.ID)
		}
		if err != nil {
			w.log.Severe("Could not update ttdc output:", err)
			return err
		}
	}
	for _, diff := range cc.TurtleDexfundOutputDiffs {
		// Verify that the diff is relevant to the wallet.
		if !w.isWalletAddress(diff.TurtleDexfundOutput.UnlockHash) {
			continue
		}

		var err error
		if diff.Direction == modules.DiffApply {
			w.log.Println("Wallet has gained a spendable siafund output:", diff.ID, "::", diff.TurtleDexfundOutput.Value)
			err = dbPutTurtleDexfundOutput(tx, diff.ID, diff.TurtleDexfundOutput)
		} else {
			w.log.Println("Wallet has lost a spendable siafund output:", diff.ID, "::", diff.TurtleDexfundOutput.Value)
			err = dbDeleteTurtleDexfundOutput(tx, diff.ID)
		}
		if err != nil {
	...
	for _, diff := range cc.TurtleDexfundPoolDiffs {
		var err error
		if diff.Direction == modules.DiffApply {
			err = dbPutTurtleDexfundPool(tx, diff.Adjusted)
		} else {
			err = dbPutTurtleDexfundPool(tx, diff.Previous)
		}
		if err != nil {
...
// outputs and collects them in a map of TurtleDexcoinOutputID -> TurtleDexcoinOutput.
func computeSpentTurtleDexcoinOutputSet(diffs []modules.TurtleDexcoinOutputDiff) spentTurtleDexcoinOutputSet {
	outputs := make(spentTurtleDexcoinOutputSet)
	for _, diff := range diffs {
		if diff.Direction == modules.DiffRevert {
			// DiffRevert means spent.
			outputs[diff.ID] = diff.TurtleDexcoinOutput
		}
	}
	return outputs
}

// computeSpentTurtleDexfundOutputSet scans a slice of TurtleDexfund output diffs for spent
// outputs and collects them in a map of TurtleDexfundOutputID -> TurtleDexfundOutput.
func computeSpentTurtleDexfundOutputSet(diffs []modules.TurtleDexfundOutputDiff) spentTurtleDexfundOutputSet {
	outputs := make(spentTurtleDexfundOutputSet)
	for _, diff := range diffs {
		if diff.Direction == modules.DiffRevert {
			// DiffRevert means spent.
			outputs[diff.ID] = diff.TurtleDexfundOutput
		}
	}
	return outputs
}
...
func (w *Wallet) computeProcessedTransactionsFromBlock(tx *bolt.Tx, block types.Block, spentTurtleDexcoinOutputs spentTurtleDexcoinOutputSet, spentTurtleDexfundOutputs spentTurtleDexfundOutputSet, consensusHeight types.BlockHeight) []modules.ProcessedTransaction {
	...
	for _, txn := range block.Transactions {
		...
		for _, sco := range txn.TurtleDexcoinOutputs {
		...
		for _, sfo := range txn.TurtleDexfundOutputs {
		...
		for _, sci := range txn.TurtleDexcoinInputs {
			pi := modules.ProcessedInput{
				...
				FundType:       types.SpecifierTurtleDexcoinInput,
				WalletAddress:  w.isWalletAddress(sci.UnlockConditions.UnlockHash()),
				RelatedAddress: sci.UnlockConditions.UnlockHash(),
				Value:          spentTurtleDexcoinOutputs[sci.ParentID].Value,
			}
			pt.Inputs = append(pt.Inputs, pi)

			// Log any wallet-relevant inputs.
			if pi.WalletAddress {
				w.log.Println("\tTurtleDexcoin Input:", pi.ParentID, "::", pi.Value.HumanString())
			}
		}

		for i, sco := range txn.TurtleDexcoinOutputs {
			po := modules.ProcessedOutput{
				ID:             types.OutputID(txn.TurtleDexcoinOutputID(uint64(i))),
				FundType:       types.SpecifierTurtleDexcoinOutput,
			...
			if po.WalletAddress {
				w.log.Println("\tTurtleDexcoin Output:", po.ID, "::", po.Value.HumanString())
			}
		}
		...
		for _, sfi := range txn.TurtleDexfundInputs {
			pi := modules.ProcessedInput{
				...
				FundType:       types.SpecifierTurtleDexfundInput,
				WalletAddress:  w.isWalletAddress(sfi.UnlockConditions.UnlockHash()),
				RelatedAddress: sfi.UnlockConditions.UnlockHash(),
				Value:          spentTurtleDexfundOutputs[sfi.ParentID].Value,
			}
			pt.Inputs = append(pt.Inputs, pi)
			// Log any wallet-relevant inputs.
			if pi.WalletAddress {
				w.log.Println("\tTurtleDexfund Input:", pi.ParentID, "::", pi.Value.HumanString())
			}
			...
			sfo := spentTurtleDexfundOutputs[sfi.ParentID]
			po := modules.ProcessedOutput{
		...
		for i, sfo := range txn.TurtleDexfundOutputs {
			po := modules.ProcessedOutput{
				ID:             types.OutputID(txn.TurtleDexfundOutputID(uint64(i))),
				FundType:       types.SpecifierTurtleDexfundOutput,
				MaturityHeight: consensusHeight,
			...
			// Log any wallet-relevant outputs.
			if po.WalletAddress {
				w.log.Println("\tTurtleDexfund Output:", po.ID, "::", po.Value.HumanString())
			}
		}
...
func (w *Wallet) applyHistory(tx *bolt.Tx, cc modules.ConsensusChange) error {
	spentTurtleDexcoinOutputs := computeSpentTurtleDexcoinOutputSet(cc.TurtleDexcoinOutputDiffs)
	spentTurtleDexfundOutputs := computeSpentTurtleDexfundOutputSet(cc.TurtleDexfundOutputDiffs)
	...
	for _, block := range cc.AppliedBlocks {
		...
		pts := w.computeProcessedTransactionsFromBlock(tx, block, spentTurtleDexcoinOutputs, spentTurtleDexfundOutputs, consensusHeight)
		for _, pt := range pts {
...
func (w *Wallet) ReceiveUpdatedUnconfirmedTransactions(diff *modules.TransactionPoolDiff) {
	...
	for _, unconfirmedTxnSet := range diff.AppliedTransactions {
		...
		spentTurtleDexcoinOutputs := make(map[types.TurtleDexcoinOutputID]types.TurtleDexcoinOutput)
		for _, scod := range unconfirmedTxnSet.Change.TurtleDexcoinOutputDiffs {
			...
			if scod.Direction == modules.DiffRevert {
				spentTurtleDexcoinOutputs[scod.ID] = scod.TurtleDexcoinOutput
			}
		}
