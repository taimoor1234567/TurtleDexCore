...
func TestDefragWallet(t *testing.T) {
	...
	ttdcOutputs := wt.wallet.dbTx.Bucket(bucketTurtleDexcoinOutputs).Stats().KeyN
	wt.wallet.mu.Unlock()
	if ttdcOutputs > defragThreshold {
		t.Fatalf("defrag should result in fewer than defragThreshold outputs, got %v wanted %v\n", ttdcOutputs, defragThreshold)
	}
}
...
func TestDefragWalletDust(t *testing.T) {
	...
	err = tbuilder.FundTurtleDexcoins(dustOutputValue.Mul64(uint64(noutputs)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDefragOutputExhaustion(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			case <-time.After(time.Millisecond * 100):
				...
				txnValue := types.TurtleDexcoinPrecision.Mul64(3000)
				...
				tbuilder.FundTurtleDexcoins(txnValue.Mul64(uint64(numOutputs)).Add(fee))
	...
	// fragmented outputs
	for i := 0; i < 30; i++ {
		sendAmount := types.TurtleDexcoinPrecision.Mul64(2000)
		_, err = wt.wallet.SendTurtleDexcoins(sendAmount, types.UnlockHash{})
		if err != nil {
			t.Errorf("%v: %v", i, err)
		}
...
func TestDefragInterrupted(t *testing.T) {
	...
	err = build.Retry(50, 100*time.Millisecond, func() error {
		...
		if ttdcOutputs > defragThreshold {
			err := fmt.Errorf("defrag should result in fewer than defragThreshold outputs, got %v wanted %v", ttdcOutputs, defragThreshold)
			return err
		}
		if spentOutputs == 0 {
