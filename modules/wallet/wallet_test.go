...
func createWalletTester(name string, deps modules.Dependencies) (*walletTester, error) {
	...
	masterKey := crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
...
func TestAdvanceLookaheadNoRescan(t *testing.T) {
	...
	for _, sk := range generateKeys(wt.wallet.primarySeed, progress, 10) {
		...
		builder.AddTurtleDexcoinOutput(sco)
...
func TestAdvanceLookaheadForceRescan(t *testing.T) {
	...
	farPayout := types.TurtleDexcoinPrecision.Mul64(8888)

	builder, err := wt.wallet.StartTransaction()
	if err != nil {
		t.Fatal(err)
	}
	builder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{
		UnlockHash: farAddr,
		Value:      farPayout,
	})
	err = builder.FundTurtleDexcoins(farPayout)
	if err != nil {
		t.Fatal(err)
	}
	...
	for uh, index := range wt.wallet.lookahead {
		...
		sco := types.TurtleDexcoinOutput{
			UnlockHash: uh,
			Value:      types.TurtleDexcoinPrecision.Mul64(1000),
		}
		builder.AddTurtleDexcoinOutput(sco)
		payout = payout.Add(sco.Value)
...
func TestDistantWallets(t *testing.T) {
	...
	for i := uint64(0); i < lookaheadBuffer/2; i++ {
		_, err = wt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision, types.UnlockHash{})
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
	...
	value := types.TurtleDexcoinPrecision.Mul64(1e3)
	tbuilder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{
		UnlockHash: farAddr,
		Value:      value,
	})
	err = tbuilder.FundTurtleDexcoins(value)
	if err != nil {
		t.Fatal(err)
	}
