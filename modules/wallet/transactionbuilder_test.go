...
func TestViewAdded(t *testing.T) {
	...
	err = b.FundTurtleDexcoins(txnFund)
	if err != nil {
		t.Fatal(err)
	}
	_ = b.AddMinerFee(txnFund)
	_ = b.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{Value: txnFund})
	...
	err = b2.FundTurtleDexcoins(txnFund)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, inputIndex := range newInputIndices {
		b.AddTurtleDexcoinInput(unfinishedTxn2.TurtleDexcoinInputs[inputIndex])
	}
...
func TestConcurrentBuilders(t *testing.T) {
	...
	// Fund each builder with a ttdc output that is smaller than all of the
	// outputs that the wallet should currently have.
	funding := types.NewCurrency64(10e3).Mul(types.TurtleDexcoinPrecision)
	err = builder1.FundTurtleDexcoins(funding)
	if err != nil {
		t.Fatal(err)
	}
	err = builder2.FundTurtleDexcoins(funding)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !startingSCConfirmed.Equals(fundedSCConfirmed) {
		t.Fatal("confirmed ttdc balance changed when no blocks have been mined", startingSCConfirmed, fundedSCConfirmed)
	}

	// Spend the transaction funds on miner fees and the void output.
	builder1.AddMinerFee(types.NewCurrency64(25).Mul(types.TurtleDexcoinPrecision))
	builder2.AddMinerFee(types.NewCurrency64(25).Mul(types.TurtleDexcoinPrecision))
	// Send the money to the void.
	output := types.TurtleDexcoinOutput{Value: types.NewCurrency64(9975).Mul(types.TurtleDexcoinPrecision)}
	builder1.AddTurtleDexcoinOutput(output)
	builder2.AddTurtleDexcoinOutput(output)
...
func TestConcurrentBuildersSingleOutput(t *testing.T) {
	...
	err = builder.FundTurtleDexcoins(scBal)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Fund each builder with a ttdc output.
	funding := types.NewCurrency64(10e3).Mul(types.TurtleDexcoinPrecision)
	err = builder1.FundTurtleDexcoins(funding)
	if err != nil {
		t.Fatal(err)
...
func TestParallelBuilders(t *testing.T) {
	...
	for i := 0; i < outputsDesired; i++ {
		...
		go func() {
			...
			err = builder.FundTurtleDexcoins(funding)
			if err != nil {
				t.Fatal(err)
			}

			// Spend the transaction funds on miner fees and the void output.
			builder.AddMinerFee(types.NewCurrency64(25).Mul(types.TurtleDexcoinPrecision))
			output := types.TurtleDexcoinOutput{Value: types.NewCurrency64(9975).Mul(types.TurtleDexcoinPrecision)}
			builder.AddTurtleDexcoinOutput(output)
			// Sign the transactions and verify that both are valid.
			tset, err := builder.Sign(true)
...
func TestUnconfirmedParents(t *testing.T) {
	...
	ttdcs, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Fatal(err)
	}
	tSet, err := wt.wallet.SendTurtleDexcoins(ttdcs.Sub(types.TurtleDexcoinPrecision), uc.UnlockHash())
	if err != nil {
		t.Fatal("Failed to send coins", err)
	}

	// Create a transaction. That transaction should use ttdc outputs from
...
func TestDoubleSpendCreation(t *testing.T) {
	...
	b.AddTurtleDexcoinOutput(output)

	unlockConditions2, err := wt.wallet.NextAddress()
	if err != nil {
		t.Fatal(err)
	}
	output2 := types.TurtleDexcoinOutput{
		Value:      txnFund,
		UnlockHash: unlockConditions2.UnlockHash(),
	}
	copyBuilder.AddTurtleDexcoinOutput(output2)
...
// TestReplaceOutput tests the ReplaceTurtleDexcoinOutput feature of the
...
func TestReplaceOutput(t *testing.T) {
	...
	unusedOutput := types.TurtleDexcoinOutput{
		Value:      txnFund,
		UnlockHash: unusedUC.UnlockHash(),
	}
	b.AddTurtleDexcoinOutput(unusedOutput)
	...
	b.ReplaceTurtleDexcoinOutput(0, replacementOutput)
	...
	for _, txn := range txnSet {
		for _, scOutput := range txn.TurtleDexcoinOutputs {
...
func TestDoubleSpendAfterMarking(t *testing.T) {
	...
	outputIndex := copyBuilder.AddTurtleDexcoinOutput(output2)
	...
	output3 := types.TurtleDexcoinOutput{
		Value:      txnFund.Sub(fee),
		UnlockHash: unlockConditions3.UnlockHash(),
	}
	err = newCopyBuilder.ReplaceTurtleDexcoinOutput(outputIndex, output3)
	...
	if len(alteredSet[1].TurtleDexcoinOutputs) < int(outputIndex) {
		t.Fatal(err)
	}
	foundUnlockHash := alteredSet[1].TurtleDexcoinOutputs[outputIndex].UnlockHash
