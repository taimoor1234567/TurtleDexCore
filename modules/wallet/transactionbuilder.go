...
// the ability to fund transactions with ttdcs and siafunds from the wallet.
...
// is compatible with both ttdc inputs and siafund inputs.
...
func (w *Wallet) checkOutput(tx *bolt.Tx, currentHeight types.BlockHeight, id types.TurtleDexcoinOutputID, output types.TurtleDexcoinOutput, dustThreshold types.Currency) error {
...
func (tb *transactionBuilder) Copy() modules.TransactionBuilder {
	...
	copyBuilder.ttdcInputs = make([]int, len(tb.ttdcInputs))
	copy(copyBuilder.ttdcInputs, tb.ttdcInputs)
...
func (tb *transactionBuilder) MarkWalletInputs() bool {
	...
	for i, scInput := range tb.transaction.TurtleDexcoinInputs {
		...
		alreadyMarked := false
		for _, storedIdx := range tb.ttdcInputs {
			if i == storedIdx {
				alreadyMarked = true
				break
			}
		...
		if !alreadyMarked {
			tb.ttdcInputs = append(tb.ttdcInputs, i)
			markedAnyInputs = true
		}
	}

	for i, sfInput := range tb.transaction.TurtleDexfundInputs {
		...
		if !alreadyMarked {
			tb.ttdcInputs = append(tb.siafundInputs, i)
			markedAnyInputs = true
		}
	}
...
// FundTurtleDexcoins will add a ttdc input of exactly 'amount' to the
// transaction. A parent transaction may be needed to achieve an input with the
// correct value. The ttdc input will not be signed until 'Sign' is called
// on the transaction builder.
func (tb *transactionBuilder) FundTurtleDexcoins(amount types.Currency) (err error) {
	if amount.IsZero() {
		return nil
	}
	...
	err = dbForEachTurtleDexcoinOutput(tb.wallet.dbTx, func(scoid types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput) {
	...
	for _, upt := range tb.wallet.unconfirmedProcessedTransactions {
		for i, sco := range upt.Transaction.TurtleDexcoinOutputs {
			// Determine if the output belongs to the wallet.
			_, exists := tb.wallet.keys[sco.UnlockHash]
			if !exists {
				continue
			}
			so.ids = append(so.ids, upt.Transaction.TurtleDexcoinOutputID(uint64(i)))
			so.outputs = append(so.outputs, sco)
		}
	}
	...
	if !amount.Equals(fund) {
		...
		refundOutput := types.TurtleDexcoinOutput{
			Value:      fund.Sub(amount),
			UnlockHash: refundUnlockConditions.UnlockHash(),
		}
		parentTxn.TurtleDexcoinOutputs = append(parentTxn.TurtleDexcoinOutputs, refundOutput)
	}
	...
	err = dbPutSpentOutput(tb.wallet.dbTx, types.OutputID(parentTxn.TurtleDexcoinOutputID(0)), consensusHeight)
	if err != nil {
		return err
	}

	// Add the exact output.
	newInput := types.TurtleDexcoinInput{
	...
	tb.ttdcInputs = append(tb.ttdcInputs, len(tb.transaction.TurtleDexcoinInputs))
	tb.transaction.TurtleDexcoinInputs = append(tb.transaction.TurtleDexcoinInputs, newInput)
...
// FundTurtleDexfunds will add a siafund input of exactly 'amount' to the
// transaction. A parent transaction may be needed to achieve an input with the
// correct value. The siafund input will not be signed until 'Sign' is called
// on the transaction builder.
func (tb *transactionBuilder) FundTurtleDexfunds(amount types.Currency) (err error) {
	if amount.IsZero() {
		return nil
	}
	...
	var potentialFund types.Currency
	parentTxn := types.Transaction{}
	var spentSfoids []types.TurtleDexfundOutputID
	c := tb.wallet.dbTx.Bucket(bucketTurtleDexfundOutputs).Cursor()
	for idBytes, sfoBytes := c.First(); idBytes != nil; idBytes, sfoBytes = c.Next() {
		var sfoid types.TurtleDexfundOutputID
		var sfo types.TurtleDexfundOutput
		...
		sfi := types.TurtleDexfundInput{
			ParentID:         sfoid,
			UnlockConditions: outputUnlockConditions,
			ClaimUnlockHash:  parentClaimUnlockConditions.UnlockHash(),
		}
		parentTxn.TurtleDexfundInputs = append(parentTxn.TurtleDexfundInputs, sfi)
	...
	exactOutput := types.TurtleDexfundOutput{
		Value:      amount,
		UnlockHash: parentUnlockConditions.UnlockHash(),
	}
	parentTxn.TurtleDexfundOutputs = append(parentTxn.TurtleDexfundOutputs, exactOutput)
	...
	if !amount.Equals(fund) {
		...
		refundOutput := types.TurtleDexfundOutput{
			Value:      fund.Sub(amount),
			UnlockHash: refundUnlockConditions.UnlockHash(),
		}
		parentTxn.TurtleDexfundOutputs = append(parentTxn.TurtleDexfundOutputs, refundOutput)
	}

	// Sign all of the inputs to the parent transaction.
	for _, sfi := range parentTxn.TurtleDexfundInputs {
	...
	newInput := types.TurtleDexfundInput{
		ParentID:         parentTxn.TurtleDexfundOutputID(0),
		UnlockConditions: parentUnlockConditions,
		ClaimUnlockHash:  claimUnlockConditions.UnlockHash(),
	}
	tb.newParents = append(tb.newParents, len(tb.parents))
	tb.parents = append(tb.parents, parentTxn)
	tb.siafundInputs = append(tb.siafundInputs, len(tb.transaction.TurtleDexfundInputs))
	tb.transaction.TurtleDexfundInputs = append(tb.transaction.TurtleDexfundInputs, newInput)
...
func (tb *transactionBuilder) Sweep(output types.TurtleDexcoinOutput) (txn types.Transaction, parents []types.Transaction) {
	builder := tb.Copy()
	builder.AddTurtleDexcoinOutput(output)
	return builder.View()
}
...
func (tb *transactionBuilder) UnconfirmedParents() (parents []types.Transaction, err error) {
	...
	for _, p := range tb.parents {
		for _, sci := range p.TurtleDexcoinInputs {
			...
			for _, txn := range tSet {
				...
				for i := range txn.TurtleDexcoinOutputs {
					if txn.TurtleDexcoinOutputID(uint64(i)) == sci.ParentID {
						break
					}
				}
			}
...
// AddTurtleDexcoinInput adds a ttdc input to the transaction, returning the index
// of the ttdc input within the transaction. When 'Sign' gets called, this
// input will be left unsigned.
func (tb *transactionBuilder) AddTurtleDexcoinInput(input types.TurtleDexcoinInput) uint64 {
	tb.transaction.TurtleDexcoinInputs = append(tb.transaction.TurtleDexcoinInputs, input)
	return uint64(len(tb.transaction.TurtleDexcoinInputs) - 1)
}

// AddTurtleDexcoinOutput adds a ttdc output to the transaction, returning the
// index of the ttdc output within the transaction.
func (tb *transactionBuilder) AddTurtleDexcoinOutput(output types.TurtleDexcoinOutput) uint64 {
	tb.transaction.TurtleDexcoinOutputs = append(tb.transaction.TurtleDexcoinOutputs, output)
	return uint64(len(tb.transaction.TurtleDexcoinOutputs) - 1)
}

// ReplaceTurtleDexcoinOutput replaces the ttdc output in the transaction at the
// given index.
func (tb *transactionBuilder) ReplaceTurtleDexcoinOutput(index uint64, output types.TurtleDexcoinOutput) error {
	if index >= uint64(len(tb.transaction.TurtleDexcoinOutputs)) {
		return errReplaceIndexOutOfBounds
	}
	tb.transaction.TurtleDexcoinOutputs[index] = output
	return nil
}
...
// AddTurtleDexfundInput adds a siafund input to the transaction, returning the index
// of the siafund input within the transaction. When 'Sign' is called, this
// input will be left unsigned.
func (tb *transactionBuilder) AddTurtleDexfundInput(input types.TurtleDexfundInput) uint64 {
	tb.transaction.TurtleDexfundInputs = append(tb.transaction.TurtleDexfundInputs, input)
	return uint64(len(tb.transaction.TurtleDexfundInputs) - 1)
}

// AddTurtleDexfundOutput adds a siafund output to the transaction, returning the
// index of the siafund output within the transaction.
func (tb *transactionBuilder) AddTurtleDexfundOutput(output types.TurtleDexfundOutput) uint64 {
	tb.transaction.TurtleDexfundOutputs = append(tb.transaction.TurtleDexfundOutputs, output)
	return uint64(len(tb.transaction.TurtleDexfundOutputs) - 1)
}
...
// added by calling 'FundTurtleDexcoins' or 'FundTurtleDexfunds'.
...
// Sign will sign any inputs added by 'FundTurtleDexcoins' or 'FundTurtleDexfunds' and
...
func (tb *transactionBuilder) Sign(wholeTransaction bool) ([]types.Transaction, error) {
	...
	if wholeTransaction {
		...
		for i := range tb.transaction.TurtleDexcoinInputs {
			coveredFields.TurtleDexcoinInputs = append(coveredFields.TurtleDexcoinInputs, uint64(i))
		}
		for i := range tb.transaction.TurtleDexcoinOutputs {
			coveredFields.TurtleDexcoinOutputs = append(coveredFields.TurtleDexcoinOutputs, uint64(i))
		}
		...
		for i := range tb.transaction.TurtleDexfundInputs {
			coveredFields.TurtleDexfundInputs = append(coveredFields.TurtleDexfundInputs, uint64(i))
		}
		for i := range tb.transaction.TurtleDexfundOutputs {
			coveredFields.TurtleDexfundOutputs = append(coveredFields.TurtleDexfundOutputs, uint64(i))
		}
	...
	// For each ttdc input in the transaction that we added, provide a
	// signature.
	tb.wallet.mu.RLock()
	defer tb.wallet.mu.RUnlock()
	for _, inputIndex := range tb.ttdcInputs {
		input := tb.transaction.TurtleDexcoinInputs[inputIndex]
	...
	for _, inputIndex := range tb.siafundInputs {
		input := tb.transaction.TurtleDexfundInputs[inputIndex]
		key, ok := tb.wallet.keys[input.UnlockConditions.UnlockHash()]
		if !ok {
			return nil, errors.New("transaction builder added an input that it cannot sign")
		}
...
// ViewAdded returns all of the ttdc inputs, siafund inputs, and parent
// transactions that have been automatically added by the builder.
func (tb *transactionBuilder) ViewAdded() (newParents, ttdcInputs, siafundInputs, transactionSignatures []int) {
	return tb.newParents, tb.ttdcInputs, tb.siafundInputs, tb.transactionSignatures
}
