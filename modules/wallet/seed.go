...
func generateSpendableKey(seed modules.Seed, index uint64) spendableKey {
	...
	return spendableKey{
		UnlockConditions: types.UnlockConditions{
			PublicKeys:         []types.TurtleDexPublicKey{types.Ed25519PublicKey(pk)},
...
// NextAddresses returns n unlock hashes that are ready to receive ttdcs or
...
// NextAddress returns an unlock hash that is ready to receive ttdcs or
...
func (w *Wallet) SweepSeed(seed modules.Seed) (coins, funds types.Currency, err error) {
	...
	if len(s.ttdcOutputs) == 0 && len(s.siafundOutputs) == 0 {
	...
	var ttdcOutputs, siafundOutputs []scannedOutput
	for _, sco := range s.ttdcOutputs {
		ttdcOutputs = append(ttdcOutputs, sco)
	}
	for _, sfo := range s.siafundOutputs {
		siafundOutputs = append(siafundOutputs, sfo)
	}

	for len(ttdcOutputs) > 0 || len(siafundOutputs) > 0 {
		// process up to maxOutputs ttdcOutputs
		txnTurtleDexcoinOutputs := make([]scannedOutput, maxOutputs)
		n := copy(txnTurtleDexcoinOutputs, ttdcOutputs)
		txnTurtleDexcoinOutputs = txnTurtleDexcoinOutputs[:n]
		ttdcOutputs = ttdcOutputs[n:]

		// process up to (maxOutputs-n) siafundOutputs
		txnTurtleDexfundOutputs := make([]scannedOutput, maxOutputs-n)
		n = copy(txnTurtleDexfundOutputs, siafundOutputs)
		txnTurtleDexfundOutputs = txnTurtleDexfundOutputs[:n]
		...
		for _, output := range txnTurtleDexcoinOutputs {
			// construct a ttdc input that spends the output
			sk := generateSpendableKey(seed, output.seedIndex)
			tb.AddTurtleDexcoinInput(types.TurtleDexcoinInput{
				ParentID:         types.TurtleDexcoinOutputID(output.id),
				UnlockConditions: sk.UnlockConditions,
			})
			// add a signature for the input
			sweptCoins = sweptCoins.Add(output.value)
		}
		for _, output := range txnTurtleDexfundOutputs {
			// construct a siafund input that spends the output
			sk := generateSpendableKey(seed, output.seedIndex)
			tb.AddTurtleDexfundInput(types.TurtleDexfundInput{
				ParentID:         types.TurtleDexfundOutputID(output.id),
				UnlockConditions: sk.UnlockConditions,
			})
			// add a signature for the input
		...
		estTxnSize := (len(txnTurtleDexcoinOutputs) + len(txnTurtleDexfundOutputs)) * outputSize
		estFee := maxFee.Mul64(uint64(estTxnSize))
		tb.AddMinerFee(estFee)

		// calculate total ttdc payout
		...
		case !txnCoins.IsZero() && txnFunds.IsZero():
			// if we're sweeping coins but not funds, add a ttdc output for
			// them
			tb.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{
				Value:      txnCoins,
		...
		case txnCoins.IsZero() && !txnFunds.IsZero():
			...
			tb.AddTurtleDexfundOutput(types.TurtleDexfundOutput{
				Value:      txnFunds,
				UnlockHash: uc.UnlockHash(),
			})
			err = tb.FundTurtleDexcoins(estFee)
			if err != nil {
				return types.Currency{}, types.Currency{}, errors.New("couldn't pay transaction fee on swept funds: " + err.Error())
			}

		case !txnCoins.IsZero() && !txnFunds.IsZero():
			// if we're sweeping both coins and funds, add a ttdc output and a
			// siafund output
			tb.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{
				Value:      txnCoins,
				UnlockHash: uc.UnlockHash(),
		...
		for _, sfo := range txnTurtleDexfundOutputs {
		...
		// this case, we iterate through the TurtleDexcoinInputs and add a signature for
		// any input that belongs to the wallet.
		w.mu.RLock()
		for _, input := range txn.TurtleDexcoinInputs {
