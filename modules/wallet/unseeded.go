...
const (
	// TurtleDexgFileExtension is the file extension to be used for siag files
	TurtleDexgFileExtension = ".siakey"

	// TurtleDexgFileHeader is the header for all siag files. Do not change. Because siag was created
	...
	TurtleDexgFileHeader = "siag"

	// TurtleDexgFileVersion is the version number to be used for siag files
	TurtleDexgFileVersion = "1.0"
)
...
// keys in versions v0.3.3.x for ttdxd.
...
// loadTurtleDexgKeys loads a set of siag keyfiles into the wallet, so that the
// wallet may spend the siafunds.
func (w *Wallet) loadTurtleDexgKeys(masterKey crypto.CipherKey, keyfiles []string) error {
	...
	for i, keyfile := range keyfiles {
		...
		if skps[i].Header != TurtleDexgFileHeader {
			return ErrUnknownHeader
		}
		if skps[i].Version != TurtleDexgFileVersion {
...
// LoadTurtleDexgKeys loads a set of siag-generated keys into the wallet.
func (w *Wallet) LoadTurtleDexgKeys(masterKey crypto.CipherKey, keyfiles []string) error {
	...
	err := func() error {
		w.mu.Lock()
		defer w.mu.Unlock()
		err := w.loadTurtleDexgKeys(masterKey, keyfiles)
		if err != nil {
			return err
		}
