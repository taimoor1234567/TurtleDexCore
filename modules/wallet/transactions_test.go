...
func TestIntegrationTransactions(t *testing.T) {
	...
	_, err = wt.wallet.SendTurtleDexcoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationTransaction(t *testing.T) {
	...
	// test sending ttdcs
	sentValue := types.NewCurrency64(5000)
	sendTxns, err := wt.wallet.SendTurtleDexcoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	...
	sendTxns, err = wt.wallet.SendTurtleDexfunds(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	_, err = wt.miner.AddBlock()
...
func TestIntegrationAddressTransactions(t *testing.T) {
	...
	_, err = wt.wallet.SendTurtleDexcoins(types.NewCurrency64(5005), addr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionInputOutputIDs(t *testing.T) {
	...
	// verify the miner payouts and ttdc outputs/inputs have correct IDs
	...
	for _, txn := range txns {
		...
		for i, output := range txn.Outputs {
			...
			if output.FundType == types.SpecifierTurtleDexcoinOutput {
				if output.ID != types.OutputID(txn.Transaction.TurtleDexcoinOutputID(uint64(i))) {
					t.Fatal("ttdc output had incorrect output ID")
				}
			}
		}
