...
// TestSendTurtleDexcoins probes the SendTurtleDexcoins method of the wallet.
func TestSendTurtleDexcoins(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	...
	// Send ttdcs. The wallet will automatically add a fee. Outgoing
	// unconfirmed ttdcs - incoming unconfirmed ttdcs should equal amount
	// sent + fee.
	sendValue := types.TurtleDexcoinPrecision.Mul64(3)
	_, tpoolFee := wt.wallet.tpool.FeeEstimation()
	tpoolFee = tpoolFee.Mul64(750)
	_, err = wt.wallet.SendTurtleDexcoins(sendValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !unconfirmedOut2.Equals(unconfirmedIn2.Add(sendValue).Add(tpoolFee)) {
		t.Error("sending ttdcs appears to be ineffective")
	}
...
// TestSendTurtleDexcoinsFeeIncluded probes the SendTurtleDexcoins method of the wallet with
// feeIncluded=true.
func TestSendTurtleDexcoinsFeeIncluded(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	wt, err := createWalletTester(t.Name(), modules.ProdDependencies)
	...
	_, tpoolFee := wt.wallet.tpool.FeeEstimation()
	tpoolFee = tpoolFee.Mul64(750)
	_, err = wt.wallet.SendTurtleDexcoinsFeeIncluded(sendValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	confirmedBal2, _, _, err := wt.wallet.ConfirmedBalance()
...
// TestIntegrationSendOverUnder sends too many ttdcs, resulting in an error,
// followed by sending few enough ttdcs that the send should complete.
//
...
func TestIntegrationSendOverUnder(t *testing.T) {
	...
	// Spend too many ttdcs.
	tooManyCoins := types.TurtleDexcoinPrecision.Mul64(1e12)
	_, err = wt.wallet.SendTurtleDexcoins(tooManyCoins, types.UnlockHash{})
	if err == nil {
		t.Error("low balance err not returned after attempting to send too many coins:", err)
	}

	// Spend a reasonable amount of ttdcs.
	reasonableCoins := types.TurtleDexcoinPrecision.Mul64(100e3)
	_, err = wt.wallet.SendTurtleDexcoins(reasonableCoins, types.UnlockHash{})
	if err != nil {
...
func TestIntegrationSpendHalfHalf(t *testing.T) {
	...
	halfPlus := types.TurtleDexcoinPrecision.Mul64(200e3)
	_, err = wt.wallet.SendTurtleDexcoins(halfPlus, types.UnlockHash{})
	if err != nil {
		t.Error("unexpected error: ", err)
	}
	_, err = wt.wallet.SendTurtleDexcoins(halfPlus, types.UnlockHash{1})
	if err == nil {
		t.Error("wallet appears to be reusing outputs when building transactions: ", err)
	}
}

// TestIntegrationSpendUnconfirmed spends an unconfirmed ttdc output.
func TestIntegrationSpendUnconfirmed(t *testing.T) {
	...
	someMore := types.TurtleDexcoinPrecision.Mul64(75e3)
	_, err = wt.wallet.SendTurtleDexcoins(someMore, types.UnlockHash{1})
	if err != nil {
...
func TestIntegrationSortedOutputsSorting(t *testing.T) {
	...
	so := sortedOutputs{
		ids: []types.TurtleDexcoinOutputID{{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}},
	...
	expectedIDSorting := []types.TurtleDexcoinOutputID{{5}, {6}, {0}, {1}, {2}, {7}, {4}, {3}}
...
// TestSendTurtleDexcoinsFailed checks if SendTurtleDexcoins and SendTurtleDexcoinsMulti behave
// correctly when funcing the Transaction succeeded but accepting it didn't.
func TestSendTurtleDexcoinsAcceptTxnSetFailed(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	deps := &dependencySendTurtleDexcoinsInterrupted{}
	...
	// Try to send coins using SendTurtleDexcoinsMulti
	numOutputs := 10
	scos := make([]types.TurtleDexcoinOutput, numOutputs)
	for i := 0; i < numOutputs; i++ {
		uc, err := wt.wallet.NextAddress()
		if err != nil {
			t.Fatal(err)
		}
		scos[i].Value = types.TurtleDexcoinPrecision
		scos[i].UnlockHash = uc.UnlockHash()
	}
	deps.fail()
	_, err = wt.wallet.SendTurtleDexcoinsMulti(scos)
	if err == nil {
		t.Fatal("SendTurtleDexcoinsMulti should have failed but didn't")
	}

	// Send some coins using SendTurtleDexcoins
	uc, err := wt.wallet.NextAddress()
	if err != nil {
		t.Fatal(err)
	}
	deps.fail()
	_, err = wt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision, uc.UnlockHash())
	if err == nil {
		t.Fatal("SendTurtleDexcoins should have failed but didn't")
	}
	...
	_, err = wt.wallet.SendTurtleDexcoinsMulti(scos)
	if err != nil {
		t.Fatalf("SendTurtleDexcoinsMulti failed: %v", err)
	}

	// Send some coins using SendTurtleDexcoins
	_, err = wt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision, uc.UnlockHash())
	if err != nil {
		t.Fatalf("SendTurtleDexcoins failed: %v", err)
	}
}
