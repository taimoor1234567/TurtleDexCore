...
//		There are plans to add newer, faster signature algorithms to TurtleDex as the
...
//		The arbitrary data field can be used to orchestrate soft-forks to TurtleDex
...
func isStandardTransaction(t types.Transaction) (uint64, error) {
	...
	// prefixes. The allowed prefixes include a 'NonTurtleDex' prefix for truly
	...
	for _, arb := range t.ArbitraryData {
		...
		if prefix == modules.PrefixHostAnnouncement ||
			prefix == modules.PrefixNonTurtleDex ||
