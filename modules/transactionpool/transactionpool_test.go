...
func TestGetTransaction(t *testing.T) {
	...
	err = txnBuilder.FundTurtleDexcoins(value)
	if err != nil {
		t.Fatal(err)
	}
	...
	childrenSet := []types.Transaction{{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID: txnSet[len(txnSet)-1].TurtleDexcoinOutputID(0),
		}},
...
func TestFeeEstimation(t *testing.T) {
	...
	for i := 0; i < numGraphs; i++ {
		...
		for j := 0; j < graphLens; j++ {
			fee := types.TurtleDexcoinPrecision.Mul64(uint64(j + i + 1)).Div64(200)
...
func TestTpoolScalability(t *testing.T) {
	...
	graphFund := types.TurtleDexcoinPrecision.Mul64(2000)
	var outputs []types.TurtleDexcoinOutput
	for i := 0; i < numGraphs+1; i++ {
		outputs = append(outputs, types.TurtleDexcoinOutput{
			UnlockHash: types.UnlockConditions{}.UnlockHash(),
	...
	for i := 0; i < numGraphs; i++ {
		...
		feeValues := types.TurtleDexcoinPrecision
		...
		for j := 0; j < rows; j++ {
			...
			// balance.
			rowValue := firstRowValues.Sub(types.TurtleDexcoinPrecision.Mul64(uint64(j + 1)))
...
func TestHeapFees(t *testing.T) {
	...
	coinFrac := types.TurtleDexcoinPrecision
	numGraphs := 110
	...
	// Add up total fees
	numTxns1 := 0
	maxFee1 := types.TurtleDexcoinPrecision.Div64(1000000)
	minFee1 := types.TurtleDexcoinPrecision.Mul64(1000000)
	...
	// Add up total fees
	numTxns2 := 0
	maxFee2 := types.TurtleDexcoinPrecision.Div64(1000000)
	minFee2 := types.TurtleDexcoinPrecision.Mul64(1000000)
	for _, txn := range block.Transactions {
		for _, fee := range txn.MinerFees {
...
func TestBigTpool(t *testing.T) {
	...
	// Create transaction graph setup.
	coinFrac := types.TurtleDexcoinPrecision.Div64(1)
	feeFrac := types.TurtleDexcoinPrecision.Div64(10)
	numGraphsPerChunk := 1000
	transactionSetSizes := []int{1, 2, 5, 10, 20}

	var outputs1 []types.TurtleDexcoinOutput
	var outputs2 []types.TurtleDexcoinOutput
	var outputs3 []types.TurtleDexcoinOutput
	...
	for i := 1; i <= numGraphsPerChunk; i++ {
		...
		outputs1 = append(outputs1, types.TurtleDexcoinOutput{
			UnlockHash: types.UnlockConditions{}.UnlockHash(),
			Value:      value,
		})
	}
	...
	for i := 0; i < numGraphsPerChunk/500; i++ {
		txns, err := tpt.wallet.SendTurtleDexcoinsMulti(outputs1[500*i : (500*i)+500])
		if err != nil {
			t.Error(err)
		}
	...
	for i := 1; i <= numGraphsPerChunk; i++ {
		...
		outputs2 = append(outputs2, types.TurtleDexcoinOutput{
			UnlockHash: types.UnlockConditions{}.UnlockHash(),
			Value:      value,
		})
	}
	...
	for i := 0; i < numGraphsPerChunk/500; i++ {
		txns, err := tpt.wallet.SendTurtleDexcoinsMulti(outputs2[500*i : (500*i)+500])
		if err != nil {
			t.Error(err)
		}
	...
	for i := 1; i <= numGraphsPerChunk; i++ {
		...
		outputs3 = append(outputs3, types.TurtleDexcoinOutput{
			UnlockHash: types.UnlockConditions{}.UnlockHash(),
			Value:      value,
		})
	}
	...
	for i := 0; i < numGraphsPerChunk/500; i++ {
		txns, err := tpt.wallet.SendTurtleDexcoinsMulti(outputs3[500*i : (500*i)+500])
		if err != nil {
			t.Error(err)
		}
	...
	var totalFee1 types.Currency
	minFee1 := types.TurtleDexcoinPrecision.Mul64(10000000000) // All the fees are much smaller than 1 SC.
	...
	minFee2 := types.TurtleDexcoinPrecision.Mul64(10000000000) // All the fees are much smaller than 1 SC.
	for _, tx := range block.Transactions {
		for _, fee := range tx.MinerFees {
			totalFee2 = totalFee2.Add(fee)
			if fee.Cmp(minFee2) < 0 {
	...
	minFee3 := types.TurtleDexcoinPrecision.Mul64(10000000000) // All the fees are much smaller than 1 SC.
	for _, tx := range block.Transactions {
		for _, fee := range tx.MinerFees {
...
func TestTpoolRevert(t *testing.T) {
	...
	totalExpectedFee := types.TurtleDexcoinPrecision.Mul64(13860).Add(types.TurtleDexcoinPrecision.Mul64(321915))
