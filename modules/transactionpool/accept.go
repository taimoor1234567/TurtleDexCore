...
func relatedObjectIDs(ts []types.Transaction) []ObjectID {
	...
	for _, t := range ts {
		...
		for i := range t.TurtleDexcoinOutputs {
			oidMap[ObjectID(t.TurtleDexcoinOutputID(uint64(i)))] = struct{}{}
		}
		...
		for i := range t.TurtleDexfundOutputs {
			oidMap[ObjectID(t.TurtleDexfundOutputID(uint64(i)))] = struct{}{}
		}
	}
...
func requiredFeesToExtendTpoolAtSize(size int) types.Currency {
	...
	return types.TurtleDexcoinPrecision.MulFloat(feeFactor).Div64(1000) // Divide by 1000 to get SC / kb
}
