...
func TestAcceptTransactionSet(t *testing.T) {
	...
	txns, err := tpt.wallet.SendTurtleDexcoins(types.NewCurrency64(100), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestConflictingTransactionSets(t *testing.T) {
	...
	err = txnBuilder.FundTurtleDexcoins(fund)
	if err != nil {
		t.Fatal(err)
	}
	...
	// other create a ttdc output.
	txnIndex := len(txnSet) - 1
	txnSet[txnIndex].MinerFees = append(txnSet[txnIndex].MinerFees, fund)
	txnSetDoubleSpend[txnIndex].TurtleDexcoinOutputs = append(txnSetDoubleSpend[txnIndex].TurtleDexcoinOutputs, types.TurtleDexcoinOutput{Value: fund})
...
func TestCheckMinerFees(t *testing.T) {
	...
	graphFund := types.TurtleDexcoinPrecision.Mul64(1000)
	...
	txns, err := tpt.wallet.SendTurtleDexcoinsMulti(outputs)
	if err != nil {
		t.Error(err)
	}
	...
	for i := 0; i < TransactionPoolSizeForFee/10e3; i++ {
		...
		copy(arbData, modules.PrefixNonTurtleDex[:])
	...
	_, err = tpt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(50), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < numGraphs; i++ {
		...
		for j := 0; j < graphLens; j++ {
			edges = append(edges, types.TransactionGraphEdge{
				Dest:   j + 1,
				Fee:    types.TurtleDexcoinPrecision,
				Source: j,
				Value:  graphFund.Sub(types.TurtleDexcoinPrecision.Mul64(uint64(j + 1))),
			})
		}
		graph, err := types.TransactionGraph(finalTxn.TurtleDexcoinOutputID(uint64(i)), edges)
		if err != nil {
			t.Fatal(err)
		}
	...
	source := finalTxn.TurtleDexcoinOutputID(uint64(numGraphs))
	lowFee := types.TurtleDexcoinPrecision.Div64(3)
	remaining := types.TurtleDexcoinPrecision.Mul64(1000).Sub(lowFee)
...
func TestTransactionGraph(t *testing.T) {
	...
	txns, err := tpt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(100), types.UnlockConditions{}.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}

	// Get the output of that transaction.
	graphSourceOutputID := txns[len(txns)-1].TurtleDexcoinOutputID(0)
	edge := types.TransactionGraphEdge{
		Dest:   1,
		Fee:    types.TurtleDexcoinPrecision.Mul64(10),
		Source: 0,
		Value:  types.TurtleDexcoinPrecision.Mul64(90),
	}
...
func TestTransactionGraphDiamond(t *testing.T) {
	...
	for i := range sources {
		edges = append(edges, types.TransactionGraphEdge{
			Dest:   dests[i],
			Fee:    types.TurtleDexcoinPrecision.Mul64(fees[i]),
			Source: sources[i],
			Value:  types.TurtleDexcoinPrecision.Mul64(values[i]),
		})
	}
...
func TestAcceptFCAndConflictingRevision(t *testing.T) {
	...
	err = builder.FundTurtleDexcoins(payout)
	if err != nil {
		t.Fatal(err)
	}
	builder.AddFileContract(types.FileContract{
		...
		Payout:             payout,
		ValidProofOutputs:  []types.TurtleDexcoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		MissedProofOutputs: []types.TurtleDexcoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		UnlockHash:         types.UnlockConditions{}.UnlockHash(),
	})
	...
	rSet := []types.Transaction{{
		FileContractRevisions: []types.FileContractRevision{{
			...
			NewValidProofOutputs:  []types.TurtleDexcoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
			NewMissedProofOutputs: []types.TurtleDexcoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		}},
	}}
...
func TestPartialConfirmationWeave(t *testing.T) {
	...
	err = builder1.FundTurtleDexcoins(funding1)
	if err != nil {
		t.Fatal(err)
	}
	scOutput1 := types.TurtleDexcoinOutput{
		Value:      funding1,
		UnlockHash: emptyUH,
	}
	i1 := builder1.AddTurtleDexcoinOutput(scOutput1)
	...
	err = builder2.FundTurtleDexcoins(funding2)
	if err != nil {
		t.Fatal(err)
	}
	scOutput2 := types.TurtleDexcoinOutput{
		Value:      funding2,
		UnlockHash: emptyUH,
	}
	i2 := builder2.AddTurtleDexcoinOutput(scOutput2)
	...
	txn1 := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID: tSet1[len(tSet1)-1].TurtleDexcoinOutputID(i1),
		}},
	...
	txn2 := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID: tSet2[len(tSet2)-1].TurtleDexcoinOutputID(i2),
		}},
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{{
			Value:      funding2,
			UnlockHash: emptyUH,
	...
	child := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{
			{
				ParentID: txn1.TurtleDexcoinOutputID(0),
			},
			{
				ParentID: txn2.TurtleDexcoinOutputID(0),
			},
		},
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{{
