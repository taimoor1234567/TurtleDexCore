...
func findSets(ts []types.Transaction) [][]types.Transaction {
	...
	for i, t := range ts {
		...
		for _, obj := range t.TurtleDexcoinInputs {
		...
		for _, obj := range t.TurtleDexfundInputs {
			txid, exists := objMap[ObjectID(obj.ParentID)]
			if exists {
				parentSet := forward(txMap[txid])
				parentSets[parentSet] = struct{}{}
		...
		// transactions.
		for j := range t.TurtleDexcoinOutputs {
			scoid := t.TurtleDexcoinOutputID(uint64(j))
		...
		for j := range t.TurtleDexfundOutputs {
			sfoid := t.TurtleDexfundOutputID(uint64(j))
