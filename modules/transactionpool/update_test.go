...
func TestFindSets(t *testing.T) {
	...
	graph1, err := types.TransactionGraph(types.TurtleDexcoinOutputID{}, edges)
	if err != nil {
		t.Fatal(err)
	}
	...
	graph2, err := types.TransactionGraph(types.TurtleDexcoinOutputID{1}, edges)
	if err != nil {
		t.Fatal(err)
	}
	...
	graph3, err := types.TransactionGraph(types.TurtleDexcoinOutputID{2}, edges)
...
func TestArbDataOnly(t *testing.T) {
	...
	txn := types.Transaction{
		ArbitraryData: [][]byte{
			append(modules.PrefixNonTurtleDex[:], []byte("arb-data")...),
		},
	}
...
func TestValidRevertedTransaction(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		txns, err := tpt.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(1000), types.UnlockHash{})
		if err != nil {
			t.Fatal(err)
		}
