...
type (
	// ObjectID is the ID of an object such as ttdc output and file
...
func (tp *TransactionPool) Transaction(id types.TransactionID) (types.Transaction, []types.Transaction, bool) {
	...
	addOutputIDs := func(txn types.Transaction) {
		...
		for _, input := range txn.TurtleDexfundInputs {
			parentIDs[types.OutputID(input.ParentID)] = struct{}{}
		}
	...
	isParent := func(t types.Transaction) bool {
		for i := range t.TurtleDexcoinOutputs {
			if _, exists := parentIDs[types.OutputID(t.TurtleDexcoinOutputID(uint64(i)))]; exists {
				return true
			}
		}
