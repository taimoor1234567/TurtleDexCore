...
// identifiers and that CombinedTurtleDexFilePath also produces unique siapaths using
// the identifiers.
func testUniqueIdentifier(t *testing.T) {
	...
	sp1 := CombinedTurtleDexFilePath(ec1)
	sp2 := CombinedTurtleDexFilePath(ec2)
	sp3 := CombinedTurtleDexFilePath(ec3)
	sp4 := CombinedTurtleDexFilePath(ec4)
	sp5 := CombinedTurtleDexFilePath(ec5)
	sp6 := CombinedTurtleDexFilePath(ec6)
	if !sp1.Equals(sp2) {
