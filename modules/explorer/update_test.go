...
// TestIntegrationExplorerFileContractMetrics checks that the ttdc
...
func TestIntegrationExplorerFileContractMetrics(t *testing.T) {
	...
	builder.FundTurtleDexcoins(types.NewCurrency64(5e9))
	fcOutputs := []types.TurtleDexcoinOutput{{Value: types.NewCurrency64(4805e6)}}
	...
	builder.FundTurtleDexcoins(types.NewCurrency64(1e9))
	fcOutputs = []types.TurtleDexcoinOutput{{Value: types.NewCurrency64(961e6)}}
	fc = types.FileContract{
		FileSize:           15e3,
		WindowStart:        et.cs.Height() + 2,
		WindowEnd:          et.cs.Height() + 3,
