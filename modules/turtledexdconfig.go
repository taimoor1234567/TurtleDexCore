...
type (
	// TurtleDexdConfig is a helper type to manage the global ttdxd config.
	TurtleDexdConfig struct {
...
var (
	...
	// throughout ttdxd. It is set using the gateway module.
	...
	ConfigName = "ttdxd.config"
)

// SetRatelimit sets the ratelimit related fields in the config and persists it
// to disk.
func (cfg *TurtleDexdConfig) SetRatelimit(readBPS, writeBPS int64) error {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	// Input validation.
...
func (cfg *TurtleDexdConfig) save() error {
	return persist.SaveJSON(configMetadata, cfg, cfg.path)
}

// load loads the config from disk.
func (cfg *TurtleDexdConfig) load(path string) error {
...
func (cfg *TurtleDexdConfig) writeBPSCompat() {
...
// yet.
func NewConfig(path string) (*TurtleDexdConfig, error) {
	var cfg TurtleDexdConfig
	cfg.path = path
