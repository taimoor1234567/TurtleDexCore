...
import (
	...
	mnemonics "github.com/turtledex/entropy-mnemonics"
...
type (
	...
	// 'TurtleDexcoinInput', 'TurtleDexfundInput'.
	...
	// A ProcessedOutput is a ttdc output that appears in a transaction.
	// Some outputs mature immediately, some are delayed, and some may never
	// mature at all (in the event of storage proofs).
	//
	// Fund type can either be 'TurtleDexcoinOutput', 'TurtleDexfundOutput', 'ClaimOutput',
	...
	// available. TurtleDexcoinInputs and TurtleDexfundInputs become available immediately.
	...
	// A UnspentOutput is a TurtleDexcoinOutput or TurtleDexfundOutput that the wallet
	// is tracking.
	...
	// 'Sign', which will sign all inputs added via the 'FundTurtleDexcoins' or
	// 'FundTurtleDexfunds' call. All modifications are additive.
	//
	...
	TransactionBuilder interface {
		...
		// with the correct value. The ttdc input will not be signed until
		...
		// transaction failed.
		FundTurtleDexcoins(amount types.Currency) error
		...
		// 'Sign' is called on the transaction builder. Any ttdcs that are
		...
		// failed.
		FundTurtleDexfunds(amount types.Currency) error
		...
		// AddTurtleDexcoinInput adds a ttdc input to the transaction, returning
		// the index of the ttdc input within the transaction. When 'Sign'
		// gets called, this input will be left unsigned.
		AddTurtleDexcoinInput(types.TurtleDexcoinInput) uint64

		// AddTurtleDexcoinOutput adds a ttdc output to the transaction, returning
		// the index of the ttdc output within the transaction.
		...
		// given index.
		ReplaceTurtleDexcoinOutput(index uint64, output types.TurtleDexcoinOutput) error
		...
		// AddTurtleDexfundInput adds a siafund input to the transaction, returning
		// the index of the siafund input within the transaction. When 'Sign'
		// is called, this input will be left unsigned.
		AddTurtleDexfundInput(types.TurtleDexfundInput) uint64

		// AddTurtleDexfundOutput adds a siafund output to the transaction, returning
		// the index of the siafund output within the transaction.
		AddTurtleDexfundOutput(types.TurtleDexfundOutput) uint64
		...
		// sign any of the inputs that were added by calling 'FundTurtleDexcoins' or
		// 'FundTurtleDexfunds'.
		...
		// Sign will sign any inputs added by 'FundTurtleDexcoins' or 'FundTurtleDexfunds'
		...
		Sweep(output types.TurtleDexcoinOutput) (txn types.Transaction, parents []types.Transaction)
		...
		// ViewAdded returns all of the ttdc inputs, siafund inputs, and
		// parent transactions that have been automatically added by the
		// builder. Items are returned by index.
		ViewAdded() (newParents, ttdcInputs, siafundInputs, transactionSignatures []int)
	...
	KeyManager interface {
		...
		// as a primary seed.
		// LoadBackup(masterKey, backupMasterKey crypto.TurtleDexKey, string) error
		...
		// LoadTurtleDexgKeys will take a set of filepaths that point to a siag key
		// and will have the siag keys loaded into the wallet so that they will
		// become spendable.
		LoadTurtleDexgKeys(crypto.CipherKey, []string) error
	...
	// Wallet stores and manages ttdcs and siafunds. The wallet file is
	...
	Wallet interface {
		...
		// refund transactions.
		ConfirmedBalance() (ttdcBalance types.Currency, siafundBalance types.Currency, ttdcClaimBalance types.Currency, err error)
		...
		// someone could result in 'outgoing: 12, incoming: 11'. TurtleDexfunds are
		// not considered in the unconfirmed balance.
		UnconfirmedBalance() (outgoingTurtleDexcoins types.Currency, incomingTurtleDexcoins types.Currency, err error)
		...
		// SendTurtleDexcoins is a tool for sending ttdcs from the wallet to an
		...
		SendTurtleDexcoins(amount types.Currency, dest types.UnlockHash) ([]types.Transaction, error)

		// SendTurtleDexcoinsFeeIncluded sends ttdcs with fees included.
		SendTurtleDexcoinsFeeIncluded(amount types.Currency, dest types.UnlockHash) ([]types.Transaction, error)

		// SendTurtleDexcoinsMulti sends coins to multiple addresses.
		SendTurtleDexcoinsMulti(outputs []types.TurtleDexcoinOutput) ([]types.Transaction, error)

		// SendTurtleDexfunds is a tool for sending siafunds from the wallet to an
		// address. Sending money usually results in multiple transactions. The
		// transactions are automatically given to the transaction pool, and
		// are also returned to the caller.
		SendTurtleDexfunds(amount types.Currency, dest types.UnlockHash) ([]types.Transaction, error)
