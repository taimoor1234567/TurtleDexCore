...
// testSpendTurtleDexcoinsBlock mines a block with a transaction spending ttdcs
// and adds it to the consensus set.
func (cst *consensusSetTester) testSpendTurtleDexcoinsBlock() {
	// Create a random destination address for the output in the transaction.
	destAddr := randAddress()

	// Create a block containing a transaction with a valid ttdc output.
	txnValue := types.NewCurrency64(1200)
	txnBuilder, err := cst.wallet.StartTransaction()
	if err != nil {
		panic(err)
	}
	err = txnBuilder.FundTurtleDexcoins(txnValue)
	if err != nil {
		panic(err)
	}
	outputIndex := txnBuilder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{Value: txnValue, UnlockHash: destAddr})
	...
	outputID := txnSet[len(txnSet)-1].TurtleDexcoinOutputID(outputIndex)
	sco, err := cst.cs.dbGetTurtleDexcoinOutput(outputID)
	if err != nil {
		panic(err)
	}
...
// TestIntegrationSpendTurtleDexcoinsBlock creates a consensus set tester and uses it
// to call testSpendTurtleDexcoinsBlock.
func TestIntegrationSpendTurtleDexcoinsBlock(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
...
func (cst *consensusSetTester) testValidStorageProofBlocks() {
	...
	fc := types.FileContract{
		...
		MissedProofOutputs: []types.TurtleDexcoinOutput{{
	...
	oldTurtleDexfundPool := cst.cs.dbGetTurtleDexfundPool()
	txnBuilder, err := cst.wallet.StartTransaction()
	if err != nil {
		panic(err)
	}
	err = txnBuilder.FundTurtleDexcoins(payout)
	if err != nil {
		panic(err)
	}
	...
	siafundPool := cst.cs.dbGetTurtleDexfundPool()
	if !siafundPool.Equals(oldTurtleDexfundPool.Add(types.Tax(cst.cs.dbBlockHeight()-1, payout))) {
	...
	postProofPool := cst.cs.dbGetTurtleDexfundPool()
...
func (cst *consensusSetTester) testFileContractRevision() {
	...
	uc := types.UnlockConditions{
		PublicKeys: []types.TurtleDexPublicKey{{
...
// testSpendTurtleDexfunds spends siafunds on the blockchain.
func (cst *consensusSetTester) testSpendTurtleDexfunds() {
	// Create a random destination address for the output in the transaction.
	destAddr := randAddress()
	...
	err = txnBuilder.FundTurtleDexfunds(txnValue)
	if err != nil {
		panic(err)
	}
	outputIndex := txnBuilder.AddTurtleDexfundOutput(types.TurtleDexfundOutput{Value: txnValue, UnlockHash: destAddr})
	txnSet, err := txnBuilder.Sign(true)
	if err != nil {
		panic(err)
	}
	...
	var claimIDs []types.TurtleDexcoinOutputID
	for _, txn := range txnSet {
		for _, sfi := range txn.TurtleDexfundInputs {
			sfo, err := cst.cs.dbGetTurtleDexfundOutput(sfi.ParentID)
			if err != nil {
			...
			poolDiff := cst.cs.dbGetTurtleDexfundPool().Sub(sfo.ClaimStart)
			value := poolDiff.Div(types.TurtleDexfundCount).Mul(sfo.Value)
			claimValues = append(claimValues, value)
			claimIDs = append(claimIDs, sfi.ParentID.TurtleDexClaimOutputID())
		}
	}
	...
	// See that the destination output was created.
	outputID := txnSet[len(txnSet)-1].TurtleDexfundOutputID(outputIndex)
	sfo, err := cst.cs.dbGetTurtleDexfundOutput(outputID)
	if err != nil {
		panic(err)
	}
	...
	if !sfo.ClaimStart.Equals(cst.cs.dbGetTurtleDexfundPool()) {
	...
	for i, id := range claimIDs {
		...
		if !dsco.Value.Equals(claimValues[i]) {
			panic("expected a different claim value on the ttdxclaim")
		}
	}
}

// TestIntegrationSpendTurtleDexfunds creates a consensus set tester and uses it
// to call testSpendTurtleDexfunds.
func (cst *consensusSetTester) TestIntegrationSpendTurtleDexfunds(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
...
func (cst *consensusSetTester) testPaymentChannelBlocks() error {
	...
	err = fundTxnBuilder.FundTurtleDexcoins(channelSize)
	if err != nil {
		return err
	}
	scoFundIndex := fundTxnBuilder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{Value: channelSize, UnlockHash: channelFundingAddr})
	fundTxnSet, err := fundTxnBuilder.Sign(true)
	if err != nil {
		return err
	}
	fundOutputID := fundTxnSet[len(fundTxnSet)-1].TurtleDexcoinOutputID(int(scoFundIndex))
	...
	// channel output.
	channelOutputID := channelTxn.TurtleDexcoinOutputID(0)
	...
	closeRefundID := closeTxn.TurtleDexcoinOutputID(0)
	closePaymentID := closeTxn.TurtleDexcoinOutputID(1)
	exists := cst.cs.db.inTurtleDexcoinOutputs(closeRefundID)
	if !exists {
		return errors.New("close txn refund output doesn't exist")
	}
	exists = cst.cs.db.inTurtleDexcoinOutputs(closePaymentID)
	if !exists {
	...
	// the initial transaction.
	{
		...
		reclaimOutputID := reclaimTxn.TurtleDexcoinOutputID(0)
		exists := cst.cs.db.inTurtleDexcoinOutputs(reclaimOutputID)
		if !exists {
	...
	// the timelocked signature.
	{
		...
		refundOutputID := refundTxn.TurtleDexcoinOutputID(0)
		exists := cst.cs.db.inTurtleDexcoinOutputs(refundOutputID)
		if !exists {
