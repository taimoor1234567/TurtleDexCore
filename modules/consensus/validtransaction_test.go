...
func TestTryValidTransactionSet(t *testing.T) {
	...
	_, err = cst.wallet.SendTurtleDexcoins(types.NewCurrency64(1), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(cc.TurtleDexcoinOutputDiffs) == 0 {
	...
	for i := 0; i < 3; i++ {
		...
		for _, k := range segmentRange {
			...
			fc := types.FileContract{
				...
				ValidProofOutputs:  []types.TurtleDexcoinOutput{{Value: types.NewCurrency64(500)}},
				MissedProofOutputs: []types.TurtleDexcoinOutput{{Value: types.NewCurrency64(500)}},
			}
			...
			err = b.FundTurtleDexcoins(types.NewCurrency64(500))
			if err != nil {
				t.Fatal(err)
			}
...
// TestValidTurtleDexcoins probes the validTurtleDexcoins method of the consensus set.
func TestValidTurtleDexcoins(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Create a transaction pointing to a nonexistent ttdc output.
	txn := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{}},
	}
	err = cst.cs.db.View(func(tx *bolt.Tx) error {
		err := validTurtleDexcoins(tx, txn)
		if !errors.Contains(err, errMissingTurtleDexcoinOutput) {
			t.Fatal(err)
		}
		return nil
	})
	...
	scoid, _, err := cst.cs.getArbTurtleDexcoinOutput()
	if err != nil {
		t.Fatal(err)
	}
	txn = types.Transaction{
...
func TestValidFileContractRevisions(t *testing.T) {
	...
	txn.FileContractRevisions[0].NewValidProofOutputs = []types.TurtleDexcoinOutput{{
		Value: types.NewCurrency64(1),
	}}
	txn.FileContractRevisions[0].NewMissedProofOutputs = []types.TurtleDexcoinOutput{{
		Value: types.NewCurrency64(1),
	}}
	err = cst.cs.dbValidFileContractRevisions(txn)
...
/*
// TestValidTurtleDexfunds probes the validTurtleDexfunds mthod of the consensus set.
func TestValidTurtleDexfunds(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	cst, err := createConsensusSetTester(t.Name())
	...
	err = cst.cs.validTurtleDexfunds(txn)
	if !errors.Contains(err, ErrMissingTurtleDexfundOutput){
		t.Error(err)
	}

	// Create a transaction with invalid unlock conditions.
	var sfoid types.TurtleDexfundOutputID
	cst.cs.db.forEachTurtleDexfundOutputs(func(mapSfoid types.TurtleDexfundOutputID, sfo types.TurtleDexfundOutput) {
		sfoid = mapSfoid
	...
	err = cst.cs.validTurtleDexfunds(txn)
	if !errors.Contains(err, ErrTurtleDexfundInputOutputMismatch){
		t.Error(err)
	}
}
...
func TestValidTransaction(t *testing.T) {
	...
	// Create a transaction with invalid ttdcs.
	txn = types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{}},
	}
	err = cst.cs.validTransaction(txn)
...
func TestValidArbitraryData(t *testing.T) {
	...
	txn := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID:         types.TurtleDexcoinOutputID{1, 2, 3},
	...
	txn.TurtleDexcoinInputs[0].UnlockConditions = failsafeUC
	if err := validate(txn, types.FoundationHardforkHeight); err != nil {
		t.Error(err)
	}

	// Try with invalid unlock conditions
	txn.TurtleDexcoinInputs[0].UnlockConditions = types.UnlockConditions{}
