...
import (
	...
	"github.com/turtledex/demotemutex"
...
func consensusSetBlockingStartup(gateway modules.Gateway, persistDir string, deps modules.Dependencies) (*ConsensusSet, error) {
	...
	for _, transaction := range types.GenesisBlock.Transactions {
		// Create the diffs for the genesis ttdc outputs.
		for i, ttdcOutput := range transaction.TurtleDexcoinOutputs {
			scid := transaction.TurtleDexcoinOutputID(uint64(i))
			scod := modules.TurtleDexcoinOutputDiff{
				Direction:     modules.DiffApply,
				ID:            scid,
				TurtleDexcoinOutput: ttdcOutput,
			}
			cs.blockRoot.TurtleDexcoinOutputDiffs = append(cs.blockRoot.TurtleDexcoinOutputDiffs, scod)
		}
		// Create the diffs for the genesis siafund outputs.
		for i, siafundOutput := range transaction.TurtleDexfundOutputs {
			sfid := transaction.TurtleDexfundOutputID(uint64(i))
			sfod := modules.TurtleDexfundOutputDiff{
				Direction:     modules.DiffApply,
				ID:            sfid,
				TurtleDexfundOutput: siafundOutput,
			}
			cs.blockRoot.TurtleDexfundOutputDiffs = append(cs.blockRoot.TurtleDexfundOutputDiffs, sfod)
		}
	}
