...
func BenchmarkAcceptSmallBlocks(b *testing.B) {
	...
	b.StopTimer()
	for j := 0; j < b.N; j++ {
		// Create a transaction with a miner fee, a normal ttdc output, and
		// a funded file contract.
		txnBuilder, err := cst.wallet.StartTransaction()
		if err != nil {
			b.Fatal(err)
		}
		err = txnBuilder.FundTurtleDexcoins(types.NewCurrency64(125e6))
		if err != nil {
			b.Fatal(err)
		}
		...
		// Add a ttdc output.
		txnBuilder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{Value: types.NewCurrency64(20e6)})
		// Add a file contract.
