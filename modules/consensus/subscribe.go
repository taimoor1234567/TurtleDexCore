...
func computeConsensusChangeDiffs(pb *processedBlock, apply bool) modules.ConsensusChangeDiffs {
	if apply {
		return modules.ConsensusChangeDiffs{
			TurtleDexcoinOutputDiffs:        pb.TurtleDexcoinOutputDiffs,
			FileContractDiffs:         pb.FileContractDiffs,
			TurtleDexfundOutputDiffs:        pb.TurtleDexfundOutputDiffs,
			DelayedTurtleDexcoinOutputDiffs: pb.DelayedTurtleDexcoinOutputDiffs,
			TurtleDexfundPoolDiffs:          pb.TurtleDexfundPoolDiffs,
		}
	}
	...
	cd := modules.ConsensusChangeDiffs{
		TurtleDexcoinOutputDiffs:        make([]modules.TurtleDexcoinOutputDiff, len(pb.TurtleDexcoinOutputDiffs)),
		FileContractDiffs:         make([]modules.FileContractDiff, len(pb.FileContractDiffs)),
		TurtleDexfundOutputDiffs:        make([]modules.TurtleDexfundOutputDiff, len(pb.TurtleDexfundOutputDiffs)),
		DelayedTurtleDexcoinOutputDiffs: make([]modules.DelayedTurtleDexcoinOutputDiff, len(pb.DelayedTurtleDexcoinOutputDiffs)),
		TurtleDexfundPoolDiffs:          make([]modules.TurtleDexfundPoolDiff, len(pb.TurtleDexfundPoolDiffs)),
	}
	for i, d := range pb.TurtleDexcoinOutputDiffs {
		d.Direction = !d.Direction
		cd.TurtleDexcoinOutputDiffs[len(cd.TurtleDexcoinOutputDiffs)-i-1] = d
	}
	for i, d := range pb.FileContractDiffs {
		d.Direction = !d.Direction
		cd.FileContractDiffs[len(cd.FileContractDiffs)-i-1] = d
	}
	for i, d := range pb.TurtleDexfundOutputDiffs {
		d.Direction = !d.Direction
		cd.TurtleDexfundOutputDiffs[len(cd.TurtleDexfundOutputDiffs)-i-1] = d
	}
	for i, d := range pb.DelayedTurtleDexcoinOutputDiffs {
		d.Direction = !d.Direction
		cd.DelayedTurtleDexcoinOutputDiffs[len(cd.DelayedTurtleDexcoinOutputDiffs)-i-1] = d
	}
	for i, d := range pb.TurtleDexfundPoolDiffs {
		d.Direction = !d.Direction
		cd.TurtleDexfundPoolDiffs[len(cd.TurtleDexfundPoolDiffs)-i-1] = d
	}
	return cd
}
