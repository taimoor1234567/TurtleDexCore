...
func consensusChecksum(tx *bolt.Tx) crypto.Hash {
	...
	consensusSetBuckets := []*bolt.Bucket{
		tx.Bucket(BlockPath),
		tx.Bucket(TurtleDexcoinOutputs),
		tx.Bucket(FileContracts),
		tx.Bucket(TurtleDexfundOutputs),
		tx.Bucket(TurtleDexfundPool),
	...
	err := tx.ForEach(func(name []byte, b *bolt.Bucket) error {
		// If the bucket is not a delayed ttdc output bucket or a file
...
// checkTurtleDexcoinCount checks that the number of ttdcs countable within the
// consensus set equal the expected number of ttdcs for the block height.
func checkTurtleDexcoinCount(tx *bolt.Tx) {
	// Iterate through all the buckets looking for the delayed ttdc output
	// buckets, and check that they are for the correct heights.
	var dscoTurtleDexcoins types.Currency
	err := tx.ForEach(func(name []byte, b *bolt.Bucket) error {
		// Check if the bucket is a delayed ttdc output bucket.
		...
		err := b.ForEach(func(_, delayedOutput []byte) error {
			...
			dscoTurtleDexcoins = dscoTurtleDexcoins.Add(sco.Value)
			return nil
		})
		if err != nil {
			return err
	...
	// Add all of the ttdc outputs.
	var scoTurtleDexcoins types.Currency
	err = tx.Bucket(TurtleDexcoinOutputs).ForEach(func(_, scoBytes []byte) error {
		var sco types.TurtleDexcoinOutput
		err := encoding.Unmarshal(scoBytes, &sco)
		if err != nil {
			manageErr(tx, err)
		}
		scoTurtleDexcoins = scoTurtleDexcoins.Add(sco.Value)
		return nil
	})
	if err != nil {
		manageErr(tx, err)
	...
	var fcTurtleDexcoins types.Currency
	err = tx.Bucket(FileContracts).ForEach(func(_, fcBytes []byte) error {
		...
		fcTurtleDexcoins = fcTurtleDexcoins.Add(fcCoins)
		return nil
	})
	if err != nil {
		manageErr(tx, err)
	...
	var claimTurtleDexcoins types.Currency
	err = tx.Bucket(TurtleDexfundOutputs).ForEach(func(_, sfoBytes []byte) error {
		...
		coinsPerFund := getTurtleDexfundPool(tx).Sub(sfo.ClaimStart)
		claimCoins := coinsPerFund.Mul(sfo.Value).Div(types.TurtleDexfundCount)
		claimTurtleDexcoins = claimTurtleDexcoins.Add(claimCoins)
		return nil
	})
	if err != nil {
		manageErr(tx, err)
	...
	expectedTurtleDexcoins := types.CalculateNumTurtleDexcoins(blockHeight(tx))
	totalTurtleDexcoins := dscoTurtleDexcoins.Add(scoTurtleDexcoins).Add(fcTurtleDexcoins).Add(claimTurtleDexcoins)
	if !totalTurtleDexcoins.Equals(expectedTurtleDexcoins) {
		diagnostics := fmt.Sprintf("Wrong number of ttdcs\nDsco: %v\nSco: %v\nFc: %v\nClaim: %v\n", dscoTurtleDexcoins, scoTurtleDexcoins, fcTurtleDexcoins, claimTurtleDexcoins)
		if totalTurtleDexcoins.Cmp(expectedTurtleDexcoins) < 0 {
			diagnostics += fmt.Sprintf("total: %v\nexpected: %v\n expected is bigger: %v", totalTurtleDexcoins, expectedTurtleDexcoins, expectedTurtleDexcoins.Sub(totalTurtleDexcoins))
		} else {
			diagnostics += fmt.Sprintf("total: %v\nexpected: %v\n expected is bigger: %v", totalTurtleDexcoins, expectedTurtleDexcoins, totalTurtleDexcoins.Sub(expectedTurtleDexcoins))
		}
		manageErr(tx, errors.New(diagnostics))
	}
}

// checkTurtleDexfundCount checks that the number of siafunds countable within the
// consensus set equal the expected number of siafunds for the block height.
func checkTurtleDexfundCount(tx *bolt.Tx) {
	var total types.Currency
	err := tx.Bucket(TurtleDexfundOutputs).ForEach(func(_, siafundOutputBytes []byte) error {
		var sfo types.TurtleDexfundOutput
	...
	if !total.Equals(types.TurtleDexfundCount) {
		manageErr(tx, errors.New("wrong number of siafunds in the consensus set"))
	}
}

// checkDSCOs scans the sets of delayed ttdc outputs and checks for
// consistency.
func checkDSCOs(tx *bolt.Tx) {
	// Create a map to track which delayed ttdc output maps exist, and
	// another map to track which ids have appeared in the dsco set.
	dscoTracker := make(map[types.BlockHeight]struct{})
	idMap := make(map[types.TurtleDexcoinOutputID]struct{})
	...
	err := tx.ForEach(func(name []byte, b *bolt.Bucket) error {
		...
		err = b.ForEach(func(idBytes, delayedOutput []byte) error {
			...
			_, exists := idMap[id]
			if exists {
				return errors.New("repeat delayed ttdc output")
			}
			idMap[id] = struct{}{}
...
func (cs *ConsensusSet) checkConsistency(tx *bolt.Tx) {
	...
	checkDSCOs(tx)
	checkTurtleDexcoinCount(tx)
	checkTurtleDexfundCount(tx)
	if build.DEBUG {
