...
var (
	...
	// TurtleDexcoinOutputs is a database bucket that contains all of the unspent
	// ttdc outputs.
	TurtleDexcoinOutputs = []byte("TurtleDexcoinOutputs")

	// TurtleDexfundOutputs is a database bucket that contains all of the unspent
	// siafund outputs.
	TurtleDexfundOutputs = []byte("TurtleDexfundOutputs")

	// TurtleDexfundPool is a database bucket storing the current value of the
	// siafund pool.
	TurtleDexfundPool = []byte("TurtleDexfundPool")
...
func (cs *ConsensusSet) createConsensusDB(tx *bolt.Tx) error {
	...
	buckets := [][]byte{
		...
		BlockPath,
		Consistency,
		TurtleDexcoinOutputs,
		FileContracts,
		TurtleDexfundOutputs,
		TurtleDexfundPool,
	}
...
// isTurtleDexcoinOutput returns true if there is a ttdc output of that id in the
// database.
func isTurtleDexcoinOutput(tx *bolt.Tx, id types.TurtleDexcoinOutputID) bool {
	bucket := tx.Bucket(TurtleDexcoinOutputs)
	sco := bucket.Get(id[:])
	return sco != nil
}

// getTurtleDexcoinOutput fetches a ttdc output from the database. An error is
// returned if the ttdc output does not exist.
func getTurtleDexcoinOutput(tx *bolt.Tx, id types.TurtleDexcoinOutputID) (types.TurtleDexcoinOutput, error) {
	scoBytes := tx.Bucket(TurtleDexcoinOutputs).Get(id[:])
	if scoBytes == nil {
		return types.TurtleDexcoinOutput{}, errNilItem
	}
...
// addTurtleDexcoinOutput adds a ttdc output to the database. An error is returned
// if the ttdc output is already in the database.
func addTurtleDexcoinOutput(tx *bolt.Tx, id types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput) {
	...
	/*
		if build.DEBUG && sco.Value.IsZero() {
			panic("discovered a zero value ttdc output")
		}
	*/
	ttdcOutputs := tx.Bucket(TurtleDexcoinOutputs)
	// Sanity check - should not be adding an item that exists.
	if build.DEBUG && ttdcOutputs.Get(id[:]) != nil {
		panic("repeat ttdc output")
	}
	err := ttdcOutputs.Put(id[:], encoding.Marshal(sco))
	if build.DEBUG && err != nil {
		panic(err)
	}
}

// removeTurtleDexcoinOutput removes a ttdc output from the database. An error is
// returned if the ttdc output is not in the database prior to removal.
func removeTurtleDexcoinOutput(tx *bolt.Tx, id types.TurtleDexcoinOutputID) {
	scoBucket := tx.Bucket(TurtleDexcoinOutputs)
	// Sanity check - should not be removing an item that is not in the db.
	if build.DEBUG && scoBucket.Get(id[:]) == nil {
		panic("nil ttdc output")
	}
...
// getTurtleDexfundOutput fetches a siafund output from the database. An error is
// returned if the siafund output does not exist.
func getTurtleDexfundOutput(tx *bolt.Tx, id types.TurtleDexfundOutputID) (types.TurtleDexfundOutput, error) {
	sfoBytes := tx.Bucket(TurtleDexfundOutputs).Get(id[:])
	if sfoBytes == nil {
		return types.TurtleDexfundOutput{}, errNilItem
	}
	var sfo types.TurtleDexfundOutput
	err := encoding.Unmarshal(sfoBytes, &sfo)
	if err != nil {
		return types.TurtleDexfundOutput{}, err
	}
	gsa := types.GenesisTurtleDexfundAllocation
...
// addTurtleDexfundOutput adds a siafund output to the database. An error is returned
// if the siafund output is already in the database.
func addTurtleDexfundOutput(tx *bolt.Tx, id types.TurtleDexfundOutputID, sfo types.TurtleDexfundOutput) {
	siafundOutputs := tx.Bucket(TurtleDexfundOutputs)
...
// removeTurtleDexfundOutput removes a siafund output from the database. An error is
// returned if the siafund output is not in the database prior to removal.
func removeTurtleDexfundOutput(tx *bolt.Tx, id types.TurtleDexfundOutputID) {
	sfoBucket := tx.Bucket(TurtleDexfundOutputs)
...
// getTurtleDexfundPool returns the current value of the siafund pool. No error is
// returned as the siafund pool should always be available.
func getTurtleDexfundPool(tx *bolt.Tx) (pool types.Currency) {
	bucket := tx.Bucket(TurtleDexfundPool)
	poolBytes := bucket.Get(TurtleDexfundPool)
...
// setTurtleDexfundPool updates the saved siafund pool on disk
func setTurtleDexfundPool(tx *bolt.Tx, c types.Currency) {
	err := tx.Bucket(TurtleDexfundPool).Put(TurtleDexfundPool, encoding.Marshal(c))
	if build.DEBUG && err != nil {
		panic(err)
	}
}
...
// addDSCO adds a delayed ttdc output to the consnesus set.
func addDSCO(tx *bolt.Tx, bh types.BlockHeight, id types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput) {
	...
	*/
	...
	if build.DEBUG && tx.Bucket(TurtleDexcoinOutputs).Get(id[:]) != nil {
...
// removeDSCO removes a delayed ttdc output from the consensus set.
func removeDSCO(tx *bolt.Tx, bh types.BlockHeight, id types.TurtleDexcoinOutputID) {
...
// createDSCOBucket creates a bucket for the delayed ttdc outputs at the
// input height.
...
// deleteDSCOBucket deletes the bucket that held a set of delayed ttdc
// outputs.
