...
func TestIntegrationDoSBlockHandling(t *testing.T) {
	...
	// transaction. The transaction has more ttdc inputs than outputs.
	txnBuilder, err := cst.wallet.StartTransaction()
	if err != nil {
		t.Fatal(err)
	}
	err = txnBuilder.FundTurtleDexcoins(types.NewCurrency64(50))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !errors.Contains(err, errTurtleDexcoinInputOutputMismatch) {
		t.Fatalf("expected %v, got %v", errTurtleDexcoinInputOutputMismatch, err)
	}
...
func TestMinerPayoutHandling(t *testing.T) {
	...
	block.MinerPayouts = append(block.MinerPayouts, types.TurtleDexcoinOutput{Value: types.NewCurrency64(1)})
...
func TestBuriedBadTransaction(t *testing.T) {
	...
	txnBuilder.AddTurtleDexcoinOutput(types.TurtleDexcoinOutput{Value: txnValue})
	...
	block := types.Block{
		...
		MinerPayouts: []types.TurtleDexcoinOutput{{Value: types.CalculateCoinbase(pb.Height + 1)}},
		Transactions: txns,
	}
...
func TestInconsistentCheck(t *testing.T) {
	...
	sfo := types.TurtleDexfundOutput{
		Value: types.NewCurrency64(1),
	}
	cst.cs.dbAddTurtleDexfundOutput(types.TurtleDexfundOutputID{}, sfo)
...
func TestTaxHardfork(t *testing.T) {
	...
	fc := types.FileContract{
		...
		Payout:             payout,
		ValidProofOutputs:  []types.TurtleDexcoinOutput{{Value: outputSize}},
		MissedProofOutputs: []types.TurtleDexcoinOutput{{Value: outputSize}},
...
func TestChainedAcceptBlock(t *testing.T) {
	...
	badBlock.Transactions = append(badBlock.Transactions, types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{{
			ParentID: types.TurtleDexcoinOutputID{1},
		}},
	})
...
func TestFoundationUpdateBlocks(t *testing.T) {
	...
	foundationOutput := func(height types.BlockHeight) (id types.TurtleDexcoinOutputID, sco types.TurtleDexcoinOutput, exists bool) {
	...
	txn = types.Transaction{
		...
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{
			{Value: sco.Value.Sub(types.TurtleDexcoinPrecision.Mul64(2)), UnlockHash: types.UnlockHash{}},
			{Value: types.TurtleDexcoinPrecision, UnlockHash: types.InitialFoundationUnlockHash},
			{Value: types.TurtleDexcoinPrecision, UnlockHash: types.InitialFoundationFailsafeUnlockHash},
		},
	...
	primarySCOID := txn.TurtleDexcoinOutputID(1)
	failsafeSCOID := txn.TurtleDexcoinOutputID(2)
	...
	txn.TurtleDexcoinInputs[0] = types.TurtleDexcoinInput{
