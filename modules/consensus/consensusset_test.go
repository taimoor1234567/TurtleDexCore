...
// addTurtleDexfunds makes a transaction that moves some testing genesis siafunds
// into the wallet.
func (cst *consensusSetTester) addTurtleDexfunds() {
	...
	txn := types.Transaction{
		TurtleDexfundInputs: []types.TurtleDexfundInput{{
			ParentID:         cst.cs.blockRoot.Block.Transactions[0].TurtleDexfundOutputID(2),
...
// mineCoins mines blocks until there are ttdcs in the wallet.
func (cst *consensusSetTester) mineTurtleDexcoins() {
...
// including ttdcs and siafunds available in the wallet.
func createConsensusSetTester(name string) (*consensusSetTester, error) {
	...
	cst.addTurtleDexfunds()
	cst.mineTurtleDexcoins()
	return cst, nil
}
