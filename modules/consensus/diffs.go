...
var (
	errApplyTurtleDexfundPoolDiffMismatch  = errors.New("committing a siafund pool diff with an invalid 'previous' field")
	...
	errNonApplyTurtleDexfundPoolDiff       = errors.New("committing a siafund pool diff that doesn't have the 'apply' direction")
	errRevertTurtleDexfundPoolDiffMismatch = errors.New("committing a siafund pool diff with an invalid 'adjusted' field")
...
// commitTurtleDexcoinOutputDiff applies or reverts a TurtleDexcoinOutputDiff.
func commitTurtleDexcoinOutputDiff(tx *bolt.Tx, scod modules.TurtleDexcoinOutputDiff, dir modules.DiffDirection) {
	if scod.Direction == dir {
		addTurtleDexcoinOutput(tx, scod.ID, scod.TurtleDexcoinOutput)
	} else {
		removeTurtleDexcoinOutput(tx, scod.ID)
	}
}
...
// commitTurtleDexfundOutputDiff applies or reverts a TurtleDexfund output diff.
func commitTurtleDexfundOutputDiff(tx *bolt.Tx, sfod modules.TurtleDexfundOutputDiff, dir modules.DiffDirection) {
	if sfod.Direction == dir {
		addTurtleDexfundOutput(tx, sfod.ID, sfod.TurtleDexfundOutput)
	} else {
		removeTurtleDexfundOutput(tx, sfod.ID)
	}
}

// commitDelayedTurtleDexcoinOutputDiff applies or reverts a delayedTurtleDexcoinOutputDiff.
func commitDelayedTurtleDexcoinOutputDiff(tx *bolt.Tx, dscod modules.DelayedTurtleDexcoinOutputDiff, dir modules.DiffDirection) {
	if dscod.Direction == dir {
		addDSCO(tx, dscod.MaturityHeight, dscod.ID, dscod.TurtleDexcoinOutput)
	} else {
		removeDSCO(tx, dscod.MaturityHeight, dscod.ID)
	}
}

// commitTurtleDexfundPoolDiff applies or reverts a TurtleDexfundPoolDiff.
func commitTurtleDexfundPoolDiff(tx *bolt.Tx, sfpd modules.TurtleDexfundPoolDiff, dir modules.DiffDirection) {
	...
	if build.DEBUG {
		...
		if sfpd.Direction != modules.DiffApply {
			panic(errNonApplyTurtleDexfundPoolDiff)
		}
	}

	if dir == modules.DiffApply {
		// Sanity check - sfpd.Previous should equal the current siafund pool.
		if build.DEBUG && !getTurtleDexfundPool(tx).Equals(sfpd.Previous) {
			panic(errApplyTurtleDexfundPoolDiffMismatch)
		}
		setTurtleDexfundPool(tx, sfpd.Adjusted)
	} else {
		// Sanity check - sfpd.Adjusted should equal the current siafund pool.
		if build.DEBUG && !getTurtleDexfundPool(tx).Equals(sfpd.Adjusted) {
			panic(errRevertTurtleDexfundPoolDiffMismatch)
		}
		setTurtleDexfundPool(tx, sfpd.Previous)
	}
}

// createUpcomingDelayeOutputdMaps creates the delayed ttdc output maps that
// will be used when applying delayed ttdc outputs in the diff set.
...
func commitNodeDiffs(tx *bolt.Tx, pb *processedBlock, dir modules.DiffDirection) {
	if dir == modules.DiffApply {
		for _, scod := range pb.TurtleDexcoinOutputDiffs {
			commitTurtleDexcoinOutputDiff(tx, scod, dir)
		}
		for _, fcd := range pb.FileContractDiffs {
			commitFileContractDiff(tx, fcd, dir)
		}
		for _, sfod := range pb.TurtleDexfundOutputDiffs {
			commitTurtleDexfundOutputDiff(tx, sfod, dir)
		}
		for _, dscod := range pb.DelayedTurtleDexcoinOutputDiffs {
			commitDelayedTurtleDexcoinOutputDiff(tx, dscod, dir)
		}
		for _, sfpd := range pb.TurtleDexfundPoolDiffs {
			commitTurtleDexfundPoolDiff(tx, sfpd, dir)
		}
	} else {
		for i := len(pb.TurtleDexcoinOutputDiffs) - 1; i >= 0; i-- {
			commitTurtleDexcoinOutputDiff(tx, pb.TurtleDexcoinOutputDiffs[i], dir)
		}
		for i := len(pb.FileContractDiffs) - 1; i >= 0; i-- {
			commitFileContractDiff(tx, pb.FileContractDiffs[i], dir)
		}
		for i := len(pb.TurtleDexfundOutputDiffs) - 1; i >= 0; i-- {
			commitTurtleDexfundOutputDiff(tx, pb.TurtleDexfundOutputDiffs[i], dir)
		}
		for i := len(pb.DelayedTurtleDexcoinOutputDiffs) - 1; i >= 0; i-- {
			commitDelayedTurtleDexcoinOutputDiff(tx, pb.DelayedTurtleDexcoinOutputDiffs[i], dir)
		}
		for i := len(pb.TurtleDexfundPoolDiffs) - 1; i >= 0; i-- {
			commitTurtleDexfundPoolDiff(tx, pb.TurtleDexfundPoolDiffs[i], dir)
		}
	}
}

// deleteObsoleteDelayedOutputMaps deletes the delayed ttdc output maps that
...
func generateAndApplyDiff(tx *bolt.Tx, pb *processedBlock) error {
	...
	// Create the bucket to hold all of the delayed ttdc outputs created by
