...
var (
	...
	errMissingTurtleDexcoinOutput       = errors.New("transaction spends a nonexisting ttdc output")
	errTurtleDexcoinInputOutputMismatch = errors.New("ttdc inputs do not equal ttdc outputs for transaction")
	errTurtleDexfundInputOutputMismatch = errors.New("siafund inputs do not equal siafund outputs for transaction")
...
// validTurtleDexcoins checks that the ttdc inputs and outputs are valid in the
// context of the current consensus set.
func validTurtleDexcoins(tx *bolt.Tx, t types.Transaction) error {
	...
	for _, sci := range t.TurtleDexcoinInputs {
		...
		if scoBytes == nil {
			return errMissingTurtleDexcoinOutput
		}
	...
	if !inputSum.Equals(t.TurtleDexcoinOutputSum()) {
		return errTurtleDexcoinInputOutputMismatch
	}
	return nil
}
...
// validTurtleDexfunds checks that the siafund portions of the transaction are valid
// in the context of the consensus set.
func validTurtleDexfunds(tx *bolt.Tx, t types.Transaction) (err error) {
	...
	if !siafundOutputSum.Equals(siafundInputSum) {
		return errTurtleDexfundInputOutputMismatch
	}
	return
}
...
// covers a TurtleDexcoinInput controlled by the primary or failsafe UnlockHash. To
...
func (cs *ConsensusSet) tryTransactionSet(txns []types.Transaction) (modules.ConsensusChange, error) {
	...
	cc := modules.ConsensusChange{
		ConsensusChangeDiffs: modules.ConsensusChangeDiffs{
			TurtleDexcoinOutputDiffs:        diffHolder.TurtleDexcoinOutputDiffs,
			FileContractDiffs:         diffHolder.FileContractDiffs,
			TurtleDexfundOutputDiffs:        diffHolder.TurtleDexfundOutputDiffs,
			DelayedTurtleDexcoinOutputDiffs: diffHolder.DelayedTurtleDexcoinOutputDiffs,
			TurtleDexfundPoolDiffs:          diffHolder.TurtleDexfundPoolDiffs,
		},
	}
	return cc, nil
}
