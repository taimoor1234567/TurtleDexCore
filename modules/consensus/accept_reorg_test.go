...
// TestIntegrationTurtleDexcoinReorg tries to reorganize a ttdc output block out
// of, and then back into, the consensus set.
func TestIntegrationTurtleDexcoinReorg(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Give a ttdc block to cstMain.
	rs.cstMain.testSpendTurtleDexcoinsBlock()
...
func TestBuriedBadFork(t *testing.T) {
	...
	badBlock := types.Block{
		...
		MinerPayouts: []types.TurtleDexcoinOutput{{Value: types.CalculateCoinbase(pb.Height)}},
		Transactions: []types.Transaction{{
			TurtleDexcoinInputs: []types.TurtleDexcoinInput{{}}, // Will trigger an error on full verification but not partial verification.
		}},
	}
