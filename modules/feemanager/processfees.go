...
func (fm *FeeManager) createAndPersistTransaction(feeUIDs []modules.FeeUID, outputs []types.TurtleDexcoinOutput) (err error) {
	// Submit the outputs and get the transactions
	txns, err := fm.staticCommon.staticWallet.SendTurtleDexcoinsMulti(outputs)
	if err != nil {
		return errors.AddContext(err, "unable to build transaction for fee")
	}

	// From SendTurtleDexcoinsMulti, the transaction that was just created is the
