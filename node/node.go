// Package node provides tooling for creating a TurtleDex node. TurtleDex nodes consist of a
...
// including templates for assembling sane no-hassle TurtleDex nodes.
package node
...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/feemanager"
...
type NodeParams struct {
	...
	TurtleDexMuxTCPAddress string
	TurtleDexMuxWSAddress  string
...
// Node is a collection of TurtleDex modules operating together as a TurtleDex node.
type Node struct {
	// The mux of the node.
...
func New(params NodeParams, loadStartTime time.Time) (*Node, <-chan error) {
	...
	// Create the siamux.
	mux, err := modules.NewTurtleDexMux(filepath.Join(dir, modules.TurtleDexMuxDir), dir, params.TurtleDexMuxTCPAddress, params.TurtleDexMuxWSAddress)
	if err != nil {
	...
	i := 1
	printfRelease("(%d/%d) Loading ttdxd...\n", i, numModules)
	// Gateway.
