...
type (
	...
	WalletGET struct {
		...
		ConfirmedTurtleDexcoinBalance     types.Currency `json:"confirmedttdcbalance"`
		UnconfirmedOutgoingTurtleDexcoins types.Currency `json:"unconfirmedoutgoingttdcs"`
		UnconfirmedIncomingTurtleDexcoins types.Currency `json:"unconfirmedincomingttdcs"`

		TurtleDexcoinClaimBalance types.Currency `json:"ttdcclaimbalance"`
		TurtleDexfundBalance      types.Currency `json:"siafundbalance"`
	...
	// WalletTurtleDexcoinsPOST contains the transaction sent in the POST call to
	// /wallet/ttdcs.
	WalletTurtleDexcoinsPOST struct {
		Transactions   []types.Transaction   `json:"transactions"`
		TransactionIDs []types.TransactionID `json:"transactionids"`
	}

	// WalletTurtleDexfundsPOST contains the transaction sent in the POST call to
	// /wallet/siafunds.
	WalletTurtleDexfundsPOST struct {
		Transactions   []types.Transaction   `json:"transactions"`
		TransactionIDs []types.TransactionID `json:"transactionids"`
	}
...
func (api *API) walletHandler(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	ttdcBal, siafundBal, ttdxclaimBal, err := api.wallet.ConfirmedBalance()
	if err != nil {
		WriteError(w, Error{fmt.Sprintf("Error when calling /wallet: %v", err)}, http.StatusBadRequest)
		return
	}
	ttdcsOut, ttdcsIn, err := api.wallet.UnconfirmedBalance()
	if err != nil {
		WriteError(w, Error{fmt.Sprintf("Error when calling /wallet: %v", err)}, http.StatusBadRequest)
		return
	}
	...
	WriteJSON(w, WalletGET{
		...
		ConfirmedTurtleDexcoinBalance:     ttdcBal,
		UnconfirmedOutgoingTurtleDexcoins: ttdcsOut,
		UnconfirmedIncomingTurtleDexcoins: ttdcsIn,

		TurtleDexfundBalance:      siafundBal,
		TurtleDexcoinClaimBalance: ttdxclaimBal,
...
// walletTurtleDexgkeyHandler handles API calls to /wallet/siagkey.
func (api *API) walletTurtleDexgkeyHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	for _, key := range potentialKeys {
		err := api.wallet.LoadTurtleDexgKeys(key, keyfiles)
		if err == nil {
			WriteSuccess(w)
			return
		}
...
// walletTurtleDexcoinsHandler handles API calls to /wallet/ttdcs.
func (api *API) walletTurtleDexcoinsHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	if req.FormValue("outputs") != "" {
		...
		txns, err = api.wallet.SendTurtleDexcoinsMulti(outputs)
		if err != nil {
			WriteError(w, Error{"error when calling /wallet/ttdcs: " + err.Error()}, http.StatusInternalServerError)
			return
		}
	} else {
		...
		if !ok {
			WriteError(w, Error{"could not read amount from POST call to /wallet/ttdcs"}, http.StatusBadRequest)
			return
		}
		dest, err := scanAddress(req.FormValue("destination"))
		if err != nil {
			WriteError(w, Error{"could not read address from POST call to /wallet/ttdcs"}, http.StatusBadRequest)
			return
		}
		feeIncluded, err := scanBool(req.FormValue("feeIncluded"))
		if err != nil {
			WriteError(w, Error{"could not read feeIncluded from POST call to /wallet/ttdcs"}, http.StatusBadRequest)
			return
		}

		if feeIncluded {
			txns, err = api.wallet.SendTurtleDexcoinsFeeIncluded(amount, dest)
		} else {
			txns, err = api.wallet.SendTurtleDexcoins(amount, dest)
		}
		if err != nil {
			WriteError(w, Error{"error when calling /wallet/ttdcs: " + err.Error()}, http.StatusInternalServerError)
			return
	...
	WriteJSON(w, WalletTurtleDexcoinsPOST{
		Transactions:   txns,
		TransactionIDs: txids,
	})
}

// walletTurtleDexfundsHandler handles API calls to /wallet/siafunds.
func (api *API) walletTurtleDexfundsHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	amount, ok := scanAmount(req.FormValue("amount"))
	if !ok {
	...
	txns, err := api.wallet.SendTurtleDexfunds(amount, dest)
	if err != nil {
	...
	WriteJSON(w, WalletTurtleDexfundsPOST{
		Transactions:   txns,
		TransactionIDs: txids,
	})
}
