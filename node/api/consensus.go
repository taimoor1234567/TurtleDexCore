...
type ConsensusGET struct {
	...
	TurtleDexfundCount           types.Currency    `json:"siafundcount"`
	TurtleDexfundPortion         *big.Rat          `json:"siafundportion"`
	...
	TurtleDexcoinPrecision types.Currency `json:"ttdcprecision"`
}
...
type ConsensusBlocksGet struct {
	...
	MinerPayouts []types.TurtleDexcoinOutput   `json:"minerpayouts"`
...
type ConsensusBlocksGetTxn struct {
	...
	TurtleDexcoinInputs         []types.TurtleDexcoinInput              `json:"ttdcinputs"`
	TurtleDexcoinOutputs        []ConsensusBlocksGetTurtleDexcoinOutput `json:"ttdcoutputs"`
	...
	TurtleDexfundInputs         []types.TurtleDexfundInput              `json:"siafundinputs"`
	TurtleDexfundOutputs        []ConsensusBlocksGetTurtleDexfundOutput `json:"siafundoutputs"`
...
type ConsensusBlocksGetFileContract struct {
	...
	ValidProofOutputs  []ConsensusBlocksGetTurtleDexcoinOutput `json:"validproofoutputs"`
	MissedProofOutputs []ConsensusBlocksGetTurtleDexcoinOutput `json:"missedproofoutputs"`
	UnlockHash         types.UnlockHash                  `json:"unlockhash"`
	RevisionNumber     uint64                            `json:"revisionnumber"`
}

// ConsensusBlocksGetTurtleDexcoinOutput contains all fields of a types.TurtleDexcoinOutput
// and an additional ID field.
type ConsensusBlocksGetTurtleDexcoinOutput struct {
	ID         types.TurtleDexcoinOutputID `json:"id"`
	Value      types.Currency        `json:"value"`
	UnlockHash types.UnlockHash      `json:"unlockhash"`
}

// ConsensusBlocksGetTurtleDexfundOutput contains all fields of a types.TurtleDexfundOutput
// and an additional ID field.
type ConsensusBlocksGetTurtleDexfundOutput struct {
	ID         types.TurtleDexfundOutputID `json:"id"`
	Value      types.Currency        `json:"value"`
	UnlockHash types.UnlockHash      `json:"unlockhash"`
}
...
func consensusBlocksGetFromBlock(b types.Block, h types.BlockHeight, d types.Currency) ConsensusBlocksGet {
	...
	for _, t := range b.Transactions {
		// Get the transaction's TurtleDexcoinOutputs.
		scos := make([]ConsensusBlocksGetTurtleDexcoinOutput, 0, len(t.TurtleDexcoinOutputs))
		for i, sco := range t.TurtleDexcoinOutputs {
			scos = append(scos, ConsensusBlocksGetTurtleDexcoinOutput{
				ID:         t.TurtleDexcoinOutputID(uint64(i)),
				Value:      sco.Value,
				UnlockHash: sco.UnlockHash,
			})
		}
		// Get the transaction's TurtleDexfundOutputs.
		sfos := make([]ConsensusBlocksGetTurtleDexfundOutput, 0, len(t.TurtleDexfundOutputs))
		for i, sfo := range t.TurtleDexfundOutputs {
			sfos = append(sfos, ConsensusBlocksGetTurtleDexfundOutput{
				ID:         t.TurtleDexfundOutputID(uint64(i)),
				Value:      sfo.Value,
		...
		for i, fc := range t.FileContracts {
			...
			vpos := make([]ConsensusBlocksGetTurtleDexcoinOutput, 0, len(fc.ValidProofOutputs))
			for j, vpo := range fc.ValidProofOutputs {
				vpos = append(vpos, ConsensusBlocksGetTurtleDexcoinOutput{
			...
			mpos := make([]ConsensusBlocksGetTurtleDexcoinOutput, 0, len(fc.MissedProofOutputs))
			for j, mpo := range fc.MissedProofOutputs {
				mpos = append(mpos, ConsensusBlocksGetTurtleDexcoinOutput{
		...
		txns = append(txns, ConsensusBlocksGetTxn{
			ID:                    t.ID(),
			TurtleDexcoinInputs:         t.TurtleDexcoinInputs,
			TurtleDexcoinOutputs:        scos,
			FileContracts:         fcos,
			FileContractRevisions: t.FileContractRevisions,
			StorageProofs:         t.StorageProofs,
			TurtleDexfundInputs:         t.TurtleDexfundInputs,
			TurtleDexfundOutputs:        sfos,
...
func (api *API) consensusHandler(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	...
	WriteJSON(w, ConsensusGET{
		...
		TurtleDexfundCount:           types.TurtleDexfundCount,
		TurtleDexfundPortion:         types.TurtleDexfundPortion,
		...
		TurtleDexcoinPrecision: types.TurtleDexcoinPrecision,
	})
}
