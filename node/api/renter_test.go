...
func setupTestDownload(t *testing.T, size int, name string, waitOnRedundancy bool) (*serverTester, string) {
	...
	// Create a file.
	path := filepath.Join(build.TurtleDexTestingDir, "api", t.Name(), name)
...
func runDownloadTest(t *testing.T, filesize, offset, length int64, useHttpResp bool, testName string) error {
	ulTurtleDexPath, err := modules.NewTurtleDexPath(testName + ".dat")
	if err != nil {
		t.Fatal(err)
	}
	st, path := setupTestDownload(t, int(filesize), ulTurtleDexPath.String(), true)
	defer func() {
	...
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&offset=%d&length=%d", ulTurtleDexPath, offset, length)
	...
	if useHttpResp {
		...
		err = build.Retry(30, time.Second, func() error {
			...
			for _, download := range rdq.Downloads {
				if download.Received == download.Filesize && download.TurtleDexPath.Equals(ulTurtleDexPath) {
					return nil
				}
			}
...
func TestRenterDownloadError(t *testing.T) {
	...
	for _, download := range rdq.Downloads {
		if download.TurtleDexPath.String() == "test.dat" && download.Received == download.Filesize && download.Error == expectedErr.Error() {
...
func runDownloadParamTest(t *testing.T, length, offset, filesize int) error {
	ulTurtleDexPath := "test.dat"

	st, _ := setupTestDownload(t, filesize, ulTurtleDexPath, true)
	defer func() {
		if err := st.server.Close(); err != nil {
			t.Fatal(err)
		}
	...
	downpath := filepath.Join(st.dir, fname)
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%s", ulTurtleDexPath, downpath)
...
func TestRenterDownloadAsyncAndHttpRespError(t *testing.T) {
	...
	st, _ := setupTestDownload(t, int(filesize), ulTurtleDexPath, true)
	defer func() {
		if err := st.server.Close(); err != nil {
			t.Fatal(err)
		}
	...
	// Download the original file from offset 40 and length 10.
	fname := "offsetsinglechunk.dat"
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%s&async=true&httpresp=true", ulTurtleDexPath, fname)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterDownloadAsyncAndNotDestinationError(t *testing.T) {
	...
	// Download the original file from offset 40 and length 10.
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&async=true", ulTurtleDexPath)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterDownloadHttpRespAndDestinationError(t *testing.T) {
	...
	// Download the original file from offset 40 and length 10.
	fname := "test.dat"
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%shttpresp=true", ulTurtleDexPath, fname)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterAsyncDownloadError(t *testing.T) {
	...
	for _, download := range rdq.Downloads {
		if download.TurtleDexPath.String() == "test.dat" && download.Received == download.Filesize && download.Error == "" {
...
func TestRenterAsyncDownload(t *testing.T) {
	...
	for start := time.Now(); time.Since(start) < 30*time.Second; time.Sleep(time.Millisecond * 10) {
		...
		for _, download := range rdq.Downloads {
			if download.Received == download.Filesize && download.TurtleDexPath.String() == "test.dat" {
				success = true
			}
		}
		if success {
...
func TestRenterPaths(t *testing.T) {
	...
	// Create a file.
	path := filepath.Join(build.TurtleDexTestingDir, "api", t.Name(), "test.dat")
	...
	if len(rf.Files) != 1 || rf.Files[0].TurtleDexPath.String() != "foo/bar/test" {
...
func TestRenterConflicts(t *testing.T) {
	...
	if len(rf.Files) != 1 || rf.Files[0].TurtleDexPath.String() != "foo/bar.sia/test" {
		t.Fatal("/renter/files did not return correct file:", rf)
	}
...
func TestAdversarialPriceRenewal(t *testing.T) {
	...
	allowanceValues := url.Values{}
	testPeriod := "10000"
	allowanceValues.Set("funds", types.TurtleDexcoinPrecision.Mul64(10000).String())
	allowanceValues.Set("period", testPeriod)
	err = st.stdPostAPI("/renter", allowanceValues)
	if err != nil {
		t.Fatal(err)
	...
	err = build.Retry(100, time.Millisecond*500, func() error {
		...
		available := true
		err := st.renter.FileList(modules.RootTurtleDexPath(), true, false, func(fi modules.FileInfo) {
			if !fi.Available {
				available = false
			}
		})
...
// verify the health information stored in the ttdxdir metadata is getting
