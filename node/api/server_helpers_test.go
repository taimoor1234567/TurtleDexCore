...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/explorer"
	...
	"github.com/turtledex/TurtleDexCore/modules/renter/proto"
...
// A Server is a collection of ttdxd modules that can be communicated with over
// an http api.
type Server struct {
	api               *API
...
func NewCustomServer(dir string, APIaddr string, requiredUserAgent string, requiredPassword string, cs modules.ConsensusSet, e modules.Explorer, fm modules.FeeManager, g modules.Gateway, h modules.Host, m modules.Miner, r modules.Renter, tp modules.TransactionPool, w modules.Wallet, apiDeps modules.Dependencies) (*Server, error) {
	...
	if err != nil {
		return nil, errors.AddContext(err, "failed to load ttdxd config")
	}
...
func assembleServerTesterWithDeps(key crypto.CipherKey, testdir string, gDeps, cDeps, tDeps, wDeps, hDeps, rDeps, hdbDeps, hcDeps, csDeps, apiDeps modules.Dependencies) (*serverTester, error) {
	...
	srv, err := NewCustomServer(testdir, "localhost:0", "TurtleDex-Agent", "", cs, nil, nil, g, h, m, r, tp, w, apiDeps)
	if err != nil {
		return nil, err
	}
...
func assembleAuthenticatedServerTester(requiredPassword string, key crypto.CipherKey, testdir string) (*serverTester, error) {
	...
	srv, err := NewServer(testdir, "localhost:0", "TurtleDex-Agent", requiredPassword, cs, nil, nil, g, h, m, r, tp, w)
	if err != nil {
		return nil, err
	}
