...
type (
	...
	RenterContract struct {
		...
		// HostVersion is the version of TurtleDex that the host is running
	...
	RenterBackupsGET struct {
		...
		SyncedHosts   []types.TurtleDexPublicKey   `json:"syncedhosts"`
		UnsyncedHosts []types.TurtleDexPublicKey   `json:"unsyncedhosts"`
	}
	...
	DownloadInfo struct {
		...
		TurtleDexPath         modules.TurtleDexPath `json:"siapath"`         // The siapath of the file used for the download.
...
// rebaseInputTurtleDexPath rebases the TurtleDexPath provided by the user to one that is
// prefixed by the user's home directory.
func rebaseInputTurtleDexPath(siaPath modules.TurtleDexPath) (modules.TurtleDexPath, error) {
...
// trimTurtleDexDirFolder is a helper method to trim /home/siafiles off of the
// siapaths of the dirinfos since the user expects a path relative to
// /home/siafiles and not relative to root.
func trimTurtleDexDirFolder(dis ...modules.DirectoryInfo) (_ []modules.DirectoryInfo, err error) {
	for i := range dis {
		dis[i].TurtleDexPath, err = dis[i].TurtleDexPath.Rebase(modules.UserFolder, modules.RootTurtleDexPath())
		if err != nil {
			return nil, err
		}
	}
...
// trimTurtleDexDirFolderOnFiles is a helper method to trim /home/siafiles off of the
// siapaths of the fileinfos since the user expects a path relative to
// /home/siafiles and not relative to root.
func trimTurtleDexDirFolderOnFiles(fis ...modules.FileInfo) (_ []modules.FileInfo, err error) {
	for i := range fis {
		fis[i].TurtleDexPath, err = fis[i].TurtleDexPath.Rebase(modules.UserFolder, modules.RootTurtleDexPath())
		if err != nil {
...
// trimTurtleDexDirInfo is a helper method to trim /home/siafiles off of the
// siapaths of the fileinfos since the user expects a path relative to
// /home/siafiles and not relative to root.
...
func (api *API) renterBubbleHandlerPOST(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	rootTurtleDexPath := false
	var err error
	if r := req.FormValue("rootsiapath"); r != "" {
		rootTurtleDexPath, err = scanBool(r)
		if err != nil {
...
func (api *API) renterBackupsHandlerGET(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	var unsyncedHosts []types.TurtleDexPublicKey
outer:
	...
	if req.FormValue("host") != "" {
		var hostKey types.TurtleDexPublicKey
...
func (api *API) renterCleanHandlerPOST(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	...
	cleanFunc := func(fi modules.FileInfo) {
		...
		deleteErrs = errors.Compose(deleteErrs, api.renter.DeleteFile(fi.TurtleDexPath))
	}
	err := api.renter.FileList(modules.RootTurtleDexPath(), true, false, cleanFunc)
...
func (api *API) parseRenterContracts(disabled, inactive, expired bool) RenterContracts {
	...
	for _, c := range api.renter.Contracts() {
		...
		contract := RenterContract{
			...
			Fees:                      c.TxnFee.Add(c.TurtleDexfundFee).Add(c.ContractFee),
...
func (api *API) renterDownloadsHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	for _, di := range dis {
		downloads = append(downloads, DownloadInfo{
			...
			Length:          di.Length,
			Offset:          di.Offset,
			TurtleDexPath:         di.TurtleDexPath,
...
func (api *API) renterFuseHandlerGET(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	...
	for i := 0; i < len(rfi.MountPoints); i++ {
		rebased, err := rfi.MountPoints[i].TurtleDexPath.Rebase(modules.UserFolder, modules.RootTurtleDexPath())
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
		rfi.MountPoints[i].TurtleDexPath = rebased
	}
...
func (api *API) renterFuseMountHandlerPOST(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	if spfv == "" {
		...
		siaPath, err = modules.NewTurtleDexPath(spfv)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
	}
	siaPath, err = rebaseInputTurtleDexPath(siaPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func (api *API) renterRenameHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	// Parse the siaPath and the newTurtleDexPath
	siaPath, err := modules.NewTurtleDexPath(ps.ByName("siapath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	newTurtleDexPath, err := modules.NewTurtleDexPath(req.FormValue("newsiapath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	...
	if !root {
		...
		newTurtleDexPath, err = rebaseInputTurtleDexPath(newTurtleDexPath)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
	}
	err = api.renter.RenameFile(siaPath, newTurtleDexPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func (api *API) renterFileHandlerGET(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if !root {
		files, err := trimTurtleDexDirFolderOnFiles(file)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
...
func (api *API) renterFilesHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	files, err = trimTurtleDexDirFolderOnFiles(files...)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusInternalServerError)
		return
	}
...
// renterValidateTurtleDexPathHandler handles the API call that validates a siapath
func (api *API) renterValidateTurtleDexPathHandler(w http.ResponseWriter, _ *http.Request, ps httprouter.Params) {
	// Try and create a new siapath, this will validate the potential siapath
	_, err := modules.NewTurtleDexPath(ps.ByName("siapath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func (api *API) renterDirHandlerGET(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if str == "" || str == "/" {
		...
		siaPath, err = modules.NewTurtleDexPath(str)
	}
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	...
	if !root {
		directories, err = trimTurtleDexDirFolder(directories...)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
...
func (api *API) renterDirHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if action == "rename" {
		...
		err = api.renter.RenameDir(siaPath, newTurtleDexPath)
		if err != nil {
