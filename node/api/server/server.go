...
// host.
func (srv *Server) HostPublicKey() (types.TurtleDexPublicKey, error) {
	if srv.node.Host == nil {
		return types.TurtleDexPublicKey{}, errors.New("can't get public host key of a non-host node")
	}
	...
	s, err := func() (*Server, error) {
		...
		srv := &Server{
			...
			apiServer: &http.Server{
				...
				// set reasonable timeout windows for requests, to prevent the TurtleDex API
		...
		// Create the TurtleDex node for the server after the server was started.
		n, errChan = node.New(nodeParams, loadStartTime)
		if err := modules.PeekErr(errChan); err != nil {
			if isAddrInUseErr(err) {
				return nil, fmt.Errorf("%v; are you running another instance of ttdxd?", err.Error())
			}
			return nil, errors.AddContext(err, "server is unable to create the TurtleDex node")
		}
