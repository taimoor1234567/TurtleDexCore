...
const (
	// The developer key is used to sign updates and other important TurtleDex-
	// related information.
...
type (
	...
	// TurtleDexConstants is a struct listing all of the constants in use.
	TurtleDexConstants struct {
	...
	// DaemonVersion holds the version information for ttdxd
...
func fetchLatestRelease() (_ gitlabRelease, err error) {
	resp, err := http.Get("https://github.com/api/v4/projects/7508674/repository/tags?order_by=name")
	if err != nil {
...
// updateToRelease updates ttdxd and ttdxc to the release specified. ttdxc is
// assumed to be in the same folder as ttdxd.
func updateToRelease(version string) (err error) {
	...
	resp, err := http.Get(fmt.Sprintf("https://turtledex.io/releases/TurtleDex-%s-SHA256SUMS.txt.asc", version))
	if err != nil {
		return err
	}
	...
	releaseFilePrefix := fmt.Sprintf("TurtleDex-%s-%s-%s", version, runtime.GOOS, runtime.GOARCH)
	zipResp, err := http.Get(fmt.Sprintf("https://turtledex.io/releases/%s.zip", releaseFilePrefix))
	if err != nil {
		return err
	}
	...
	// Process zip, finding ttdxd/ttdxc binaries and validate the checksum against
	// the signed checksums file.
	for _, binary := range []string{"ttdxd", "ttdxc"} {
...
// daemonUpdateHandlerPOST handles the API call that updates ttdxd and ttdxc.
...
func (api *API) daemonSettingsHandlerPOST(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	// Set the limit.
	if err := api.ttdxdConfig.SetRatelimit(maxDownloadSpeed, maxUploadSpeed); err != nil {
