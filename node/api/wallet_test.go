...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/consensus"
	"github.com/turtledex/TurtleDexCore/modules/gateway"
	"github.com/turtledex/TurtleDexCore/modules/miner"
	"github.com/turtledex/TurtleDexCore/modules/transactionpool"
	"github.com/turtledex/TurtleDexCore/modules/wallet"
...
func TestWalletGETEncrypted(t *testing.T) {
	...
	srv, err := NewServer(testdir, "localhost:0", "TurtleDex-Agent", "", cs, nil, nil, g, nil, nil, nil, tp, w)
	if err != nil {
		t.Fatal(err)
	}
...
// TestWalletGETTurtleDexcoins probes the GET call to /wallet when the
// ttdc balance is being manipulated.
func TestWalletGETTurtleDexcoins(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Check the initial wallet is encrypted, unlocked, and has the ttdcs
	// that got mined.
	var wg WalletGET
	err = st.getAPI("/wallet", &wg)
	if err != nil {
	...
	if wg.ConfirmedTurtleDexcoinBalance.Cmp(calculateCoinbase(1)) != 0 {
		t.Error("reported wallet balance does not reflect the single block that has been mined")
	}
	if wg.UnconfirmedOutgoingTurtleDexcoins.Cmp64(0) != 0 {
		t.Error("there should not be unconfirmed outgoing ttdcs")
	}
	if wg.UnconfirmedIncomingTurtleDexcoins.Cmp64(0) != 0 {
		t.Error("there should not be unconfirmed incoming ttdcs")
	}
	...
	sendTurtleDexcoinsValues := url.Values{}
	sendTurtleDexcoinsValues.Set("amount", "1234")
	sendTurtleDexcoinsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/ttdcs", sendTurtleDexcoinsValues)
	if err != nil {
		t.Fatal(err)
	}

	// Check that the wallet is reporting unconfirmed ttdcs.
	err = st.getAPI("/wallet", &wg)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.UnconfirmedOutgoingTurtleDexcoins.Cmp64(0) <= 0 {
		t.Error("there should be unconfirmed outgoing ttdcs")
	}
	if wg.UnconfirmedIncomingTurtleDexcoins.Cmp64(0) <= 0 {
		t.Error("there should be unconfirmed incoming ttdcs")
	}
	if wg.UnconfirmedOutgoingTurtleDexcoins.Cmp(wg.UnconfirmedIncomingTurtleDexcoins) <= 0 {
		t.Error("net movement of ttdcs should be outgoing (miner fees)")
	}
	...
	if wg.ConfirmedTurtleDexcoinBalance.Cmp(calculateCoinbase(2)) >= 0 {
...
func TestIntegrationWalletSweepSeedPOST(t *testing.T) {
	...
	key := crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
	...
	st.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision.Mul64(100), addr.UnlockHash())
	_, err = st.miner.AddBlock()
	if err != nil {
		t.Fatal(err)
	}
	...
	if wsp.Coins.Cmp(types.TurtleDexcoinPrecision.Mul64(80)) <= 0 {
		t.Fatalf("swept fewer coins (%v SC) than expected %v+", wsp.Coins.Div(types.TurtleDexcoinPrecision), 80)
	}
...
func TestIntegrationWalletLoadSeedPOST(t *testing.T) {
	...
	key2 := crypto.GenerateTurtleDexKey(crypto.TypeDefaultWallet)
...
func TestWalletTransactionGETid(t *testing.T) {
	...
	// Query the details of a transaction where ttdcs were sent.
	//
	// NOTE: We call the SendTurtleDexcoins method directly to get convenient access
	// to the txid.
	sentValue := types.TurtleDexcoinPrecision.Mul64(3)
	txns, err := st.wallet.SendTurtleDexcoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	_, err = st.miner.AddBlock()
	...
	sendTurtleDexcoinsValues := url.Values{}
	sendTurtleDexcoinsValues.Set("amount", sentValue.String())
	sendTurtleDexcoinsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/ttdcs", sendTurtleDexcoinsValues)
	if err != nil {
		t.Fatal(err)
	...
	// Check that undocumented API behavior used in TurtleDex-UI still works with
	// current API.
...
func TestWalletRelativePathErrorTurtleDexg(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	loadTurtleDexgAbsoluteError := "error when calling /wallet/siagkey: keyfiles contains a non-absolute path"

	// This should fail.
	loadTurtleDexgValues := url.Values{}
	loadTurtleDexgValues.Set("keyfiles", "test.dat")
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err == nil || !strings.Contains(err.Error(), loadTurtleDexgAbsoluteError) {
		t.Fatal(err)
	}
	...
	// This should fail.
	loadTurtleDexgValues = url.Values{}
	loadTurtleDexgValues.Set("keyfiles", "/test.dat,test.dat,../test.dat")
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err == nil || !strings.Contains(err.Error(), loadTurtleDexgAbsoluteError) {
		t.Fatal(err)
	}
	...
	// As should this.
	loadTurtleDexgValues = url.Values{}
	loadTurtleDexgValues.Set("keyfiles", "../test.dat,/test.dat")
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err == nil || !strings.Contains(err.Error(), loadTurtleDexgAbsoluteError) {
		t.Fatal(err)
	}
	...
	loadTurtleDexgValues.Set("keyfiles", filepath.Join(walletTestDir, "test.dat"))
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err == nil || err.Error() == loadTurtleDexgAbsoluteError {
		t.Fatal(err)
	}
	...
	loadTurtleDexgValues.Set("keyfiles", filepath.Join(walletTestDir, "test.dat")+","+filepath.Join(walletTestDir, "test1.dat"))
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err == nil || err.Error() == loadTurtleDexgAbsoluteError {
		t.Fatal(err)
	}
...
func TestWalletTurtleDexfunds(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	loadTurtleDexgValues := url.Values{}
	loadTurtleDexgValues.Set("keyfiles", siagPath)
	loadTurtleDexgValues.Set("encryptionpassword", walletPassword)
	err = st.stdPostAPI("/wallet/siagkey", loadTurtleDexgValues)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.TurtleDexfundBalance.Cmp64(2000) != 0 {
		t.Fatalf("bad siafund balance: expected %v, got %v", 2000, wg.TurtleDexfundBalance)
	}
	...
	sendTurtleDexfundsValues := url.Values{}
	sendTurtleDexfundsValues.Set("amount", "2000")
	sendTurtleDexfundsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/siafunds", sendTurtleDexfundsValues)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.TurtleDexcoinClaimBalance.IsZero() {
...
// TestWalletTurtleDexcoins tests the /wallet/ttdcs endpoint, including sending
// to multiple addresses.
func TestWalletTurtleDexcoins(t *testing.T) {
	...
	sendAmount := types.TurtleDexcoinPrecision.Mul64(10000)
	var wag WalletAddressGET
	err = st2.getAPI("/wallet/address", &wag)
	if err != nil {
		t.Fatal(err)
	...
	sendTurtleDexcoinsValues := url.Values{}
	outputsJSON, _ := json.Marshal([]types.TurtleDexcoinOutput{{
		UnlockHash: wag.Address,
		Value:      sendAmount,
	}})
	sendTurtleDexcoinsValues.Set("outputs", string(outputsJSON))
	if err = st.stdPostAPI("/wallet/ttdcs", sendTurtleDexcoinsValues); err != nil {
		t.Fatal(err)
	}
	...
	for _, w := range wallets[2:5] {
		...
		outputs = append(outputs, types.TurtleDexcoinOutput{
			UnlockHash: wag.Address,
			Value:      sendAmount,
		})
	}
	...
	for i, w := range wallets[1:] {
		...
		if !wg.ConfirmedTurtleDexcoinBalance.Equals(sendAmount) {
			t.Errorf("wallet %d should have %v coins, has %v", i+2, sendAmount, wg.ConfirmedTurtleDexcoinBalance)
		}
	}
}
...
func TestWalletManyTransactions(t *testing.T) {
	...
	minedBlocks := 0
	for i, uc := range ucs {
		st.wallet.SendTurtleDexcoins(types.TurtleDexcoinPrecision, uc.UnlockHash())
		if i%100 == 0 {
...
func TestWalletTransactionsGetAddr(t *testing.T) {
	...
	sentValue := types.TurtleDexcoinPrecision.Mul64(3)
	_, err = st.wallet.SendTurtleDexcoins(sentValue, addr)
	if err != nil {
		t.Fatal(err)
	}
