...
var (
	...
	DefaultSkynetPricePerMS = types.TurtleDexcoinPrecision.MulFloat(1e-7) // 100 nS
)
...
// skynetSkylinkPinHandlerPOST will pin a skylink to this TurtleDex node, ensuring
...
func (api *API) skynetSkyfileHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if params.convertPath == "" {
		...
		// measure.
		file, err := api.renter.File(sup.TurtleDexPath)
		extendedPath := sup.TurtleDexPath
	...
	convertPath, err := modules.NewTurtleDexPath(params.convertPath)
	if err != nil {
		WriteError(w, Error{"invalid convertpath provided: " + err.Error()}, http.StatusBadRequest)
		return
	}
	convertPath, err = rebaseInputTurtleDexPath(convertPath)
	if err != nil {
		WriteError(w, Error{"invalid convertpath provided - can't rebase: " + err.Error()}, http.StatusBadRequest)
		return
	}
	skylink, err := api.renter.CreateSkylinkFromTurtleDexfile(sup, convertPath)
	if errors.Contains(err, renter.ErrSkylinkBlocked) {
		WriteError(w, Error{err.Error()}, http.StatusUnavailableForLegalReasons)
		return
	}
...
func (api *API) skynetStatsHandlerGET(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	dis, err := api.renter.DirList(modules.RootTurtleDexPath())
	if err == nil {
