...
func (uc *UnsafeClient) SkynetSkyfilePostRawResponse(params modules.SkyfileUploadParameters) (http.Header, []byte, error) {
	...
	query := fmt.Sprintf("/skynet/skyfile/%s?%s", params.TurtleDexPath.String(), values.Encode())
	return uc.postRawResponse(query, params.Reader)
}

// RenterSkyfileGet wraps RenterFileRootGet to query a skyfile.
func (c *Client) RenterSkyfileGet(siaPath modules.TurtleDexPath, root bool) (rf api.RenterFile, err error) {
	if !root {
...
func (c *Client) SkynetSkylinkPinPostWithTimeout(skylink string, params modules.SkyfilePinParameters, timeout int) error {
	...
	rootStr := fmt.Sprintf("%t", params.Root)
	values.Set("root", rootStr)
	values.Set("siapath", params.TurtleDexPath.String())
	values.Set("timeout", fmt.Sprintf("%d", timeout))
...
// SkynetConvertTurtleDexfileToSkyfilePost uses the /skynet/skyfile endpoint to
// convert an existing siafile to a skyfile. The input TurtleDexPath 'convert' is the
...
// skyfile.
func (c *Client) SkynetConvertTurtleDexfileToSkyfilePost(lup modules.SkyfileUploadParameters, convert modules.TurtleDexPath) (api.SkynetSkyfileHandlerPOST, error) {
	// Set the url values.
	values := url.Values{}
	...
	// Make the call to upload the file.
	query := fmt.Sprintf("/skynet/skyfile/%s?%s", lup.TurtleDexPath.String(), values.Encode())
...
func (c *Client) RegistryRead(spk types.TurtleDexPublicKey, dataKey crypto.Hash) (modules.SignedRegistryValue, error) {
	return c.RegistryReadWithTimeout(spk, dataKey, 0)
}

// RegistryReadWithTimeout queries the /skynet/registry [GET] endpoint with the
// specified timeout.
func (c *Client) RegistryReadWithTimeout(spk types.TurtleDexPublicKey, dataKey crypto.Hash, timeout time.Duration) (modules.SignedRegistryValue, error) {
	// Set the values.
	values := url.Values{}
...
func (c *Client) RegistryUpdate(spk types.TurtleDexPublicKey, dataKey crypto.Hash, revision uint64, sig crypto.Signature, skylink modules.Skylink) error {
