...
type (
	// A Client makes requests to the ttdxd HTTP API.
	Client struct {
		Options
	}
	...
	// client.
	Options struct {
		// Address is the API address of the ttdxd server.
		Address string

		// Password must match the password of the ttdxd server.
		Password string

		// UserAgent must match the User-Agent required by the ttdxd server. If not
		// set, it defaults to "TurtleDex-Agent".
		UserAgent string
...
// using build.APIPassword and the user agent will be set to "TurtleDex-Agent". Both
...
// setting the default ttdxd user agent to "TurtleDex-Agent" and setting the password
...
func DefaultOptions() (Options, error) {
	...
	return Options{
		...
		Password:  pw,
		UserAgent: "TurtleDex-Agent",
	}, nil
}

// NewRequest constructs a request to the ttdxd HTTP API, setting the correct
