...
gitlab-runner.exe register --non-interactive --url https://github.com/ --registration-token $token --executor shell --pre-clone-script "remove-item .\builds\*\*\*\*\.cache -Force -Recurse -ErrorAction SilentlyContinue" --description Win10-Server-Hetzner --tag-list nebulous-windows
