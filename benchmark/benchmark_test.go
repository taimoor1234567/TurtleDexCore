...
const (
	...
	siaDir           = "upload-download-benchmark"     // TurtleDex directory to upload files to
	...
	// ttdxd
	ttdxdPort     = "9980" // Port of ttdxd node (if not using a test group)
	useTestGroup = true   // Whether to use a test group (true) or TurtleDex network (false)
)
...
var (
	...
	c  *client.Client     // TurtleDex client for uploads and downloads
...
// TestTurtleDexUploadsDownloads concurrently uploads and then downloads files. Before execution
// be sure to have enough storage for concurrent upload and download files.
func TestTurtleDexUploadsDownloads(t *testing.T) {
	if !build.VLONG {
		t.SkipNow()
	}
	...
	// TODO: https://github.com/turtledex/TurtleDexCore/-/issues/4242
...
// benchmarkTestDir creates a temporary TurtleDex testing directory for a benchmark
...
// health is reached
func checkMaxHealthReached(c *client.Client, siaPath modules.TurtleDexPath) error {
...
// initClient initializes a TurtleDex http client
func initClient() {
	if useTestGroup {
	...
	check(err)
	opts.Address = "localhost:" + ttdxdPort
	c = client.New(opts)
}
...
func threadedCreateAndUploadFiles(timestamp string, workerIndex int) {
	for {
		...
		// Upload the file file to TurtleDex
		uploadFile(filename)
...
func threadedDownloadFiles(workerIndex int) {
	for {
		...
		siaPath, err := modules.NewTurtleDexPath(filepath.Join(siaDir, filename))
		check(err)
...
// uploadFile uploads file to TurtleDex
func uploadFile(filename string) {
	start := time.Now()

	log.Printf("Uploading file: %s\n", filename)

	// Upload file to TurtleDex
	siaPath, err := modules.NewTurtleDexPath(filepath.Join(siaDir, filename))
	check(err)
