...
func TestTransactionReorg(t *testing.T) {
	...
	wsp, err := miner1.WalletTurtleDexcoinsPost(types.TurtleDexcoinPrecision, uc.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	blocks := 1
...
func TestSignTransaction(t *testing.T) {
	...
	txn := types.Transaction{
		TurtleDexcoinInputs: []types.TurtleDexcoinInput{
			{
				ParentID:         types.TurtleDexcoinOutputID(outputs[0].ID),
				UnlockConditions: wucg1.UnlockConditions,
			},
			{
				ParentID:         types.TurtleDexcoinOutputID(outputs[1].ID),
	...
	for _, output := range unspentResp.Outputs {
		if output.ID == types.OutputID(txn.TurtleDexcoinInputs[0].ParentID) {
...
func TestWatchOnly(t *testing.T) {
	...
	_, err = testNode.WalletTurtleDexcoinsPost(types.TurtleDexcoinPrecision.Mul64(77), addr, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFileContractUnspentOutputs(t *testing.T) {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		var found bool
		for _, o := range wug.Outputs {
			if types.TurtleDexcoinOutputID(o.ID) == outputID {
				found = true
			}
		}
		if !found {
...
// TestWalletSend tests sending ttdcs with and without fees included.
func TestWalletSend(t *testing.T) {
	...
	originalBalance1 := wg.ConfirmedTurtleDexcoinBalance
	wg, err = renter2.WalletGet()
	if err != nil {
		t.Fatal(err)
	}
	originalBalance2 := wg.ConfirmedTurtleDexcoinBalance
	...
	_, err = renter1.WalletTurtleDexcoinsPost(sentAmount, uc.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		newBalance2 := wg.ConfirmedTurtleDexcoinBalance
	...
	newBalance1 := wg.ConfirmedTurtleDexcoinBalance
	...
	_, err = renter2.WalletTurtleDexcoinsPost(sentAmount, uc.Address, true)
	if err != nil {
		t.Fatal(err)
	}
	err = build.Retry(100, 100*time.Millisecond, func() error {
...
// trying to send ttdcs or siafunds if the consensus is not fully synced
func TestWalletSendUnsynced(t *testing.T) {
	...
	// Check error returned from ttdcs multi post
	_, err = wallet.WalletTurtleDexcoinsMultiPost([]types.TurtleDexcoinOutput{})
	if err == nil {
		t.Fatal("expected an error to be returned for not being synced")
	}
	if !strings.Contains(err.Error(), "cannot send ttdc until fully synced") {
		t.Fatal("expected to get synced error but got:", err)
	}

	// Check error returned from single ttdc post
	_, err = wallet.WalletTurtleDexcoinsPost(types.ZeroCurrency, types.UnlockHash{}, false)
	if err == nil {
		t.Fatal("expected an error to be returned for not being synced")
	}
	if !strings.Contains(err.Error(), "cannot send ttdc until fully synced") {
	...
	_, err = wallet.WalletTurtleDexfundsPost(types.ZeroCurrency, types.UnlockHash{})
	if err == nil {
		t.Fatal("expected an error to be returned for not being synced")
	}
