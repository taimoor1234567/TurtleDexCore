...
func TestFeeManagerProcessFee(t *testing.T) {
	...
	nebulousStartingBal := nwg.ConfirmedTurtleDexcoinBalance
	rwg, err := r.WalletGet()
	if err != nil {
		t.Fatal(err)
	}
	renterStartingBal := rwg.ConfirmedTurtleDexcoinBalance
	fmwg, err := fm.WalletGet()
	if err != nil {
		t.Fatal(err)
	}
	fmStartingBal := fmwg.ConfirmedTurtleDexcoinBalance
	...
	amount := types.TurtleDexcoinPrecision.Mul64(10)
	appUID := modules.AppUID("testapp")
	recurring := fastrand.Intn(2) == 0
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		if nwg.ConfirmedTurtleDexcoinBalance.Cmp(nebExpected) != 0 {
			return fmt.Errorf("Incorrect Nebulous wallet balance; expected %v, got %v", nebExpected.HumanString(), nwg.ConfirmedTurtleDexcoinBalance.HumanString())
		}
		...
		if rwg.ConfirmedTurtleDexcoinBalance.Cmp(devExpected) != 0 {
			return fmt.Errorf("Incorrect Renter wallet balance; expected %v, got %v", devExpected.HumanString(), rwg.ConfirmedTurtleDexcoinBalance.HumanString())
		}
		...
		if fmwg.ConfirmedTurtleDexcoinBalance.Cmp(fmExpected) >= 0 {
			return fmt.Errorf("Incorrect FeeManager wallet balance; expected less than %v, got %v", fmExpected.HumanString(), fmwg.ConfirmedTurtleDexcoinBalance.HumanString())
		}
		return nil
	})
	if err != nil {
