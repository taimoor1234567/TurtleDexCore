...
var (
	// TurtleDexTestingDir is the directory that contains all of the files and
	// folders created during testing.
	TurtleDexTestingDir = filepath.Join(os.TempDir(), "TurtleDexTesting")
)

// TestDir joins the provided directories and prefixes them with the TurtleDex
// testing directory, removing any files or directories that previously existed
// at that location.
func TestDir(dirs ...string) string {
	path := filepath.Join(TurtleDexTestingDir, "siatest", filepath.Join(dirs...))
