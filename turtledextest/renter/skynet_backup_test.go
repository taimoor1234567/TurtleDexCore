...
func TestSkynetBackupAndRestore(t *testing.T) {
	...
	subTests := []siatest.SubTest{
		...
		{Name: "ConvertedTurtleDexfile", Test: testConvertedTurtleDexFile},
	}
...
func testSingleFileRegular(t *testing.T, tg *siatest.TestGroup) {
	...
	singleFileTest := func(filename, skykeyName string, data []byte) {
		...
		err = verifyBackupAndRestore(tg, portal1, portal2, skylink, sup.TurtleDexPath.String())
		if err != nil {
...
// testConvertedTurtleDexFile verifies that a skyfile that was converted from
// a siafile can be backed up by its skylink and then restored.
func testConvertedTurtleDexFile(t *testing.T, tg *siatest.TestGroup) {
	// Grab the portals
	portals := tg.Portals()
	portal1 := portals[0]
	portal2 := portals[1]
	...
	convertTest := func(filename, skykeyName string, size int) {
		...
		sshp, err := portal1.SkynetConvertTurtleDexfileToSkyfilePost(sup, rf.TurtleDexPath())
		if skykeyName != "" {
			if err == nil {
				// Future proofing the test to fail when siafile conversion with
				// encryption is supported
				t.Fatal("TurtleDexfile Conversions with Encryption now supported, update test")
			}
			return
		}
		if err != nil {
		...
		// Verify the backup and restoration of the skylink
		err = verifyBackupAndRestore(tg, portal1, portal2, sshp.Skylink, sup.TurtleDexPath.String())
		if err != nil {
			t.Errorf("Test %v failed to backup and restore: %v", filename, err)
		}
	}
	...
	// Small siafile
	convertTest("smallTurtleDexfile", "", smallSize)
	// Small siafile with encrypted conversion
	convertTest("smallTurtleDexfile_Encryption", sk.Name, smallSize)
	// Large siafile
	convertTest("largeTurtleDexfile", "", largeSize)
	// Large siafile with encrypted conversion
	convertTest("largeTurtleDexfile_Encryption", sk.Name, largeSize)
}
...
func verifyBackupAndRestore(tg *siatest.TestGroup, portal1, portal2 *siatest.TestNode, skylink, siaPath string) error {
	...
	skyTurtleDexPath, err := modules.SkynetFolder.Join(siaPath)
	if err != nil {
		return err
	}
	err = portal1.RenterFileDeleteRootPost(skyTurtleDexPath)
	if err != nil {
		return err
	}
	skyTurtleDexPathExtended, err := skyTurtleDexPath.Join(modules.ExtendedSuffix)
	if err != nil {
		return err
	}
	err = portal1.RenterFileDeleteRootPost(skyTurtleDexPathExtended)
