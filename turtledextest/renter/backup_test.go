...
func TestCreateLoadBackup(t *testing.T) {
	...
	dirMDPath := filepath.Join(r.Dir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), "subDir", modules.TurtleDexDirExtension)
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatalf(".ttdxdir file at %v doesn't exist", dirMDPath)
	}
	...
	if _, err := os.Stat(dirMDPath); os.IsExist(err) {
		t.Fatal(".ttdxdir file does exist:", dirMDPath)
	}
	// Create a backup.
	...
	// loops to avoid updating the .ttdxdir file.
	...
	// The .ttdxdir file should also be recovered.
	dirMDPath = filepath.Join(r.Dir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), "subDir", modules.TurtleDexDirExtension)
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatalf(".ttdxdir file at %v doesn't exist", dirMDPath)
	}
	// There shouldn't be a .ttdxdir_1 file as we don't replace existing .ttdxdir
	// files.
	if _, err := os.Stat(dirMDPath + "_1"); os.IsExist(err) {
		t.Fatal(".ttdxdir_1 file does exist:", err)
	}
	...
	if err := r.RenterFileDeletePost(rf.TurtleDexPath()); err != nil {
		t.Fatal(err)
	}
	subDir, err = r.FilesDir().CreateDir("subDir")
	if err != nil {
	...
	sp, err := modules.NewTurtleDexPath(rf.TurtleDexPath().String() + "_1")
	if err != nil {
		t.Fatal(err)
	}
	...
	// The .ttdxdir file should still exist.
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatal(".ttdxdir file doesn't exist")
	}
	// There shouldn't be a .ttdxdir_1 file as we don't replace existing .ttdxdir
	// files.
	if _, err := os.Stat(dirMDPath + "_1"); !os.IsNotExist(err) {
		t.Fatal(".ttdxdir_1 file does exist")
	}
}
...
func TestRemoteBackup(t *testing.T) {
	...
	if err := r.RenterFileDeletePost(rf2.TurtleDexPath()); err != nil {
		t.Fatal(err)
	}
	...
	// Confirm ttdxdir exists by querying directory
	rd, err := r.RenterDirGet(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	rd, err = r.RenterDirGet(rd.Directories[1].TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	// pubkey.
	_, err = r.RenterBackupsOnHost(types.TurtleDexPublicKey{})
	if err == nil {
		t.Error(err)
	}
}
