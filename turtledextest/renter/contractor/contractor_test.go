...
import (
	...
	"github.com/turtledex/TurtleDexCore/sync"
...
func TestRenterContractRecovery(t *testing.T) {
	...
	oldPath := filepath.Join(r.Dir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), lf.FileName()+modules.TurtleDexFileExtension)
	siaFile, err := ioutil.ReadFile(oldPath)
	if err != nil {
		t.Fatal(err)
	}
	newRenterDir := filepath.Join(testDir, "renter")
	newPath := filepath.Join(newRenterDir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), lf.FileName()+modules.TurtleDexFileExtension)
...
func TestRenterDownloadWithDrainedContract(t *testing.T) {
	...
	_, err = renter.RenterStreamGet(files[fastrand.Intn(len(files))].TurtleDexPath, true, false)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestContractorHostRemoval(t *testing.T) {
	...
	hostPrice := types.TurtleDexcoinPrecision.Mul64(5000) // 5 KS
...
func TestWatchdogExtraDependencyRegression(t *testing.T) {
	...
	balance := wg.ConfirmedTurtleDexcoinBalance.Add(wg.UnconfirmedIncomingTurtleDexcoins).Sub(wg.UnconfirmedOutgoingTurtleDexcoins)

	// Generate a new address and send all the ttdcs to it.
	...
	_, err = renter.WalletTurtleDexcoinsPost(balance.Sub(fee), addressGet.Address, false)
	if err != nil {
		t.Fatal(err)
	}
