...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/host/registry"
...
func TestSkynetSuite(t *testing.T) {
	...
	subTests := []siatest.SubTest{
		...
		{Name: "ConvertTurtleDexFile", Test: testConvertTurtleDexFile},
		...
		{Name: "RenameTurtleDexPath", Test: testRenameTurtleDexPath},
...
func testSkynetBasic(t *testing.T, tg *siatest.TestGroup) {
	...
	filename := "testSmall"
	uploadTurtleDexPath, err := modules.NewTurtleDexPath("testSmallPath")
	if err != nil {
		t.Fatal(err)
	}
	...
	sup := modules.SkyfileUploadParameters{
		TurtleDexPath:             uploadTurtleDexPath,
		Force:               force,
		Root:                false,
	...
	skynetUploadPath, err := modules.SkynetFolder.Join(uploadTurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	rootUploadTurtleDexPath, err := modules.NewTurtleDexPath("rootTestSmallPath")
	if err != nil {
		t.Fatal(err)
	}
	// Quick fuzz on the force value so that sometimes it is set, sometimes it
	...
	rootLup := modules.SkyfileUploadParameters{
		TurtleDexPath:             rootUploadTurtleDexPath,
		Force:               rootForce,
		Root:                true,
	...
	// Get the list of files in the skynet directory and see if the file is
	// present.
	rootRdg, err := r.RenterDirRootGet(modules.RootTurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	err = build.Retry(250, 250*time.Millisecond, func() error {
		uploadedFile, err := r.RenterFileRootGet(rootUploadTurtleDexPath)
		if err != nil {
			return err
		}
	...
	largeTurtleDexPath, err := modules.NewTurtleDexPath("testLargePath")
	if err != nil {
		t.Fatal(err)
	}
	var force2 bool
	...
	largeLup := modules.SkyfileUploadParameters{
		TurtleDexPath:             largeTurtleDexPath,
		Force:               force2,
		Root:                false,
		BaseChunkRedundancy: 2,
	...
	largeUploadPath, err := modules.NewTurtleDexPath("testLargePath")
	if err != nil {
		t.Fatal(err)
	}
	...
	//
	// Try to download the file behind the skylink.
	pinTurtleDexPath, err := modules.NewTurtleDexPath("testSmallPinPath")
	if err != nil {
		t.Fatal(err)
	}
	pinLUP := modules.SkyfilePinParameters{
		TurtleDexPath:             pinTurtleDexPath,
		Force:               force,
		Root:                false,
		BaseChunkRedundancy: 2,
	}
	...
	// Get the list of files in the skynet directory and see if the file is
	// present.
	fullPinTurtleDexPath, err := modules.SkynetFolder.Join(pinTurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	// See if the file is present.
	pinnedFile, err := r.RenterFileRootGet(fullPinTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterFileDeleteRootPost(fullPinTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	// Make sure the file is no longer present.
	_, err = r.RenterFileRootGet(fullPinTurtleDexPath)
	...
	largePinTurtleDexPath, err := modules.NewTurtleDexPath("testLargePinPath")
	if err != nil {
		t.Fatal(err)
	}
	largePinLUP := modules.SkyfilePinParameters{
		TurtleDexPath:             largePinTurtleDexPath,
		Force:               force,
		Root:                false,
		BaseChunkRedundancy: 2,
	}
	...
	// Use a different TurtleDexpath to avoid path conflict.
	largePinTurtleDexPath, err = modules.NewTurtleDexPath("testLargePinPath2")
	if err != nil {
		t.Fatal(err)
	}
	largePinLUP = modules.SkyfilePinParameters{
	...
	// See if the file is present.
	fullLargePinTurtleDexPath, err := modules.SkynetFolder.Join(largePinTurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	pinnedFile, err = r.RenterFileRootGet(fullLargePinTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	if len(pinnedFile.File.Skylinks) != 1 {
	...
	err = r.RenterFileDeleteRootPost(fullLargePinTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	// Make sure the file is no longer present.
	_, err = r.RenterFileRootGet(fullLargePinTurtleDexPath)
	if err == nil || !strings.Contains(err.Error(), filesystem.ErrNotExist.Error()) {
		t.Fatal("skyfile still present after deletion")
	}
...
// testConvertTurtleDexFile tests converting a siafile to a skyfile. This test checks
// for 1-of-N redundancies and N-of-M redundancies.
func testConvertTurtleDexFile(t *testing.T, tg *siatest.TestGroup) {
	r := tg.Renters()[0]
	...
	_, err = r.SkynetConvertTurtleDexfileToSkyfilePost(sup, remoteFile.TurtleDexPath())
	if err != nil {
		t.Fatal("Expected conversion from TurtleDexfile to Skyfile Post to succeed.")
	}
	...
	sshp, err := r.SkynetConvertTurtleDexfileToSkyfilePost(sup, remoteFile.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetMultipartUpload(t *testing.T, tg *siatest.TestGroup) {
	...
	largeTestFunc := func(files []siatest.TestFile, fileName, skykeyName string) {
		...
		// Check the metadata of the siafile, see that the metadata of the siafile
		// has the skylink referenced.
		largeSkyfilePath, err := modules.SkynetFolder.Join(sup.TurtleDexPath.String())
		if err != nil {
			t.Fatal(err)
		}
		largeRenterFile, err := r.RenterFileRootGet(largeSkyfilePath)
...
func testSkynetStats(t *testing.T, tg *siatest.TestGroup) {
	...
	var sps []modules.TurtleDexPath
	for name, size := range files {
		_, sup, _, err := r.UploadNewSkyfileBlocking(name, size, false)
		if err != nil {
			t.Fatal(err)
		}
		sp, err := sup.TurtleDexPath.Rebase(modules.RootTurtleDexPath(), modules.SkynetFolder)
		if err != nil {
			t.Fatal(err)
		}
		sps = append(sps, sp)
	...
	sup := modules.SkyfileUploadParameters{
		TurtleDexPath: rf.TurtleDexPath(),
		Mode:    modules.DefaultFilePerm,
		Force:   false,
		Root:    false,
	}
	_, err = r.SkynetConvertTurtleDexfileToSkyfilePost(sup, rf.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, sp := range sps {
		...
		extSP, err := modules.NewTurtleDexPath(sp.String() + modules.ExtendedSuffix)
		if err != nil {
			t.Fatal(err)
		}
	...
	err = r.RenterFileDeletePost(rf.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	convertSP, err := rf.TurtleDexPath().Rebase(modules.RootTurtleDexPath(), modules.SkynetFolder)
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetInvalidFilename(t *testing.T, tg *siatest.TestGroup) {
	...
	for _, filename := range filenames {
		uploadTurtleDexPath, err := modules.NewTurtleDexPath("testInvalidFilename" + persist.RandomSuffix())
		if err != nil {
			t.Fatal(err)
		}
		...
		// Call the upload skyfile client call.
		uploadTurtleDexPath, err = modules.NewTurtleDexPath("testInvalidFilenameMultipart" + persist.RandomSuffix())
		if err != nil {
			t.Fatal(err)
		}
	...
	uploadTurtleDexPath, err := modules.NewTurtleDexPath("testInvalidFilename")
	if err != nil {
		t.Fatal(err)
	}
	sup := modules.SkyfileUploadParameters{
	...
	subfiles := make(modules.SkyfileSubfiles)
	subfiles[subfile.Filename] = subfile
	uploadTurtleDexPath, err = modules.NewTurtleDexPath("testInvalidFilenameMultipart")
	if err != nil {
		t.Fatal(err)
	}
	mup := modules.SkyfileMultipartUploadParameters{
...
func testSkynetDownloadFormats(t *testing.T, tg *siatest.TestGroup) {
	...
	uploadTurtleDexPath, err := modules.NewTurtleDexPath("testSkynetDownloadFormats")
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetSubDirDownload(t *testing.T, tg *siatest.TestGroup) {
	...
	uploadTurtleDexPath, err := modules.NewTurtleDexPath(name)
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetBlocklist(t *testing.T, tg *siatest.TestGroup, isHash bool) {
	...
	var blockedTurtleDexPaths []modules.TurtleDexPath

	// Confirm that the skyfile and its extended info are registered with the
	// renter
	sp, err := modules.SkynetFolder.Join(sup.TurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	spExtended, err := modules.NewTurtleDexPath(sp.String() + modules.ExtendedSuffix)
	if err != nil {
		t.Fatal(err)
	}
	...
	blockedTurtleDexPaths = append(blockedTurtleDexPaths, sp, spExtended)
	...
	// Verify that the TurtleDexPath and Extended TurtleDexPath were removed from the renter
	...
	pinlup := modules.SkyfilePinParameters{
		TurtleDexPath:             sup.TurtleDexPath,
		BaseChunkRedundancy: 2,
		Force:               true,
	}
	err = r.SkynetSkylinkPinPost(skylink, pinlup)
	...
	convertSSHP, err := r.SkynetConvertTurtleDexfileToSkyfilePost(convertUP, rf.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = r.RenterFileGet(rf.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	skyfilePath, err := modules.SkynetFolder.Join(rf.TurtleDexPath().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Make sure all blockedTurtleDexPaths are root paths
	sp, err = modules.UserFolder.Join(rf.TurtleDexPath().String())
	if err != nil {
		t.Fatal(err)
	}
	blockedTurtleDexPaths = append(blockedTurtleDexPaths, sp, skyfilePath)
	...
	_, err = r.SkynetConvertTurtleDexfileToSkyfilePost(convertUP, rf.TurtleDexPath())
	if err == nil || !strings.Contains(err.Error(), renter.ErrSkylinkBlocked.Error()) {
		t.Fatalf("Expected error %v but got %v", renter.ErrSkylinkBlocked, err)
	}
	...
	// in the renter.
	for _, siaPath := range blockedTurtleDexPaths {
...
func testSkynetDryRunUpload(t *testing.T, tg *siatest.TestGroup) {
	r := tg.Renters()[0]
	siaPath, err := modules.NewTurtleDexPath(t.Name())
	if err != nil {
		t.Fatal(err)
	}
	...
	verifyDryRun := func(sup modules.SkyfileUploadParameters, dataSize int) {
		...
		skyfilePath, err := modules.SkynetFolder.Join(sup.TurtleDexPath.String())
		if err != nil {
			t.Fatal(err)
		}
		_, err = r.RenterFileRootGet(skyfilePath)
	...
	uploadTurtleDexPath, err := modules.NewTurtleDexPath(fmt.Sprintf("%s%s", t.Name(), "S"))
	if err != nil {
		t.Fatal(err)
	}
	...
	uploadTurtleDexPath, err = modules.NewTurtleDexPath(fmt.Sprintf("%s%s", t.Name(), "L"))
	if err != nil {
		t.Fatal(err)
	}
	verifyDryRun(modules.SkyfileUploadParameters{
...
func testSkynetRequestTimeout(t *testing.T, tg *siatest.TestGroup) {
	...
	pinTurtleDexPath, err := modules.NewTurtleDexPath(t.Name())
	if err != nil {
		t.Fatal(err)
	}
	pinLUP := modules.SkyfilePinParameters{
...
// testRenameTurtleDexPath verifies that the siapath to the skyfile can be renamed.
func testRenameTurtleDexPath(t *testing.T, tg *siatest.TestGroup) {
	// Grab Renter
	r := tg.Renters()[0]
	...
	siaPath := sup.TurtleDexPath

	// Rename Skyfile with root set to false should fail
	err = r.RenterRenamePost(siaPath, modules.RandomTurtleDexPath(), false)
	if err == nil {
	...
	newTurtleDexPath, err := modules.SkynetFolder.Join(persist.RandomSuffix())
	if err != nil {
		t.Fatal(err)
	}
	err = r.RenterRenamePost(siaPath, newTurtleDexPath, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkynetCleanupOnError(t *testing.T) {
	...
	skyfileDeleted := func(path modules.TurtleDexPath) bool {
	...
	smallPath, err := modules.SkynetFolder.Join(small.TurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	largePath, err := modules.SkynetFolder.Join(large.TurtleDexPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	largePathExtended, err := modules.NewTurtleDexPath(largePath.String() + modules.ExtendedSuffix)
	if err != nil {
		t.Fatal(err)
	}
