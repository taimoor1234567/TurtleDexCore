...
// TestTurtleDexmuxRequired checks that the hostdb will count a host as offline if the
// host is not running siamux.
func TestTurtleDexmuxRequired(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// dependency.
	deps := &dependencies.DependencyDisableHostTurtleDexmux{}
...
func testFilterMode(tg *siatest.TestGroup, renter *siatest.TestNode, fm modules.FilterMode) error {
	...
	var filteredHosts []types.TurtleDexPublicKey
	numHosts := 0
	...
	var nullList []types.TurtleDexPublicKey
