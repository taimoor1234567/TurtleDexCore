...
func testStreamRepair(t *testing.T, tg *siatest.TestGroup) {
	...
	if err := r.RenterUploadStreamRepairPost(bytes.NewReader(corruptB), remoteFile.TurtleDexPath()); err == nil {
		t.Fatal("Corrupt file repair should fail")
	}
	if err := r.WaitForDecreasingRedundancy(remoteFile, 0); err != nil {
		t.Fatal("Redundancy isn't staying at 0", err)
	}
	if err := r.RenterUploadStreamRepairPost(bytes.NewReader(b), remoteFile.TurtleDexPath()); err != nil {
		t.Fatal(err)
	}
