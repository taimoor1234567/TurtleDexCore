...
func TestRenterSpendingReporting(t *testing.T) {
	...
	// Get renter's initial ttdc balance
	wg, err := r.WalletGet()
	if err != nil {
		t.Fatal("Failed to get wallet:", err)
	}
	initialBalance := wg.ConfirmedTurtleDexcoinBalance
	...
	initialPeriodEndBalance := wg.ConfirmedTurtleDexcoinBalance
	...
	if initialPeriodEndBalance.Cmp(wg.ConfirmedTurtleDexcoinBalance) > 0 {
...
// TestStresstestTurtleDexFileSet is a vlong test that performs multiple operations
// which modify the siafileset in parallel for a period of time.
func TestStresstestTurtleDexFileSet(t *testing.T) {
	if !build.VLONG {
		t.SkipNow()
	}
	...
	go func() {
		...
		for {
			...
			// Upload the file.
			sp := files[fastrand.Intn(len(files))].TurtleDexPath
			lf, err := r.FilesDir().NewFile(size)
			if err != nil {
				t.Error(errors.AddContext(err, fmt.Sprintf("%v: unable to create NewFile", threadName)))
				return
	...
	go func() {
		...
		for {
			...
			sp := files[fastrand.Intn(len(files))].TurtleDexPath
			err = r.RenterRenamePost(sp, modules.RandomTurtleDexPath(), false)
			if isErr(err) {
	...
	go func() {
		...
		for {
			...
			if fastrand.Intn(2) == 0 {
				...
				// decided that `ttdxdir.New` returns a potentially existing
				// directory instead.
				if isErr(err) {
					t.Error(errors.AddContext(err, fmt.Sprintf("%v: error creating dir", threadName)))
					return
...
// TestHostChurnTurtleDexfileDefragRegression tests that constant host churn won't
// ever stop the TurtleDexFile from being repaired to full health again.
func TestHostChurnTurtleDexfileDefragRegression(t *testing.T) {
	if testing.Short() || !build.VLONG {
		t.SkipNow()
	}
	t.Parallel()
