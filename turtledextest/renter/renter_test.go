...
import (
	...
	"github.com/turtledex/TurtleDexCore/modules/host/contractmanager"
	"github.com/turtledex/TurtleDexCore/modules/renter"
	"github.com/turtledex/TurtleDexCore/modules/renter/contractor"
...
func TestRenterTwo(t *testing.T) {
	...
	subTests := []siatest.SubTest{
		...
		{Name: "TestTurtleDexFileTimestamps", Test: testTurtleDexfileTimestamps},
...
// testTurtleDexfileTimestamps tests if timestamps are set correctly when creating,
// uploading, downloading and modifying a file.
func testTurtleDexfileTimestamps(t *testing.T, tg *siatest.TestGroup) {
	if len(tg.Hosts()) < 2 {
	...
	newTurtleDexPath, err := modules.NewTurtleDexPath("newsiapath")
	if err != nil {
		t.Fatal(err)
	}
	// Rename the file and check that only the ChangeTime changed.
	rf, err = r.Rename(rf, newTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenterFour(t *testing.T) {
	...
	// Specify subtests to run
	subTests := []siatest.SubTest{
		{Name: "TestValidateTurtleDexPath", Test: testValidateTurtleDexPath},
		...
		{Name: "TestEscapeTurtleDexPath", Test: testEscapeTurtleDexPath}, // Runs last because it uploads many files
	}
...
// downloads to stall in the UI and ttdxc is gone.
func testReceivedFieldEqualsFileSize(t *testing.T, tg *siatest.TestGroup) {
	...
	if !d.TurtleDexPath.Equals(rf.TurtleDexPath()) {
		t.Fatal(d.TurtleDexPath.String(), rf.TurtleDexPath().String())
	}
	sp, err := rf.TurtleDexPath().Rebase(modules.RootTurtleDexPath(), modules.UserFolder)
	if err != nil {
		t.Fatal(err)
	}
	if !rdgr.Downloads[0].TurtleDexPath.Equals(sp) {
		t.Fatal(d.TurtleDexPath.String(), rf.TurtleDexPath().String())
	}
}
...
func testClearDownloadHistory(t *testing.T, tg *siatest.TestGroup) {
	...
	if len(rdg.Downloads) < numDownloads {
		...
		dest := filepath.Join(siatest.TurtleDexTestingDir, strconv.Itoa(fastrand.Intn(math.MaxInt32)))
		for i := 0; i < remainingDownloads; i++ {
			_, err = r.RenterDownloadGet(rf.Files[0].TurtleDexPath, dest, 0, rf.Files[0].Filesize, false, false, false)
			if err != nil {
...
func testDirectories(t *testing.T, tg *siatest.TestGroup) {
	...
	// Check directory
	rgd, err := r.RenterDirGet(rd.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	if rgd.Directories[0].TurtleDexPath != rd.TurtleDexPath() {
		t.Fatalf("TurtleDexPaths do not match %v and %v", rgd.Directories[0].TurtleDexPath, rd.TurtleDexPath())
	}
	if len(rgd.Files) != 0 {
	...
	siaPath, err := rf.TurtleDexPath().Dir()
	if err != nil {
		t.Fatal(err)
	}
	...
	subDir1, err := modules.NewTurtleDexPath("subDir1")
	if err != nil {
		t.Fatal(err)
	}
	newTurtleDexPath := modules.RandomTurtleDexPath()
	if err = r.RenterDirRenamePost(subDir1, newTurtleDexPath); err != nil {
		t.Fatal(err)
	}
	// Renamed directory should have 0 files and 1 sub directory.
	rgd, err = r.RenterDirGet(newTurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	if len(rgd.Files) != 0 {
	...
	rgd, err = r.RenterDirGet(rgd.Directories[1].TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	if len(rgd.Files) != 0 {
	...
	if _, _, err := r.RenterDownloadHTTPResponseGet(rgd.Files[0].TurtleDexPath, 0, uint64(size), true, false); err != nil {
		t.Fatal(err)
	}

	// Check that the old ttdxdir was deleted from disk
	_, err = os.Stat(subDir1.TurtleDexDirSysPath(r.RenterFilesDir()))
	...
	err = r.RenterFileDeletePost(rf1.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}

	// Test deleting directory
	if err = r.RenterDirDeletePost(rd.TurtleDexPath()); err != nil {
		t.Fatal(err)
	}
	...
	rf2RootPath, err := modules.NewTurtleDexPath("/home/user/" + rf2.TurtleDexPath().Path)
	...
	rd2RootPath, err := modules.NewTurtleDexPath("/home/user/" + rd2.TurtleDexPath().Path)
	if err = r.RenterDirDeleteRootPost(rd2RootPath); err != nil {
		t.Fatal(fmt.Errorf(err.Error() + " => " + rd2RootPath.Path))
	}

	// Check that ttdxdir was deleted from disk
	_, err = os.Stat(rd.TurtleDexPath().TurtleDexDirSysPath(r.RenterFilesDir()))
	if !os.IsNotExist(err) {
		t.Fatal("Expected IsNotExist err, but got err:", err)
	}
	// Check that ttdxdir was deleted from disk by root path
	_, err = os.Stat(rd2.TurtleDexPath().TurtleDexDirSysPath(r.RenterFilesDir()))
	if !os.IsNotExist(err) {
		t.Fatal("Expected IsNotExist err, but got err:", err)
	}
}
...
func testLocalRepair(t *testing.T, tg *siatest.TestGroup) {
	...
	for hostsRemoved = 0; float64(hostsRemoved)/float64(parityPieces) < renter.AlertTurtleDexfileLowRedundancyThreshold; hostsRemoved++ {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		for _, alert := range dag.Alerts {
			expectedCause := fmt.Sprintf("TurtleDexfile 'home/user/%v' has a health of %v and redundancy of %v", remoteFile.TurtleDexPath().String(), f.MaxHealth, f.Redundancy)
			if alert.Msg == renter.AlertMSGTurtleDexfileLowRedundancy &&
...
func testPriceTablesUpdated(t *testing.T, tg *siatest.TestGroup) {
	...
	// Get a random worker
	var host types.TurtleDexPublicKey
...
func testSingleFileGet(t *testing.T, tg *siatest.TestGroup) {
	...
	for i := range files {
		// Get Single File
		rf, err := renter.RenterFileGet(files[i].TurtleDexPath)
		if err != nil {
			t.Fatal(err)
		}
		...
		if files[i].TurtleDexPath != rf.File.TurtleDexPath {
			t.Error("mismatch")
		}
...
func testCancelAsyncDownload(t *testing.T, tg *siatest.TestGroup) {
	...
	cancelID, err := renter.RenterDownloadGet(remoteFile.TurtleDexPath(), dst, 0, fileSize, true, true, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := range rdg.Downloads {
		d := rdg.Downloads[i]
		if remoteFile.TurtleDexPath() == d.TurtleDexPath && dst == d.Destination {
			di = &d
			break
		}
	}
...
func testUploadDownload(t *testing.T, tg *siatest.TestGroup) {
	...
	rootPath, err := remoteFile.TurtleDexPath().Rebase(modules.RootTurtleDexPath(), modules.UserFolder)
	if err != nil {
		t.Fatal(err)
	}
...
func testOverspendAllowance(t *testing.T, tg *siatest.TestGroup) {
	...
	allowance.Funds = types.TurtleDexcoinPrecision.Mul64(4)
...
func TestRenterLosingHosts(t *testing.T) {
	...
	file, err := r.RenterFileGet(rf.TurtleDexPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	var pk types.TurtleDexPublicKey
...
// Since 4 hosts are in the worker pool but only 2 are needed, TurtleDex will put 2
// of them on standby and try to download from the other 2. Since only 1 worker
// can succeed, TurtleDex should wake up one worker after another until it finally
...
func testZeroByteFile(t *testing.T, tg *siatest.TestGroup) {
	...
	for _, file := range rfs {
		if file.TurtleDexPath.Equals(rf.TurtleDexPath) {
			found = true
			rf2 = file
			break
		}
...
func TestUploadAfterDelete(t *testing.T) {
	...
	// Repeatedly upload and delete a file with the same TurtleDexPath without
	// closing the entry. That shouldn't cause issues.
	for i := 0; i < 5; i++ {
		// Delete the file.
		if err := renter.RenterFileDeletePost(remoteFile.TurtleDexPath()); err != nil {
			t.Fatal(err)
		}
	...
	sp, err := modules.NewTurtleDexPath("dir.sia")
	if err != nil {
		t.Fatal(err)
	}
...
// TestTurtleDexfileCompatCodeV137 checks that legacy renters can upgrade from the
// v137 siafile format.
func TestTurtleDexfileCompatCodeV137(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	expectedTurtleDexPath, err := modules.NewTurtleDexPath("sub1/sub2/testfile")
	if err != nil {
		t.Fatal(err)
	}
	...
	if fis[0].TurtleDexPath != expectedTurtleDexPath {
		t.Fatalf("TurtleDexpath should be '%v' but was '%v'",
			expectedTurtleDexPath, fis[0].TurtleDexPath)
	}
	...
	err = build.Retry(100, time.Second, func() error {
		...
		if sf.LocalPath != "/tmp/TurtleDexTesting/siatest/TestRenterTwo/gctwr-EKYAZSVOZ6U2T4HZYIAQ/files/4096bytes 16951a61" {
...
// TestTurtleDexfileCompatCodeV140 checks that legacy renters can upgrade from the
// v140 siafile format.
func TestTurtleDexfileCompatCodeV140(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	dummyTurtleDexfile := "foo.sia"
	dummySnapshot := "bar.sia"
	var f *os.File
	var err error
	if f, err = os.Create(filepath.Join(siafilesDir, dummyTurtleDexfile)); err != nil {
		t.Fatal(err)
	}
	...
	expectedLocation := filepath.Join(renterDir, modules.FileSystemRoot, modules.UserFolder.String(), dummyTurtleDexfile)
	if _, err := os.Stat(expectedLocation); err != nil {
		t.Fatal(err)
	}
...
func testSetFileStuck(t *testing.T, tg *siatest.TestGroup) {
	...
	if err := r.RenterSetFileStuckPost(f.TurtleDexPath, false, !f.Stuck); err != nil {
		t.Fatal(err)
	}
	// Check if it was set correctly.
	fi, err := r.RenterFileGet(f.TurtleDexPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	rebased, err := f.TurtleDexPath.Rebase(modules.RootTurtleDexPath(), modules.UserFolder)
	if err != nil {
		t.Fatal(err)
	}
...
// testEscapeTurtleDexPath tests that TurtleDexPaths are escaped correctly to handle escape
// characters
func testEscapeTurtleDexPath(t *testing.T, tg *siatest.TestGroup) {
	// Grab the first of the group's renters
	r := tg.Renters()[0]
	...
	for _, s := range names {
		// Create TurtleDexPath
		siaPath, err := modules.NewTurtleDexPath(s)
		if err != nil {
			t.Fatal(err)
		}
...
// testValidateTurtleDexPath tests the validate siapath endpoint
func testValidateTurtleDexPath(t *testing.T, tg *siatest.TestGroup) {
	// Grab the first of the group's renters
	r := tg.Renters()[0]
	...
	for _, pathTest := range pathTests {
		err := r.RenterValidateTurtleDexPathPost(pathTest.path)
		// Verify expected Error
		if err != nil && pathTest.valid {
			t.Fatal("validateTurtleDexpath failed on valid path: ", pathTest.path)
		}
		if err == nil && !pathTest.valid {
			t.Fatal("validateTurtleDexpath succeeded on invalid path: ", pathTest.path)
		}
	}

	// Create TurtleDexPaths that contain escape characters
	...
	for _, escapeCharTest := range escapeCharTests {
		...
		err := r.RenterValidateTurtleDexPathPost(path)
		// Verify expected Error
		if err != nil && escapeCharTest.valid {
			t.Fatalf("validateTurtleDexpath failed on valid path %v, escaped %v ", escapeCharTest.path, path)
		}
		if err == nil && !escapeCharTest.valid {
			t.Fatalf("validateTurtleDexpath succeeded on invalid path %v, escaped %v ", escapeCharTest.path, path)
		}
	}
}
...
func testDirMode(t *testing.T, tg *siatest.TestGroup) {
	...
	dirSP, err := modules.NewTurtleDexPath("dir")
	if err != nil {
		t.Fatal(err)
	}
	...
	dir2SP := modules.RandomTurtleDexPath()
	...
	dir3SP := modules.RandomTurtleDexPath()
...
func TestWorkerStatus(t *testing.T) {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		for _, worker := range rwg.Workers {
			...
			// Account checks
			if !worker.AccountBalanceTarget.Equals(types.TurtleDexcoinPrecision) {
...
func TestWorkerSyncBalanceWithHost(t *testing.T) {
	...
	delta := types.TurtleDexcoinPrecision.Div64(10)
...
func TestRenterLimitGFUContracts(t *testing.T) {
	...
	test := func(hosts uint64, portalMode bool) error {
		...
		if portalMode {
			allowance.PaymentContractInitialFunding = types.TurtleDexcoinPrecision
		}
		err := renter.RenterPostAllowance(allowance)
		if err != nil {
			t.Fatal(err)
...
func TestRenterClean(t *testing.T) {
	...
	// Upload 2 TurtleDexFiles then delete one of the TurtleDexFile's localFile so that it
	...
	cleanAndVerify := func(numTurtleDexFiles, numSkyFiles int) {
		// Clean renter
		...
		// Check for the expected TurtleDexFiles
		rds, err := r.RenterDirRootGet(modules.UserFolder)
		if err != nil {
			t.Fatal(err)
		}
		if len(rds.Files) != numTurtleDexFiles {
			t.Fatal("unexpected number of files in user folder:", len(rds.Files))
		}
		// The file should be the 2nd siafile uploaded.
		siaPath := rds.Files[0].TurtleDexPath
		expected, err := modules.UserFolder.Join(rf2.TurtleDexPath().String())
		if err != nil {
			t.Fatal(err)
		}
	...
	// First test should only remove the 1 unrecoverable TurtleDexfile
	cleanAndVerify(1, 1)
...
func TestRenterRepairSize(t *testing.T) {
	...
	// Define helper
	m := tg.Miners()[0]
	checkRepairSize := func(dirTurtleDexPath modules.TurtleDexPath, repairExpected, stuckExpected uint64) error {
		return build.Retry(15, time.Second, func() error {
			...
			dis, err := r.RenterDirRootGet(modules.RootTurtleDexPath())
			if err != nil {
				return err
			}
			...
			// If the passed in dirTurtleDexPath is also root then return
			if dirTurtleDexPath.IsRoot() {
				return rootErrs
			}

			// Grab renter's siafile's directory
			dis, err = r.RenterDirRootGet(dirTurtleDexPath)
			if err != nil {
			...
			var dirErrs error
			if dir.AggregateRepairSize != repairExpected {
				dirErrs = errors.Compose(dirErrs, fmt.Errorf("%v: AggregateRepairSize should be %v but was %v", dirTurtleDexPath, repairExpected, dir.AggregateRepairSize))
			}
			if dir.AggregateStuckSize != stuckExpected {
				dirErrs = errors.Compose(dirErrs, fmt.Errorf("%v: AggregateStuckSize should be %v but was %v", dirTurtleDexPath, stuckExpected, dir.AggregateStuckSize))
			}
			if dir.RepairSize != repairExpected {
				dirErrs = errors.Compose(dirErrs, fmt.Errorf("%v: RepairSize should be %v but was %v", dirTurtleDexPath, repairExpected, dir.RepairSize))
			}
			if dir.StuckSize != stuckExpected {
				dirErrs = errors.Compose(dirErrs, fmt.Errorf("%v: StuckSize should be %v but was %v", dirTurtleDexPath, stuckExpected, dir.StuckSize))
			}
	...
	if err := checkRepairSize(modules.RootTurtleDexPath(), 0, 0); err != nil {
	...
	dirTurtleDexPath, err := rf.TurtleDexPath().Dir()
	if err != nil {
		t.Fatal(err)
	}

	// Renter root directory should show 0 repair bytes needed
	if err := checkRepairSize(dirTurtleDexPath, 0, 0); err != nil {
	...
	// Mark as stuck
	err = r.RenterSetFileStuckPost(rf.TurtleDexPath(), false, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := checkRepairSize(dirTurtleDexPath, 0, expected); err != nil {
		t.Log("First host stuck check failed")
		t.Error(err)
	}

	// Mark as not stuck
	err = r.RenterSetFileStuckPost(rf.TurtleDexPath(), false, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, host := range hosts {
		...
		expected += modules.SectorSize
		if err := checkRepairSize(dirTurtleDexPath, expected, 0); err != nil {
...
func TestRenterBubble(t *testing.T) {
	...
	checkDirInfo := func(found, expected modules.DirectoryInfo) error {
		...
		// Match up the UID and TurtleDexPath
		expected.UID = found.UID
		expected.TurtleDexPath = found.TurtleDexPath
	...
	initDirInfo := modules.DirectoryInfo{
		AggregateHealth:              ttdxdir.DefaultDirHealth,
		AggregateMaxHealth:           ttdxdir.DefaultDirHealth,
		AggregateMaxHealthPercentage: 100,
		AggregateMinRedundancy:       ttdxdir.DefaultDirRedundancy,
		// The exact time is not important, just needs to be initialized
		AggregateMostRecentModTime: time.Now(),
		AggregateStuckHealth:       ttdxdir.DefaultDirHealth,

		Health:              ttdxdir.DefaultDirHealth,
		MaxHealth:           ttdxdir.DefaultDirHealth,
		MaxHealthPercentage: 100,
		MinRedundancy:       ttdxdir.DefaultDirRedundancy,
		DirMode:             modules.DefaultDirPerm,
		// The exact time is not important, just needs to be initialized
		MostRecentModTime: time.Now(),
		StuckHealth:       ttdxdir.DefaultDirHealth,
	}
	...
	var tests = []struct {
		...
		{modules.RootTurtleDexPath(), initDirInfo},
	...
	err = r.RenterBubblePost(modules.RootTurtleDexPath(), force, recursive)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests = []struct {
		...
		{modules.RootTurtleDexPath(), rootDirInfo},
		{modules.HomeFolder, initDirInfo},
		{modules.UserFolder, initDirInfo},
		{modules.BackupFolder, initDirInfo},
		{modules.VarFolder, initDirInfo},
