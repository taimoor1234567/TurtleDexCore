...
func siaPathToFusePath(sp modules.TurtleDexPath, fuseRoot modules.TurtleDexPath, mountpoint string) (string, error) {
	rebased, err := sp.Rebase(modules.RootTurtleDexPath(), fuseRoot)
	if err != nil {
...
func TestFuse(t *testing.T) {
	...
	err = r.RenterFuseMount(mountpoint1, modules.RootTurtleDexPath(), defaultOpts)
	if err != nil {
		t.Fatal(err)
	}
	...
	path := remoteFile.TurtleDexPath()
	fusePath, err := siaPathToFusePath(path, modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	path = remotefd1f1.TurtleDexPath()
	fusePath, err = siaPathToFusePath(path, modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	// See if we can read the new file in fuse.
	path = remotefd1f2.TurtleDexPath()
	fusePath, err = siaPathToFusePath(path, modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	localfd2f1Path, err := siaPathToFusePath(remotefd2f1.TurtleDexPath(), modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	customDirTurtleDexPath, err := modules.RootTurtleDexPath().Join("custom-dir-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterDirCreateWithModePost(customDirTurtleDexPath, customDirPerm)
	if err != nil {
		t.Fatal(err)
	}
	...
	customFileTurtleDexPath, err := customDirTurtleDexPath.Join("custom-file-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterUploadPost(customFilePath, customFileTurtleDexPath, 1, 1)
	if err != nil {
		t.Fatal(err)
	}
	// Open the new custom dir and file in fuse and ensure that the modes are
	// set correctly.
	customDirFusePath, err := siaPathToFusePath(customDirTurtleDexPath, modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	customFileFusePath, err := siaPathToFusePath(customFileTurtleDexPath, modules.RootTurtleDexPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	customFileRenamedTurtleDexPath, err := customDirTurtleDexPath.Join("custom-file-1-renamed")
	if err != nil {
		t.Fatal(err)
	}
	err = r.RenterRenamePost(customFileTurtleDexPath, customFileRenamedTurtleDexPath, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterFuseMount(inodeMount, modules.RootTurtleDexPath(), defaultOpts)
	if err != nil {
		t.Fatal(err)
	}
	inodeFile1Path, err := siaPathToFusePath(remoteFile.TurtleDexPath(), modules.RootTurtleDexPath(), inodeMount)
	if err != nil {
		t.Fatal(err)
	}
	...
	inodeDir1Path, err := siaPathToFusePath(remotefd1.TurtleDexPath(), modules.RootTurtleDexPath(), inodeMount)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < threads; i++ {
		go func(id int) {
			...
			var homeTurtleDexPath modules.TurtleDexPath
			if siatest.Fuzz() < 1 {
				home = localfd1
				homeTurtleDexPath = remotefd1.TurtleDexPath()
			} else {
				home = localfd2
				homeTurtleDexPath = remotefd2.TurtleDexPath()
			}
			...
			for i := 0; i < mountIters; i++ {
				var err error
				var siaPathToMount modules.TurtleDexPath
				if siatest.Fuzz() < 1 {
					siaPathToMount = homeTurtleDexPath
				} else {
					siaPathToMount = threadRemoteDir.TurtleDexPath()
				}
				if err != nil {
					err = errors.AddContext(err, "unable to create mountpoint")
					errMu.Lock()
			...
			err = r.RenterFuseMount(threadMount, modules.RootTurtleDexPath(), defaultOpts)
			if err != nil {
				err = errors.AddContext(err, "unable to mount thread mount")
				errMu.Lock()
				groupErr = errors.Compose(groupErr, err)
			...
			for i := 0; i < readIters; i++ {
				path := remoteFile.TurtleDexPath()
				fusePath, err := siaPathToFusePath(path, modules.RootTurtleDexPath(), threadMount)
				if err != nil {
			...
			if siatest.Fuzz() == -1 {
				...
				siaPathToMount = modules.RootTurtleDexPath()
			}
			err = r.RenterFuseMount(threadMount, siaPathToMount, defaultOpts)
			if err != nil {
				err = errors.AddContext(err, "unable to mount thread mount")
