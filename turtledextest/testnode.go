...
func newCleanNode(nodeParams node.NodeParams, asyncSync bool) (*TestNode, error) {
	...
	// Check if the TurtleDexMuxAddress is set, if not we want to set it to use a
	// random port in testing
	if nodeParams.TurtleDexMuxTCPAddress == "" {
		nodeParams.TurtleDexMuxTCPAddress = "localhost:0"
	}
	if nodeParams.TurtleDexMuxWSAddress == "" {
		nodeParams.TurtleDexMuxWSAddress = "localhost:0"
	}
...
// TurtleDexPath returns the siapath of a local file or directory to be used for
// uploading
func (tn *TestNode) TurtleDexPath(path string) modules.TurtleDexPath {
