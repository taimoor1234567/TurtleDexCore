...
// CheckBalanceVsSpending checks the renters confirmed ttdc balance in their
...
func CheckBalanceVsSpending(r *TestNode, initialBalance types.Currency) error {
	...
	if expectedBalance.Cmp(wg.ConfirmedTurtleDexcoinBalance) != 0 {
		details := fmt.Sprintf(`Initial balance minus Renter Reported Spending does not equal wallet Confirmed TurtleDexcoin Balance
		Expected Balance:   %v
		Wallet Balance:     %v
		Actual difference:  %v
		`, expectedBalance.HumanString(), wg.ConfirmedTurtleDexcoinBalance.HumanString(), initialBalance.Sub(wg.ConfirmedTurtleDexcoinBalance).HumanString())
		var diff string
		if expectedBalance.Cmp(wg.ConfirmedTurtleDexcoinBalance) > 0 {
			diff = fmt.Sprintf("Under reported by:  %v\n", expectedBalance.Sub(wg.ConfirmedTurtleDexcoinBalance).HumanString())
		} else {
			diff = fmt.Sprintf("Over reported by:   %v\n", wg.ConfirmedTurtleDexcoinBalance.Sub(expectedBalance).HumanString())
		}
		err := details + diff
...
func DrainContractsByUploading(renter *TestNode, tg *TestGroup) (startingUploadSpend types.Currency, err error) {
	...
	maxStoragePrice := types.TurtleDexcoinPrecision.Mul64(3e6).Div(modules.BlockBytesPerMonthTerabyte)
