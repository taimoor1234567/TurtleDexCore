...
// deterministic order.
func (tn *TestNode) Dirs() ([]modules.TurtleDexPath, error) {
	// dirs always contains the root dir.
	dirs := []modules.TurtleDexPath{modules.RootTurtleDexPath()}
	toVisit := []modules.TurtleDexPath{modules.RootTurtleDexPath()}
	...
	for len(toVisit) > 0 {
		...
		for _, di := range rd.Directories {
			if di.TurtleDexPath != d {
				toVisit = append(toVisit, di.TurtleDexPath)
			}
		}
	}
	return dirs, nil
...
func (tn *TestNode) DownloadByStreamWithDiskFetch(rf *RemoteFile, disableLocalFetch bool) (uid modules.DownloadID, data []byte, err error) {
	...
	uid, data, err = tn.RenterDownloadHTTPResponseGet(rf.TurtleDexPath(), 0, fi.Filesize, disableLocalFetch, false)
...
func (tn *TestNode) DownloadInfo(lf *LocalFile, rf *RemoteFile) (*api.DownloadInfo, error) {
	...
	for i := range rdq.Downloads {
		d := rdq.Downloads[i]
		if rf.siaPath == d.TurtleDexPath && lf.path == d.Destination {
			di = &d
			break
		}
	}
...
func (tn *TestNode) DownloadToDiskWithDiskFetch(rf *RemoteFile, async bool, disableLocalFetch bool) (modules.DownloadID, *LocalFile, error) {
	...
	fileName := fmt.Sprintf("%dbytes %s", fi.Filesize, persist.RandomSuffix())
	dest := filepath.Join(tn.downloadDir.path, fileName)
	uid, err := tn.RenterDownloadGet(rf.TurtleDexPath(), dest, 0, fi.Filesize, async, disableLocalFetch, false)
	if err != nil {
		return "", nil, errors.AddContext(err, "failed to download file")
	}
	// Make sure the download is in the history.
...
func (tn *TestNode) File(rf *RemoteFile) (fi modules.FileInfo, err error) {
	...
	if rf.Root() {
		rfile, err = tn.RenterFileRootGet(rf.TurtleDexPath())
	} else {
		rfile, err = tn.RenterFileGet(rf.TurtleDexPath())
	}
	if err != nil {
		return
	}
...
// the new file.
func (tn *TestNode) Rename(rf *RemoteFile, newPath modules.TurtleDexPath) (*RemoteFile, error) {
...
func (tn *TestNode) RenameRoot(rf *RemoteFile, newPath modules.TurtleDexPath, root bool) (*RemoteFile, error) {
	err := tn.RenterRenamePost(rf.TurtleDexPath(), newPath, root)
	if err != nil {
		return nil, err
	}
	rf.mu.Lock()
...
func (tn *TestNode) Upload(lf *LocalFile, siapath modules.TurtleDexPath, dataPieces, parityPieces uint64, force bool) (*RemoteFile, error) {
	// Upload file
...
func (tn *TestNode) UploadDirectory(ld *LocalDir) (*RemoteDir, error) {
	...
	rd := &RemoteDir{
		siapath: tn.TurtleDexPath(ld.path),
	}
	return rd, nil
}
...
func (tn *TestNode) UploadNewFile(filesize int, dataPieces uint64, parityPieces uint64, force bool) (*LocalFile, *RemoteFile, error) {
	...
	remoteFile, err := tn.Upload(localFile, tn.TurtleDexPath(localFile.path), dataPieces, parityPieces, force)
	if err != nil {
...
// WaitForFileAvailable waits for a file to become available on the TurtleDex network
// (redundancy of 1).
...
func (tn *TestNode) WaitForStuckChunksToBubble() error {
	...
	return build.Retry(1000, 100*time.Millisecond, func() error {
		rd, err := tn.RenterDirGet(modules.RootTurtleDexPath())
		if err != nil {
			return err
		}
