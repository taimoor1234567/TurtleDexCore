...
var (
	...
	DefaultPaymentContractInitialFunding = types.TurtleDexcoinPrecision.Mul64(10)
...
func FundNodes(miner *TestNode, nodes map[*TestNode]struct{}) error {
	...
	scos := make([]types.TurtleDexcoinOutput, 0, uint64(len(nodes))*txnsPerNode)
	funding := wg.ConfirmedTurtleDexcoinBalance.Div64(uint64(len(nodes))).Div64(txnsPerNode + 1)
	for node := range nodes {
		...
		for i := uint64(0); i < txnsPerNode; i++ {
			scos = append(scos, types.TurtleDexcoinOutput{
				Value:      funding,
	...
	_, err = miner.WalletTurtleDexcoinsMultiPost(scos)
	if err != nil {
...
func (tg *TestGroup) setupNodes(setHosts, setNodes, setRenters map[*TestNode]struct{}) error {
	...
	for m := range tg.miners {
		...
		if wg.ConfirmedTurtleDexcoinBalance.Cmp(balance) > 0 {
			miner = m
			balance = wg.ConfirmedTurtleDexcoinBalance
		}
	}
	// Get all the nodes.
