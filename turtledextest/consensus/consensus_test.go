		...
		for _, tx := range cbhg.Transactions {
			...
			for i, fc := range tx.FileContracts {
				...
				for _, vp := range fc.ValidProofOutputs {
					txn.FileContracts[i].ValidProofOutputs = append(txn.FileContracts[i].ValidProofOutputs, types.TurtleDexcoinOutput{
						Value:      vp.Value,
						UnlockHash: vp.UnlockHash,
					})
				}
				for _, mp := range fc.MissedProofOutputs {
					txn.FileContracts[i].MissedProofOutputs = append(txn.FileContracts[i].MissedProofOutputs, types.TurtleDexcoinOutput{
						Value:      mp.Value,
						UnlockHash: mp.UnlockHash,
					})
				}
			}
			for _, sfo := range tx.TurtleDexfundOutputs {
				txn.TurtleDexfundOutputs = append(txn.TurtleDexfundOutputs, types.TurtleDexfundOutput{
					Value:      sfo.Value,
			...
			// Verify TurtleDexcoinOutput IDs
			for i, sco := range tx.TurtleDexcoinOutputs {
				if sco.ID != txn.TurtleDexcoinOutputID(uint64(i)) {
					t.Fatalf("TurtleDexcoinOutputID not as expected, got %v expected %v", sco.ID, txn.TurtleDexcoinOutputID(uint64(i)))
				}
			}
			...
			// Verify TurtleDexfundOutput IDs
			for i, sfo := range tx.TurtleDexfundOutputs {
				// Failing, switch back to !=
				if sfo.ID != txn.TurtleDexfundOutputID(uint64(i)) {
					t.Fatalf("TurtleDexfundOutputID not as expected, got %v expected %v", sfo.ID, txn.TurtleDexfundOutputID(uint64(i)))
				}
			}
		}
	}
...
func TestFoundationHardfork(t *testing.T) {
	...
	// Have the renter upload some files to TurtleDex prior to the foundation fork
	...
	if balance.Cmp(types.TurtleDexcoinPrecision.Mul64(100)) < 0 {
		t.Log(balance)
	...
	primaryWSP, err := w.WalletTurtleDexcoinsPost(types.TurtleDexcoinPrecision, foundationPrimaryAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	failsafeWSP, err := w.WalletTurtleDexcoinsPost(types.TurtleDexcoinPrecision, foundationFailsafeAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	failsafeWSP2, err := w.WalletTurtleDexcoinsPost(types.TurtleDexcoinPrecision, foundationFailsafeAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	//
	// Grab the output ids for the ttdc outputs that were sent to the
	// foundation addresses. These will be needed to create the transaction that
	// updates the subsidy ownership.
	var primaryOutputID types.TurtleDexcoinOutputID
	var failsafeOutputID types.TurtleDexcoinOutputID
	var failsafeOutputID2 types.TurtleDexcoinOutputID
	found := false
	for _, txn := range primaryWSP.Transactions {
		for i, output := range txn.TurtleDexcoinOutputs {
			if output.UnlockHash == foundationPrimaryAddress {
				found = true
				primaryOutputID = txn.TurtleDexcoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	for _, txn := range failsafeWSP.Transactions {
		for i, output := range txn.TurtleDexcoinOutputs {
			if output.UnlockHash == foundationFailsafeAddress {
				found = true
				failsafeOutputID = txn.TurtleDexcoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	for _, txn := range failsafeWSP2.Transactions {
		for i, output := range txn.TurtleDexcoinOutputs {
			if output.UnlockHash == foundationFailsafeAddress {
				found = true
				failsafeOutputID2 = txn.TurtleDexcoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		for _, txn := range tptg.Transactions {
			for _, output := range txn.TurtleDexcoinOutputs {
				if output.UnlockHash == foundationPrimaryAddress {
					primaryFound = true
					continue
				}
	...
	updateWithPrimaryTxn := types.Transaction{
		...
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{
			{Value: types.TurtleDexcoinPrecision},
		},
	...
	txn := types.Transaction{
		...
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{{
			Value:      types.InitialFoundationSubsidy.Sub(types.TurtleDexcoinPrecision),
			UnlockHash: addr,
		}},
		MinerFees: []types.Currency{
			types.TurtleDexcoinPrecision,
		},
		TransactionSignatures: make([]types.TransactionSignature, foundationPrimaryUnlockConditions.SignaturesRequired),
	}
	...
	_, err = w.WalletTurtleDexcoinsPost(types.InitialFoundationSubsidy, wag.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	waitForTxns(2)
	...
	txn = types.Transaction{
		...
		TurtleDexcoinOutputs: []types.TurtleDexcoinOutput{{
			Value:      subsidyCoins.Sub(types.TurtleDexcoinPrecision),
			UnlockHash: addr,
		}},
		MinerFees: []types.Currency{
			types.TurtleDexcoinPrecision,
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		expectedBalance := originalBalance.Add(subsidyCoins).Sub(types.TurtleDexcoinPrecision)
	...
	_, err = w.WalletTurtleDexcoinsPost(balance.Sub(types.TurtleDexcoinPrecision), wag.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	waitForTxns(2)
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		if balance.Cmp(types.TurtleDexcoinPrecision) >= 0 {
	...
	if newBalance.Cmp(balance.Add(subsidyCoins).Sub(types.TurtleDexcoinPrecision)) != 0 {
