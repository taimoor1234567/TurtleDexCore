...
func TestMinimizeTransactionSet(t *testing.T) {
	...
	var sources []types.TurtleDexcoinOutputID
	numSources := 2
	sourceSize := types.TurtleDexcoinPrecision.Mul64(1e3)
	...
	wsmp, err := minerA.WalletTurtleDexcoinsMultiPost(outputs)
	if err != nil {
		t.Fatal(err)
	}
	lastTxn := len(wsmp.Transactions) - 1
	for i := 0; i < numSources; i++ {
		sources = append(sources, wsmp.Transactions[lastTxn].TurtleDexcoinOutputID(uint64(i)))
	}
	...
	source1Index, err := graph1.AddTurtleDexcoinSource(sources[0], sourceSize)
	if err != nil {
		t.Fatal(err)
	}
	source2Index, err := graph1.AddTurtleDexcoinSource(sources[1], sourceSize)
	if err != nil {
		t.Fatal(err)
	}
	// Add txn1, which consumes src1 and produces out1
	txn1Outputs, err := graph1.AddTransaction(typesutil.SimpleTransaction{
		TurtleDexcoinInputs:  []int{source1Index},
		TurtleDexcoinOutputs: []types.Currency{types.TurtleDexcoinPrecision.Mul64(999)},
	...
	txn2Outputs, err := graph1.AddTransaction(typesutil.SimpleTransaction{
		TurtleDexcoinInputs: []int{source2Index},
		TurtleDexcoinOutputs: []types.Currency{
			types.TurtleDexcoinPrecision.Mul64(499),
			types.TurtleDexcoinPrecision.Mul64(500),
		},
	...
	_, err = graph1.AddTransaction(typesutil.SimpleTransaction{
		TurtleDexcoinInputs: []int{
			txn1Outputs[0],
			txn2Outputs[0],
		},
		TurtleDexcoinOutputs: []types.Currency{
			types.TurtleDexcoinPrecision.Mul64(998),
			types.TurtleDexcoinPrecision.Mul64(499),
		},
	...
	graph2Source, err := graph2.AddTurtleDexcoinSource(sources[0], sourceSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = graph2.AddTransaction(typesutil.SimpleTransaction{
		TurtleDexcoinInputs:  []int{graph2Source},
		TurtleDexcoinOutputs: []types.Currency{types.TurtleDexcoinPrecision.Mul64(998)},

		MinerFees: []types.Currency{types.TurtleDexcoinPrecision.Mul64(2)},
	})
	if err != nil {
		t.Fatal(err)
	}
