...
func TestTransactionFollowsMinimumValues(t *testing.T) {
	...
	txn := Transaction{
		TurtleDexcoinOutputs: []TurtleDexcoinOutput{{Value: NewCurrency64(1)}},
		FileContracts:  []FileContract{{Payout: NewCurrency64(1)}},
		TurtleDexfundOutputs: []TurtleDexfundOutput{{Value: NewCurrency64(1)}},
	...
	txn.TurtleDexcoinOutputs[0].Value = ZeroCurrency
	err = txn.followsMinimumValues()
	if !errors.Contains(err, ErrZeroOutput) {
		t.Error(err)
	}
	txn.TurtleDexcoinOutputs[0].Value = NewCurrency64(1)
	...
	txn.TurtleDexfundOutputs[0].Value = ZeroCurrency
	err = txn.followsMinimumValues()
	if !errors.Contains(err, ErrZeroOutput) {
		t.Error(err)
	}
	txn.TurtleDexfundOutputs[0].Value = NewCurrency64(1)
	...
	txn.TurtleDexfundOutputs[0].ClaimStart = NewCurrency64(1)
	err = txn.followsMinimumValues()
	if !errors.Contains(err, ErrNonZeroClaimStart) {
		t.Error(err)
	}
	txn.TurtleDexfundOutputs[0].ClaimStart = ZeroCurrency
}
...
func TestTransactionFollowsStorageProofRules(t *testing.T) {
	...
	// Try a transaction with a storage proof and a TurtleDexcoinOutput.
	txn.TurtleDexcoinOutputs = append(txn.TurtleDexcoinOutputs, TurtleDexcoinOutput{})
	err = txn.followsStorageProofRules()
	if !errors.Contains(err, ErrStorageProofWithOutputs) {
		t.Error(err)
	}
	txn.TurtleDexcoinOutputs = nil
	...
	// Try a transaction with a storage proof and a FileContractRevision.
	txn.TurtleDexfundOutputs = append(txn.TurtleDexfundOutputs, TurtleDexfundOutput{})
	err = txn.followsStorageProofRules()
	if !errors.Contains(err, ErrStorageProofWithOutputs) {
		t.Error(err)
	}
	txn.TurtleDexfundOutputs = nil
}
...
func TestTransactionNoRepeats(t *testing.T) {
	...
	// Try a transaction double spending a ttdc output.
	txn.TurtleDexcoinInputs = append(txn.TurtleDexcoinInputs, TurtleDexcoinInput{})
	err = txn.noRepeats()
	if !errors.Contains(err, ErrDoubleSpend) {
		t.Error(err)
	}
	txn.TurtleDexcoinInputs = txn.TurtleDexcoinInputs[:1]
	...
	txn.TurtleDexfundInputs = append(txn.TurtleDexfundInputs, TurtleDexfundInput{})
	err = txn.noRepeats()
	if !errors.Contains(err, ErrDoubleSpend) {
		t.Error(err)
	}
	txn.TurtleDexfundInputs = txn.TurtleDexfundInputs[:1]
}
...
func TestTransactionValidUnlockConditions(t *testing.T) {
	...
	// Try with illegal conditions in the ttdc inputs.
	txn.TurtleDexcoinInputs[0].UnlockConditions.Timelock = 5
	err = txn.validUnlockConditions(4)
	if err == nil {
		t.Error(err)
	}
	txn.TurtleDexcoinInputs[0].UnlockConditions.Timelock = 3
	...
	// Try with illegal conditions in the siafund inputs.
	txn.TurtleDexfundInputs[0].UnlockConditions.Timelock = 5
	err = txn.validUnlockConditions(4)
	if err == nil {
		t.Error(err)
	}
	txn.TurtleDexfundInputs[0].UnlockConditions.Timelock = 3
}
...
func TestTransactionStandaloneValid(t *testing.T) {
	...
	txn.TurtleDexcoinOutputs = []TurtleDexcoinOutput{{}}
	txn.TurtleDexcoinOutputs[0].Value = NewCurrency64(1)
	err = txn.StandaloneValid(0)
	if err == nil {
	...
	// Violate noRepeats
	txn.TurtleDexcoinInputs = []TurtleDexcoinInput{{}, {}}
	err = txn.StandaloneValid(0)
	if err == nil {
		t.Error("failed to trigger noRepeats error")
	}
	txn.TurtleDexcoinInputs = nil
	...
	txn.TurtleDexcoinInputs = []TurtleDexcoinInput{{}}
	txn.TurtleDexcoinInputs[0].UnlockConditions.Timelock = 1
	err = txn.StandaloneValid(0)
	if err == nil {
