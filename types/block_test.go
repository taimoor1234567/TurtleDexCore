...
func TestCalculateCoinbase(t *testing.T) {
	...
	if c.Cmp(NewCurrency64(InitialCoinbase).Mul(TurtleDexcoinPrecision)) != 0 {
		t.Error("Unexpected CalculateCoinbase result")
	}

	c = CalculateCoinbase(1)
	if c.Cmp(NewCurrency64(InitialCoinbase-1).Mul(TurtleDexcoinPrecision)) != 0 {
		t.Error("Unexpected CalculateCoinbase result")
	}

	c = CalculateCoinbase(295000)
	if c.Cmp(NewCurrency64(MinimumCoinbase).Mul(TurtleDexcoinPrecision)) != 0 {
		t.Error(c)
		t.Error(NewCurrency64(MinimumCoinbase).Mul(TurtleDexcoinPrecision))
		t.Error("Unexpected CalculateCoinbase result")
	}
...
// TestCalculateNumTurtleDexcoins checks that the ttdc calculator is correctly
// determining the number of ttdcs in circulation. The check is performed by
// doing a naive computation, instead of by doing the optimized computation.
func TestCalculateNumTurtleDexcoins(t *testing.T) {
	c := CalculateNumTurtleDexcoins(0)
	if !c.Equals(CalculateCoinbase(0).Add(numGenesisTurtleDexcoins)) {
	...
	for i := BlockHeight(0); i < 500e3; i++ {
		...
		if totalCoins.Cmp(CalculateNumTurtleDexcoins(i)) != 0 {
			t.Fatal("coin miscalculation", i, totalCoins, CalculateNumTurtleDexcoins(i))
		}
	}
}
...
func TestBlockHeader(t *testing.T) {
	...
	b.Nonce[2] = 2
	b.Timestamp = 3
	b.MinerPayouts = []TurtleDexcoinOutput{{Value: NewCurrency64(4)}}
...
func TestBlockID(t *testing.T) {
	...
	ids = append(ids, b.ID())
	b.MinerPayouts = append(b.MinerPayouts, TurtleDexcoinOutput{Value: CalculateCoinbase(0)})
	ids = append(ids, b.ID())
	b.MinerPayouts = append(b.MinerPayouts, TurtleDexcoinOutput{Value: CalculateCoinbase(0)})
	ids = append(ids, b.ID())
...
func TestBlockMinerPayoutID(t *testing.T) {
	...
	knownIDs := make(map[TurtleDexcoinOutputID]struct{})
	for i, id := range ids {
		_, exists := knownIDs[id]
		if exists {
			t.Error("id repeat for index", i)
