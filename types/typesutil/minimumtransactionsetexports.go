...
func MinimumTransactionSet(requiredTxns []types.Transaction, relatedTxns []types.Transaction) []types.Transaction {
	...
	for i := 0; i < len(relatedTxns); i++ {
		for j := range relatedTxns[i].TurtleDexcoinOutputs {
			potentialSources[objectID(relatedTxns[i].TurtleDexcoinOutputID(uint64(j)))] = &relatedTxns[i]
		}
		for j := range relatedTxns[i].FileContracts {
			potentialSources[objectID(relatedTxns[i].FileContractID(uint64(j)))] = &relatedTxns[i]
		}
		for j := range relatedTxns[i].TurtleDexfundOutputs {
			potentialSources[objectID(relatedTxns[i].TurtleDexfundOutputID(uint64(j)))] = &relatedTxns[i]
		}
	}
