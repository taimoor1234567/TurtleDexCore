...
// constants.go contains the TurtleDex constants. Depending on which build tags are
...
var (
	...
	// TurtleDexcoinInputs and TurtleDexfundInputs when calculating SigHashes to protect
	...
	// GenesisTurtleDexcoinAllocation is the set of TurtleDexcoinOutputs created in the Genesis
	// block
	GenesisTurtleDexcoinAllocation []TurtleDexcoinOutput
	// GenesisTurtleDexfundAllocation is the set of TurtleDexfundOutputs created in the Genesis
	// block.
	GenesisTurtleDexfundAllocation []TurtleDexfundOutput
	...
	// block subsidies.
	InitialFoundationSubsidy = TurtleDexcoinPrecision.Mul64(30e3).Mul64(uint64(BlocksPerYear))
	...
	// blocks, where for TurtleDex this number is typically 11.
	...
	// TurtleDex following a hardfork at block 135e3.
	...
	RootTarget Target
	// TurtleDexcoinPrecision is the number of base units in a ttdc. The TurtleDex network has a very
	// large number of base units. We call 10^24 of these a ttdc.
	//
	...
	TurtleDexcoinPrecision = NewCurrency(new(big.Int).Exp(big.NewInt(10), big.NewInt(24), nil))
	// TurtleDexfundCount is the total number of TurtleDexfunds in existence.
	TurtleDexfundCount = NewCurrency64(10000)
	// TurtleDexfundPortion is the percentage of ttdcs that is taxed from FileContracts.
	TurtleDexfundPortion = big.NewRat(39, 1000)
...
func init() {
	if build.Release == "dev" {
		...
		// Populate the void address with 1 billion ttdcs in the genesis block.
		GenesisTurtleDexcoinAllocation = []TurtleDexcoinOutput{
			{
				Value:      NewCurrency64(1000000000).Mul(TurtleDexcoinPrecision),
		...
		// classes of payouts in TurtleDex that depend on a previous block - if that
		...
		// primarily to stop difficulty raising attacks. TurtleDex's safety margin is
		// similar to Bitcoin's despite the looser clamp because TurtleDex's
		...
		// acceptable due to TurtleDex's more vulnerable position as an altcoin.
		...
		// decreases by 1 every time, it means that TurtleDex's coinbase will have an
	...
	GenesisBlock = Block{
		...
		Transactions: []Transaction{
			{TurtleDexfundOutputs: GenesisTurtleDexfundAllocation},
		},
	}
...
func GenerateDeterministicMultisig(m, n int, salt string) (UnlockConditions, []crypto.SecretKey) {
	uc := UnlockConditions{
		PublicKeys:         make([]TurtleDexPublicKey, n),
