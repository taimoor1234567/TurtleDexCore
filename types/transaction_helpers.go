...
func TransactionGraph(sourceOutput TurtleDexcoinOutputID, edges []TransactionGraphEdge) ([]Transaction, error) {
	...
	txnInputs := make([][]TurtleDexcoinOutputID, biggest+1)
	txnInputs[0] = []TurtleDexcoinOutputID{sourceOutput}
	...
	for i < len(edges) {
		...
		for _, outputID := range txnInputs[j] {
			t.TurtleDexcoinInputs = append(t.TurtleDexcoinInputs, TurtleDexcoinInput{
				ParentID: outputID,
			})
		}
		...
		for i < len(edges) && edges[i].Source == current {
			t.TurtleDexcoinOutputs = append(t.TurtleDexcoinOutputs, TurtleDexcoinOutput{
				Value:      edges[i].Value,
		...
		for k := startingPoint; k < i; k++ {
			txnInputs[edges[k].Dest] = append(txnInputs[edges[k].Dest], t.TurtleDexcoinOutputID(uint64(k-startingPoint)))
		}
		ts[j] = t
		j++
	}
