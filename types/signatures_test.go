...
// TestTurtleDexPublicKeyEquals tests the functionality of the implementation of
// Equals on a TurtleDexPublicKey
func TestTurtleDexPublicKeyEquals(t *testing.T) {
	var x, y TurtleDexPublicKey
	key := func() []byte { return fastrand.Bytes(32) }
	spk := func(algo Specifier, key []byte) TurtleDexPublicKey {
		return TurtleDexPublicKey{
			Algorithm: algo,
			Key:       key,
		}
	}
...
// TestTurtleDexPublicKeyToPublicKey tests the functionality of
// 'ToPublicKey' on the TurtleDexPublicKey object
func TestTurtleDexPublicKeyToPublicKey(t *testing.T) {
	// randomSPK returns a random TurtleDexPublicKey with specified key length
	randomSPK := func(keyLength int) TurtleDexPublicKey {
		return TurtleDexPublicKey{
	...
	if !bytes.Equal(cpk[:], spk.Key[:]) {
		t.Fatal("Expected crytpo.PublicKey to equal the Key in TurtleDexPublicKey")
	}
	...
	defer func() {
		if r := recover(); r == nil {
			t.Error("Converting a TurtleDexPublicKey, with an incorrect key length, to a crypto.PublicKey did not cause a panic")
		}
	}()
	_ = spk.ToPublicKey()
}
		...
		TransactionSignatures: []TransactionSignature{
			...
			},
			{
				CoveredFields: CoveredFields{
					...
					TurtleDexcoinOutputs:        []uint64{0},
					FileContracts:         []uint64{0},
					FileContractRevisions: []uint64{0},
					StorageProofs:         []uint64{0},
					TurtleDexfundInputs:         []uint64{0},
					TurtleDexfundOutputs:        []uint64{0},
					MinerFees:             []uint64{0},
...
func TestTransactionValidCoveredFields(t *testing.T) {
	...
	// Add ttdc output coverage to the first signature. This should violate
	// rules, as the fields are not allowed to be set when 'WholeTransaction'
	// is set.
	txn.TransactionSignatures[0].CoveredFields.TurtleDexcoinOutputs = []uint64{0}
	err = txn.validCoveredFields()
	...
	txn.TransactionSignatures[0].CoveredFields.TurtleDexcoinOutputs = nil
...
func TestTransactionValidSignatures(t *testing.T) {
	...
	txn.TurtleDexfundInputs[0].ParentID[0] = 2         // can't overlap with other objects
	...
	txn.TransactionSignatures[0].CoveredFields.TurtleDexcoinInputs = []uint64{33}
	err = txn.validSignatures(10)
	if err == nil {
		t.Error("failed to flunk the validCoveredFields check")
	}
	txn.TransactionSignatures[0].CoveredFields.TurtleDexcoinInputs = nil

	// Double spend a TurtleDexcoinInput, FileContractTermination, and TurtleDexfundInput.
	txn.TurtleDexcoinInputs = append(txn.TurtleDexcoinInputs, TurtleDexcoinInput{UnlockConditions: UnlockConditions{}})
	err = txn.validSignatures(10)
	if err == nil {
		t.Error("failed to double spend a ttdc input")
	}
	txn.TurtleDexcoinInputs = txn.TurtleDexcoinInputs[:len(txn.TurtleDexcoinInputs)-1]
	...
	txn.TurtleDexfundInputs = append(txn.TurtleDexfundInputs, TurtleDexfundInput{UnlockConditions: UnlockConditions{}})
	err = txn.validSignatures(10)
	if err == nil {
		t.Error("failed to double spend a siafund input")
	}
	txn.TurtleDexfundInputs = txn.TurtleDexfundInputs[:len(txn.TurtleDexfundInputs)-1]
	...
	txn.TurtleDexcoinInputs[0].UnlockConditions.PublicKeys[0].Key = []byte{'b', 'a', 'd'}
	err = txn.validSignatures(10)
	if err == nil {
		t.Error(err)
	}
	txn.TurtleDexcoinInputs[0].UnlockConditions.PublicKeys[0].Key = pk[:]
