...
var heavyBlock = func() Block {
	b := Block{
		MinerPayouts: []TurtleDexcoinOutput{
			{Value: CalculateCoinbase(0)},
			{Value: CalculateCoinbase(1)},
		},
		Transactions: []Transaction{
			{
				TurtleDexcoinInputs: []TurtleDexcoinInput{{
					UnlockConditions: UnlockConditions{
						PublicKeys: []TurtleDexPublicKey{{
				...
				FileContracts: []FileContract{{
					...
					RevisionNumber: 8,
					ValidProofOutputs: []TurtleDexcoinOutput{{
						Value: NewCurrency64(2),
					}},
					MissedProofOutputs: []TurtleDexcoinOutput{{
				...
				FileContractRevisions: []FileContractRevision{{
					...
					NewValidProofOutputs: []TurtleDexcoinOutput{{
						Value: NewCurrency64(4),
					}},
					NewMissedProofOutputs: []TurtleDexcoinOutput{{
				...
				TurtleDexfundInputs: []TurtleDexfundInput{{
					UnlockConditions: UnlockConditions{
						PublicKeys: []TurtleDexPublicKey{{
							Algorithm: SignatureEd25519,
							Key:       fastrand.Bytes(32),
				...
				TransactionSignatures: []TransactionSignature{{
					...
					CoveredFields: CoveredFields{
						...
						TurtleDexcoinInputs:         []uint64{0},
						TurtleDexcoinOutputs:        []uint64{1},
						FileContracts:         []uint64{2},
						FileContractRevisions: []uint64{3},
						StorageProofs:         []uint64{4},
						TurtleDexfundInputs:         []uint64{5},
						TurtleDexfundOutputs:        []uint64{6},
						MinerFees:             []uint64{7},
	...
	fastrand.Read(b.Transactions[0].TurtleDexcoinInputs[0].ParentID[:])
	fastrand.Read(b.Transactions[0].TurtleDexcoinOutputs[0].UnlockHash[:])
	...
	fastrand.Read(b.Transactions[0].TurtleDexfundInputs[0].ParentID[:])
	fastrand.Read(b.Transactions[0].TurtleDexfundInputs[0].ClaimUnlockHash[:])
	fastrand.Read(b.Transactions[0].TurtleDexfundOutputs[0].UnlockHash[:])
	fastrand.Read(b.Transactions[0].TransactionSignatures[0].ParentID[:])
	return b
}()

// TestBlockEncodes probes the MarshalTurtleDex and UnmarshalTurtleDex methods of the
// Block type.
...
// TestCurrencyMarshalTurtleDex probes the MarshalTurtleDex and UnmarshalTurtleDex functions of
// the currency type.
func TestCurrencyMarshalTurtleDex(t *testing.T) {
	c := NewCurrency64(1656)
	buf := new(bytes.Buffer)
	err := c.MarshalTurtleDex(buf)
	if err != nil {
		t.Fatal(err)
	}
	var cUmar Currency
	cUmar.UnmarshalTurtleDex(buf)
	if c.Cmp(cUmar) != 0 {
...
func TestCurrencyUnsafeDecode(t *testing.T) {
	// Scan
	backup := TurtleDexcoinPrecision.Mul64(1)
	c := TurtleDexcoinPrecision
	_, err := fmt.Sscan("7", &c)
	if err != nil {
		t.Error(err)
	} else if !TurtleDexcoinPrecision.Equals(backup) {
		t.Errorf("Scan changed value of TurtleDexcoinPrecision: %v -> %v", backup, TurtleDexcoinPrecision)
	}
	...
	if err != nil {
		...
		t.Errorf("UnmarshalTurtleDex changed value of TurtleDexcoinPrecision: %v -> %v", backup, TurtleDexcoinPrecision)
	}
}
...
// TestTurtleDexcoinInputEncoding tests that optimizations applied to the encoding
// of the TurtleDexcoinInput type do not change its encoding.
func TestTurtleDexcoinInputEncoding(t *testing.T) {
	var sci TurtleDexcoinInput
	if h := hashStr(sci); h != "2f806f905436dc7c5079ad8062467266e225d8110a3c58d17628d609cb1c99d0" {
		t.Error("encoding mismatch:", h)
	}

	sci = TurtleDexcoinInput{
		ParentID:         TurtleDexcoinOutputID{1, 2, 3},
		UnlockConditions: UnlockConditions{},
	}
...
// TestTurtleDexcoinOutputEncoding tests that optimizations applied to the encoding
// of the TurtleDexcoinOutput type do not change its encoding.
func TestTurtleDexcoinOutputEncoding(t *testing.T) {
	var sco TurtleDexcoinOutput
	if h := hashStr(sco); h != "4a1931803561f431decab002e7425f0a8531d5e456a1a47fd9998a2530c0f800" {
		t.Error("encoding mismatch:", h)
	}

	sco = TurtleDexcoinOutput{
...
// TestTurtleDexfundInputEncoding tests that optimizations applied to the encoding
// of the TurtleDexfundInput type do not change its encoding.
func TestTurtleDexfundInputEncoding(t *testing.T) {
	var sci TurtleDexfundInput
	if h := hashStr(sci); h != "978a948b1a92bcddcea382bafc7718a25f8cc49b8fb11db5d9159afa960cf70a" {
		t.Error("encoding mismatch:", h)
	}

	sci = TurtleDexfundInput{
		ParentID:         TurtleDexfundOutputID{1, 2, 3},
...
// TestTurtleDexfundOutputEncoding tests that optimizations applied to the encoding
// of the TurtleDexfundOutput type do not change its encoding.
func TestTurtleDexfundOutputEncoding(t *testing.T) {
	var sco TurtleDexfundOutput
	if h := hashStr(sco); h != "df69a516de12056d0895fdea7a0274c5aba67091543238670513104c1af69c1f" {
		t.Error("encoding mismatch:", h)
	}

	sco = TurtleDexfundOutput{
		Value:      NewCurrency64(0),
		UnlockHash: UnlockHash{1, 2, 3},
...
// TestTurtleDexPublicKeyEncoding tests that optimizations applied to the encoding
// of the TurtleDexPublicKey type do not change its encoding.
func TestTurtleDexPublicKeyEncoding(t *testing.T) {
	var spk TurtleDexPublicKey
	if h := hashStr(spk); h != "19ea4a516c66775ea1f648d71f6b8fa227e8b0c1a0c9203f82c33b89c4e759b5" {
		t.Error("encoding mismatch:", h)
	}

	spk = TurtleDexPublicKey{
...
// TestTurtleDexPublicKeyLoadString checks that the LoadString method is the proper
// inverse of the String() method, also checks that there are no stupid panics
// or severe errors.
func TestTurtleDexPublicKeyLoadString(t *testing.T) {
	spk := TurtleDexPublicKey{
		Algorithm: SignatureEd25519,
		Key:       fastrand.Bytes(32),
	}
...
// TestTurtleDexPublicKeyString does a quick check to verify that the String method
// on the TurtleDexPublicKey is producing the expected output.
func TestTurtleDexPublicKeyString(t *testing.T) {
	spk := TurtleDexPublicKey{
		Algorithm: SignatureEd25519,
		Key:       make([]byte, 32),
	}
...
// TestTurtleDexPublicKeyShortString does a quick check to verify that the ShortString
// method on the TurtleDexPublicKey is producing the expected output.
func TestTurtleDexPublicKeyShortString(t *testing.T) {
	var spk TurtleDexPublicKey
	if spk.ShortString() != "" {
...
// TestTurtleDexPublicKeyUnmarshalJSON checks that UnmarshalJSON supports both
// encodings of TurtleDexPublicKey.
func TestTurtleDexPublicKeyUnmarshalJSON(t *testing.T) {
	js1 := `{ "algorithm": "ed25519", "key": "5GhilFqVBKtSCedCZc6TIthzxvyBH9gPqqf+Z9hsfBo=" }`
	var spk1 TurtleDexPublicKey
	if err := json.Unmarshal([]byte(js1), &spk1); err != nil {
		t.Error(err)
	}

	js2 := `"ed25519:e46862945a9504ab5209e74265ce9322d873c6fc811fd80faaa7fe67d86c7c1a"`
	var spk2 TurtleDexPublicKey
...
func TestCurrencyUnits(t *testing.T) {
	tests := []struct {
		...
		{NewCurrency64(1).Mul(TurtleDexcoinPrecision), "1 SC"},
		{NewCurrency64(1000).Mul(TurtleDexcoinPrecision), "1 KS"},
		{NewCurrency64(1000000).Mul(TurtleDexcoinPrecision), "1 MS"},
		{NewCurrency64(1000000000).Mul(TurtleDexcoinPrecision), "1 GS"},
		{NewCurrency64(1000000000000).Mul(TurtleDexcoinPrecision), "1 TS"},
		{NewCurrency64(1234560000000).Mul(TurtleDexcoinPrecision), "1.235 TS"},
		{NewCurrency64(1234560000000000).Mul(TurtleDexcoinPrecision), "1235 TS"},
	}
...
// TestTransactionMarshalTurtleDexSize tests that the txn.MarshalTurtleDexSize method is
// always consistent with len(encoding.Marshal(txn)).
func TestTransactionMarshalTurtleDexSize(t *testing.T) {
	txn := Transaction{
		TurtleDexcoinInputs:         []TurtleDexcoinInput{{}},
		TurtleDexcoinOutputs:        []TurtleDexcoinOutput{{}},
		FileContracts:         []FileContract{{}},
	...
	if txn.MarshalTurtleDexSize() != len(encoding.Marshal(txn)) {
		t.Errorf("sizes do not match: expected %v, got %v", len(encoding.Marshal(txn)), txn.MarshalTurtleDexSize())
	}
}
