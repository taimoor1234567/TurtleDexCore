...
type (
	...
	// is sent to the siafund pool, which is a set of ttdcs only spendable
	// by siafund owners.
	//
	...
	FileContract struct {
		...
		ValidProofOutputs  []TurtleDexcoinOutput `json:"validproofoutputs"`
		MissedProofOutputs []TurtleDexcoinOutput `json:"missedproofoutputs"`
	...
	FileContractRevision struct {
		...
		NewValidProofOutputs  []TurtleDexcoinOutput `json:"newvalidproofoutputs"`
		NewMissedProofOutputs []TurtleDexcoinOutput `json:"newmissedproofoutputs"`
	...
	//
	// A transaction with a StorageProof cannot have any TurtleDexcoinOutputs,
	// TurtleDexfundOutputs, or FileContracts. This is because a mundane reorg can
...
func (fcr FileContractRevision) HostPublicKey() TurtleDexPublicKey {
...
func (fcr FileContractRevision) PaymentRevision(amount Currency) (FileContractRevision, error) {
	...
	rev.NewValidProofOutputs = append([]TurtleDexcoinOutput{}, fcr.NewValidProofOutputs...)
	rev.NewMissedProofOutputs = append([]TurtleDexcoinOutput{}, fcr.NewMissedProofOutputs...)
...
func (fcr FileContractRevision) ExecuteProgramRevision(revisionNumber uint64, transfer Currency, newRoot crypto.Hash, newSize uint64) (FileContractRevision, error) {
	...
	// need to manually copy slice memory
	newRevision.NewValidProofOutputs = append([]TurtleDexcoinOutput{}, fcr.NewValidProofOutputs...)
	newRevision.NewMissedProofOutputs = append([]TurtleDexcoinOutput{}, fcr.NewMissedProofOutputs...)
...
func (fc FileContract) ValidRenterOutput() TurtleDexcoinOutput {
...
func (fc FileContract) ValidHostOutput() TurtleDexcoinOutput {
...
func (fc FileContract) MissedRenterOutput() TurtleDexcoinOutput {
...
func (fc FileContract) MissedHostOutput() TurtleDexcoinOutput {
	return fc.MissedProofOutputs[1]
}

// MissedVoidOutput gets the void's missed proof output.
func (fc FileContract) MissedVoidOutput() (TurtleDexcoinOutput, error) {
	if len(fc.MissedProofOutputs) <= 2 {
		return TurtleDexcoinOutput{}, ErrMissingVoidOutput
	}
...
// ValidRenterOutput gets the renter's valid proof output.
func (fcr FileContractRevision) ValidRenterOutput() TurtleDexcoinOutput {
...
// ValidHostOutput sets gets host's missed proof output.
func (fcr FileContractRevision) ValidHostOutput() TurtleDexcoinOutput {
...
// MissedRenterOutput gets the renter's missed proof output.
func (fcr FileContractRevision) MissedRenterOutput() TurtleDexcoinOutput {
...
// MissedHostOutput gets the host's missed proof output.
func (fcr FileContractRevision) MissedHostOutput() TurtleDexcoinOutput {
...
// MissedVoidOutput gets the void's missed proof output.
func (fcr FileContractRevision) MissedVoidOutput() (TurtleDexcoinOutput, error) {
	if len(fcr.NewMissedProofOutputs) <= 2 {
		return TurtleDexcoinOutput{}, ErrMissingVoidOutput
	}
...
func (fcid FileContractID) StorageProofOutputID(proofStatus ProofStatus, i uint64) TurtleDexcoinOutputID {
	return TurtleDexcoinOutputID(crypto.HashAll(
...
func Tax(height BlockHeight, payout Currency) Currency {
	...
	if height < TaxHardforkHeight {
		return payout.MulFloat(0.039).RoundDown(TurtleDexfundCount)
	}
	return payout.MulTax().RoundDown(TurtleDexfundCount)
}
