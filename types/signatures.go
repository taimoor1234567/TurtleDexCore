...
type (
	...
	CoveredFields struct {
		...
		TurtleDexcoinInputs         []uint64 `json:"ttdcinputs"`
		TurtleDexcoinOutputs        []uint64 `json:"ttdcoutputs"`
		...
		TurtleDexfundInputs         []uint64 `json:"siafundinputs"`
		TurtleDexfundOutputs        []uint64 `json:"siafundoutputs"`
	...
	// A TurtleDexPublicKey is a public key prefixed by a Specifier. The Specifier
	...
	// are three types that use UnlockConditions: TurtleDexcoinInputs, TurtleDexfundInputs,
	...
	// certain actions, such as spending a TurtleDexcoinOutput or terminating a
	// FileContract.
	//
	...
	UnlockConditions struct {
		Timelock           BlockHeight    `json:"timelock"`
		PublicKeys         []TurtleDexPublicKey `json:"publickeys"`
	...
	inputSignatures struct {
		...
		possibleKeys        []TurtleDexPublicKey
		usedKeys            map[uint64]struct{}
		index               int
	}
)

// Ed25519PublicKey returns pk as a TurtleDexPublicKey, denoting its algorithm as
// Ed25519.
func Ed25519PublicKey(pk crypto.PublicKey) TurtleDexPublicKey {
...
// Equals compares two TurtleDexPublicKey types for equality
func (x TurtleDexPublicKey) Equals(y TurtleDexPublicKey) bool {
	return x.Algorithm == y.Algorithm && bytes.Equal(x.Key, y.Key)
}

// ToPublicKey converts the TurtleDexPublicKey into a crypto.PublicKey
func (x TurtleDexPublicKey) ToPublicKey() (pk crypto.PublicKey) {
	// sanity check key length
	if len(pk) != len(x.Key) {
		build.Critical(fmt.Sprintf("Could not convert TurtleDexPublicKey to crypto.PublicKey, incorrect key length %v != %v", len(x.Key), len(pk)))
	}
	copy(pk[:], x.Key)
	return
}
...
func (t *Transaction) wholeSigHash(sig TransactionSignature, height BlockHeight) (hash crypto.Hash) {
	...
	for _, i := range sig.CoveredFields.TransactionSignatures {
		t.TransactionSignatures[i].MarshalTurtleDex(h)
	}
...
func (t *Transaction) partialSigHash(cf CoveredFields, height BlockHeight) (hash crypto.Hash) {
	...
	for _, input := range cf.TurtleDexcoinInputs {
		h.Write(replayPrefix(height))
		t.TurtleDexcoinInputs[input].MarshalTurtleDex(h)
	}
	for _, output := range cf.TurtleDexcoinOutputs {
		t.TurtleDexcoinOutputs[output].MarshalTurtleDex(h)
	}
	for _, contract := range cf.FileContracts {
		t.FileContracts[contract].MarshalTurtleDex(h)
	}
	for _, revision := range cf.FileContractRevisions {
		t.FileContractRevisions[revision].MarshalTurtleDex(h)
	}
	for _, storageProof := range cf.StorageProofs {
		t.StorageProofs[storageProof].MarshalTurtleDex(h)
	}
	for _, siafundInput := range cf.TurtleDexfundInputs {
		h.Write(replayPrefix(height))
		t.TurtleDexfundInputs[siafundInput].MarshalTurtleDex(h)
	}
	for _, siafundOutput := range cf.TurtleDexfundOutputs {
		t.TurtleDexfundOutputs[siafundOutput].MarshalTurtleDex(h)
	}
	for _, minerFee := range cf.MinerFees {
		t.MinerFees[minerFee].MarshalTurtleDex(h)
	}
	...
	for _, sig := range cf.TransactionSignatures {
		t.TransactionSignatures[sig].MarshalTurtleDex(h)
	}
...
func (t Transaction) validCoveredFields() error {
	for _, sig := range t.TransactionSignatures {
		...
		fieldMaxs := []struct {
			...
			{cf.TurtleDexcoinInputs, len(t.TurtleDexcoinInputs)},
			{cf.TurtleDexcoinOutputs, len(t.TurtleDexcoinOutputs)},
			...
			{cf.TurtleDexfundInputs, len(t.TurtleDexfundInputs)},
			{cf.TurtleDexfundOutputs, len(t.TurtleDexfundOutputs)},
