...
// block.go defines the Block type for TurtleDex, and provides some helper functions
...
type (
	...
	Block struct {
		...
		MinerPayouts []TurtleDexcoinOutput `json:"minerpayouts"`
...
// equation is:
//
//     coinbase := max(InitialCoinbase - height, MinimumCoinbase) * TurtleDexcoinPrecision
func CalculateCoinbase(height BlockHeight) Currency {
	...
	return NewCurrency64(base).Mul(TurtleDexcoinPrecision)
}

// CalculateNumTurtleDexcoins calculates the number of ttdcs in circulation at a
// given height.
func CalculateNumTurtleDexcoins(height BlockHeight) (total Currency) {
	total = numGenesisTurtleDexcoins
	deflationBlocks := BlockHeight(InitialCoinbase - MinimumCoinbase)
	avgDeflationTurtleDexcoins := CalculateCoinbase(0).Add(CalculateCoinbase(height)).Div64(2)
	if height <= deflationBlocks {
		total = total.Add(avgDeflationTurtleDexcoins.Mul64(uint64(height + 1)))
	} else {
		total = total.Add(avgDeflationTurtleDexcoins.Mul(NewCurrency64(uint64(deflationBlocks + 1))))
...
var numGenesisTurtleDexcoins = func() Currency {
	// Sum all the values for the genesis ttdc outputs.
	numGenesisTurtleDexcoins := NewCurrency64(0)
	for _, transaction := range GenesisBlock.Transactions {
		for _, ttdcOutput := range transaction.TurtleDexcoinOutputs {
			numGenesisTurtleDexcoins = numGenesisTurtleDexcoins.Add(ttdcOutput.Value)
		}
	}
	return numGenesisTurtleDexcoins
}()
...
func (b Block) MerkleRoot() crypto.Hash {
	...
	for _, payout := range b.MinerPayouts {
		payout.MarshalTurtleDex(e)
		tree.Push(buf.Bytes())
		buf.Reset()
	}
	for _, txn := range b.Transactions {
		txn.MarshalTurtleDex(e)
		tree.Push(buf.Bytes())
		buf.Reset()
	}
	return tree.Root()
...
// index.
func (b Block) MinerPayoutID(i uint64) TurtleDexcoinOutputID {
...
// SpecifierFoundation.
func (bid BlockID) FoundationSubsidyID() TurtleDexcoinOutputID {
	return TurtleDexcoinOutputID(crypto.HashAll(bid, SpecifierFoundation))
}
