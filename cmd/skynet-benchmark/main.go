...
func main() {
	...
	// Determine which port to use when talking to ttdxd.
	args := os.Args
	addr := "localhost:9980"
	var password string
	var cmd string
	if len(args) == 1 {
		...
		fmt.Println("Usage: ./skynet-benchmark [optional: which test to run] [optional: endpoint for ttdxd api, defaults to \"localhost:9980\"] [optional: api password]\n\tTest options: 'dl' and 'basic'")
		return
	}

	// Create the client that will be used to talk to ttdxd.
	opts, err := client.DefaultOptions()
	if err != nil {
		fmt.Println("Unable to get TurtleDex client options:", err)
		return
	}
	opts.Address = addr
	if password != "" {
