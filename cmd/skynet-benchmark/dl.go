...
const (
	// The TurtleDexPath that will be used by the program to upload and store all of
	// the files when performing test downloads.
	testTurtleDexDirDL = "var/skynet-benchmark-dl"
...
func dl() {
	...
	dirBasePath, err := modules.NewTurtleDexPath(testTurtleDexDirDL)
	if err != nil {
	...
	paths := make(map[string]modules.TurtleDexPath)
...
// a dir.
func downloadFileSet(dir modules.TurtleDexPath, fileSize int, threads uint64) (stats.Float64Data, error) {
	now := time.Now()
...
// have skylinks
func getMissingFiles(dir modules.TurtleDexPath, expectedFileSize uint64, expectedFetchSize uint64) (map[int]struct{}, error) {
	...
	for _, file := range rdg.Files {
		...
		cleanName := strings.TrimSuffix(file.TurtleDexPath.Name(), "-extended")
...
// that already exist.
func uploadFileSet(dir modules.TurtleDexPath, fileSize uint64, expectedFetchSize uint64) error {
