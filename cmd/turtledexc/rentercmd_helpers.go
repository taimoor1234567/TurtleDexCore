...
func (s byDirectoryInfo) Less(i, j int) bool {
	return s[i].dir.TurtleDexPath.String() < s[j].dir.TurtleDexPath.String()
}

// byTurtleDexPathFile implements sort.Interface for [] modules.FileInfo based on the
// TurtleDexPath field.
type byTurtleDexPathFile []modules.FileInfo

func (s byTurtleDexPathFile) Len() int           { return len(s) }
func (s byTurtleDexPathFile) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s byTurtleDexPathFile) Less(i, j int) bool { return s[i].TurtleDexPath.String() < s[j].TurtleDexPath.String() }

// byTurtleDexPathDir implements sort.Interface for [] modules.DirectoryInfo based on the
// TurtleDexPath field.
type byTurtleDexPathDir []modules.DirectoryInfo

func (s byTurtleDexPathDir) Len() int           { return len(s) }
func (s byTurtleDexPathDir) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s byTurtleDexPathDir) Less(i, j int) bool { return s[i].TurtleDexPath.String() < s[j].TurtleDexPath.String() }

// byValue sorts contracts by their value in ttdcs, high to low. If two
...
// into a single error.
func downloadDir(siaPath modules.TurtleDexPath, destination string) (tfs []trackedFile, skipped []string, totalSize uint64, err error) {
	// Get dir info.
	...
	for _, file := range rd.Files {
		...
		dst := filepath.Join(destination, file.TurtleDexPath.Name())
		...
		_, err = httpClient.RenterDownloadFullGet(file.TurtleDexPath, dst, true, true)
		if err != nil {
	...
	for i := 1; i < len(rd.Directories); i++ {
		...
		rtfs, rskipped, totalSubSize, rerr := downloadDir(subDir.TurtleDexPath, filepath.Join(destination, subDir.TurtleDexPath.Name()))
		tfs = append(tfs, rtfs...)
...
func downloadProgress(tfs []trackedFile) []api.DownloadInfo {
	...
	tfsMap := make(map[modules.TurtleDexPath]trackedFile)
	measurements := make(map[modules.TurtleDexPath][]progressMeasurement)
	for _, tf := range tfs {
	...
	for range time.Tick(OutputRefreshRate) {
		...
		for _, d := range rdg.Downloads {
			key := d.TurtleDexPath.String() + d.Destination
	...
	for _, dir := range dirs {
		for _, file := range dir.files {
			...
			default:
				unrecoverable++
				if printLostFiles {
					fmt.Println(file.TurtleDexPath)
				}
			}
		}
	}
...
func getDir(siaPath modules.TurtleDexPath, root, recursive bool) (dirs []directoryInfo) {
	...
	for _, subDir := range subDirs {
		rdirs := getDir(subDir.TurtleDexPath, root, recursive)
...
// this also get called by 'ttdxc renter -v' which is why it's in its own
// function
...
func renterFilesAndContractSummary() error {
	rf, err := httpClient.RenterDirRootGet(modules.RootTurtleDexPath())
...
// renterFilesDownload downloads the file at the specified path from the TurtleDex
