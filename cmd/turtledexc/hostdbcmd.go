...
// hostdbcmd is the handler for the command `ttdxc hostdb`.
...
// hostdbfiltermodecmd is the handler for the command `ttdxc hostdb
// filtermode`.
...
// hostdbsetfiltermodecmd is the handler for the command `ttdxc hostdb
...
func hostdbsetfiltermodecmd(cmd *cobra.Command, args []string) {
	...
	var hosts []types.TurtleDexPublicKey
	switch len(args) {
	case 0:
		_ = cmd.UsageFunc()(cmd)
...
// hostdbviewcmd is the handler for the command `ttdxc hostdb view`.
// shows detailed information about a host in the hostdb.
func hostdbviewcmd(pubkey string) {
	var publicKey types.TurtleDexPublicKey
