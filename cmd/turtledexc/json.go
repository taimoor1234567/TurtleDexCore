...
var (
	jsonCmd = &cobra.Command{
		Use:   "json",
		Short: "provide a json dump of ttdxd's current status",
		Long:  "queries a large number of endpoints in the ttdxd api and produces a json dump with all of the information",
		Run:   wrap(jsoncmd),
	}
)

// jsoncmd queries a large number of endpoints in the ttdxd api and aggregates
...
func jsoncmd() {
	...
	rs.TotalTurtleDexfiles = rf.Directories[0].AggregateNumFiles
	rs.TotalTurtleDexdirs = rf.Directories[0].AggregateNumSubDirs
	...
	rs.WalletFunds = wg.ConfirmedTurtleDexcoinBalance.Add(wg.UnconfirmedIncomingTurtleDexcoins).Sub(wg.UnconfirmedOutgoingTurtleDexcoins)
