...
var (
	...
	walletBalanceCmd = &cobra.Command{
		...
		Long:  "View wallet balance, including confirmed and unconfirmed ttdcs and siafunds.",
...
Units:
The smallest unit of ttdcs is the hasting. One ttdc is 10^24 hastings. Other supported units are:
  pS (pico,  10^-12 SC)
  nS (nano,  10^-9 SC)
  uS (micro, 10^-6 SC)
  mS (milli, 10^-3 SC)
...
By default the wallet encryption / unlock password is the same as the generated seed.`,
	...
	walletSendCmd = &cobra.Command{
		Use:   "send",
		Short: "Send either ttdcs or siafunds to an address",
		Long:  "Send either ttdcs or siafunds to an address",
		// Run field is not set, as the send command itself is not a valid command.
		// A subcommand must be provided.
	}

	walletSendTurtleDexcoinsCmd = &cobra.Command{
		Use:   "ttdcs [amount] [dest]",
		Short: "Send ttdcs to an address",
		Long: `Send ttdcs to an address. 'dest' must be a 76-byte hexadecimal address.
...
A dynamic transaction fee is applied depending on the size of the transaction and how busy the network is.`,
		Run: wrap(walletsendttdcscmd),
	}

	walletSendTurtleDexfundsCmd = &cobra.Command{
		Use:   "siafunds [amount] [dest]",
		Short: "Send siafunds",
		Long: `Send siafunds to an address, and transfer the claim ttdcs to your wallet.
Run 'wallet send --help' to see a list of available units.`,
	...
	walletSignCmd = &cobra.Command{
		...
		Long: `Sign a transaction. If ttdxd is running with an unlocked wallet, the
...
provided, the wallet will fill in every TransactionSignature it has keys for.`,
	...
	walletSweepCmd = &cobra.Command{
		Use:   "sweep",
		Short: "Sweep ttdcs and siafunds from a seed.",
		Long: `Sweep ttdcs and siafunds from a seed. The outputs belonging to the seed
will be sent to your wallet.`,
	...
	walletTransactionsCmd = &cobra.Command{
		...
		Long:  "View transactions related to addresses spendable by the wallet, providing a net flow of ttdcs and siafunds for each transaction",
...
func walletloadsiagcmd(keyfiles string) {
	...
	err = httpClient.WalletTurtleDexgKeyPost(keyfiles, password)
	if err != nil {
...
// walletsendttdcscmd sends ttdcs to a destination address.
func walletsendttdcscmd(amount, dest string) {
	...
	_, err = httpClient.WalletTurtleDexcoinsPost(value, hash, walletTxnFeeIncluded)
	if err != nil {
		die("Could not send ttdcs:", err)
	}
...
func walletsendsiafundscmd(amount, dest string) {
	...
	_, err := httpClient.WalletTurtleDexfundsPost(value, hash)
	if err != nil {
...
`, encStatus)
	...
	unconfirmedBalance := status.ConfirmedTurtleDexcoinBalance.Add(status.UnconfirmedIncomingTurtleDexcoins).Sub(status.UnconfirmedOutgoingTurtleDexcoins)
	var delta string
	if unconfirmedBalance.Cmp(status.ConfirmedTurtleDexcoinBalance) >= 0 {
		delta = "+" + currencyUnits(unconfirmedBalance.Sub(status.ConfirmedTurtleDexcoinBalance))
	} else {
		delta = "-" + currencyUnits(status.ConfirmedTurtleDexcoinBalance.Sub(unconfirmedBalance))
	}
...
Exact:               %v H
TurtleDexfunds:            %v SF
TurtleDexfund Claims:      %v H

Estimated Fee:       %v / KB
`, encStatus, status.Height, currencyUnits(status.ConfirmedTurtleDexcoinBalance), delta,
		status.ConfirmedTurtleDexcoinBalance, status.TurtleDexfundBalance, status.TurtleDexcoinClaimBalance,
...
func walletsigncmd(cmd *cobra.Command, args []string) {
	...
	if err == nil {
		...
		// if ttdxd is running, but the wallet is locked, assume the user
		// wanted to sign with ttdxd
		if strings.Contains(err.Error(), modules.ErrLockedWallet.Error()) {
			die("Signing via API failed: ttdxd is running, but the wallet is locked.")
		}

		// ttdxd is not running; fallback to offline keygen
...
// transactions without ttdxd.
func walletsigncmdoffline(txn *types.Transaction, toSign []crypto.Hash) {
	fmt.Println("Enter your wallet seed to generate the signing key(s) now and sign without ttdxd.")
...
// providing a net flow of ttdcs and siafunds for each.
func wallettransactionscmd() {
	...
	fmt.Println("             [timestamp]    [height]                                                   [transaction id]    [net ttdcs]   [net siafunds]")
	...
	for _, txn := range sts {
		// Determine the number of outgoing ttdcs and siafunds.
		var outgoingTurtleDexfunds types.Currency
		for _, input := range txn.Inputs {
			if input.FundType == types.SpecifierTurtleDexfundInput && input.WalletAddress {
				outgoingTurtleDexfunds = outgoingTurtleDexfunds.Add(input.Value)
			}
		}

		// Determine the number of incoming ttdcs and siafunds.
		var incomingTurtleDexfunds types.Currency
		for _, output := range txn.Outputs {
			if output.FundType == types.SpecifierTurtleDexfundOutput && output.WalletAddress {
				incomingTurtleDexfunds = incomingTurtleDexfunds.Add(output.Value)
			}
		}

		// Convert the ttdcs to a float.
		incomingTurtleDexcoinsFloat, _ := new(big.Rat).SetFrac(txn.ConfirmedIncomingValue.Big(), types.TurtleDexcoinPrecision.Big()).Float64()
		outgoingTurtleDexcoinsFloat, _ := new(big.Rat).SetFrac(txn.ConfirmedOutgoingValue.Big(), types.TurtleDexcoinPrecision.Big()).Float64()
		...
		fmt.Printf("%67v%15.2f SC", txn.TransactionID, incomingTurtleDexcoinsFloat-outgoingTurtleDexcoinsFloat)
		// For siafunds, need to avoid having a negative types.Currency.
		if incomingTurtleDexfunds.Cmp(outgoingTurtleDexfunds) >= 0 {
			fmt.Printf("%14v SF\n", incomingTurtleDexfunds.Sub(outgoingTurtleDexfunds))
		} else {
			fmt.Printf("-%14v SF\n", outgoingTurtleDexfunds.Sub(incomingTurtleDexfunds))
		}
	}
}
