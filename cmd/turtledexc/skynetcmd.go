...
since the list returned from 'ttdxc skynet blocklist' is a list of hashes of the skylinks' 
...
get uploaded will be pinned to this TurtleDex node, meaning that this node will pay
...
func skynetconvertcmd(sourceTurtleDexPathStr, destTurtleDexPathStr string) {
	// Create the siapaths.
	sourceTurtleDexPath, err := modules.NewTurtleDexPath(sourceTurtleDexPathStr)
	if err != nil {
		die("Could not parse source siapath:", err)
	}
	destTurtleDexPath, err := modules.NewTurtleDexPath(destTurtleDexPathStr)
	if err != nil {
	...
	sup := modules.SkyfileUploadParameters{
		TurtleDexPath: destTurtleDexPath,
	}
	sup = parseAndAddSkykey(sup)
	sshp, err := httpClient.SkynetConvertTurtleDexfileToSkyfilePost(sup, sourceTurtleDexPath)
	...
	var skypath modules.TurtleDexPath
	if skynetUploadRoot {
		skypath = destTurtleDexPath
	} else {
		skypath, err = modules.SkynetFolder.Join(destTurtleDexPath.String())
		if err != nil {
...
// skynetlscmd is the handler for the command `ttdxc skynet ls`. Works very
// similar to 'ttdxc renter ls' but defaults to the SkynetFolder and only
...
func skynetlscmd(cmd *cobra.Command, args []string) {
	...
	sizePerDir := make(map[modules.TurtleDexPath]uint64)
	for _, dir := range dirs {
		sizePerDir[dir.dir.TurtleDexPath] = dir.dir.AggregateSize
	}

	// Drop any files that are not tracking skylinks.
	var numDropped uint64
	numOmittedPerDir := make(map[modules.TurtleDexPath]int)
	for j := 0; j < len(dirs); j++ {
		for i := 0; i < len(dirs[j].files); i++ {
			...
			siaPath := dirs[j].dir.TurtleDexPath
			numDropped++
	...
	if skynetLsRecursive {
		...
		numFilesDirs -= uint64(numOmittedPerDir[root.dir.TurtleDexPath])
	}

	// Print totals.
	totalStoredStr := modules.FilesizeUnits(sizePerDir[root.dir.TurtleDexPath])
	fmt.Printf("\nListing %v files/dirs:\t%9s\n\n", numFilesDirs, totalStoredStr)

	// Print dirs.
	for _, dir := range dirs {
		fmt.Printf("%v/", dir.dir.TurtleDexPath)
		if numOmitted := numOmittedPerDir[dir.dir.TurtleDexPath]; numOmitted > 0 {
		...
		for _, subDir := range dir.subDirs {
			subDirName := subDir.TurtleDexPath.Name() + "/"
			sizeUnits := modules.FilesizeUnits(sizePerDir[subDir.TurtleDexPath])
...
// provided TurtleDexPath
func skynetPin(skylink string, siaPath modules.TurtleDexPath) (string, error) {
...
func skynetpincmd(sourceSkylink, destTurtleDexPath string) {
	skylink := strings.TrimPrefix(sourceSkylink, "sia://")
	// Create the siapath.
	siaPath, err := modules.NewTurtleDexPath(destTurtleDexPath)
	if err != nil {
		die("Could not parse destination siapath:", err)
	}
...
func skynetunpincmd(cmd *cobra.Command, skyPathStrs []string) {
	...
	for _, skyPathStr := range skyPathStrs {
		// Create the skypath.
		skyPath, err := modules.NewTurtleDexPath(skyPathStr)
		if err != nil {
...
func skynetuploadcmd(_ *cobra.Command, args []string) {
	...
	sourcePath, destTurtleDexPath := args[0], args[1]
	...
	if !fi.IsDir() {
		skynetUploadFile(sourcePath, sourcePath, destTurtleDexPath, pbs)
	...
	if skynetUploadSeparately {
		skynetUploadFilesSeparately(sourcePath, destTurtleDexPath, pbs)
		return
	}
	skynetUploadDirectory(sourcePath, destTurtleDexPath)
}

// skynetuploadpipecmd will upload a file or directory to Skynet. If --dry-run is
// passed, it will fetch the skylinks without uploading.
func skynetuploadpipecmd(destTurtleDexPath string) {
...
func skynetUploadFile(basePath, sourcePath string, destTurtleDexPath string, pbs *mpb.Progress) {
	// Create the siapath.
	siaPath, err := modules.NewTurtleDexPath(destTurtleDexPath)
	if err != nil {
		die("Could not parse destination siapath:", err)
...
func skynetUploadFilesSeparately(sourcePath, destTurtleDexPath string, pbs *mpb.Progress) {
	...
	for i := 0; i < SimultaneousSkynetUploads; i++ {
		...
		go func() {
			...
			for filename := range filesChan {
				// get only the filename and path, relative to the original destTurtleDexPath
				// in order to figure out where to put the file
				newDestTurtleDexPath := filepath.Join(destTurtleDexPath, strings.TrimPrefix(filename, sourcePath))
				skynetUploadFile(sourcePath, filename, newDestTurtleDexPath, pbs)
			}
		}()
	}
...
func skynetUploadDirectory(sourcePath, destTurtleDexPath string) {
	skyfilePath, err := modules.NewTurtleDexPath(destTurtleDexPath)
	if err != nil {
		fmt.Println("Failed to create siapath", destTurtleDexPath)
		die(err)
	}
...
func skynetUploadFileFromReader(source io.Reader, filename string, siaPath modules.TurtleDexPath, mode os.FileMode) (skylink string) {
