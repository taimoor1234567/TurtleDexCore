...
// TODO: If you run ttdxc from a non-existent directory, the abs() function does
...
the command 'ttdxc renter lost' to see the renter's lost files.`,
	...
	renterFilesListCmd = &cobra.Command{
		...
		Long:  "List the status of a specific file or all files known to the renter within the specified folder on the TurtleDex network. To query the root dir either '\"\"', '/' or '.' can be supplied",
	...
	renterFuseMountCmd = &cobra.Command{
		...
		Short: "Mount a TurtleDex folder to your disk",
		Long: `Mount a TurtleDex folder to your disk. Applications will be able to see this folder
as though it is a normal part of your filesystem.  Currently experimental, and
read-only. When TurtleDex is ready to support read-write fuse mounting, ttdxc will be
...
read-only mode is used, you must use the API.`,
	...
	renterFuseUnmountCmd = &cobra.Command{
		Use:   "unmount [path]",
		Short: "Unmount a TurtleDex folder",
		Long: `Unmount a TurtleDex folder that has previously been mounted. Unmount by specifying the
local path where the TurtleDex folder is mounted.`,
	...
	renterFilesUploadCmd = &cobra.Command{
		...
		Long: `Upload a file or folder to [path] on the TurtleDex network. The --data-pieces and --parity-pieces
...
For Example: 'ttdxc renter upload pause 3h' would pause uploads for 3 hours.`,
...
Note that setting the allowance will cause ttdxd to immediately begin forming
...
func renterlostcmd() {
	...
	dirs := getDir(modules.RootTurtleDexPath(), true, true)
...
// renteruploadscmd is the handler for the command `ttdxc renter uploads`.
...
func renteruploadscmd() {
	...
	for _, file := range filteredFiles {
		fmt.Printf("%13s  %s (uploading, %0.2f%%)\n", modules.FilesizeUnits(file.Filesize), file.TurtleDexPath, file.UploadProgress)
	}
}

// renterdownloadscmd is the handler for the command `ttdxc renter downloads`.
...
func renterdownloadscmd() {
	...
	if len(downloading) == 0 {
		...
		for _, file := range downloading {
			fmt.Printf("%s: %5.1f%% %s -> %s\n", file.StartTime.Format("Jan 02 03:04 PM"), 100*float64(file.Received)/float64(file.Filesize), file.TurtleDexPath, file.Destination)
		}
	}
	...
	if len(downloaded) == 0 {
		...
		for _, file := range downloaded {
			fmt.Printf("%s: %s -> %s\n", file.StartTime.Format("Jan 02 03:04 PM"), file.TurtleDexPath, file.Destination)
		}
	}
}

// renterallowancecmd is the handler for the command `ttdxc renter allowance`.
...
// renterallowancecancelcmd is the handler for `ttdxc renter allowance cancel`.
...
// rentersetallowancecmd is the handler for `ttdxc renter setallowance`.
...
// rentersetallowancecmdInteractive is the interactive handler for `ttdxc renter
// setallowance`.
...
Funds determines the number of ttdcs that the renter will spend when forming
contracts with hosts. The renter will not allocate more than this amount of
ttdcs into the set of contracts each billing period. If the renter spends all
...
The command 'ttdxc renter allowance' can be used to see a breakdown of spending.

The following units can be used to set the allowance:

    H  (10^24 H per ttdc)
    SC (1 ttdc per SC)
    KS (1000 ttdcs per KS)`)
	fmt.Println()
...
Hosts sets the number of hosts that will be used to form the allowance. TurtleDex
...
TurtleDex network. This value is important to calibrate the spending habits of ttdxd.
Because TurtleDex is decentralized, there is no easy way for ttdxd to know what the
real world cost of storage is, nor what the real world price of a ttdc is. To
overcome this deficiency, ttdxd depends on the user for guidance.

If the user has a low allowance and a high amount of expected storage, ttdxd will
...
and a low amount of expected storage, ttdxd will prioritize hosts that post more
...
ttdxd will try to optimize for saving money; ttdxd tries to meet the users storage
...
Expected upload tells ttdxd how much uploading the user expects to do each
period. If this value is high, ttdxd will more strongly prefer hosts that have a
low upload bandwidth price. If this value is low, ttdxd will focus on other
...
The user should not consider upload bandwidth used during repairs, ttdxd will
...
Expected download tells ttdxd how much downloading the user expects to do each
period. If this value is high, ttdxd will more strongly prefer hosts that have a
low download bandwidth price. If this value is low, ttdxd will focus on other
...
The user should not consider download bandwidth used during repairs, ttdxd will
consider repair bandwidth separately.
...
// renterbubblecmd is the handler for the command `ttdxc renter
// bubble`.
func renterbubblecmd(directory string) {
	// Parse the siapath
	var siaPath modules.TurtleDexPath
	if directory == "." {
		directory = "root" // For UX
		siaPath = modules.RootTurtleDexPath()
	} else {
...
// renterbackcreatecmd is the handler for the command `ttdxc renter
// createbackup`.
...
// renterbackuprestorecmd is the handler for the command `ttdxc renter
// restorebackup`.
...
// renterbackuplistcmd is the handler for the command `ttdxc renter listbackups`.
...
// rentercontractscmd is the handler for the command `ttdxc renter contracts`.
...
// rentercontractsviewcmd is the handler for the command `ttdxc renter contracts <id>`.
...
// renterfilesdownload downloads the dir at the given path from the TurtleDex network
// to the local specified destination.
func renterdirdownload(path, destination string) {
	destination = abs(destination)
	// Parse TurtleDexPath.
	siaPath, err := modules.NewTurtleDexPath(path)
	if err != nil {
		die("Couldn't parse TurtleDexPath:", err)
	}
	// If root is not set we need to rebase.
	if !renterDownloadRoot {
		siaPath, err = siaPath.Rebase(modules.RootTurtleDexPath(), modules.UserFolder)
		if err != nil {
			die("Couldn't rebase TurtleDexPath:", err)
		}
	}
	// Download dir.
	start := time.Now()
	...
	for _, fd := range failedDownloads {
		fmt.Printf("Download of file '%v' to destination '%v' failed: %v\n", fd.TurtleDexPath, fd.Destination, fd.Error)
	}
	os.Exit(1)
}

// renterdownloadcancelcmd is the handler for the command `ttdxc renter download cancel [cancelID]`
...
// renterfilesdeletecmd is the handler for the command `ttdxc renter delete [path]`.
// Removes the specified path from the TurtleDex network.
...
// renterfilesdownload is the handler for the command `ttdxc renter download
...
// renterfileslistcmd is the handler for the command `ttdxc renter ls`. Lists
...
func renterfileslistcmd(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < len(dirs); i++ {
		sort.Sort(byTurtleDexPathDir(dirs[i].subDirs))
		sort.Sort(byTurtleDexPathFile(dirs[i].files))
	}
	...
	if !verbose {
		for _, dir := range dirs {
			fmt.Printf("%v/\n", dir.dir.TurtleDexPath)
			w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
			for _, subDir := range dir.subDirs {
				name := subDir.TurtleDexPath.Name() + "/"
				size := modules.FilesizeUnits(subDir.AggregateSize)
				fmt.Fprintf(w, "  %v\t%9v\n", name, size)
			}

			for _, file := range dir.files {
				name := file.TurtleDexPath.Name()
	...
	for _, dir := range dirs {
		fmt.Println(dir.dir.TurtleDexPath.String() + "/")
		w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
...
// renterfilesrenamecmd is the handler for the command `ttdxc renter rename [path] [newpath]`.
// Renames a file on the TurtleDex network.
func renterfilesrenamecmd(path, newpath string) {
	// Parse TurtleDexPath.
	siaPath, err1 := modules.NewTurtleDexPath(path)
	newTurtleDexPath, err2 := modules.NewTurtleDexPath(newpath)
	if err := errors.Compose(err1, err2); err != nil {
		die("Couldn't parse TurtleDexPath:", err)
	}
	err := httpClient.RenterRenamePost(siaPath, newTurtleDexPath, renterRenameRoot)
	if err != nil {
...
func renterfusecmd() {
	...
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	fmt.Fprintf(w, "\t%s\t%s\n", "Mount Point", "TurtleDexPath")
	for _, mp := range mountPoints {
		siaPathStr := mp.TurtleDexPath.String()
		if siaPathStr == "" {
			siaPathStr = "{root}"
		}
...
// renterfusemountcmd is the handler for the command `ttdxc renter fuse mount [path] [siapath]`.
func renterfusemountcmd(path, siaPathStr string) {
	// TODO: Once read-write is supported on the backend, the 'true' flag can be
	// set to 'false' - ttdxc will support mounting read-write by default. Need
	...
	if siaPathStr == "" || siaPathStr == "/" {
		...
		siaPath, err = modules.NewTurtleDexPath(siaPathStr)
		if err != nil {
...
// renterfuseunmountcmd is the handler for the command `ttdxc renter fuse unmount [path]`.
...
//rentersetlocalpathcmd is the handler for the command `ttdxc renter setlocalpath [siapath] [newlocalpath]`
//Changes the trackingpath of the file
//through API Endpoint
func rentersetlocalpathcmd(siapath, newlocalpath string) {
	//Parse TurtleDexpath
	siaPath, err := modules.NewTurtleDexPath(siapath)
	if err != nil {
		die("Couldn't parse TurtleDexpath:", err)
	}
...
// renterfilesunstuckcmd is the handler for the command `ttdxc renter
...
func renterfilesunstuckcmd() {
	...
	toUnstuck := make(chan modules.TurtleDexPath)
	worker := func() {
...
// renterfilesuploadcmd is the handler for the command `ttdxc renter upload
// [source] [path]`. Uploads the [source] file to [path] on the TurtleDex network.
...
func renterfilesuploadcmd(source, path string) {
	...
	if stat.IsDir() {
		...
		for _, file := range files {
			...
			// Parse TurtleDexPath.
			fTurtleDexPath, err := modules.NewTurtleDexPath(fpath)
			if err != nil {
				die("Couldn't parse TurtleDexPath:", err)
			}
			err = httpClient.RenterUploadPost(abs(file), fTurtleDexPath, uint64(numDataPieces), uint64(numParityPieces))
			if err != nil {
				failed++
...
// renterfilesuploadpausecmd is the handler for the command `ttdxc renter upload
...
// renterfilesuploadresumecmd is the handler for the command `ttdxc renter upload
...
// renterpricescmd is the handler for the command `ttdxc renter prices`, which
...
// renterratelimitcmd is the handler for the command `ttdxc renter ratelimit`
...
// renterworkerscmd is the handler for the command `ttdxc renter workers`.
...
// renterworkerseacmd is the handler for the command `ttdxc renter workers ea`.
...
// renterworkersdownloadscmd is the handler for the command `ttdxc renter workers
...
// renterworkersptcmd is the handler for the command `ttdxc renter workers pt`.
...
// renterworkersrjcmd is the handler for the command `ttdxc renter workers rj`.
...
// renterworkershsjcmd is the handler for the command `ttdxc renter workers hs`.
...
// renterworkersuploadscmd is the handler for the command `ttdxc renter workers
...
// renterworkerreadregistrycmd is the handler for the command `ttdxc renter workers
...
// renterworkerupdateregistrycmd is the handler for the command `ttdxc renter
