...
var (
	...
	stopCmd = &cobra.Command{
		Use:   "stop",
		Short: "Stop the TurtleDex daemon",
		Long:  "Stop the TurtleDex daemon.",
		Run:   wrap(stopcmd),
	}
...
directory in the ttdxd data directory.`,
	...
	updateCmd = &cobra.Command{
		Use:   "update",
		Short: "Update TurtleDex",
		Long:  "Check for (and/or download) available updates for TurtleDex.",
		Run:   wrap(updatecmd),
	}
...
// alerts as critical alerts are printed on every ttdxc command
func alertscmd() {
...
// version prints the version of ttdxc and ttdxd.
func versioncmd() {
	fmt.Println("TurtleDex Client")
	if build.ReleaseTag == "" {
...
// stopcmd is the handler for the command `ttdxc stop`.
// Stops the daemon.
func stopcmd() {
	...
	fmt.Println("TurtleDex daemon stopped.")
}

// stackcmd is the handler for the command `ttdxc stack` and writes the current
...
// updatecmd is the handler for the command `ttdxc update`.
...
func updatecmd() {
	...
	fmt.Printf("Updated to version %s! Restart ttdxd now.\n", update.Version)
}

// updatecheckcmd is the handler for the command `ttdxc check`.
...
func updatecheckcmd() {
	...
	if update.Available {
		fmt.Printf("A new release (v%s) is available! Run 'ttdxc update' to install it.\n", update.Version)
	} else {
		fmt.Println("Up to date.")
	}
}

// globalratelimitcmd is the handler for the command `ttdxc ratelimit`.
