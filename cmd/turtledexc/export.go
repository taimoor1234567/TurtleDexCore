...
var (
	...
	renterExportContractTxnsCmd = &cobra.Command{
		...
		Short: "export the renter's contracts for import to `https://rankings.turtledex.io/`",
		Long: "Export the renter's current contract set in JSON format to the specified " +
			"file. Intended for upload to `https://rankings.turtledex.io/`.",
		Run: wrap(renterexportcontracttxnscmd),
	}
)

// renterexportcontracttxnscmd is the handler for the command `ttdxc renter export contract-txns`.
