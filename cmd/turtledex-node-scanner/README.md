...
The TurtleDex-Node-Scanner tries to learn the IP addresses of all nodes in the TurtleDex
network in order to get an accurate count of current active nodes.

## Overview

The TurtleDex-Node-Scanner uses a TurtleDex gateway to explore the TurtleDex peer-to-peer network.
It attempts to connect to as many TurtleDex nodes as possible, and uses the ShareNodes
RPC with TurtleDex nodes to learn the IP addresses of more nodes in the network.
...
scanner.  Then running `sia-node-scanner` will create a TurtleDexNodeScanner directory
...
Use the `-dir` flag to specify which directory to make the `TurtleDexNodeScanner`
directory in. The flag defaults to the current working directory.

Ex:
```
TurtleDexNodeScanner
