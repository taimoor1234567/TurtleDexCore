...
import (
	...
	siaPersist "github.com/turtledex/TurtleDexCore/persist"
)
...
func TestSendShareNodesRequests(t *testing.T) {
	...
	mainGateway, err := gateway.New("localhost:0", true, build.TempDir("TurtleDexNodeScannerTestGateway"))
	if err != nil {
	...
	for i := 0; i < numTestingGateways; i++ {
		g, err := gateway.New("localhost:0", true, build.TempDir(fmt.Sprintf("TurtleDexNodeScannerTestGateway-%d", i)))
		if err != nil {
			t.Fatal("Error making new gateway: ", err)
		}
...
func TestRestartScanner(t *testing.T) {
	...
	testDir := build.TempDir("TurtleDexNodeScanner-TestRestartScanner")
	...
	for i := 0; i < numTestingGateways; i++ {
		g, err := gateway.New(fmt.Sprintf("localhost:4444%d", i), true, build.TempDir(fmt.Sprintf("TurtleDexNodeScannerTestGateway-%d", i)))
		if err != nil {
			t.Fatal("Error making new gateway: ", err)
		}
		gateways = append(gateways, g)
