...
import (
	...
	"github.com/turtledex/TurtleDexCore/node/api/server"
	"github.com/turtledex/TurtleDexCore/profile"
)
...
func verifyAPISecurity(config Config) error {
	...
	if !config.TurtleDexd.AllowAPIBind {
		addr := modules.NetAddress(config.TurtleDexd.APIaddr)
		if !addr.IsLoopback() {
			if addr.Host() == "" {
				return fmt.Errorf("a blank host will listen on all interfaces, did you mean localhost:%v?\nyou must pass --disable-api-security to bind TurtleDexd to a non-localhost address", addr.Port())
			}
			return errors.New("you must pass --disable-api-security to bind TurtleDexd to a non-localhost address")
		}
		return nil
	}
	...
	if config.TurtleDexd.AllowAPIBind && !config.TurtleDexd.AuthenticateAPI {
...
func processConfig(config Config) (Config, error) {
	var err1, err2 error
	config.TurtleDexd.APIaddr = processNetAddr(config.TurtleDexd.APIaddr)
	config.TurtleDexd.RPCaddr = processNetAddr(config.TurtleDexd.RPCaddr)
	config.TurtleDexd.HostAddr = processNetAddr(config.TurtleDexd.HostAddr)
	config.TurtleDexd.Modules, err1 = processModules(config.TurtleDexd.Modules)
	if config.TurtleDexd.Profile != "" {
		config.TurtleDexd.Profile, err2 = profile.ProcessProfileFlags(config.TurtleDexd.Profile)
	}
...
func loadAPIPassword(config Config) (_ Config, err error) {
	if config.TurtleDexd.AuthenticateAPI {
		if config.TurtleDexd.TempPassword {
...
func printVersionAndRevision() {
	fmt.Println("TurtleDex Daemon v" + build.Version)
...
func tryAutoUnlock(srv *server.Server) {
	if password := build.WalletPassword(); password != "" {
		fmt.Println("TurtleDex Wallet Password found, attempting to auto-unlock wallet")
...
// startDaemon uses the config parameters to initialize TurtleDex modules and start
// ttdxd.
func startDaemon(config Config) (err error) {
	...
	// Print the ttdxd Version and GitRevision
	...
	srv, err := server.New(config.TurtleDexd.APIaddr, config.TurtleDexd.RequiredUserAgent, config.APIPassword, nodeParams, loadStart)
	if err != nil {
		return err
	}
...
func startDaemonCmd(cmd *cobra.Command, _ []string) {
	...
	// Parse profile flags
	profileCPU := strings.Contains(config.TurtleDexd.Profile, "c")
	profileMem := strings.Contains(config.TurtleDexd.Profile, "m")
	profileTrace := strings.Contains(config.TurtleDexd.Profile, "t")
	...
	if profileCPU || profileMem || profileTrace {
		...
		if cmd.Root().Flag("profile-directory").Changed {
			profileDir = config.TurtleDexd.ProfileDir
		} else {
			profileDir = filepath.Join(config.TurtleDexd.TurtleDexDir, config.TurtleDexd.ProfileDir)
		}
		go profile.StartContinuousProfile(profileDir, profileCPU, profileMem, profileTrace)
	}

	// Start ttdxd. startDaemon will only return when it is shutting down.
