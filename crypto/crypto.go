...
type (
	...
	// CipherKey is a key with TurtleDex specific encryption/decryption methods.
	CipherKey interface {
...
func NewWalletKey(entropy Hash) CipherKey {
	sk, err := NewTurtleDexKey(TypeDefaultWallet, entropy[:])
	if err != nil {
		panic(err)
	}
	return sk
...
// NewTurtleDexKey creates a new TurtleDexKey from the provided type and entropy.
func NewTurtleDexKey(ct CipherType, entropy []byte) (CipherKey, error) {
	switch ct {
	case TypePlain:
...
// GenerateTurtleDexKey creates a new TurtleDexKey from the provided type.
func GenerateTurtleDexKey(ct CipherType) CipherKey {
	switch ct {
	case TypePlain:
