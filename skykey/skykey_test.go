...
func TestSkykeyManager(t *testing.T) {
	...
	err = skykey.marshalTurtleDex(&buf)
	if err != nil {
		t.Fatal(err)
	}

	var decodedSkykey Skykey
	err = decodedSkykey.unmarshalTurtleDex(&buf)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkykeyFormatCompat(t *testing.T) {
	...
	_, err = crypto.NewTurtleDexKey(oldSkykey.ciphertype, oldSkykey.entropy)
	if err != nil {
	...
	var buf bytes.Buffer
	err = oldSkykey2.marshalTurtleDex(&buf)
	if err != nil {
		t.Fatal(err)
	}

	var decodedOK2 compatSkykeyV148
	err = decodedOK2.unmarshalTurtleDex(&buf)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkykeyMarshalling(t *testing.T) {
	...
	cipherKey := crypto.GenerateTurtleDexKey(skykeyType.CipherType())
	skykey := Skykey{
		Type:    skykeyType,
