...
// unmarshalTurtleDex decodes the Skykey into the reader.
func (skOld *compatSkykeyV148) unmarshalTurtleDex(r io.Reader) error {
...
// marshalTurtleDex encodes the Skykey into the writer.
func (skOld compatSkykeyV148) marshalTurtleDex(w io.Writer) error {
...
func (skOld *compatSkykeyV148) fromString(s string) error {
	...
	return skOld.unmarshalTurtleDex(bytes.NewReader(keyBytes))
}
...
func (skOld compatSkykeyV148) convertToUpdatedFormat() (Skykey, error) {
	...
	_, err := crypto.NewTurtleDexKey(sk.CipherType(), sk.Entropy)
	if err != nil {
...
func (sk *Skykey) unmarshalAndConvertFromOldFormat(r io.Reader) error {
	...
	err := oldFormatSkykey.unmarshalTurtleDex(r)
	if err != nil {
		return err
	}
...
// unmarshalTurtleDex decodes the Skykey data and name into the reader.
func (sk *Skykey) unmarshalTurtleDex(r io.Reader) error {
...
// marshalTurtleDex encodes the Skykey data and name into the writer.
func (sk Skykey) marshalTurtleDex(w io.Writer) error {
...
func (sk *Skykey) SubkeyWithNonce(nonce []byte) (Skykey, error) {
	...
	// Sanity check that we can actually make a CipherKey with this.
	_, err := crypto.NewTurtleDexKey(sk.CipherType(), entropy)
	if err != nil {
...
func (sk *Skykey) CipherKey() (crypto.CipherKey, error) {
	return crypto.NewTurtleDexKey(sk.CipherType(), sk.Entropy)
}
